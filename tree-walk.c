...
end_function
begin_comment
comment|/*  * This is Linux's built-in max for the number of symlinks to follow.  * That limit, of course, does not affect git, but it's a reasonable  * choice.  */
end_comment
begin_define
DECL|macro|GET_TREE_ENTRY_FOLLOW_SYMLINKS_MAX_LINKS
define|#
directive|define
name|GET_TREE_ENTRY_FOLLOW_SYMLINKS_MAX_LINKS
value|40
end_define
begin_comment
comment|/**  * Find a tree entry by following symlinks in tree_sha (which is  * assumed to be the root of the repository).  In the event that a  * symlink points outside the repository (e.g. a link to /foo or a  * root-level link to ../foo), the portion of the link which is  * outside the repository will be returned in result_path, and *mode  * will be set to 0.  It is assumed that result_path is uninitialized.  * If there are no symlinks, or the end result of the symlink chain  * points to an object inside the repository, result will be filled in  * with the sha1 of the found object, and *mode will hold the mode of  * the object.  *  * See the code for enum follow_symlink_result for a description of  * the return values.  */
end_comment
begin_function
DECL|function|get_tree_entry_follow_symlinks
name|enum
name|follow_symlinks_result
name|get_tree_entry_follow_symlinks
parameter_list|(
name|unsigned
name|char
modifier|*
...
expr_stmt|;
block|}
comment|/* Handle symlinks to e.g. a//b by removing leading slashes */
while|while
condition|(
name|namebuf
operator|.
...
expr_stmt|;
block|}
comment|/* Split namebuf into a first component and a remainder */
if|if
condition|(
operator|(
name|first_slash
...
name|parent
decl_stmt|;
comment|/* 			 * We could end up with .. in the namebuf if it 			 * appears in a symlink. 			 */
if|if
condition|(
name|parents_nr
operator|==
...
continue|continue;
block|}
comment|/* We could end up here via a symlink to dir/.. */
if|if
condition|(
name|namebuf
operator|.
...
goto|;
block|}
comment|/* Look up the first (or only) path component in the tree. */
name|find_result
operator|=
name|find_tree_entry
argument_list|(
...
condition|)
block|{
comment|/* Too many symlinks followed */
name|retval
operator|=
name|SYMLINK_LOOP
expr_stmt|;
...
goto|;
block|}
comment|/* 			 * At this point, we have followed at a least 			 * one symlink, so on error we need to report this. 			 */
name|retval
operator|=
