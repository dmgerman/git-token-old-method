...
condition|)
block|{
comment|/* Add a trailing '>' to email, since map_user returns plain emails 		   Note: It already has '<', since we replace from mail+1 */
name|mailpos
operator|=
name|memchr
argument_list|(
...
end_function
begin_comment
comment|/*  * Porcelain/Incremental format wants to show a lot of details per  * commit.  Instead of repeating this every line, emit it only once,  * the first time each commit appears in the output.  */
end_comment
begin_function
