...
block|{
comment|/* The file describes a pack that is no longer here; 		 * dependencies between packs needs to be recalculated. 		 */
return|return
literal|1
return|;
block|}
...
name|that
condition|)
comment|/* The pack this one depends on does not 			 * exist; this should not happen because 			 * we write out the list of packs first and 			 * then dependency information, but it means 			 * the file is useless anyway. 			 */
return|return
literal|1
return|;
name|this
...
end_function
begin_comment
comment|/* We sort the packs according to the date of the latest commit.  That  * in turn indicates how young the pack is, and in general we would  * want to depend on younger packs.  */
end_comment
begin_function
...
name|i
expr_stmt|;
comment|/* we need to fix up the dependency information 	 * for the old ones. 	 */
name|dep_temp
operator|=
name|NULL
expr_stmt|;
...
name|die
argument_list|(
literal|"internal error renumbering"
argument_list|)
expr_stmt|;
name|dep_temp
index|[
...
parameter_list|)
block|{
comment|/* 	 * We are interested in objects that are not referenced, 	 * and objects that are referenced but not internal. 	 */
if|if
condition|(
name|o
operator|->
...
name|i
decl_stmt|;
comment|/* Which pack contains this object?  That is what 		 * pack_ix can depend on.  We earlier sorted info 		 * array from youngest to oldest, so try newer packs 		 * first to favor them here. 		 */
for|for
control|(
name|i
operator|=
...
expr_stmt|;
block|}
comment|/* Mark all the internal ones */
for|for
control|(
name|i
operator|=
...
name|commit_type
condition|)
comment|/* tag pointing at a non-commit */
return|return
literal|0
return|;
return|return
...
argument_list|)
expr_stmt|;
comment|/* update the rev-cache database */
