...
argument_list|)
expr_stmt|;
comment|/* no previous entry overwritten */
return|return
name|ret
return|;
block|}
...
expr_stmt|;
comment|/* 	 * The branches file would have URL and optionally 	 * #branch specified.  The "master" (or specified) branch is 	 * fetched and stored in the local branch matching the 	 * remote name. 	 */
name|frag
operator|=
name|strchr
argument_list|(
...
name|xstrfmt
argument_list|(
literal|"refs/heads/%s:refs/heads/%s"
argument_list|,
name|frag
argument_list|,
name|remote
...
name|_
argument_list|(
literal|"Cannot fetch both %s and %s to %s"
argument_list|)
argument_list|,
name|ref1
operator|->
...
name|_
argument_list|(
literal|"%s usually tracks %s, not %s"
argument_list|)
argument_list|,
name|ref2
operator|->
...
block|{
comment|/* 			 * This last possibility doesn't occur because 			 * FETCH_HEAD_IGNORE entries always appear at 			 * the end of the list. 			 */
name|die
argument_list|(
name|_
argument_list|(
...
name|error
argument_list|(
literal|"query_refspecs_multiple: need either src or dst"
argument_list|)
expr_stmt|;
for|for
control|(
...
end_function
begin_comment
comment|/*  * Given only the set of local refs, sanity-check the set of push  * refspecs. We can't catch all errors that match_push_refs would,  * but we can catch some errors early before even talking to the  * remote side.  */
end_comment
begin_function
...
condition|)
block|{
comment|/* 		 * no merge config; let's make sure we don't confuse callers 		 * with a non-zero merge_nr but a NULL merge 		 */
name|ret
operator|->
name|merge_nr
operator|=
...
condition|)
block|{
comment|/* 		 * no merge config; is it because the user didn't define any, 		 * or because it is not a real branch, and get_branch 		 * auto-vivified it? 		 */
if|if
condition|(
operator|!
name|ref_exists
...
name|_
argument_list|(
literal|"push destination '%s' on remote '%s' has no local tracking branch"
argument_list|)
argument_list|,
name|refname
argument_list|,
...
name|_
argument_list|(
literal|"branch '%s' has no remote for pushing"
argument_list|)
argument_list|,
name|branch
operator|->
...
name|_
argument_list|(
literal|"push refspecs for '%s' do not include '%s'"
argument_list|)
argument_list|,
name|remote
operator|->
...
name|_
argument_list|(
literal|"push has no destination (push.default is 'nothing')"
argument_list|)
argument_list|)
return|;
case|case
...
name|_
argument_list|(
literal|"cannot resolve 'simple' push to a single destination"
argument_list|)
argument_list|)
return|;
return|return
...
name|die
argument_list|(
literal|"BUG: unhandled push situation"
argument_list|)
expr_stmt|;
block|}
end_function
...
goto|;
comment|/* No matches */
comment|/* 	 * If we did find a suitable refspec and it's not a symref and 	 * it's not in the list of refs that currently exist in that 	 * remote, we consider it to be stale. In order to deal with 	 * overlapping refspecs, we need to go over all of the 	 * matching refs. 	 */
if|if
condition|(
name|flags
operator|&
...
return|;
block|}
comment|/* "--<option>=refname" or "--<option>=refname:value" */
name|colon
operator|=
name|strchrnul
argument_list|(
...
name|error
argument_list|(
literal|"cannot parse expected object name '%s'"
argument_list|,
name|colon
operator|+
literal|1
...
end_function
begin_comment
comment|/*  * Look at remote.fetch refspec and see if we have a remote  * tracking branch for the refname there.  Fill its current  * value in sha1[].  * If we cannot do so, return negative to signal an error.  */
end_comment
begin_function
...
literal|1
return|;
comment|/* no tracking ref for refname at remote */
if|if
condition|(
name|read_ref
argument_list|(
...
literal|1
return|;
comment|/* we know what the tracking ref is but we cannot read it */
return|return
literal|0
return|;
block|}
...
name|i
decl_stmt|;
comment|/* Find an explicit --<option>=<name>[:<value>] entry */
for|for
control|(
name|i
operator|=
...
return|return;
block|}
comment|/* Are we using "--<option>" to cover all? */
if|if
condition|(
operator|!
name|cas
