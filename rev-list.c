...
condition|)
return|return;
comment|/* Only single-parent commits don't have TREECHANGE */
operator|*
name|pp
operator|=
name|p
...
return|return;
block|}
comment|/* 	 * Ok, the commit wasn't uninteresting. If it 	 * is a merge, try to find the parent that has 	 * no differences in the path set if one exists. 	 */
if|if
condition|(
name|paths
operator|&&
...
name|next
expr_stmt|;
comment|/* 		 * Exactly one parent? Check if it leaves the tree 		 * unchanged 		 */
if|if
condition|(
name|parent
operator|&&
