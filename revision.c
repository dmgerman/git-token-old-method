...
condition|)
continue|continue;
comment|/* We want to show boundary commits only when their 		 * children are shown.  When path-limiter is in effect, 		 * rewrite_parents() drops some commits from getting shown, 		 * and there is no point showing boundary parents that 		 * are not shown.  After rewrite_parents() rewrites the 		 * parents of a commit that is shown, we mark the boundary 		 * parents with BOUNDARY_SHOW. 		 */
if|if
condition|(
name|commit
operator|->
...
name|list
decl_stmt|;
comment|/* 		 * rev_info.reverse is used to note the fact that we 		 * want to output the list of revisions in reverse 		 * order.  To accomplish this goal, reverse can have 		 * different values: 		 * 		 *  0  do nothing 		 *  1  reverse the list 		 *  2  internal use:  we have already obtained and 		 *     reversed the list, now we only need to yield 		 *     its items. 		 */
if|if
condition|(
name|revs
operator|->
...
expr_stmt|;
block|}
comment|/* all remaining commits are boundary commits */
name|revs
operator|->
name|max_count
operator|=
