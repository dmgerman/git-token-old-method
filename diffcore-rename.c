...
end_function
begin_comment
comment|/*  * Returns 0 on success, -1 if we found a duplicate.  */
end_comment
begin_function
...
name|b_
decl_stmt|;
comment|/* sink the unused ones to the bottom */
if|if
condition|(
name|a
operator|->
...
literal|1
decl_stmt|;
comment|/* 	 * Find the best source match for specified destination. 	 */
name|p
operator|=
...
continue|continue;
block|}
comment|/* Give higher scores to sources that haven't been used already */
name|score
operator|=
operator|!
name|source
...
argument_list|)
expr_stmt|;
comment|/* Walk the destinations and find best source match */
for|for
control|(
name|i
operator|=
...
argument_list|)
expr_stmt|;
comment|/* Free the hash data structure and entries */
name|hashmap_free
argument_list|(
operator|&
name|file_table
...
name|i
expr_stmt|;
comment|/* is it better than the worst one? */
if|if
condition|(
name|score_compare
argument_list|(
...
end_function
begin_comment
comment|/*  * Returns:  * 0 if we are under the limit;  * 1 if we need to disable inexact rename detection;  * 2 if we would be under the limit if we were given -C instead of -C -C.  */
end_comment
begin_function
DECL|function|too_many_rename_candidates
specifier|static
name|int
name|too_many_rename_candidates
parameter_list|(
name|int
name|num_create
parameter_list|,
...
name|num_create
expr_stmt|;
comment|/* Are we running under -C -C? */
if|if
condition|(
operator|!
name|DIFF_OPT_TST
...
literal|1
return|;
comment|/* Would we bust the limit if we were running under -C? */
for|for
control|(
name|num_src
operator|=
