...
end_function
begin_comment
comment|/*  * Find exact renames first.  *  * The first round matches up the up-to-date entries,  * and then during the second round we try to match  * cache-dirty entries as well.  *  * Note: the rest of the rename logic depends on this  * phase also populating all the filespecs for any  * entry that isn't matched up with an exact rename,  * see "is_exact_match()".  */
end_comment
begin_function
