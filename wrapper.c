...
end_endif
begin_comment
comment|/**  * xopen() is the same as open(), but it die()s if the open() fails.  */
end_comment
begin_function
DECL|function|xopen
name|int
...
name|ap
decl_stmt|;
comment|/* 	 * va_arg() will have undefined behavior if the specified type is not 	 * compatible with the argument type. Since integers are promoted to 	 * ints, we fetch the next argument as an int, and then cast it to a 	 * mode_t to avoid undefined behavior. 	 */
name|va_start
argument_list|(
name|ap
argument_list|,
...
name|_
argument_list|(
literal|"could not open '%s' for reading and writing"
argument_list|)
argument_list|,
name|path
argument_list|)
...
name|fd
expr_stmt|;
comment|/* 				 * it is OK if this poll() failed; we 				 * want to leave this infinite loop 				 * only when read() returns with 				 * success, or an expected failure, 				 * which would be checked by the next 				 * call to read(2). 				 */
name|poll
argument_list|(
operator|&
name|pfd
...
end_function
begin_comment
comment|/*  * xpread() is the same as pread(), but it automatically restarts pread()  * operations with a recoverable error (EAGAIN and EINTR). xpread() DOES  * NOT GUARANTEE that "len" bytes is read even if the data is available.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/**  * xfopen() is the same as fopen(), but it die()s if the fopen() fails.  */
end_comment
begin_function
...
end_function
begin_function
DECL|function|fopen_for_writing
name|FILE
modifier|*
...
name|die
argument_list|(
literal|"BUG: your snprintf is broken"
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|die
argument_list|(
literal|"BUG: attempt to snprintf into too-small buffer"
argument_list|)
expr_stmt|;
return|return
name|len
