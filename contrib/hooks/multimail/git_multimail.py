...
op|':'
newline|'\n'
comment|"# Python < 2.6 do not have ssl, but that's OK if we don't use it."
nl|'\n'
indent|'    '
name|'pass'
newline|'\n'
...
op|':'
newline|'\n'
comment|'# Try outputing with the default encoding. If it fails,'
nl|'\n'
comment|'# try UTF-8.'
nl|'\n'
indent|'        '
...
newline|'\n'
nl|'\n'
name|'LINK_TEXT_TEMPLATE'
op|'='
string|'"""\\\nView the commit online:\n%(browse_url)s\n\n"""'
newline|'\n'
nl|'\n'
name|'LINK_HTML_TEMPLATE'
op|'='
...
newline|'\n'
nl|'\n'
comment|'# Convert to unicode, if required.'
nl|'\n'
name|'if'
name|'not'
name|'isinstance'
...
name|'staticmethod'
newline|'\n'
DECL|member|add_config_parameters
name|'def'
name|'add_config_parameters'
op|'('
name|'c'
op|')'
op|':'
...
string|"' '"
newline|'\n'
comment|'# git expects GIT_CONFIG_PARAMETERS to be of the form'
nl|'\n'
comment|'#    "\'name1=value1\' \'name2=value2\' \'name3=value3\'"'
nl|'\n'
comment|'# including everything inside the double quotes (but not the double'
nl|'\n'
comment|'# quotes themselves).  Spacing is critical.  Also, if a value contains'
nl|'\n'
comment|'# a literal single quote that quote must be represented using the'
nl|'\n'
comment|"# four character sequence: '\\''"
nl|'\n'
dedent|''
name|'parameters'
op|'+='
...
newline|'\n'
indent|'        '
string|'"""Python 2 backward compatibility"""'
newline|'\n'
name|'return'
name|'self'
op|'.'
...
name|'self'
op|'.'
name|'_contains_html_diff'
op|'='
name|'False'
newline|'\n'
nl|'\n'
DECL|member|_contains_diff
dedent|''
name|'def'
name|'_contains_diff'
op|'('
...
op|':'
newline|'\n'
comment|'# We do contain a diff, should it be rendered in HTML?'
nl|'\n'
indent|'        '
name|'if'
name|'self'
...
newline|'\n'
nl|'\n'
comment|'# Aliases usable in template strings. Tuple of pairs (destination,'
nl|'\n'
comment|'# source).'
nl|'\n'
DECL|variable|VALUES_ALIAS
dedent|''
name|'VALUES_ALIAS'
op|'='
op|'('
...
newline|'\n'
nl|'\n'
DECL|member|generate_browse_link
dedent|''
name|'def'
name|'generate_browse_link'
op|'('
name|'self'
op|','
name|'base_url'
...
newline|'\n'
indent|'        '
string|'"""Generate a link to an online repository browser."""'
newline|'\n'
name|'return'
name|'iter'
op|'('
...
op|':'
newline|'\n'
comment|'# "white-space: pre" is the default, but we need to'
nl|'\n'
comment|'# specify it again in case the message is viewed in a'
nl|'\n'
comment|'# webmail which wraps it in an element setting white-space'
nl|'\n'
comment|'# to something else (Zimbra does this and sets'
nl|'\n'
comment|'# white-space: pre-line).'
nl|'\n'
indent|'            '
name|'yield'
string|'\'<pre style="white-space: pre; background: #F8F8F8">\''
newline|'\n'
dedent|''
name|'for'
name|'line'
...
op|':'
newline|'\n'
comment|'# This is very, very naive. It would be much better to really'
nl|'\n'
comment|'# parse the diff, i.e. look at how many lines do we have in'
nl|'\n'
comment|'# the hunk headers instead of blindly highlighting everything'
nl|'\n'
comment|'# that looks like it might be part of a diff.'
nl|'\n'
indent|'                '
name|'bgcolor'
op|'='
...
newline|'\n'
nl|'\n'
comment|"# Chop the trailing LF, we don't want it inside <pre>."
nl|'\n'
dedent|''
name|'line'
op|'='
...
name|'style'
op|'='
string|"'display:block; white-space:pre;'"
newline|'\n'
name|'if'
name|'bgcolor'
op|':'
...
string|"';'"
newline|'\n'
comment|"# Use a <span style='display:block> to color the"
nl|'\n'
comment|'# whole line. The newline must be inside the span'
nl|'\n'
comment|'# to display properly both in Firefox and in'
nl|'\n'
comment|'# text-based browser.'
nl|'\n'
dedent|''
name|'line'
op|'='
string|'"<span style=\'%s\'>%s\\n</span>"'
op|'%'
op|'('
name|'style'
op|','
...
newline|'\n'
nl|'\n'
DECL|member|get_alt_fromaddr
dedent|''
dedent|''
name|'def'
name|'get_alt_fromaddr'
op|'('
name|'self'
op|')'
op|':'
...
name|'NotImplementedError'
op|'('
string|'"Content-type %s unsupported. Please report it as a bug."'
op|')'
newline|'\n'
nl|'\n'
DECL|member|generate_email_intro
...
newline|'\n'
nl|'\n'
DECL|function|revision_gen_link
name|'def'
name|'revision_gen_link'
op|'('
name|'base_url'
op|')'
op|':'
newline|'\n'
comment|'# revision is used only to generate the body, and'
nl|'\n'
comment|'# _content_type is set while generating headers. Get it'
nl|'\n'
comment|'# from the BranchChange object.'
nl|'\n'
indent|'            '
name|'revision'
op|'.'
...
op|':'
newline|'\n'
comment|'# Old Python versions do not have timeout= argument.'
nl|'\n'
indent|'                    '
name|'return'
name|'klass'
...
op|'('
nl|'\n'
string|'"Checking certificate is not supported for ssl, prefer starttls"'
nl|'\n'
op|')'
newline|'\n'
dedent|''
...
op|'('
nl|'\n'
string|"'*** Your Python version does not have the ssl library installed\\n'"
nl|'\n'
string|"'*** smtpEncryption=tls is not available.\\n'"
nl|'\n'
string|"'*** Either upgrade Python to 2.6 or later\\n'"
nl|'\n'
string|"'    or use git_multimail.py version 1.2.\\n'"
op|')'
newline|'\n'
dedent|''
name|'if'
...
op|')'
newline|'\n'
comment|'# start: ehlo + starttls'
nl|'\n'
comment|'# equivalent to'
nl|'\n'
comment|'#     self.smtp.ehlo()'
nl|'\n'
comment|'#     self.smtp.starttls()'
nl|'\n'
comment|'# with acces to the ssl layer'
nl|'\n'
name|'self'
op|'.'
name|'smtp'
...
name|'SMTPException'
op|'('
string|'"STARTTLS extension not supported by server"'
op|')'
newline|'\n'
dedent|''
name|'resp'
...
name|'SMTPException'
op|'('
string|'"Wrong answer to the STARTTLS command"'
op|')'
newline|'\n'
dedent|''
name|'if'
...
op|'('
nl|'\n'
string|"'*** Warning, the server certificat is not verified (smtp) ***\\n'"
nl|'\n'
string|"'***          set the option smtpCACerts                   ***\\n'"
nl|'\n'
op|')'
newline|'\n'
dedent|''
...
op|':'
newline|'\n'
comment|'# using httplib.FakeSocket with Python 2.5.x or earlier'
nl|'\n'
indent|'                    '
name|'self'
op|'.'
...
name|'smtp'
newline|'\n'
comment|'# delete the field before quit() so that in case of'
nl|'\n'
comment|'# error, self.smtp is deleted anyway.'
nl|'\n'
name|'del'
name|'self'
op|'.'
...
name|'write'
op|'('
string|"'*** Error closing the SMTP connection ***\\n'"
op|')'
newline|'\n'
name|'sys'
op|'.'
...
string|"''"
op|'}'
comment|'# %()s expands to the empty string.'
newline|'\n'
nl|'\n'
name|'for'
name|'key'
...
newline|'\n'
nl|'\n'
DECL|member|get_default_ref_ignore_regex
dedent|''
name|'def'
name|'get_default_ref_ignore_regex'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
comment|'# The commit messages of git notes are essentially meaningless'
nl|'\n'
comment|'# and "filenames" in git notes commits are an implementational'
nl|'\n'
comment|'# detail that might surprise users at first.  As such, we'
nl|'\n'
comment|'# would need a completely different method for handling emails'
nl|'\n'
comment|'# of git notes in order for them to be of benefit for users,'
nl|'\n'
comment|'# which we simply do not have right now.'
nl|'\n'
indent|'        '
name|'return'
string|'"^refs/notes/"'
...
name|'staticmethod'
newline|'\n'
DECL|member|forbid_field_values
name|'def'
name|'forbid_field_values'
op|'('
name|'name'
op|','
name|'value'
...
op|'('
nl|'\n'
string|'\'"%s" is not an allowed setting for %s\''
op|'%'
op|'('
name|'value'
op|','
...
op|'('
nl|'\n'
string|"'*** Unknown value for multimailhook.commitEmailFormat: %s\\n'"
op|'%'
nl|'\n'
name|'commit_email_format'
op|'+'
nl|'\n'
string|'\'*** Expected either "text" or "html".  Ignoring.\\n\''
nl|'\n'
op|')'
newline|'\n'
dedent|''
...
op|']'
newline|'\n'
comment|'# Single "none" is a special value equivalen to empty string.'
nl|'\n'
name|'if'
name|'lines'
op|'=='
...
nl|'\n'
nl|'\n'
DECL|class|StaticRefFilterEnvironmentMixin
dedent|''
dedent|''
dedent|''
name|'class'
name|'StaticRefFilterEnvironmentMixin'
op|'('
name|'Environment'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Set branch filter statically based on constructor parameters."""'
newline|'\n'
nl|'\n'
DECL|member|__init__
name|'def'
...
name|'self'
op|','
name|'ref_filter_incl_regex'
op|','
name|'ref_filter_excl_regex'
op|','
nl|'\n'
name|'ref_filter_do_send_regex'
op|','
name|'ref_filter_dont_send_regex'
op|','
nl|'\n'
op|'**'
name|'kw'
...
op|'('
nl|'\n'
string|'"Cannot specify both a ref inclusion and exclusion regex."'
op|')'
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'__is_inclusion_filter'
op|'='
name|'bool'
op|'('
name|'ref_filter_incl_regex'
...
op|'('
nl|'\n'
string|'\'Invalid Ref Filter Regex "%s": %s\''
op|'%'
op|'('
name|'ref_filter_regex'
op|','
...
op|'('
nl|'\n'
string|'"Cannot specify both a ref doSend and dontSend regex."'
op|')'
newline|'\n'
dedent|''
name|'if'
...
newline|'\n'
nl|'\n'
DECL|member|get_ref_filter_regex
dedent|''
dedent|''
name|'def'
name|'get_ref_filter_regex'
op|'('
name|'self'
op|','
name|'send_filter'
...
dedent|''
name|'class'
name|'ConfigRefFilterEnvironmentMixin'
op|'('
nl|'\n'
name|'ConfigEnvironmentMixin'
op|','
...
name|'StaticRefFilterEnvironmentMixin'
nl|'\n'
DECL|class|ConfigRefFilterEnvironmentMixin
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Determine branch filtering statically based on config."""'
newline|'\n'
nl|'\n'
DECL|member|_get_regex
name|'def'
name|'_get_regex'
op|'('
name|'self'
...
name|'config'
op|','
string|"'refFilterInclusionRegex'"
op|')'
op|','
nl|'\n'
name|'ref_filter_excl_regex'
...
name|'config'
op|','
string|"'refFilterExclusionRegex'"
op|')'
op|','
nl|'\n'
name|'ref_filter_do_send_regex'
...
op|'.'
name|'__next__'
comment|'# Python 2 backward compatibility'
newline|'\n'
nl|'\n'
...
nl|'\n'
nl|'\n'
DECL|class|StashEnvironmentMixin
dedent|''
name|'class'
name|'StashEnvironmentMixin'
op|'('
name|'Environment'
op|')'
op|':'
...
nl|'\n'
nl|'\n'
DECL|class|GerritEnvironmentMixin
dedent|''
name|'class'
name|'GerritEnvironmentMixin'
op|'('
name|'Environment'
op|')'
op|':'
...
name|'self'
op|'.'
name|'__update_method'
op|'='
name|'update_method'
newline|'\n'
string|'"Make an \'update_method\' value available for templates."'
newline|'\n'
name|'self'
op|'.'
name|'COMPUTED_KEYS'
...
op|':'
newline|'\n'
comment|'# Submitter has a configured email, we transformed'
nl|'\n'
comment|'# __submitter into an RFC 2822 string already.'
nl|'\n'
indent|'                '
name|'return'
name|'re'
...
op|':'
newline|'\n'
comment|"# Submitter has no configured email, it's just his name."
nl|'\n'
indent|'                '
name|'return'
name|'self'
...
op|':'
newline|'\n'
comment|'# If we arrive here, this means someone pushed "Submit" from'
nl|'\n'
comment|'# the gerrit web UI for the CR (or used one of the programmatic'
nl|'\n'
comment|'# APIs to do the same, such as gerrit review) and the'
nl|'\n'
comment|'# merge/push was done by the Gerrit user.  It was technically'
nl|'\n'
comment|'# triggered by someone else, but sadly we have no way of'
nl|'\n'
comment|'# determining who that someone else is at this point.'
nl|'\n'
indent|'            '
name|'return'
string|"'Gerrit'"
...
name|'default'
op|'+'
string|"'|^refs/changes/|^refs/cache-automerge/|^refs/meta/'"
newline|'\n'
nl|'\n'
DECL|member|get_revision_recipients
dedent|''
...
op|':'
newline|'\n'
comment|'# Merge commits created by Gerrit when users hit "Submit this patchset"'
nl|'\n'
comment|'# in the Web UI (or do equivalently with REST APIs or the gerrit review'
nl|'\n'
comment|'# command) are not something users want to see an individual email for.'
nl|'\n'
...
newline|'\n'
nl|'\n'
DECL|member|get_update_method
dedent|''
dedent|''
name|'def'
name|'get_update_method'
op|'('
name|'self'
op|')'
op|':'
...
name|'else'
op|':'
comment|"# exclusion filter -- we include the ref if the regex doesn't match"
newline|'\n'
indent|'        '
name|'return'
name|'not'
...
nl|'\n'
nl|'\n'
DECL|function|compute_gerrit_options
dedent|''
name|'def'
name|'compute_gerrit_options'
op|'('
name|'options'
op|','
name|'args'
op|','
name|'required_gerrit_options'
op|')'
op|':'
newline|'\n'
indent|'    '
...
name|'SystemExit'
op|'('
string|'"Error: Specify all of --oldrev, --newrev, --refname, "'
nl|'\n'
string|'"and --project; or none of them."'
op|')'
newline|'\n'
nl|'\n'
dedent|''
...
name|'SystemExit'
op|'('
string|'"Non-gerrit environments incompatible with --oldrev, "'
nl|'\n'
string|'"--newrev, --refname, and --project"'
op|')'
newline|'\n'
dedent|''
name|'options'
...
name|'SystemExit'
op|'('
string|'"Error: Positional parameters not allowed with "'
nl|'\n'
string|'"--oldrev, --newrev, and --refname."'
op|')'
newline|'\n'
nl|'\n'
comment|"# Gerrit oddly omits 'refs/heads/' in the refname when calling"
nl|'\n'
comment|'# ref-updated hook; put it back.'
nl|'\n'
dedent|''
name|'git_dir'
op|'='
...
newline|'\n'
nl|'\n'
comment|'# Convert each string option unicode for Python3.'
nl|'\n'
dedent|''
name|'if'
name|'PYTHON3'
...
newline|'\n'
nl|'\n'
comment|'# New revisions can appear in a gerrit repository either due to someone'
nl|'\n'
comment|'# pushing directly (in which case options.submitter will be set), or they'
nl|'\n'
comment|'# can press "Submit this patchset" in the web UI for some CR (in which'
nl|'\n'
comment|'# case options.submitter will not be set and gerrit will not have provided'
nl|'\n'
comment|'# us the information about who pressed the button).'
nl|'\n'
comment|'#'
nl|'\n'
comment|"# Note for the nit-picky: I'm lumping in REST API calls and the ssh"
nl|'\n'
comment|'# gerrit review command in with "Submit this patchset" button, since they'
nl|'\n'
comment|'# have the same effect.'
nl|'\n'
dedent|''
dedent|''
dedent|''
...
string|"'pushed'"
newline|'\n'
comment|'# The submitter argument is almost an RFC 2822 email address; change it'
nl|'\n'
comment|"# from 'User Name (email@domain)' to 'User Name <email@domain>' so it is"
nl|'\n'
name|'options'
op|'.'
name|'submitter'
...
string|"'submitted'"
newline|'\n'
comment|'# Gerrit knew who submitted this patchset, but threw that information'
nl|'\n'
comment|'# away when it invoked this hook.  However, *IF* Gerrit created a'
nl|'\n'
comment|"# merge to bring the patchset in (project 'Submit Type' is either"
nl|'\n'
comment|'# "Always Merge", or is "Merge if Necessary" and happens to be'
nl|'\n'
comment|'# necessary for this particular CR), then it will have the committer'
nl|'\n'
comment|"# of that merge be 'Gerrit Code Review' and the author will be the"
nl|'\n'
comment|'# person who requested the submission of the CR.  Since this is fairly'
nl|'\n'
comment|"# likely for most gerrit installations (of a reasonable size), it's"
nl|'\n'
comment|'# worth the extra effort to try to determine the actual submitter.'
nl|'\n'
name|'rev_info'
op|'='
name|'read_git_lines'
...
newline|'\n'
nl|'\n'
comment|'# We pass back refname, oldrev, newrev as args because then the'
nl|'\n'
comment|'# gerrit ref-updated hook is much like the git update hook'
nl|'\n'
dedent|''
dedent|''
name|'return'
...
nl|'\n'
nl|'\n'
DECL|function|check_hook_specific_args
dedent|''
name|'def'
name|'check_hook_specific_args'
op|'('
name|'options'
op|','
name|'args'
...
op|':'
newline|'\n'
comment|'# First check for stash arguments'
nl|'\n'
indent|'    '
name|'if'
op|'('
...
name|'SystemExit'
op|'('
string|'"Error: Specify both of --stash-user and "'
nl|'\n'
string|'"--stash-repo or neither."'
op|')'
newline|'\n'
dedent|''
name|'if'
...
newline|'\n'
nl|'\n'
comment|'# Finally, check for gerrit specific arguments'
nl|'\n'
dedent|''
name|'required_gerrit_options'
op|'='
...
newline|'\n'
nl|'\n'
comment|'# No special options in use, just return what we started with'
nl|'\n'
dedent|''
name|'return'
name|'options'
...
op|'('
nl|'\n'
string|"'Pass a configuration parameter through to git.  The value given '"
nl|'\n'
string|"'will override values from configuration files.  See the -c option '"
nl|'\n'
string|"'of git(1) for more details.  (Only works with git >= 1.7.3)'"
nl|'\n'
op|')'
op|','
nl|'\n'
...
op|'('
nl|'\n'
string|'"Display git-multimail\'s version"'
nl|'\n'
op|')'
op|','
nl|'\n'
op|')'
newline|'\n'
comment|'# The following options permit this script to be run as a gerrit'
nl|'\n'
comment|'# ref-updated hook.  See e.g.'
nl|'\n'
comment|'# code.google.com/p/gerrit/source/browse/Documentation/config-hooks.txt'
nl|'\n'
comment|'# We suppress help for these items, since these are specific to gerrit,'
nl|'\n'
comment|"# and we don't want users directly using them any way other than how the"
nl|'\n'
comment|'# gerrit ref-updated hook is called.'
nl|'\n'
name|'parser'
op|'.'
name|'add_option'
...
newline|'\n'
nl|'\n'
comment|'# The following allow this to be run as a stash asynchronous post-receive'
nl|'\n'
comment|'# hook (almost identical to a git post-receive hook but triggered also for'
nl|'\n'
comment|'# merges of pull requests from the UI).  We suppress help for these items,'
nl|'\n'
comment|'# since these are specific to stash.'
nl|'\n'
name|'parser'
op|'.'
name|'add_option'
...
op|'+'
nl|'\n'
string|"'\\' raised. Please report this as a bug to\\n'"
op|')'
newline|'\n'
name|'sys'
op|'.'
...
name|'write'
op|'('
string|"'https://github.com/git-multimail/git-multimail/issues\\n'"
op|')'
newline|'\n'
name|'sys'
op|'.'
...
name|'write'
op|'('
string|"'with the information below:\\n\\n'"
op|')'
newline|'\n'
name|'sys'
op|'.'
