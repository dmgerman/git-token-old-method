...
end_comment
begin_function
DECL|function|do_compare_entry_piecewise
specifier|static
name|int
name|do_compare_entry_piecewise
parameter_list|(
specifier|const
name|struct
name|cache_entry
...
name|cmp
decl_stmt|;
comment|/* 	 * If we have not precomputed the traverse path, it is quicker 	 * to avoid doing so.  But if we have precomputed it, 	 * it is quicker to use the precomputed version. 	 */
if|if
condition|(
operator|!
name|info
...
condition|)
block|{
comment|/* 			 * Check if we can skip future cache checks 			 * (because we're already past all possible 			 * entries in the traverse path). 			 */
if|if
condition|(
name|info
operator|->
