begin_unit
begin_include
include|#
directive|include
file|"cache.h"
end_include
begin_include
include|#
directive|include
file|"dir.h"
end_include
begin_include
include|#
directive|include
file|"string-list.h"
end_include
begin_decl_stmt
DECL|variable|inside_git_dir
specifier|static
name|int
name|inside_git_dir
init|=
operator|-
literal|1
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|inside_work_tree
specifier|static
name|int
name|inside_work_tree
init|=
operator|-
literal|1
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|work_tree_config_is_bogus
specifier|static
name|int
name|work_tree_config_is_bogus
decl_stmt|;
end_decl_stmt
begin_comment
comment|/*  * The input parameter must contain an absolute path, and it must already be  * normalized.  *  * Find the part of an absolute path that lies inside the work tree by  * dereferencing symlinks outside the work tree, for example:  * /dir1/repo/dir2/file   (work tree is /dir1/repo)      -> dir2/file  * /dir/file              (work tree is /)               -> dir/file  * /dir/symlink1/symlink2 (symlink1 points to work tree) -> symlink2  * /dir/repolink/file     (repolink points to /dir/repo) -> file  * /dir/repo              (exactly equal to work tree)   -> (empty string)  */
end_comment
begin_function
DECL|function|abspath_part_inside_repo
specifier|static
name|int
name|abspath_part_inside_repo
parameter_list|(
name|char
modifier|*
name|path
parameter_list|)
block|{
name|size_t
name|len
decl_stmt|;
name|size_t
name|wtlen
decl_stmt|;
name|char
modifier|*
name|path0
decl_stmt|;
name|int
name|off
decl_stmt|;
specifier|const
name|char
modifier|*
name|work_tree
init|=
name|get_git_work_tree
argument_list|()
decl_stmt|;
if|if
condition|(
operator|!
name|work_tree
condition|)
return|return
operator|-
literal|1
return|;
name|wtlen
operator|=
name|strlen
argument_list|(
name|work_tree
argument_list|)
expr_stmt|;
name|len
operator|=
name|strlen
argument_list|(
name|path
argument_list|)
expr_stmt|;
name|off
operator|=
name|offset_1st_component
argument_list|(
name|path
argument_list|)
expr_stmt|;
comment|/* check if work tree is already the prefix */
if|if
condition|(
name|wtlen
operator|<=
name|len
operator|&&
operator|!
name|strncmp
argument_list|(
name|path
argument_list|,
name|work_tree
argument_list|,
name|wtlen
argument_list|)
condition|)
block|{
if|if
condition|(
name|path
index|[
name|wtlen
index|]
operator|==
literal|'/'
condition|)
block|{
name|memmove
argument_list|(
name|path
argument_list|,
name|path
operator|+
name|wtlen
operator|+
literal|1
argument_list|,
name|len
operator|-
name|wtlen
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
elseif|else
if|if
condition|(
name|path
index|[
name|wtlen
operator|-
literal|1
index|]
operator|==
literal|'/'
operator|||
name|path
index|[
name|wtlen
index|]
operator|==
literal|'\0'
condition|)
block|{
comment|/* work tree is the root, or the whole path */
name|memmove
argument_list|(
name|path
argument_list|,
name|path
operator|+
name|wtlen
argument_list|,
name|len
operator|-
name|wtlen
operator|+
literal|1
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
comment|/* work tree might match beginning of a symlink to work tree */
name|off
operator|=
name|wtlen
expr_stmt|;
block|}
name|path0
operator|=
name|path
expr_stmt|;
name|path
operator|+=
name|off
expr_stmt|;
comment|/* check each '/'-terminated level */
while|while
condition|(
operator|*
name|path
condition|)
block|{
name|path
operator|++
expr_stmt|;
if|if
condition|(
operator|*
name|path
operator|==
literal|'/'
condition|)
block|{
operator|*
name|path
operator|=
literal|'\0'
expr_stmt|;
if|if
condition|(
name|strcmp
argument_list|(
name|real_path
argument_list|(
name|path0
argument_list|)
argument_list|,
name|work_tree
argument_list|)
operator|==
literal|0
condition|)
block|{
name|memmove
argument_list|(
name|path0
argument_list|,
name|path
operator|+
literal|1
argument_list|,
name|len
operator|-
operator|(
name|path
operator|-
name|path0
operator|)
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
operator|*
name|path
operator|=
literal|'/'
expr_stmt|;
block|}
block|}
comment|/* check whole path */
if|if
condition|(
name|strcmp
argument_list|(
name|real_path
argument_list|(
name|path0
argument_list|)
argument_list|,
name|work_tree
argument_list|)
operator|==
literal|0
condition|)
block|{
operator|*
name|path0
operator|=
literal|'\0'
expr_stmt|;
return|return
literal|0
return|;
block|}
return|return
operator|-
literal|1
return|;
block|}
end_function
begin_comment
comment|/*  * Normalize "path", prepending the "prefix" for relative paths. If  * remaining_prefix is not NULL, return the actual prefix still  * remains in the path. For example, prefix = sub1/sub2/ and path is  *  *  foo          -> sub1/sub2/foo  (full prefix)  *  ../foo       -> sub1/foo       (remaining prefix is sub1/)  *  ../../bar    -> bar            (no remaining prefix)  *  ../../sub1/sub2/foo -> sub1/sub2/foo (but no remaining prefix)  *  `pwd`/../bar -> sub1/bar       (no remaining prefix)  */
end_comment
begin_function
DECL|function|prefix_path_gently
name|char
modifier|*
name|prefix_path_gently
parameter_list|(
specifier|const
name|char
modifier|*
name|prefix
parameter_list|,
name|int
name|len
parameter_list|,
name|int
modifier|*
name|remaining_prefix
parameter_list|,
specifier|const
name|char
modifier|*
name|path
parameter_list|)
block|{
specifier|const
name|char
modifier|*
name|orig
init|=
name|path
decl_stmt|;
name|char
modifier|*
name|sanitized
decl_stmt|;
if|if
condition|(
name|is_absolute_path
argument_list|(
name|orig
argument_list|)
condition|)
block|{
name|sanitized
operator|=
name|xmallocz
argument_list|(
name|strlen
argument_list|(
name|path
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|remaining_prefix
condition|)
operator|*
name|remaining_prefix
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|normalize_path_copy_len
argument_list|(
name|sanitized
argument_list|,
name|path
argument_list|,
name|remaining_prefix
argument_list|)
condition|)
block|{
name|free
argument_list|(
name|sanitized
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
if|if
condition|(
name|abspath_part_inside_repo
argument_list|(
name|sanitized
argument_list|)
condition|)
block|{
name|free
argument_list|(
name|sanitized
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
block|}
else|else
block|{
name|sanitized
operator|=
name|xstrfmt
argument_list|(
literal|"%.*s%s"
argument_list|,
name|len
argument_list|,
name|prefix
argument_list|,
name|path
argument_list|)
expr_stmt|;
if|if
condition|(
name|remaining_prefix
condition|)
operator|*
name|remaining_prefix
operator|=
name|len
expr_stmt|;
if|if
condition|(
name|normalize_path_copy_len
argument_list|(
name|sanitized
argument_list|,
name|sanitized
argument_list|,
name|remaining_prefix
argument_list|)
condition|)
block|{
name|free
argument_list|(
name|sanitized
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
block|}
return|return
name|sanitized
return|;
block|}
end_function
begin_function
DECL|function|prefix_path
name|char
modifier|*
name|prefix_path
parameter_list|(
specifier|const
name|char
modifier|*
name|prefix
parameter_list|,
name|int
name|len
parameter_list|,
specifier|const
name|char
modifier|*
name|path
parameter_list|)
block|{
name|char
modifier|*
name|r
init|=
name|prefix_path_gently
argument_list|(
name|prefix
argument_list|,
name|len
argument_list|,
name|NULL
argument_list|,
name|path
argument_list|)
decl_stmt|;
if|if
condition|(
operator|!
name|r
condition|)
name|die
argument_list|(
literal|"'%s' is outside repository"
argument_list|,
name|path
argument_list|)
expr_stmt|;
return|return
name|r
return|;
block|}
end_function
begin_function
DECL|function|path_inside_repo
name|int
name|path_inside_repo
parameter_list|(
specifier|const
name|char
modifier|*
name|prefix
parameter_list|,
specifier|const
name|char
modifier|*
name|path
parameter_list|)
block|{
name|int
name|len
init|=
name|prefix
condition|?
name|strlen
argument_list|(
name|prefix
argument_list|)
else|:
literal|0
decl_stmt|;
name|char
modifier|*
name|r
init|=
name|prefix_path_gently
argument_list|(
name|prefix
argument_list|,
name|len
argument_list|,
name|NULL
argument_list|,
name|path
argument_list|)
decl_stmt|;
if|if
condition|(
name|r
condition|)
block|{
name|free
argument_list|(
name|r
argument_list|)
expr_stmt|;
return|return
literal|1
return|;
block|}
return|return
literal|0
return|;
block|}
end_function
begin_function
DECL|function|check_filename
name|int
name|check_filename
parameter_list|(
specifier|const
name|char
modifier|*
name|prefix
parameter_list|,
specifier|const
name|char
modifier|*
name|arg
parameter_list|)
block|{
specifier|const
name|char
modifier|*
name|name
decl_stmt|;
name|struct
name|stat
name|st
decl_stmt|;
if|if
condition|(
name|starts_with
argument_list|(
name|arg
argument_list|,
literal|":/"
argument_list|)
condition|)
block|{
if|if
condition|(
name|arg
index|[
literal|2
index|]
operator|==
literal|'\0'
condition|)
comment|/* ":/" is root dir, always exists */
return|return
literal|1
return|;
name|name
operator|=
name|arg
operator|+
literal|2
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|prefix
condition|)
name|name
operator|=
name|prefix_filename
argument_list|(
name|prefix
argument_list|,
name|strlen
argument_list|(
name|prefix
argument_list|)
argument_list|,
name|arg
argument_list|)
expr_stmt|;
else|else
name|name
operator|=
name|arg
expr_stmt|;
if|if
condition|(
operator|!
name|lstat
argument_list|(
name|name
argument_list|,
operator|&
name|st
argument_list|)
condition|)
return|return
literal|1
return|;
comment|/* file exists */
if|if
condition|(
name|errno
operator|==
name|ENOENT
operator|||
name|errno
operator|==
name|ENOTDIR
condition|)
return|return
literal|0
return|;
comment|/* file does not exist */
name|die_errno
argument_list|(
literal|"failed to stat '%s'"
argument_list|,
name|arg
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|die_verify_filename
specifier|static
name|void
name|NORETURN
name|die_verify_filename
parameter_list|(
specifier|const
name|char
modifier|*
name|prefix
parameter_list|,
specifier|const
name|char
modifier|*
name|arg
parameter_list|,
name|int
name|diagnose_misspelt_rev
parameter_list|)
block|{
if|if
condition|(
operator|!
name|diagnose_misspelt_rev
condition|)
name|die
argument_list|(
literal|"%s: no such path in the working tree.\n"
literal|"Use 'git<command> --<path>...' to specify paths that do not exist locally."
argument_list|,
name|arg
argument_list|)
expr_stmt|;
comment|/* 	 * Saying "'(icase)foo' does not exist in the index" when the 	 * user gave us ":(icase)foo" is just stupid.  A magic pathspec 	 * begins with a colon and is followed by a non-alnum; do not 	 * let maybe_die_on_misspelt_object_name() even trigger. 	 */
if|if
condition|(
operator|!
operator|(
name|arg
index|[
literal|0
index|]
operator|==
literal|':'
operator|&&
operator|!
name|isalnum
argument_list|(
name|arg
index|[
literal|1
index|]
argument_list|)
operator|)
condition|)
name|maybe_die_on_misspelt_object_name
argument_list|(
name|arg
argument_list|,
name|prefix
argument_list|)
expr_stmt|;
comment|/* ... or fall back the most general message. */
name|die
argument_list|(
literal|"ambiguous argument '%s': unknown revision or path not in the working tree.\n"
literal|"Use '--' to separate paths from revisions, like this:\n"
literal|"'git<command> [<revision>...] -- [<file>...]'"
argument_list|,
name|arg
argument_list|)
expr_stmt|;
block|}
end_function
begin_comment
comment|/*  * Verify a filename that we got as an argument for a pathspec  * entry. Note that a filename that begins with "-" never verifies  * as true, because even if such a filename were to exist, we want  * it to be preceded by the "--" marker (or we want the user to  * use a format like "./-filename")  *  * The "diagnose_misspelt_rev" is used to provide a user-friendly  * diagnosis when dying upon finding that "name" is not a pathname.  * If set to 1, the diagnosis will try to diagnose "name" as an  * invalid object name (e.g. HEAD:foo). If set to 0, the diagnosis  * will only complain about an inexisting file.  *  * This function is typically called to check that a "file or rev"  * argument is unambiguous. In this case, the caller will want  * diagnose_misspelt_rev == 1 when verifying the first non-rev  * argument (which could have been a revision), and  * diagnose_misspelt_rev == 0 for the next ones (because we already  * saw a filename, there's not ambiguity anymore).  */
end_comment
begin_function
DECL|function|verify_filename
name|void
name|verify_filename
parameter_list|(
specifier|const
name|char
modifier|*
name|prefix
parameter_list|,
specifier|const
name|char
modifier|*
name|arg
parameter_list|,
name|int
name|diagnose_misspelt_rev
parameter_list|)
block|{
if|if
condition|(
operator|*
name|arg
operator|==
literal|'-'
condition|)
name|die
argument_list|(
literal|"bad flag '%s' used after filename"
argument_list|,
name|arg
argument_list|)
expr_stmt|;
if|if
condition|(
name|check_filename
argument_list|(
name|prefix
argument_list|,
name|arg
argument_list|)
operator|||
operator|!
name|no_wildcard
argument_list|(
name|arg
argument_list|)
condition|)
return|return;
name|die_verify_filename
argument_list|(
name|prefix
argument_list|,
name|arg
argument_list|,
name|diagnose_misspelt_rev
argument_list|)
expr_stmt|;
block|}
end_function
begin_comment
comment|/*  * Opposite of the above: the command line did not have -- marker  * and we parsed the arg as a refname.  It should not be interpretable  * as a filename.  */
end_comment
begin_function
DECL|function|verify_non_filename
name|void
name|verify_non_filename
parameter_list|(
specifier|const
name|char
modifier|*
name|prefix
parameter_list|,
specifier|const
name|char
modifier|*
name|arg
parameter_list|)
block|{
if|if
condition|(
operator|!
name|is_inside_work_tree
argument_list|()
operator|||
name|is_inside_git_dir
argument_list|()
condition|)
return|return;
if|if
condition|(
operator|*
name|arg
operator|==
literal|'-'
condition|)
return|return;
comment|/* flag */
if|if
condition|(
operator|!
name|check_filename
argument_list|(
name|prefix
argument_list|,
name|arg
argument_list|)
condition|)
return|return;
name|die
argument_list|(
literal|"ambiguous argument '%s': both revision and filename\n"
literal|"Use '--' to separate paths from revisions, like this:\n"
literal|"'git<command> [<revision>...] -- [<file>...]'"
argument_list|,
name|arg
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|get_common_dir
name|int
name|get_common_dir
parameter_list|(
name|struct
name|strbuf
modifier|*
name|sb
parameter_list|,
specifier|const
name|char
modifier|*
name|gitdir
parameter_list|)
block|{
specifier|const
name|char
modifier|*
name|git_env_common_dir
init|=
name|getenv
argument_list|(
name|GIT_COMMON_DIR_ENVIRONMENT
argument_list|)
decl_stmt|;
if|if
condition|(
name|git_env_common_dir
condition|)
block|{
name|strbuf_addstr
argument_list|(
name|sb
argument_list|,
name|git_env_common_dir
argument_list|)
expr_stmt|;
return|return
literal|1
return|;
block|}
else|else
block|{
return|return
name|get_common_dir_noenv
argument_list|(
name|sb
argument_list|,
name|gitdir
argument_list|)
return|;
block|}
block|}
end_function
begin_function
DECL|function|get_common_dir_noenv
name|int
name|get_common_dir_noenv
parameter_list|(
name|struct
name|strbuf
modifier|*
name|sb
parameter_list|,
specifier|const
name|char
modifier|*
name|gitdir
parameter_list|)
block|{
name|struct
name|strbuf
name|data
init|=
name|STRBUF_INIT
decl_stmt|;
name|struct
name|strbuf
name|path
init|=
name|STRBUF_INIT
decl_stmt|;
name|int
name|ret
init|=
literal|0
decl_stmt|;
name|strbuf_addf
argument_list|(
operator|&
name|path
argument_list|,
literal|"%s/commondir"
argument_list|,
name|gitdir
argument_list|)
expr_stmt|;
if|if
condition|(
name|file_exists
argument_list|(
name|path
operator|.
name|buf
argument_list|)
condition|)
block|{
if|if
condition|(
name|strbuf_read_file
argument_list|(
operator|&
name|data
argument_list|,
name|path
operator|.
name|buf
argument_list|,
literal|0
argument_list|)
operator|<=
literal|0
condition|)
name|die_errno
argument_list|(
name|_
argument_list|(
literal|"failed to read %s"
argument_list|)
argument_list|,
name|path
operator|.
name|buf
argument_list|)
expr_stmt|;
while|while
condition|(
name|data
operator|.
name|len
operator|&&
operator|(
name|data
operator|.
name|buf
index|[
name|data
operator|.
name|len
operator|-
literal|1
index|]
operator|==
literal|'\n'
operator|||
name|data
operator|.
name|buf
index|[
name|data
operator|.
name|len
operator|-
literal|1
index|]
operator|==
literal|'\r'
operator|)
condition|)
name|data
operator|.
name|len
operator|--
expr_stmt|;
name|data
operator|.
name|buf
index|[
name|data
operator|.
name|len
index|]
operator|=
literal|'\0'
expr_stmt|;
name|strbuf_reset
argument_list|(
operator|&
name|path
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|is_absolute_path
argument_list|(
name|data
operator|.
name|buf
argument_list|)
condition|)
name|strbuf_addf
argument_list|(
operator|&
name|path
argument_list|,
literal|"%s/"
argument_list|,
name|gitdir
argument_list|)
expr_stmt|;
name|strbuf_addbuf
argument_list|(
operator|&
name|path
argument_list|,
operator|&
name|data
argument_list|)
expr_stmt|;
name|strbuf_addstr
argument_list|(
name|sb
argument_list|,
name|real_path
argument_list|(
name|path
operator|.
name|buf
argument_list|)
argument_list|)
expr_stmt|;
name|ret
operator|=
literal|1
expr_stmt|;
block|}
else|else
name|strbuf_addstr
argument_list|(
name|sb
argument_list|,
name|gitdir
argument_list|)
expr_stmt|;
name|strbuf_release
argument_list|(
operator|&
name|data
argument_list|)
expr_stmt|;
name|strbuf_release
argument_list|(
operator|&
name|path
argument_list|)
expr_stmt|;
return|return
name|ret
return|;
block|}
end_function
begin_comment
comment|/*  * Test if it looks like we're at a git directory.  * We want to see:  *  *  - either an objects/ directory _or_ the proper  *    GIT_OBJECT_DIRECTORY environment variable  *  - a refs/ directory  *  - either a HEAD symlink or a HEAD file that is formatted as  *    a proper "ref:", or a regular file HEAD that has a properly  *    formatted sha1 object name.  */
end_comment
begin_function
DECL|function|is_git_directory
name|int
name|is_git_directory
parameter_list|(
specifier|const
name|char
modifier|*
name|suspect
parameter_list|)
block|{
name|struct
name|strbuf
name|path
init|=
name|STRBUF_INIT
decl_stmt|;
name|int
name|ret
init|=
literal|0
decl_stmt|;
name|size_t
name|len
decl_stmt|;
comment|/* Check worktree-related signatures */
name|strbuf_addf
argument_list|(
operator|&
name|path
argument_list|,
literal|"%s/HEAD"
argument_list|,
name|suspect
argument_list|)
expr_stmt|;
if|if
condition|(
name|validate_headref
argument_list|(
name|path
operator|.
name|buf
argument_list|)
condition|)
goto|goto
name|done
goto|;
name|strbuf_reset
argument_list|(
operator|&
name|path
argument_list|)
expr_stmt|;
name|get_common_dir
argument_list|(
operator|&
name|path
argument_list|,
name|suspect
argument_list|)
expr_stmt|;
name|len
operator|=
name|path
operator|.
name|len
expr_stmt|;
comment|/* Check non-worktree-related signatures */
if|if
condition|(
name|getenv
argument_list|(
name|DB_ENVIRONMENT
argument_list|)
condition|)
block|{
if|if
condition|(
name|access
argument_list|(
name|getenv
argument_list|(
name|DB_ENVIRONMENT
argument_list|)
argument_list|,
name|X_OK
argument_list|)
condition|)
goto|goto
name|done
goto|;
block|}
else|else
block|{
name|strbuf_setlen
argument_list|(
operator|&
name|path
argument_list|,
name|len
argument_list|)
expr_stmt|;
name|strbuf_addstr
argument_list|(
operator|&
name|path
argument_list|,
literal|"/objects"
argument_list|)
expr_stmt|;
if|if
condition|(
name|access
argument_list|(
name|path
operator|.
name|buf
argument_list|,
name|X_OK
argument_list|)
condition|)
goto|goto
name|done
goto|;
block|}
name|strbuf_setlen
argument_list|(
operator|&
name|path
argument_list|,
name|len
argument_list|)
expr_stmt|;
name|strbuf_addstr
argument_list|(
operator|&
name|path
argument_list|,
literal|"/refs"
argument_list|)
expr_stmt|;
if|if
condition|(
name|access
argument_list|(
name|path
operator|.
name|buf
argument_list|,
name|X_OK
argument_list|)
condition|)
goto|goto
name|done
goto|;
name|ret
operator|=
literal|1
expr_stmt|;
name|done
label|:
name|strbuf_release
argument_list|(
operator|&
name|path
argument_list|)
expr_stmt|;
return|return
name|ret
return|;
block|}
end_function
begin_function
DECL|function|is_nonbare_repository_dir
name|int
name|is_nonbare_repository_dir
parameter_list|(
name|struct
name|strbuf
modifier|*
name|path
parameter_list|)
block|{
name|int
name|ret
init|=
literal|0
decl_stmt|;
name|int
name|gitfile_error
decl_stmt|;
name|size_t
name|orig_path_len
init|=
name|path
operator|->
name|len
decl_stmt|;
name|assert
argument_list|(
name|orig_path_len
operator|!=
literal|0
argument_list|)
expr_stmt|;
name|strbuf_complete
argument_list|(
name|path
argument_list|,
literal|'/'
argument_list|)
expr_stmt|;
name|strbuf_addstr
argument_list|(
name|path
argument_list|,
literal|".git"
argument_list|)
expr_stmt|;
if|if
condition|(
name|read_gitfile_gently
argument_list|(
name|path
operator|->
name|buf
argument_list|,
operator|&
name|gitfile_error
argument_list|)
operator|||
name|is_git_directory
argument_list|(
name|path
operator|->
name|buf
argument_list|)
condition|)
name|ret
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|gitfile_error
operator|==
name|READ_GITFILE_ERR_OPEN_FAILED
operator|||
name|gitfile_error
operator|==
name|READ_GITFILE_ERR_READ_FAILED
condition|)
name|ret
operator|=
literal|1
expr_stmt|;
name|strbuf_setlen
argument_list|(
name|path
argument_list|,
name|orig_path_len
argument_list|)
expr_stmt|;
return|return
name|ret
return|;
block|}
end_function
begin_function
DECL|function|is_inside_git_dir
name|int
name|is_inside_git_dir
parameter_list|(
name|void
parameter_list|)
block|{
if|if
condition|(
name|inside_git_dir
operator|<
literal|0
condition|)
name|inside_git_dir
operator|=
name|is_inside_dir
argument_list|(
name|get_git_dir
argument_list|()
argument_list|)
expr_stmt|;
return|return
name|inside_git_dir
return|;
block|}
end_function
begin_function
DECL|function|is_inside_work_tree
name|int
name|is_inside_work_tree
parameter_list|(
name|void
parameter_list|)
block|{
if|if
condition|(
name|inside_work_tree
operator|<
literal|0
condition|)
name|inside_work_tree
operator|=
name|is_inside_dir
argument_list|(
name|get_git_work_tree
argument_list|()
argument_list|)
expr_stmt|;
return|return
name|inside_work_tree
return|;
block|}
end_function
begin_function
DECL|function|setup_work_tree
name|void
name|setup_work_tree
parameter_list|(
name|void
parameter_list|)
block|{
specifier|const
name|char
modifier|*
name|work_tree
decl_stmt|,
modifier|*
name|git_dir
decl_stmt|;
specifier|static
name|int
name|initialized
init|=
literal|0
decl_stmt|;
if|if
condition|(
name|initialized
condition|)
return|return;
if|if
condition|(
name|work_tree_config_is_bogus
condition|)
name|die
argument_list|(
literal|"unable to set up work tree using invalid config"
argument_list|)
expr_stmt|;
name|work_tree
operator|=
name|get_git_work_tree
argument_list|()
expr_stmt|;
name|git_dir
operator|=
name|get_git_dir
argument_list|()
expr_stmt|;
if|if
condition|(
operator|!
name|is_absolute_path
argument_list|(
name|git_dir
argument_list|)
condition|)
name|git_dir
operator|=
name|real_path
argument_list|(
name|get_git_dir
argument_list|()
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|work_tree
operator|||
name|chdir
argument_list|(
name|work_tree
argument_list|)
condition|)
name|die
argument_list|(
literal|"This operation must be run in a work tree"
argument_list|)
expr_stmt|;
comment|/* 	 * Make sure subsequent git processes find correct worktree 	 * if $GIT_WORK_TREE is set relative 	 */
if|if
condition|(
name|getenv
argument_list|(
name|GIT_WORK_TREE_ENVIRONMENT
argument_list|)
condition|)
name|setenv
argument_list|(
name|GIT_WORK_TREE_ENVIRONMENT
argument_list|,
literal|"."
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|set_git_dir
argument_list|(
name|remove_leading_path
argument_list|(
name|git_dir
argument_list|,
name|work_tree
argument_list|)
argument_list|)
expr_stmt|;
name|initialized
operator|=
literal|1
expr_stmt|;
block|}
end_function
begin_function
DECL|function|check_repo_format
specifier|static
name|int
name|check_repo_format
parameter_list|(
specifier|const
name|char
modifier|*
name|var
parameter_list|,
specifier|const
name|char
modifier|*
name|value
parameter_list|,
name|void
modifier|*
name|vdata
parameter_list|)
block|{
name|struct
name|repository_format
modifier|*
name|data
init|=
name|vdata
decl_stmt|;
specifier|const
name|char
modifier|*
name|ext
decl_stmt|;
if|if
condition|(
name|strcmp
argument_list|(
name|var
argument_list|,
literal|"core.repositoryformatversion"
argument_list|)
operator|==
literal|0
condition|)
name|data
operator|->
name|version
operator|=
name|git_config_int
argument_list|(
name|var
argument_list|,
name|value
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|skip_prefix
argument_list|(
name|var
argument_list|,
literal|"extensions."
argument_list|,
operator|&
name|ext
argument_list|)
condition|)
block|{
comment|/* 		 * record any known extensions here; otherwise, 		 * we fall through to recording it as unknown, and 		 * check_repository_format will complain 		 */
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|ext
argument_list|,
literal|"noop"
argument_list|)
condition|)
empty_stmt|;
elseif|else
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|ext
argument_list|,
literal|"preciousobjects"
argument_list|)
condition|)
name|data
operator|->
name|precious_objects
operator|=
name|git_config_bool
argument_list|(
name|var
argument_list|,
name|value
argument_list|)
expr_stmt|;
else|else
name|string_list_append
argument_list|(
operator|&
name|data
operator|->
name|unknown_extensions
argument_list|,
name|ext
argument_list|)
expr_stmt|;
block|}
return|return
literal|0
return|;
block|}
end_function
begin_function_decl
specifier|static
name|int
name|read_repository_format_1
parameter_list|(
name|struct
name|repository_format
modifier|*
parameter_list|,
name|config_fn_t
parameter_list|,
specifier|const
name|char
modifier|*
parameter_list|)
function_decl|;
end_function_decl
begin_function
DECL|function|check_repository_format_gently
specifier|static
name|int
name|check_repository_format_gently
parameter_list|(
specifier|const
name|char
modifier|*
name|gitdir
parameter_list|,
name|int
modifier|*
name|nongit_ok
parameter_list|)
block|{
name|struct
name|strbuf
name|sb
init|=
name|STRBUF_INIT
decl_stmt|;
name|struct
name|strbuf
name|err
init|=
name|STRBUF_INIT
decl_stmt|;
name|struct
name|repository_format
name|candidate
decl_stmt|;
name|config_fn_t
name|fn
decl_stmt|;
if|if
condition|(
name|get_common_dir
argument_list|(
operator|&
name|sb
argument_list|,
name|gitdir
argument_list|)
condition|)
name|fn
operator|=
name|check_repo_format
expr_stmt|;
else|else
name|fn
operator|=
name|check_repository_format_version
expr_stmt|;
name|strbuf_addstr
argument_list|(
operator|&
name|sb
argument_list|,
literal|"/config"
argument_list|)
expr_stmt|;
name|read_repository_format_1
argument_list|(
operator|&
name|candidate
argument_list|,
name|fn
argument_list|,
name|sb
operator|.
name|buf
argument_list|)
expr_stmt|;
name|strbuf_release
argument_list|(
operator|&
name|sb
argument_list|)
expr_stmt|;
comment|/* 	 * For historical use of check_repository_format() in git-init, 	 * we treat a missing config as a silent "ok", even when nongit_ok 	 * is unset. 	 */
if|if
condition|(
name|candidate
operator|.
name|version
operator|<
literal|0
condition|)
return|return
literal|0
return|;
if|if
condition|(
name|verify_repository_format
argument_list|(
operator|&
name|candidate
argument_list|,
operator|&
name|err
argument_list|)
operator|<
literal|0
condition|)
block|{
if|if
condition|(
name|nongit_ok
condition|)
block|{
name|warning
argument_list|(
literal|"%s"
argument_list|,
name|err
operator|.
name|buf
argument_list|)
expr_stmt|;
name|strbuf_release
argument_list|(
operator|&
name|err
argument_list|)
expr_stmt|;
operator|*
name|nongit_ok
operator|=
operator|-
literal|1
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
name|die
argument_list|(
literal|"%s"
argument_list|,
name|err
operator|.
name|buf
argument_list|)
expr_stmt|;
block|}
name|repository_format_version
operator|=
name|candidate
operator|.
name|version
expr_stmt|;
name|repository_format_precious_objects
operator|=
name|candidate
operator|.
name|precious_objects
expr_stmt|;
name|string_list_clear
argument_list|(
operator|&
name|candidate
operator|.
name|unknown_extensions
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|candidate
operator|.
name|is_bare
operator|!=
operator|-
literal|1
condition|)
block|{
name|is_bare_repository_cfg
operator|=
name|candidate
operator|.
name|is_bare
expr_stmt|;
if|if
condition|(
name|is_bare_repository_cfg
operator|==
literal|1
condition|)
name|inside_work_tree
operator|=
operator|-
literal|1
expr_stmt|;
block|}
if|if
condition|(
name|candidate
operator|.
name|work_tree
condition|)
block|{
name|free
argument_list|(
name|git_work_tree_cfg
argument_list|)
expr_stmt|;
name|git_work_tree_cfg
operator|=
name|candidate
operator|.
name|work_tree
expr_stmt|;
name|inside_work_tree
operator|=
operator|-
literal|1
expr_stmt|;
block|}
return|return
literal|0
return|;
block|}
end_function
begin_function
DECL|function|read_repository_format_1
specifier|static
name|int
name|read_repository_format_1
parameter_list|(
name|struct
name|repository_format
modifier|*
name|format
parameter_list|,
name|config_fn_t
name|fn
parameter_list|,
specifier|const
name|char
modifier|*
name|path
parameter_list|)
block|{
name|memset
argument_list|(
name|format
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|format
argument_list|)
argument_list|)
expr_stmt|;
name|format
operator|->
name|version
operator|=
operator|-
literal|1
expr_stmt|;
name|format
operator|->
name|is_bare
operator|=
operator|-
literal|1
expr_stmt|;
name|string_list_init
argument_list|(
operator|&
name|format
operator|->
name|unknown_extensions
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|git_config_from_file
argument_list|(
name|fn
argument_list|,
name|path
argument_list|,
name|format
argument_list|)
expr_stmt|;
return|return
name|format
operator|->
name|version
return|;
block|}
end_function
begin_function
DECL|function|read_repository_format
name|int
name|read_repository_format
parameter_list|(
name|struct
name|repository_format
modifier|*
name|format
parameter_list|,
specifier|const
name|char
modifier|*
name|path
parameter_list|)
block|{
return|return
name|read_repository_format_1
argument_list|(
name|format
argument_list|,
name|check_repository_format_version
argument_list|,
name|path
argument_list|)
return|;
block|}
end_function
begin_function
DECL|function|verify_repository_format
name|int
name|verify_repository_format
parameter_list|(
specifier|const
name|struct
name|repository_format
modifier|*
name|format
parameter_list|,
name|struct
name|strbuf
modifier|*
name|err
parameter_list|)
block|{
if|if
condition|(
name|GIT_REPO_VERSION_READ
operator|<
name|format
operator|->
name|version
condition|)
block|{
name|strbuf_addf
argument_list|(
name|err
argument_list|,
literal|"Expected git repo version<= %d, found %d"
argument_list|,
name|GIT_REPO_VERSION_READ
argument_list|,
name|format
operator|->
name|version
argument_list|)
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
if|if
condition|(
name|format
operator|->
name|version
operator|>=
literal|1
operator|&&
name|format
operator|->
name|unknown_extensions
operator|.
name|nr
condition|)
block|{
name|int
name|i
decl_stmt|;
name|strbuf_addstr
argument_list|(
name|err
argument_list|,
literal|"unknown repository extensions found:"
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|format
operator|->
name|unknown_extensions
operator|.
name|nr
condition|;
name|i
operator|++
control|)
name|strbuf_addf
argument_list|(
name|err
argument_list|,
literal|"\n\t%s"
argument_list|,
name|format
operator|->
name|unknown_extensions
operator|.
name|items
index|[
name|i
index|]
operator|.
name|string
argument_list|)
expr_stmt|;
return|return
operator|-
literal|1
return|;
block|}
return|return
literal|0
return|;
block|}
end_function
begin_comment
comment|/*  * Try to read the location of the git directory from the .git file,  * return path to git directory if found.  *  * On failure, if return_error_code is not NULL, return_error_code  * will be set to an error code and NULL will be returned. If  * return_error_code is NULL the function will die instead (for most  * cases).  */
end_comment
begin_function
DECL|function|read_gitfile_gently
specifier|const
name|char
modifier|*
name|read_gitfile_gently
parameter_list|(
specifier|const
name|char
modifier|*
name|path
parameter_list|,
name|int
modifier|*
name|return_error_code
parameter_list|)
block|{
specifier|const
name|int
name|max_file_size
init|=
literal|1
operator|<<
literal|20
decl_stmt|;
comment|/* 1MB */
name|int
name|error_code
init|=
literal|0
decl_stmt|;
name|char
modifier|*
name|buf
init|=
name|NULL
decl_stmt|;
name|char
modifier|*
name|dir
init|=
name|NULL
decl_stmt|;
specifier|const
name|char
modifier|*
name|slash
decl_stmt|;
name|struct
name|stat
name|st
decl_stmt|;
name|int
name|fd
decl_stmt|;
name|ssize_t
name|len
decl_stmt|;
if|if
condition|(
name|stat
argument_list|(
name|path
argument_list|,
operator|&
name|st
argument_list|)
condition|)
block|{
name|error_code
operator|=
name|READ_GITFILE_ERR_STAT_FAILED
expr_stmt|;
goto|goto
name|cleanup_return
goto|;
block|}
if|if
condition|(
operator|!
name|S_ISREG
argument_list|(
name|st
operator|.
name|st_mode
argument_list|)
condition|)
block|{
name|error_code
operator|=
name|READ_GITFILE_ERR_NOT_A_FILE
expr_stmt|;
goto|goto
name|cleanup_return
goto|;
block|}
if|if
condition|(
name|st
operator|.
name|st_size
operator|>
name|max_file_size
condition|)
block|{
name|error_code
operator|=
name|READ_GITFILE_ERR_TOO_LARGE
expr_stmt|;
goto|goto
name|cleanup_return
goto|;
block|}
name|fd
operator|=
name|open
argument_list|(
name|path
argument_list|,
name|O_RDONLY
argument_list|)
expr_stmt|;
if|if
condition|(
name|fd
operator|<
literal|0
condition|)
block|{
name|error_code
operator|=
name|READ_GITFILE_ERR_OPEN_FAILED
expr_stmt|;
goto|goto
name|cleanup_return
goto|;
block|}
name|buf
operator|=
name|xmallocz
argument_list|(
name|st
operator|.
name|st_size
argument_list|)
expr_stmt|;
name|len
operator|=
name|read_in_full
argument_list|(
name|fd
argument_list|,
name|buf
argument_list|,
name|st
operator|.
name|st_size
argument_list|)
expr_stmt|;
name|close
argument_list|(
name|fd
argument_list|)
expr_stmt|;
if|if
condition|(
name|len
operator|!=
name|st
operator|.
name|st_size
condition|)
block|{
name|error_code
operator|=
name|READ_GITFILE_ERR_READ_FAILED
expr_stmt|;
goto|goto
name|cleanup_return
goto|;
block|}
if|if
condition|(
operator|!
name|starts_with
argument_list|(
name|buf
argument_list|,
literal|"gitdir: "
argument_list|)
condition|)
block|{
name|error_code
operator|=
name|READ_GITFILE_ERR_INVALID_FORMAT
expr_stmt|;
goto|goto
name|cleanup_return
goto|;
block|}
while|while
condition|(
name|buf
index|[
name|len
operator|-
literal|1
index|]
operator|==
literal|'\n'
operator|||
name|buf
index|[
name|len
operator|-
literal|1
index|]
operator|==
literal|'\r'
condition|)
name|len
operator|--
expr_stmt|;
if|if
condition|(
name|len
operator|<
literal|9
condition|)
block|{
name|error_code
operator|=
name|READ_GITFILE_ERR_NO_PATH
expr_stmt|;
goto|goto
name|cleanup_return
goto|;
block|}
name|buf
index|[
name|len
index|]
operator|=
literal|'\0'
expr_stmt|;
name|dir
operator|=
name|buf
operator|+
literal|8
expr_stmt|;
if|if
condition|(
operator|!
name|is_absolute_path
argument_list|(
name|dir
argument_list|)
operator|&&
operator|(
name|slash
operator|=
name|strrchr
argument_list|(
name|path
argument_list|,
literal|'/'
argument_list|)
operator|)
condition|)
block|{
name|size_t
name|pathlen
init|=
name|slash
operator|+
literal|1
operator|-
name|path
decl_stmt|;
name|dir
operator|=
name|xstrfmt
argument_list|(
literal|"%.*s%.*s"
argument_list|,
operator|(
name|int
operator|)
name|pathlen
argument_list|,
name|path
argument_list|,
call|(
name|int
call|)
argument_list|(
name|len
operator|-
literal|8
argument_list|)
argument_list|,
name|buf
operator|+
literal|8
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|buf
argument_list|)
expr_stmt|;
name|buf
operator|=
name|dir
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|is_git_directory
argument_list|(
name|dir
argument_list|)
condition|)
block|{
name|error_code
operator|=
name|READ_GITFILE_ERR_NOT_A_REPO
expr_stmt|;
goto|goto
name|cleanup_return
goto|;
block|}
name|path
operator|=
name|real_path
argument_list|(
name|dir
argument_list|)
expr_stmt|;
name|cleanup_return
label|:
if|if
condition|(
name|return_error_code
condition|)
operator|*
name|return_error_code
operator|=
name|error_code
expr_stmt|;
elseif|else
if|if
condition|(
name|error_code
condition|)
block|{
switch|switch
condition|(
name|error_code
condition|)
block|{
case|case
name|READ_GITFILE_ERR_STAT_FAILED
case|:
case|case
name|READ_GITFILE_ERR_NOT_A_FILE
case|:
comment|/* non-fatal; follow return path */
break|break;
case|case
name|READ_GITFILE_ERR_OPEN_FAILED
case|:
name|die_errno
argument_list|(
literal|"Error opening '%s'"
argument_list|,
name|path
argument_list|)
expr_stmt|;
case|case
name|READ_GITFILE_ERR_TOO_LARGE
case|:
name|die
argument_list|(
literal|"Too large to be a .git file: '%s'"
argument_list|,
name|path
argument_list|)
expr_stmt|;
case|case
name|READ_GITFILE_ERR_READ_FAILED
case|:
name|die
argument_list|(
literal|"Error reading %s"
argument_list|,
name|path
argument_list|)
expr_stmt|;
case|case
name|READ_GITFILE_ERR_INVALID_FORMAT
case|:
name|die
argument_list|(
literal|"Invalid gitfile format: %s"
argument_list|,
name|path
argument_list|)
expr_stmt|;
case|case
name|READ_GITFILE_ERR_NO_PATH
case|:
name|die
argument_list|(
literal|"No path in gitfile: %s"
argument_list|,
name|path
argument_list|)
expr_stmt|;
case|case
name|READ_GITFILE_ERR_NOT_A_REPO
case|:
name|die
argument_list|(
literal|"Not a git repository: %s"
argument_list|,
name|dir
argument_list|)
expr_stmt|;
default|default:
name|assert
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
block|}
name|free
argument_list|(
name|buf
argument_list|)
expr_stmt|;
return|return
name|error_code
condition|?
name|NULL
else|:
name|path
return|;
block|}
end_function
begin_function
DECL|function|setup_explicit_git_dir
specifier|static
specifier|const
name|char
modifier|*
name|setup_explicit_git_dir
parameter_list|(
specifier|const
name|char
modifier|*
name|gitdirenv
parameter_list|,
name|struct
name|strbuf
modifier|*
name|cwd
parameter_list|,
name|int
modifier|*
name|nongit_ok
parameter_list|)
block|{
specifier|const
name|char
modifier|*
name|work_tree_env
init|=
name|getenv
argument_list|(
name|GIT_WORK_TREE_ENVIRONMENT
argument_list|)
decl_stmt|;
specifier|const
name|char
modifier|*
name|worktree
decl_stmt|;
name|char
modifier|*
name|gitfile
decl_stmt|;
name|int
name|offset
decl_stmt|;
if|if
condition|(
name|PATH_MAX
operator|-
literal|40
operator|<
name|strlen
argument_list|(
name|gitdirenv
argument_list|)
condition|)
name|die
argument_list|(
literal|"'$%s' too big"
argument_list|,
name|GIT_DIR_ENVIRONMENT
argument_list|)
expr_stmt|;
name|gitfile
operator|=
operator|(
name|char
operator|*
operator|)
name|read_gitfile
argument_list|(
name|gitdirenv
argument_list|)
expr_stmt|;
if|if
condition|(
name|gitfile
condition|)
block|{
name|gitfile
operator|=
name|xstrdup
argument_list|(
name|gitfile
argument_list|)
expr_stmt|;
name|gitdirenv
operator|=
name|gitfile
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|is_git_directory
argument_list|(
name|gitdirenv
argument_list|)
condition|)
block|{
if|if
condition|(
name|nongit_ok
condition|)
block|{
operator|*
name|nongit_ok
operator|=
literal|1
expr_stmt|;
name|free
argument_list|(
name|gitfile
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
name|die
argument_list|(
literal|"Not a git repository: '%s'"
argument_list|,
name|gitdirenv
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|check_repository_format_gently
argument_list|(
name|gitdirenv
argument_list|,
name|nongit_ok
argument_list|)
condition|)
block|{
name|free
argument_list|(
name|gitfile
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
comment|/* #3, #7, #11, #15, #19, #23, #27, #31 (see t1510) */
if|if
condition|(
name|work_tree_env
condition|)
name|set_git_work_tree
argument_list|(
name|work_tree_env
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|is_bare_repository_cfg
operator|>
literal|0
condition|)
block|{
if|if
condition|(
name|git_work_tree_cfg
condition|)
block|{
comment|/* #22.2, #30 */
name|warning
argument_list|(
literal|"core.bare and core.worktree do not make sense"
argument_list|)
expr_stmt|;
name|work_tree_config_is_bogus
operator|=
literal|1
expr_stmt|;
block|}
comment|/* #18, #26 */
name|set_git_dir
argument_list|(
name|gitdirenv
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|gitfile
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
elseif|else
if|if
condition|(
name|git_work_tree_cfg
condition|)
block|{
comment|/* #6, #14 */
if|if
condition|(
name|is_absolute_path
argument_list|(
name|git_work_tree_cfg
argument_list|)
condition|)
name|set_git_work_tree
argument_list|(
name|git_work_tree_cfg
argument_list|)
expr_stmt|;
else|else
block|{
name|char
modifier|*
name|core_worktree
decl_stmt|;
if|if
condition|(
name|chdir
argument_list|(
name|gitdirenv
argument_list|)
condition|)
name|die_errno
argument_list|(
literal|"Could not chdir to '%s'"
argument_list|,
name|gitdirenv
argument_list|)
expr_stmt|;
if|if
condition|(
name|chdir
argument_list|(
name|git_work_tree_cfg
argument_list|)
condition|)
name|die_errno
argument_list|(
literal|"Could not chdir to '%s'"
argument_list|,
name|git_work_tree_cfg
argument_list|)
expr_stmt|;
name|core_worktree
operator|=
name|xgetcwd
argument_list|()
expr_stmt|;
if|if
condition|(
name|chdir
argument_list|(
name|cwd
operator|->
name|buf
argument_list|)
condition|)
name|die_errno
argument_list|(
literal|"Could not come back to cwd"
argument_list|)
expr_stmt|;
name|set_git_work_tree
argument_list|(
name|core_worktree
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|core_worktree
argument_list|)
expr_stmt|;
block|}
block|}
elseif|else
if|if
condition|(
operator|!
name|git_env_bool
argument_list|(
name|GIT_IMPLICIT_WORK_TREE_ENVIRONMENT
argument_list|,
literal|1
argument_list|)
condition|)
block|{
comment|/* #16d */
name|set_git_dir
argument_list|(
name|gitdirenv
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|gitfile
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
else|else
comment|/* #2, #10 */
name|set_git_work_tree
argument_list|(
literal|"."
argument_list|)
expr_stmt|;
comment|/* set_git_work_tree() must have been called by now */
name|worktree
operator|=
name|get_git_work_tree
argument_list|()
expr_stmt|;
comment|/* both get_git_work_tree() and cwd are already normalized */
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|cwd
operator|->
name|buf
argument_list|,
name|worktree
argument_list|)
condition|)
block|{
comment|/* cwd == worktree */
name|set_git_dir
argument_list|(
name|gitdirenv
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|gitfile
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
name|offset
operator|=
name|dir_inside_of
argument_list|(
name|cwd
operator|->
name|buf
argument_list|,
name|worktree
argument_list|)
expr_stmt|;
if|if
condition|(
name|offset
operator|>=
literal|0
condition|)
block|{
comment|/* cwd inside worktree? */
name|set_git_dir
argument_list|(
name|real_path
argument_list|(
name|gitdirenv
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|chdir
argument_list|(
name|worktree
argument_list|)
condition|)
name|die_errno
argument_list|(
literal|"Could not chdir to '%s'"
argument_list|,
name|worktree
argument_list|)
expr_stmt|;
name|strbuf_addch
argument_list|(
name|cwd
argument_list|,
literal|'/'
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|gitfile
argument_list|)
expr_stmt|;
return|return
name|cwd
operator|->
name|buf
operator|+
name|offset
return|;
block|}
comment|/* cwd outside worktree */
name|set_git_dir
argument_list|(
name|gitdirenv
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|gitfile
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
end_function
begin_function
DECL|function|setup_discovered_git_dir
specifier|static
specifier|const
name|char
modifier|*
name|setup_discovered_git_dir
parameter_list|(
specifier|const
name|char
modifier|*
name|gitdir
parameter_list|,
name|struct
name|strbuf
modifier|*
name|cwd
parameter_list|,
name|int
name|offset
parameter_list|,
name|int
modifier|*
name|nongit_ok
parameter_list|)
block|{
if|if
condition|(
name|check_repository_format_gently
argument_list|(
name|gitdir
argument_list|,
name|nongit_ok
argument_list|)
condition|)
return|return
name|NULL
return|;
comment|/* --work-tree is set without --git-dir; use discovered one */
if|if
condition|(
name|getenv
argument_list|(
name|GIT_WORK_TREE_ENVIRONMENT
argument_list|)
operator|||
name|git_work_tree_cfg
condition|)
block|{
if|if
condition|(
name|offset
operator|!=
name|cwd
operator|->
name|len
operator|&&
operator|!
name|is_absolute_path
argument_list|(
name|gitdir
argument_list|)
condition|)
name|gitdir
operator|=
name|xstrdup
argument_list|(
name|real_path
argument_list|(
name|gitdir
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|chdir
argument_list|(
name|cwd
operator|->
name|buf
argument_list|)
condition|)
name|die_errno
argument_list|(
literal|"Could not come back to cwd"
argument_list|)
expr_stmt|;
return|return
name|setup_explicit_git_dir
argument_list|(
name|gitdir
argument_list|,
name|cwd
argument_list|,
name|nongit_ok
argument_list|)
return|;
block|}
comment|/* #16.2, #17.2, #20.2, #21.2, #24, #25, #28, #29 (see t1510) */
if|if
condition|(
name|is_bare_repository_cfg
operator|>
literal|0
condition|)
block|{
name|set_git_dir
argument_list|(
name|offset
operator|==
name|cwd
operator|->
name|len
condition|?
name|gitdir
else|:
name|real_path
argument_list|(
name|gitdir
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|chdir
argument_list|(
name|cwd
operator|->
name|buf
argument_list|)
condition|)
name|die_errno
argument_list|(
literal|"Could not come back to cwd"
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
comment|/* #0, #1, #5, #8, #9, #12, #13 */
name|set_git_work_tree
argument_list|(
literal|"."
argument_list|)
expr_stmt|;
if|if
condition|(
name|strcmp
argument_list|(
name|gitdir
argument_list|,
name|DEFAULT_GIT_DIR_ENVIRONMENT
argument_list|)
condition|)
name|set_git_dir
argument_list|(
name|gitdir
argument_list|)
expr_stmt|;
name|inside_git_dir
operator|=
literal|0
expr_stmt|;
name|inside_work_tree
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|offset
operator|==
name|cwd
operator|->
name|len
condition|)
return|return
name|NULL
return|;
comment|/* Make "offset" point to past the '/', and add a '/' at the end */
name|offset
operator|++
expr_stmt|;
name|strbuf_addch
argument_list|(
name|cwd
argument_list|,
literal|'/'
argument_list|)
expr_stmt|;
return|return
name|cwd
operator|->
name|buf
operator|+
name|offset
return|;
block|}
end_function
begin_comment
comment|/* #16.1, #17.1, #20.1, #21.1, #22.1 (see t1510) */
end_comment
begin_function
DECL|function|setup_bare_git_dir
specifier|static
specifier|const
name|char
modifier|*
name|setup_bare_git_dir
parameter_list|(
name|struct
name|strbuf
modifier|*
name|cwd
parameter_list|,
name|int
name|offset
parameter_list|,
name|int
modifier|*
name|nongit_ok
parameter_list|)
block|{
name|int
name|root_len
decl_stmt|;
if|if
condition|(
name|check_repository_format_gently
argument_list|(
literal|"."
argument_list|,
name|nongit_ok
argument_list|)
condition|)
return|return
name|NULL
return|;
name|setenv
argument_list|(
name|GIT_IMPLICIT_WORK_TREE_ENVIRONMENT
argument_list|,
literal|"0"
argument_list|,
literal|1
argument_list|)
expr_stmt|;
comment|/* --work-tree is set without --git-dir; use discovered one */
if|if
condition|(
name|getenv
argument_list|(
name|GIT_WORK_TREE_ENVIRONMENT
argument_list|)
operator|||
name|git_work_tree_cfg
condition|)
block|{
specifier|const
name|char
modifier|*
name|gitdir
decl_stmt|;
name|gitdir
operator|=
name|offset
operator|==
name|cwd
operator|->
name|len
condition|?
literal|"."
else|:
name|xmemdupz
argument_list|(
name|cwd
operator|->
name|buf
argument_list|,
name|offset
argument_list|)
expr_stmt|;
if|if
condition|(
name|chdir
argument_list|(
name|cwd
operator|->
name|buf
argument_list|)
condition|)
name|die_errno
argument_list|(
literal|"Could not come back to cwd"
argument_list|)
expr_stmt|;
return|return
name|setup_explicit_git_dir
argument_list|(
name|gitdir
argument_list|,
name|cwd
argument_list|,
name|nongit_ok
argument_list|)
return|;
block|}
name|inside_git_dir
operator|=
literal|1
expr_stmt|;
name|inside_work_tree
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|offset
operator|!=
name|cwd
operator|->
name|len
condition|)
block|{
if|if
condition|(
name|chdir
argument_list|(
name|cwd
operator|->
name|buf
argument_list|)
condition|)
name|die_errno
argument_list|(
literal|"Cannot come back to cwd"
argument_list|)
expr_stmt|;
name|root_len
operator|=
name|offset_1st_component
argument_list|(
name|cwd
operator|->
name|buf
argument_list|)
expr_stmt|;
name|strbuf_setlen
argument_list|(
name|cwd
argument_list|,
name|offset
operator|>
name|root_len
condition|?
name|offset
else|:
name|root_len
argument_list|)
expr_stmt|;
name|set_git_dir
argument_list|(
name|cwd
operator|->
name|buf
argument_list|)
expr_stmt|;
block|}
else|else
name|set_git_dir
argument_list|(
literal|"."
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
end_function
begin_function
DECL|function|setup_nongit
specifier|static
specifier|const
name|char
modifier|*
name|setup_nongit
parameter_list|(
specifier|const
name|char
modifier|*
name|cwd
parameter_list|,
name|int
modifier|*
name|nongit_ok
parameter_list|)
block|{
if|if
condition|(
operator|!
name|nongit_ok
condition|)
name|die
argument_list|(
literal|"Not a git repository (or any of the parent directories): %s"
argument_list|,
name|DEFAULT_GIT_DIR_ENVIRONMENT
argument_list|)
expr_stmt|;
if|if
condition|(
name|chdir
argument_list|(
name|cwd
argument_list|)
condition|)
name|die_errno
argument_list|(
literal|"Cannot come back to cwd"
argument_list|)
expr_stmt|;
operator|*
name|nongit_ok
operator|=
literal|1
expr_stmt|;
return|return
name|NULL
return|;
block|}
end_function
begin_function
DECL|function|get_device_or_die
specifier|static
name|dev_t
name|get_device_or_die
parameter_list|(
specifier|const
name|char
modifier|*
name|path
parameter_list|,
specifier|const
name|char
modifier|*
name|prefix
parameter_list|,
name|int
name|prefix_len
parameter_list|)
block|{
name|struct
name|stat
name|buf
decl_stmt|;
if|if
condition|(
name|stat
argument_list|(
name|path
argument_list|,
operator|&
name|buf
argument_list|)
condition|)
block|{
name|die_errno
argument_list|(
literal|"failed to stat '%*s%s%s'"
argument_list|,
name|prefix_len
argument_list|,
name|prefix
condition|?
name|prefix
else|:
literal|""
argument_list|,
name|prefix
condition|?
literal|"/"
else|:
literal|""
argument_list|,
name|path
argument_list|)
expr_stmt|;
block|}
return|return
name|buf
operator|.
name|st_dev
return|;
block|}
end_function
begin_comment
comment|/*  * A "string_list_each_func_t" function that canonicalizes an entry  * from GIT_CEILING_DIRECTORIES using real_path_if_valid(), or  * discards it if unusable.  The presence of an empty entry in  * GIT_CEILING_DIRECTORIES turns off canonicalization for all  * subsequent entries.  */
end_comment
begin_function
DECL|function|canonicalize_ceiling_entry
specifier|static
name|int
name|canonicalize_ceiling_entry
parameter_list|(
name|struct
name|string_list_item
modifier|*
name|item
parameter_list|,
name|void
modifier|*
name|cb_data
parameter_list|)
block|{
name|int
modifier|*
name|empty_entry_found
init|=
name|cb_data
decl_stmt|;
name|char
modifier|*
name|ceil
init|=
name|item
operator|->
name|string
decl_stmt|;
if|if
condition|(
operator|!
operator|*
name|ceil
condition|)
block|{
operator|*
name|empty_entry_found
operator|=
literal|1
expr_stmt|;
return|return
literal|0
return|;
block|}
elseif|else
if|if
condition|(
operator|!
name|is_absolute_path
argument_list|(
name|ceil
argument_list|)
condition|)
block|{
return|return
literal|0
return|;
block|}
elseif|else
if|if
condition|(
operator|*
name|empty_entry_found
condition|)
block|{
comment|/* Keep entry but do not canonicalize it */
return|return
literal|1
return|;
block|}
else|else
block|{
specifier|const
name|char
modifier|*
name|real_path
init|=
name|real_path_if_valid
argument_list|(
name|ceil
argument_list|)
decl_stmt|;
if|if
condition|(
operator|!
name|real_path
condition|)
return|return
literal|0
return|;
name|free
argument_list|(
name|item
operator|->
name|string
argument_list|)
expr_stmt|;
name|item
operator|->
name|string
operator|=
name|xstrdup
argument_list|(
name|real_path
argument_list|)
expr_stmt|;
return|return
literal|1
return|;
block|}
block|}
end_function
begin_comment
comment|/*  * We cannot decide in this function whether we are in the work tree or  * not, since the config can only be read _after_ this function was called.  */
end_comment
begin_function
DECL|function|setup_git_directory_gently_1
specifier|static
specifier|const
name|char
modifier|*
name|setup_git_directory_gently_1
parameter_list|(
name|int
modifier|*
name|nongit_ok
parameter_list|)
block|{
specifier|const
name|char
modifier|*
name|env_ceiling_dirs
init|=
name|getenv
argument_list|(
name|CEILING_DIRECTORIES_ENVIRONMENT
argument_list|)
decl_stmt|;
name|struct
name|string_list
name|ceiling_dirs
init|=
name|STRING_LIST_INIT_DUP
decl_stmt|;
specifier|static
name|struct
name|strbuf
name|cwd
init|=
name|STRBUF_INIT
decl_stmt|;
specifier|const
name|char
modifier|*
name|gitdirenv
decl_stmt|,
modifier|*
name|ret
decl_stmt|;
name|char
modifier|*
name|gitfile
decl_stmt|;
name|int
name|offset
decl_stmt|,
name|offset_parent
decl_stmt|,
name|ceil_offset
init|=
operator|-
literal|1
decl_stmt|;
name|dev_t
name|current_device
init|=
literal|0
decl_stmt|;
name|int
name|one_filesystem
init|=
literal|1
decl_stmt|;
comment|/* 	 * We may have read an incomplete configuration before 	 * setting-up the git directory. If so, clear the cache so 	 * that the next queries to the configuration reload complete 	 * configuration (including the per-repo config file that we 	 * ignored previously). 	 */
name|git_config_clear
argument_list|()
expr_stmt|;
comment|/* 	 * Let's assume that we are in a git repository. 	 * If it turns out later that we are somewhere else, the value will be 	 * updated accordingly. 	 */
if|if
condition|(
name|nongit_ok
condition|)
operator|*
name|nongit_ok
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|strbuf_getcwd
argument_list|(
operator|&
name|cwd
argument_list|)
condition|)
name|die_errno
argument_list|(
literal|"Unable to read current working directory"
argument_list|)
expr_stmt|;
name|offset
operator|=
name|cwd
operator|.
name|len
expr_stmt|;
comment|/* 	 * If GIT_DIR is set explicitly, we're not going 	 * to do any discovery, but we still do repository 	 * validation. 	 */
name|gitdirenv
operator|=
name|getenv
argument_list|(
name|GIT_DIR_ENVIRONMENT
argument_list|)
expr_stmt|;
if|if
condition|(
name|gitdirenv
condition|)
return|return
name|setup_explicit_git_dir
argument_list|(
name|gitdirenv
argument_list|,
operator|&
name|cwd
argument_list|,
name|nongit_ok
argument_list|)
return|;
if|if
condition|(
name|env_ceiling_dirs
condition|)
block|{
name|int
name|empty_entry_found
init|=
literal|0
decl_stmt|;
name|string_list_split
argument_list|(
operator|&
name|ceiling_dirs
argument_list|,
name|env_ceiling_dirs
argument_list|,
name|PATH_SEP
argument_list|,
operator|-
literal|1
argument_list|)
expr_stmt|;
name|filter_string_list
argument_list|(
operator|&
name|ceiling_dirs
argument_list|,
literal|0
argument_list|,
name|canonicalize_ceiling_entry
argument_list|,
operator|&
name|empty_entry_found
argument_list|)
expr_stmt|;
name|ceil_offset
operator|=
name|longest_ancestor_length
argument_list|(
name|cwd
operator|.
name|buf
argument_list|,
operator|&
name|ceiling_dirs
argument_list|)
expr_stmt|;
name|string_list_clear
argument_list|(
operator|&
name|ceiling_dirs
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|ceil_offset
operator|<
literal|0
operator|&&
name|has_dos_drive_prefix
argument_list|(
name|cwd
operator|.
name|buf
argument_list|)
condition|)
name|ceil_offset
operator|=
literal|1
expr_stmt|;
comment|/* 	 * Test in the following order (relative to the cwd): 	 * - .git (file containing "gitdir:<path>") 	 * - .git/ 	 * - ./ (bare) 	 * - ../.git 	 * - ../.git/ 	 * - ../ (bare) 	 * - ../../.git/ 	 *   etc. 	 */
name|one_filesystem
operator|=
operator|!
name|git_env_bool
argument_list|(
literal|"GIT_DISCOVERY_ACROSS_FILESYSTEM"
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|one_filesystem
condition|)
name|current_device
operator|=
name|get_device_or_die
argument_list|(
literal|"."
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
name|gitfile
operator|=
operator|(
name|char
operator|*
operator|)
name|read_gitfile
argument_list|(
name|DEFAULT_GIT_DIR_ENVIRONMENT
argument_list|)
expr_stmt|;
if|if
condition|(
name|gitfile
condition|)
name|gitdirenv
operator|=
name|gitfile
operator|=
name|xstrdup
argument_list|(
name|gitfile
argument_list|)
expr_stmt|;
else|else
block|{
if|if
condition|(
name|is_git_directory
argument_list|(
name|DEFAULT_GIT_DIR_ENVIRONMENT
argument_list|)
condition|)
name|gitdirenv
operator|=
name|DEFAULT_GIT_DIR_ENVIRONMENT
expr_stmt|;
block|}
if|if
condition|(
name|gitdirenv
condition|)
block|{
name|ret
operator|=
name|setup_discovered_git_dir
argument_list|(
name|gitdirenv
argument_list|,
operator|&
name|cwd
argument_list|,
name|offset
argument_list|,
name|nongit_ok
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|gitfile
argument_list|)
expr_stmt|;
return|return
name|ret
return|;
block|}
name|free
argument_list|(
name|gitfile
argument_list|)
expr_stmt|;
if|if
condition|(
name|is_git_directory
argument_list|(
literal|"."
argument_list|)
condition|)
return|return
name|setup_bare_git_dir
argument_list|(
operator|&
name|cwd
argument_list|,
name|offset
argument_list|,
name|nongit_ok
argument_list|)
return|;
name|offset_parent
operator|=
name|offset
expr_stmt|;
while|while
condition|(
operator|--
name|offset_parent
operator|>
name|ceil_offset
operator|&&
name|cwd
operator|.
name|buf
index|[
name|offset_parent
index|]
operator|!=
literal|'/'
condition|)
empty_stmt|;
if|if
condition|(
name|offset_parent
operator|<=
name|ceil_offset
condition|)
return|return
name|setup_nongit
argument_list|(
name|cwd
operator|.
name|buf
argument_list|,
name|nongit_ok
argument_list|)
return|;
if|if
condition|(
name|one_filesystem
condition|)
block|{
name|dev_t
name|parent_device
init|=
name|get_device_or_die
argument_list|(
literal|".."
argument_list|,
name|cwd
operator|.
name|buf
argument_list|,
name|offset
argument_list|)
decl_stmt|;
if|if
condition|(
name|parent_device
operator|!=
name|current_device
condition|)
block|{
if|if
condition|(
name|nongit_ok
condition|)
block|{
if|if
condition|(
name|chdir
argument_list|(
name|cwd
operator|.
name|buf
argument_list|)
condition|)
name|die_errno
argument_list|(
literal|"Cannot come back to cwd"
argument_list|)
expr_stmt|;
operator|*
name|nongit_ok
operator|=
literal|1
expr_stmt|;
return|return
name|NULL
return|;
block|}
name|strbuf_setlen
argument_list|(
operator|&
name|cwd
argument_list|,
name|offset
argument_list|)
expr_stmt|;
name|die
argument_list|(
literal|"Not a git repository (or any parent up to mount point %s)\n"
literal|"Stopping at filesystem boundary (GIT_DISCOVERY_ACROSS_FILESYSTEM not set)."
argument_list|,
name|cwd
operator|.
name|buf
argument_list|)
expr_stmt|;
block|}
block|}
if|if
condition|(
name|chdir
argument_list|(
literal|".."
argument_list|)
condition|)
block|{
name|strbuf_setlen
argument_list|(
operator|&
name|cwd
argument_list|,
name|offset
argument_list|)
expr_stmt|;
name|die_errno
argument_list|(
literal|"Cannot change to '%s/..'"
argument_list|,
name|cwd
operator|.
name|buf
argument_list|)
expr_stmt|;
block|}
name|offset
operator|=
name|offset_parent
expr_stmt|;
block|}
block|}
end_function
begin_function
DECL|function|setup_git_directory_gently
specifier|const
name|char
modifier|*
name|setup_git_directory_gently
parameter_list|(
name|int
modifier|*
name|nongit_ok
parameter_list|)
block|{
specifier|const
name|char
modifier|*
name|prefix
decl_stmt|;
name|prefix
operator|=
name|setup_git_directory_gently_1
argument_list|(
name|nongit_ok
argument_list|)
expr_stmt|;
if|if
condition|(
name|prefix
condition|)
name|setenv
argument_list|(
name|GIT_PREFIX_ENVIRONMENT
argument_list|,
name|prefix
argument_list|,
literal|1
argument_list|)
expr_stmt|;
else|else
name|setenv
argument_list|(
name|GIT_PREFIX_ENVIRONMENT
argument_list|,
literal|""
argument_list|,
literal|1
argument_list|)
expr_stmt|;
if|if
condition|(
name|startup_info
condition|)
block|{
name|startup_info
operator|->
name|have_repository
operator|=
operator|!
name|nongit_ok
operator|||
operator|!
operator|*
name|nongit_ok
expr_stmt|;
name|startup_info
operator|->
name|prefix
operator|=
name|prefix
expr_stmt|;
block|}
return|return
name|prefix
return|;
block|}
end_function
begin_function
DECL|function|git_config_perm
name|int
name|git_config_perm
parameter_list|(
specifier|const
name|char
modifier|*
name|var
parameter_list|,
specifier|const
name|char
modifier|*
name|value
parameter_list|)
block|{
name|int
name|i
decl_stmt|;
name|char
modifier|*
name|endptr
decl_stmt|;
if|if
condition|(
name|value
operator|==
name|NULL
condition|)
return|return
name|PERM_GROUP
return|;
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|value
argument_list|,
literal|"umask"
argument_list|)
condition|)
return|return
name|PERM_UMASK
return|;
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|value
argument_list|,
literal|"group"
argument_list|)
condition|)
return|return
name|PERM_GROUP
return|;
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|value
argument_list|,
literal|"all"
argument_list|)
operator|||
operator|!
name|strcmp
argument_list|(
name|value
argument_list|,
literal|"world"
argument_list|)
operator|||
operator|!
name|strcmp
argument_list|(
name|value
argument_list|,
literal|"everybody"
argument_list|)
condition|)
return|return
name|PERM_EVERYBODY
return|;
comment|/* Parse octal numbers */
name|i
operator|=
name|strtol
argument_list|(
name|value
argument_list|,
operator|&
name|endptr
argument_list|,
literal|8
argument_list|)
expr_stmt|;
comment|/* If not an octal number, maybe true/false? */
if|if
condition|(
operator|*
name|endptr
operator|!=
literal|0
condition|)
return|return
name|git_config_bool
argument_list|(
name|var
argument_list|,
name|value
argument_list|)
condition|?
name|PERM_GROUP
else|:
name|PERM_UMASK
return|;
comment|/* 	 * Treat values 0, 1 and 2 as compatibility cases, otherwise it is 	 * a chmod value to restrict to. 	 */
switch|switch
condition|(
name|i
condition|)
block|{
case|case
name|PERM_UMASK
case|:
comment|/* 0 */
return|return
name|PERM_UMASK
return|;
case|case
name|OLD_PERM_GROUP
case|:
comment|/* 1 */
return|return
name|PERM_GROUP
return|;
case|case
name|OLD_PERM_EVERYBODY
case|:
comment|/* 2 */
return|return
name|PERM_EVERYBODY
return|;
block|}
comment|/* A filemode value was given: 0xxx */
if|if
condition|(
operator|(
name|i
operator|&
literal|0600
operator|)
operator|!=
literal|0600
condition|)
name|die
argument_list|(
literal|"Problem with core.sharedRepository filemode value "
literal|"(0%.3o).\nThe owner of files must always have "
literal|"read and write permissions."
argument_list|,
name|i
argument_list|)
expr_stmt|;
comment|/* 	 * Mask filemode value. Others can not get write permission. 	 * x flags for directories are handled separately. 	 */
return|return
operator|-
operator|(
name|i
operator|&
literal|0666
operator|)
return|;
block|}
end_function
begin_function
DECL|function|check_repository_format_version
name|int
name|check_repository_format_version
parameter_list|(
specifier|const
name|char
modifier|*
name|var
parameter_list|,
specifier|const
name|char
modifier|*
name|value
parameter_list|,
name|void
modifier|*
name|cb
parameter_list|)
block|{
name|struct
name|repository_format
modifier|*
name|data
init|=
name|cb
decl_stmt|;
name|int
name|ret
init|=
name|check_repo_format
argument_list|(
name|var
argument_list|,
name|value
argument_list|,
name|cb
argument_list|)
decl_stmt|;
if|if
condition|(
name|ret
condition|)
return|return
name|ret
return|;
if|if
condition|(
name|strcmp
argument_list|(
name|var
argument_list|,
literal|"core.bare"
argument_list|)
operator|==
literal|0
condition|)
block|{
name|data
operator|->
name|is_bare
operator|=
name|git_config_bool
argument_list|(
name|var
argument_list|,
name|value
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|strcmp
argument_list|(
name|var
argument_list|,
literal|"core.worktree"
argument_list|)
operator|==
literal|0
condition|)
block|{
if|if
condition|(
operator|!
name|value
condition|)
return|return
name|config_error_nonbool
argument_list|(
name|var
argument_list|)
return|;
name|data
operator|->
name|work_tree
operator|=
name|xstrdup
argument_list|(
name|value
argument_list|)
expr_stmt|;
block|}
return|return
literal|0
return|;
block|}
end_function
begin_function
DECL|function|check_repository_format
name|void
name|check_repository_format
parameter_list|(
name|void
parameter_list|)
block|{
name|check_repository_format_gently
argument_list|(
name|get_git_dir
argument_list|()
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
block|}
end_function
begin_comment
comment|/*  * Returns the "prefix", a path to the current working directory  * relative to the work tree root, or NULL, if the current working  * directory is not a strict subdirectory of the work tree root. The  * prefix always ends with a '/' character.  */
end_comment
begin_function
DECL|function|setup_git_directory
specifier|const
name|char
modifier|*
name|setup_git_directory
parameter_list|(
name|void
parameter_list|)
block|{
return|return
name|setup_git_directory_gently
argument_list|(
name|NULL
argument_list|)
return|;
block|}
end_function
begin_function
DECL|function|resolve_gitdir
specifier|const
name|char
modifier|*
name|resolve_gitdir
parameter_list|(
specifier|const
name|char
modifier|*
name|suspect
parameter_list|)
block|{
if|if
condition|(
name|is_git_directory
argument_list|(
name|suspect
argument_list|)
condition|)
return|return
name|suspect
return|;
return|return
name|read_gitfile
argument_list|(
name|suspect
argument_list|)
return|;
block|}
end_function
begin_comment
comment|/* if any standard file descriptor is missing open it to /dev/null */
end_comment
begin_function
DECL|function|sanitize_stdfds
name|void
name|sanitize_stdfds
parameter_list|(
name|void
parameter_list|)
block|{
name|int
name|fd
init|=
name|open
argument_list|(
literal|"/dev/null"
argument_list|,
name|O_RDWR
argument_list|,
literal|0
argument_list|)
decl_stmt|;
while|while
condition|(
name|fd
operator|!=
operator|-
literal|1
operator|&&
name|fd
operator|<
literal|2
condition|)
name|fd
operator|=
name|dup
argument_list|(
name|fd
argument_list|)
expr_stmt|;
if|if
condition|(
name|fd
operator|==
operator|-
literal|1
condition|)
name|die_errno
argument_list|(
literal|"open /dev/null or dup failed"
argument_list|)
expr_stmt|;
if|if
condition|(
name|fd
operator|>
literal|2
condition|)
name|close
argument_list|(
name|fd
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|daemonize
name|int
name|daemonize
parameter_list|(
name|void
parameter_list|)
block|{
ifdef|#
directive|ifdef
name|NO_POSIX_GOODIES
name|errno
operator|=
name|ENOSYS
expr_stmt|;
return|return
operator|-
literal|1
return|;
else|#
directive|else
switch|switch
condition|(
name|fork
argument_list|()
condition|)
block|{
case|case
literal|0
case|:
break|break;
case|case
operator|-
literal|1
case|:
name|die_errno
argument_list|(
literal|"fork failed"
argument_list|)
expr_stmt|;
default|default:
name|exit
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|setsid
argument_list|()
operator|==
operator|-
literal|1
condition|)
name|die_errno
argument_list|(
literal|"setsid failed"
argument_list|)
expr_stmt|;
name|close
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|close
argument_list|(
literal|1
argument_list|)
expr_stmt|;
name|close
argument_list|(
literal|2
argument_list|)
expr_stmt|;
name|sanitize_stdfds
argument_list|()
expr_stmt|;
return|return
literal|0
return|;
endif|#
directive|endif
block|}
end_function
end_unit
