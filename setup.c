...
end_function
begin_comment
comment|/*  * If no worktree was given, and we are outside of a default work tree,  * now is the time to set it.  *  * In other words, if the user calls git with something like  *  *	git --git-dir=/some/where/else/.git bla  *  * default to /some/where/else as working directory; if the specified  * git-dir does not end in "/.git", the cwd is used as working directory.  */
end_comment
begin_function
...
literal|1
decl_stmt|;
comment|/* strip the variable 'dir' of the postfix "/.git" if it has it */
name|len
operator|=
name|strlen
argument_list|(
...
name|die
argument_list|(
literal|"directory name too long"
argument_list|)
expr_stmt|;
name|memcpy
argument_list|(
...
literal|'\0'
expr_stmt|;
comment|/* are we inside the default work tree? */
name|rel
operator|=
...
expr_stmt|;
block|}
comment|/* if rel is set, the cwd is _not_ the current working tree */
if|if
condition|(
name|rel
operator|&&
