...
name|char
modifier|*
name|wt_status_unmerged_status_string
parameter_list|(
name|int
name|stagemask
parameter_list|)
...
name|char
modifier|*
name|wt_status_diff_status_string
parameter_list|(
name|int
name|status
parameter_list|)
...
condition|)
block|{
comment|/* If DIFF_STATUS_* uses outside the range [A..Z], we're in trouble */
name|label_width
operator|=
name|maxwidth
argument_list|(
...
name|_
argument_list|(
literal|"Submodules changed but not updated:"
argument_list|)
argument_list|)
expr_stmt|;
else|else
...
name|_
argument_list|(
literal|"Submodule changes to be committed:"
argument_list|)
argument_list|)
expr_stmt|;
name|strbuf_addstr
...
name|_
argument_list|(
literal|"Do not touch the line above.\nEverything below will be removed."
argument_list|)
decl_stmt|;
name|struct
name|strbuf
...
literal|0
expr_stmt|;
comment|/* 	 * If we're not going to stdout, then we definitely don't 	 * want color, since we are going to the commit message 	 * file (and even the "auto" setting won't work, since it 	 * will have checked isatty on stdout). But we then do want 	 * to insert the scissor line here to reliably remove the 	 * diff before committing. 	 */
if|if
condition|(
name|s
operator|->
...
condition|)
block|{
comment|/* print_updated() printed a header, so do we */
if|if
condition|(
name|s
operator|->
...
expr_stmt|;
block|}
comment|/* else use prefix as per user config */
name|run_diff_index
argument_list|(
operator|&
name|rev
...
end_function
begin_comment
comment|/*  * Turn  * "pick d6a2f0303e897ec257dd0e0a39a5ccb709bc2047 some message"  * into  * "pick d6a2f03 some message"  *  * The function assumes that the line does not contain useless spaces  * before or after the command.  */
end_comment
begin_function
DECL|function|abbrev_sha1_in_line
specifier|static
name|void
name|abbrev_sha1_in_line
parameter_list|(
name|struct
name|strbuf
modifier|*
...
name|abbrev
decl_stmt|;
comment|/* 		 * strbuf_split_max left a space. Trim it and re-add 		 * it after abbreviation. 		 */
name|strbuf_trim
argument_list|(
name|split
index|[
...
name|die_errno
argument_list|(
literal|"Could not open file %s for reading"
argument_list|,
name|git_path
argument_list|(
literal|"%s"
...
name|read_rebase_todolist
argument_list|(
literal|"rebase-merge/git-rebase-todo"
argument_list|,
operator|&
name|yet_to_do
argument_list|)
...
name|Q_
argument_list|(
literal|"Last command done (%d command done):"
argument_list|,
literal|"Last commands done (%d commands done):"
argument_list|,
name|have_done
operator|.
name|nr
...
name|Q_
argument_list|(
literal|"Next command to do (%d remaining command):"
argument_list|,
literal|"Next commands to do (%d remaining commands):"
argument_list|,
name|yet_to_do
operator|.
name|nr
...
name|_
argument_list|(
literal|"  (use \"git rebase --edit-todo\" to view and edit)"
argument_list|)
argument_list|)
expr_stmt|;
block|}
...
condition|)
block|{
comment|/* HEAD is relative. Resolve it to the right reflog entry. */
name|strbuf_reset
argument_list|(
operator|&
name|cb
...
end_function
begin_function
DECL|function|wt_status_check_rebase
name|int
...
end_function
begin_function
DECL|function|wt_status_check_bisect
name|int
