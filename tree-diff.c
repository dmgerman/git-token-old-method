...
argument_list|)
expr_stmt|;
comment|/* NOTE! Ignore the first diff! That was the old one! */
for|for
control|(
name|i
operator|=
...
index|]
decl_stmt|;
comment|/* 		 * Found a source? Not only do we use that for the new 		 * diff_queued_diff, we also use that as the path in 		 * the future! 		 */
if|if
condition|(
operator|(
name|p
...
block|}
block|}
comment|/* 	 * Then, ignore any but the first entry! It might be the old one, 	 * or it might be the rename/copy we found 	 */
name|diff_queued_diff
operator|.
name|nr
operator|=
