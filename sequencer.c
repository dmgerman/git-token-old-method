...
argument_list|()
expr_stmt|;
comment|/* 		 * If index_unchanged is less than 0, that indicates we either 		 * couldn't parse HEAD or the index, so error out here. 		 */
if|if
condition|(
name|index_unchanged
operator|<
...
name|index_unchanged
condition|)
comment|/* 			 * The head tree and the index match 			 * meaning the commit is empty.  Since it wasn't created 			 * empty (based on the previous test), we can conclude 			 * the commit has been made redundant.  Since we don't 			 * want to keep redundant commits, we can just return 			 * here, skipping this commit 			 */
return|return
literal|0
return|;
if|if
