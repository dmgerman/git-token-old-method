...
end_function
begin_comment
comment|/*  * Returns 0 for non-conforming footer  * Returns 1 for conforming footer  * Returns 2 when sob exists within conforming footer  * Returns 3 when sob exists within conforming footer as last entry  */
end_comment
begin_function
...
name|name
decl_stmt|;
comment|/* This happens when using --stdin. */
if|if
condition|(
operator|!
name|strlen
...
name|_
argument_list|(
literal|"%s: can't cherry-pick a %s"
argument_list|)
argument_list|,
name|name
argument_list|,
...
argument_list|)
expr_stmt|;
comment|/* 	 * If the whole message buffer is equal to the sob, pretend that we 	 * found a conforming footer with a matching sob 	 */
if|if
condition|(
name|msgbuf
operator|->
