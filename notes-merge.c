...
parameter_list|)
block|{
comment|/* 	 * Iterate through files in .git/NOTES_MERGE_WORKTREE and add all 	 * found notes to 'partial_tree'. Write the updated notes tree to 	 * the DB, and commit the resulting tree object while reusing the 	 * commit message and parents from 'partial_commit'. 	 * Finally store the new commit object SHA1 into 'result_sha1'. 	 */
name|DIR
modifier|*
name|dir
decl_stmt|;
...
name|printf
argument_list|(
literal|"Skipping non-SHA1 entry '%s%s'\n"
argument_list|,
name|path
operator|.
name|buf
