...
name|'write'
op|'('
string|'"git-p4: requires Python 2.4 or later.\\n"'
op|')'
newline|'\n'
name|'sys'
op|'.'
...
op|':'
newline|'\n'
comment|'# from python2.7:subprocess.py'
nl|'\n'
comment|'# Exception classes used by this module.'
nl|'\n'
DECL|class|CalledProcessError
indent|'    '
name|'class'
name|'CalledProcessError'
op|'('
...
newline|'\n'
indent|'        '
string|'"""This exception is raised when a process run by check_call() returns\n        a non-zero exit status.  The exit status will be stored in the\n        returncode attribute."""'
newline|'\n'
...
indent|'            '
name|'return'
string|'"Command \'%s\' returned non-zero exit status %d"'
op|'%'
op|'('
name|'self'
op|'.'
...
newline|'\n'
nl|'\n'
comment|'# Grab changes in blocks of this many revisions, unless otherwise requested'
nl|'\n'
DECL|variable|defaultBlockSize
...
newline|'\n'
indent|'    '
string|'"""Return free space in bytes on the disk of the given dirname."""'
newline|'\n'
name|'if'
name|'platform'
op|'.'
...
newline|'\n'
nl|'\n'
DECL|function|p4_has_move_command
dedent|''
name|'def'
...
newline|'\n'
indent|'    '
string|'"""See if the move command exists, that it supports -k, and that\n       it has not been administratively disabled.  The arguments\n       must be correct, but the filenames do not have to exist.  Use\n       ones with wildcards so even if they exist, it will fail."""'
newline|'\n'
nl|'\n'
name|'if'
name|'not'
...
op|')'
newline|'\n'
comment|'# return code will be 1 in either case'
nl|'\n'
name|'if'
name|'err'
op|'.'
...
name|'False'
newline|'\n'
comment|'# assume it failed because @... was invalid changelist'
nl|'\n'
dedent|''
name|'return'
name|'True'
...
newline|'\n'
nl|'\n'
DECL|variable|_p4_version_string
dedent|''
dedent|''
name|'_p4_version_string'
op|'='
name|'None'
newline|'\n'
DECL|function|p4_version_string
name|'def'
...
newline|'\n'
indent|'    '
string|'"""Read the version string, showing just the last line, which\n       hopefully is the interesting version bit.\n\n       $ p4 -V\n       Perforce - The Fast Software Configuration Management System.\n       Copyright 1995-2011 Perforce Software.  All rights reserved.\n       Rev. P4/NTX86/2011.1/393975 (2011/12/16).\n    """'
newline|'\n'
name|'global'
name|'_p4_version_string'
newline|'\n'
...
newline|'\n'
indent|'    '
string|'"""Make sure it returns a valid result by checking for\n       the presence of field "time".  Return a dict of the\n       results."""'
newline|'\n'
nl|'\n'
name|'ds'
op|'='
...
name|'die'
op|'('
string|'"p4 describe -s %d did not return 1 result: %s"'
op|'%'
op|'('
name|'change'
op|','
...
name|'die'
op|'('
string|'"p4 describe -s %d exited with %d: %s"'
op|'%'
op|'('
name|'change'
op|','
...
name|'die'
op|'('
string|'"p4 describe -s %d returned error code: %s"'
op|'%'
op|'('
name|'change'
op|','
...
name|'die'
op|'('
string|'"p4 describe -s %d returned no \\"time\\": %s"'
op|'%'
op|'('
name|'change'
op|','
...
op|':'
newline|'\n'
comment|"# Search for the base client side depot path, as long as it starts with the branch's P4 path."
nl|'\n'
comment|'# The base path always ends with "/...".'
nl|'\n'
indent|'            '
name|'if'
name|'entry'
...
newline|'\n'
indent|'    '
string|'"""Return a bool, using git config --bool.  It is True only if the\n       variable is set to true, and False if set to false or not present\n       in the config."""'
newline|'\n'
nl|'\n'
name|'if'
name|'not'
...
newline|'\n'
indent|'    '
string|'"""Find all the branches whose names start with "p4/", looking\n       in remotes or heads as specified by the argument.  Return\n       a dictionary of { branch: revision } for each one found.\n       The branch names are the short names, without any\n       "p4/" prefix."""'
newline|'\n'
nl|'\n'
name|'branches'
op|'='
...
name|'continue'
newline|'\n'
comment|'# special symbolic ref to p4/master'
nl|'\n'
dedent|''
name|'if'
name|'line'
...
newline|'\n'
indent|'    '
string|'"""Make sure that the given ref name really exists."""'
newline|'\n'
nl|'\n'
name|'cmd'
op|'='
...
name|'False'
newline|'\n'
comment|'# expect exactly one line of output: the branch name'
nl|'\n'
dedent|''
name|'return'
name|'out'
...
nl|'\n'
nl|'\n'
DECL|function|p4ParseNumericChangeRange
dedent|''
name|'def'
name|'p4ParseNumericChangeRange'
op|'('
name|'parts'
op|')'
op|':'
...
newline|'\n'
nl|'\n'
comment|'# Parse the change range into start and end. Try to find integer'
nl|'\n'
comment|'# revision ranges as these can be broken up into blocks to avoid'
nl|'\n'
comment|'# hitting server-side limits (maxrows, maxscanresults). But if'
nl|'\n'
comment|"# that doesn't work, fall back to using the raw revision specifier"
nl|'\n'
comment|'# strings, without using block mode.'
nl|'\n'
nl|'\n'
name|'if'
name|'changeRange'
...
name|'die'
op|'('
string|'"cannot use --changes-block-size with non-numeric revisions"'
op|')'
newline|'\n'
dedent|''
name|'block_size'
...
newline|'\n'
nl|'\n'
comment|'# Retrieve changes a block at a time, to prevent running'
nl|'\n'
comment|'# into a MaxResults/MaxScanRows error from the server.'
nl|'\n'
nl|'\n'
name|'while'
name|'True'
...
newline|'\n'
nl|'\n'
comment|'# Insert changes in chronological order'
nl|'\n'
dedent|''
name|'for'
name|'line'
...
newline|'\n'
indent|'    '
string|'"""Base class for large file system support."""'
newline|'\n'
nl|'\n'
DECL|member|__init__
name|'def'
...
newline|'\n'
indent|'        '
string|'"""Return the content of a pointer file that is stored in Git instead of\n           the actual content."""'
newline|'\n'
name|'assert'
name|'False'
op|','
string|'"Method \'generatePointer\' required in "'
op|'+'
name|'self'
op|'.'
name|'__class__'
...
newline|'\n'
indent|'        '
string|'"""Push the actual content which is not stored in the Git repository to\n           a server."""'
newline|'\n'
name|'assert'
name|'False'
op|','
string|'"Method \'pushFile\' required in "'
op|'+'
name|'self'
op|'.'
name|'__class__'
...
newline|'\n'
nl|'\n'
DECL|member|hasLargeFileExtension
dedent|''
name|'def'
name|'hasLargeFileExtension'
op|'('
name|'self'
op|','
name|'relPath'
...
name|'gitConfigList'
op|'('
string|"'git-p4.largeFileExtensions'"
op|')'
op|']'
op|','
nl|'\n'
...
newline|'\n'
nl|'\n'
DECL|member|generateTempFile
dedent|''
name|'def'
name|'generateTempFile'
op|'('
name|'self'
op|','
name|'contents'
...
newline|'\n'
nl|'\n'
DECL|member|exceedsLargeFileThreshold
dedent|''
name|'def'
name|'exceedsLargeFileThreshold'
op|'('
name|'self'
op|','
name|'relPath'
...
name|'gitConfigInt'
op|'('
string|"'git-p4.largeFileThreshold'"
op|')'
op|':'
newline|'\n'
indent|'            '
...
name|'gitConfigInt'
op|'('
string|"'git-p4.largeFileCompressedThreshold'"
op|')'
op|':'
newline|'\n'
indent|'            '
...
newline|'\n'
nl|'\n'
DECL|member|removeLargeFile
dedent|''
name|'def'
name|'removeLargeFile'
op|'('
name|'self'
op|','
name|'relPath'
...
newline|'\n'
indent|'        '
string|'"""Processes the content of git fast import. This method decides if a\n           file is stored in the large file system and handles all necessary\n           steps."""'
newline|'\n'
name|'if'
name|'self'
op|'.'
...
newline|'\n'
nl|'\n'
comment|'# Move temp file to final location in large file system'
nl|'\n'
name|'largeFileDir'
op|'='
name|'os'
...
name|'write'
op|'('
string|'"%s moved to large file system (%s)\\n"'
op|'%'
op|'('
name|'relPath'
op|','
...
newline|'\n'
indent|'    '
string|'"""Mock large file system for testing."""'
newline|'\n'
nl|'\n'
DECL|member|generatePointer
name|'def'
...
newline|'\n'
indent|'        '
string|'"""The remote filename of the large file storage is the same as the local\n           one but in a different directory.\n           """'
newline|'\n'
name|'remotePath'
op|'='
name|'os'
...
newline|'\n'
indent|'    '
string|'"""Git LFS as backend for the git-p4 large file system.\n       See https://git-lfs.github.com/ for details."""'
newline|'\n'
nl|'\n'
DECL|member|__init__
name|'def'
...
name|'die'
op|'('
string|"'git-lfs pointer command failed. Did you install the extension?'"
op|')'
newline|'\n'
nl|'\n'
comment|"# Git LFS removed the preamble in the output of the 'pointer' command"
nl|'\n'
comment|'# starting from version 1.2.0. Check for the preamble here to support'
nl|'\n'
comment|'# earlier versions.'
nl|'\n'
comment|'# c.f. https://github.com/github/git-lfs/commit/da2935d9a739592bc775c98d8ef4df9c72ea3b43'
nl|'\n'
dedent|''
name|'if'
name|'pointerFile'
...
op|')'
newline|'\n'
comment|'# LFS Spec states that pointer files should not have the executable bit set.'
nl|'\n'
name|'gitMode'
op|'='
string|"'100644'"
...
name|'die'
op|'('
string|"'git-lfs push command failed. Did you define a remote?'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|generateGitAttributes
dedent|''
dedent|''
name|'def'
name|'generateGitAttributes'
op|'('
name|'self'
op|')'
op|':'
...
op|','
nl|'\n'
string|"'# Git LFS (see https://git-lfs.github.com/)\\n'"
op|','
nl|'\n'
string|"'#\\n'"
op|','
...
nl|'\n'
dedent|''
name|'mapUserConfigRegex'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|'r"^\\s*(\\S+)\\s*=\\s*(.+)\\s*<(\\S+)>\\s*$"'
op|','
name|'re'
op|'.'
name|'VERBOSE'
...
newline|'\n'
nl|'\n'
DECL|variable|conflict_behavior_choices
indent|'    '
name|'conflict_behavior_choices'
op|'='
op|'('
string|'"ask"'
op|','
...
name|'make_option'
op|'('
string|'"--prepare-p4-only"'
op|','
name|'dest'
op|'='
string|'"prepare_p4_only"'
op|','
name|'action'
op|'='
string|'"store_true"'
...
name|'die'
op|'('
string|'"Large file system not supported for git-p4 submit command. Please remove it from config."'
op|')'
newline|'\n'
nl|'\n'
DECL|member|check
...
newline|'\n'
nl|'\n'
DECL|member|get_diff_description
dedent|''
dedent|''
dedent|''
name|'def'
name|'get_diff_description'
op|'('
name|'self'
op|','
name|'editedFiles'
...
op|':'
newline|'\n'
comment|'# turn off read-only attribute'
nl|'\n'
indent|'                    '
name|'os'
op|'.'
...
name|'diffcmd'
op|'='
string|'"git diff-tree --full-index -p \\"%s\\""'
op|'%'
op|'('
name|'id'
op|')'
...
op|')'
newline|'\n'
comment|'# File is being deleted, so not open in p4.  Must'
nl|'\n'
comment|'# disable the read-only bit on windows.'
nl|'\n'
dedent|''
name|'if'
name|'self'
...
comment|'#'
nl|'\n'
comment|'# Apply the patch for real, and do add/delete/+x handling.'
nl|'\n'
comment|'#'
nl|'\n'
dedent|''
...
comment|'#'
nl|'\n'
comment|'# Build p4 change description, starting with the contents'
nl|'\n'
comment|'# of the git commit message.'
nl|'\n'
comment|'#'
nl|'\n'
dedent|''
...
comment|'#'
nl|'\n'
comment|'# Leave the p4 tree prepared, and the submit template around'
nl|'\n'
comment|'# and let the user decide what to do next'
nl|'\n'
comment|'#'
nl|'\n'
indent|'            '
...
newline|'\n'
name|'print'
string|'"P4 workspace prepared for submission."'
newline|'\n'
name|'print'
string|'"To submit or revert, go to client workspace"'
newline|'\n'
name|'print'
string|'"  "'
op|'+'
...
newline|'\n'
name|'print'
string|'"To submit, use \\"p4 submit\\" to write a new description,"'
newline|'\n'
name|'print'
string|'"or \\"p4 submit -i <%s\\" to use the one prepared by"'
string|'" \\"git p4\\"."'
op|'%'
name|'fileName'
newline|'\n'
name|'print'
string|'"You can delete the file \\"%s\\" when finished."'
op|'%'
name|'fileName'
newline|'\n'
nl|'\n'
...
indent|'                '
name|'print'
string|'"To preserve change ownership by user %s, you must\\n"'
string|'"do \\"p4 change -f <change>\\" after submitting and\\n"'
string|'"edit the User field."'
newline|'\n'
dedent|''
name|'if'
name|'pureRenameCopy'
...
indent|'                '
name|'print'
string|'"After submitting, renamed files must be re-synced."'
newline|'\n'
name|'print'
string|'"Invoke \\"p4 sync -f\\" on each of these files:"'
newline|'\n'
name|'for'
name|'f'
name|'in'
...
newline|'\n'
name|'print'
string|'"To revert the changes, use \\"p4 revert ...\\", and delete"'
newline|'\n'
name|'print'
string|'"the submit template file \\"%s\\""'
op|'%'
name|'fileName'
newline|'\n'
name|'if'
...
indent|'                '
name|'print'
string|'"Since the commit adds new files, they must be deleted:"'
newline|'\n'
name|'for'
name|'f'
name|'in'
...
comment|'#'
nl|'\n'
comment|'# Let the user edit the change description, then submit it.'
nl|'\n'
comment|'#'
nl|'\n'
dedent|''
...
indent|'                '
name|'print'
string|'"Would create p4 label %s for tag"'
op|'%'
name|'name'
newline|'\n'
dedent|''
...
indent|'                '
name|'print'
string|'"Not creating p4 label %s for tag due to option"'
string|'" --prepare-p4-only"'
op|'%'
name|'name'
newline|'\n'
...
newline|'\n'
nl|'\n'
comment|'# if not set from the command line, try the config file'
nl|'\n'
dedent|''
dedent|''
name|'if'
...
name|'die'
op|'('
string|'"Invalid value \'%s\' for config git-p4.conflict"'
op|'%'
name|'val'
op|')'
newline|'\n'
...
newline|'\n'
nl|'\n'
comment|'# Check for the existance of P4 branches'
nl|'\n'
dedent|''
name|'branchesDetected'
op|'='
op|'('
name|'len'
op|'('
...
indent|'            '
name|'print'
string|'"Would synchronize p4 checkout in %s"'
op|'%'
name|'self'
op|'.'
name|'clientPath'
...
comment|'#'
nl|'\n'
comment|'# Apply the commits, one at a time.  On failure, ask if should'
nl|'\n'
comment|'# continue to try the rest of the patches, or quit.'
nl|'\n'
comment|'#'
nl|'\n'
dedent|''
...
indent|'                    '
name|'print'
string|'"Processing only the first commit due to option"'
string|'" --prepare-p4-only"'
newline|'\n'
name|'break'
newline|'\n'
...
op|':'
newline|'\n'
comment|'# prompt for what to do, or use the option/variable'
nl|'\n'
indent|'                        '
name|'if'
name|'self'
...
name|'raw_input'
op|'('
string|'"[s]kip this commit but apply"'
nl|'\n'
string|'" the rest, or [q]uit? "'
op|')'
newline|'\n'
name|'if'
name|'not'
...
name|'die'
op|'('
string|'"Unknown conflict_behavior \'%s\'"'
op|'%'
nl|'\n'
name|'self'
op|'.'
...
indent|'                            '
name|'print'
string|'"Skipping this commit, but applying the rest"'
newline|'\n'
name|'break'
newline|'\n'
dedent|''
...
newline|'\n'
nl|'\n'
comment|'# exit with error unless everything applied perfectly'
nl|'\n'
dedent|''
name|'if'
name|'len'
...
name|'client_name'
newline|'\n'
comment|'# cache results of "p4 where" to lookup client file locations'
nl|'\n'
name|'self'
op|'.'
name|'client_spec_path_cache'
op|'='
op|'{'
op|'}'
newline|'\n'
...
newline|'\n'
nl|'\n'
comment|'# prefix - means exclude this path, leave out of mappings'
nl|'\n'
dedent|''
name|'exclude'
op|'='
...
newline|'\n'
nl|'\n'
DECL|member|convert_client_path
dedent|''
dedent|''
name|'def'
...
op|':'
newline|'\n'
comment|'# chop off //client/ part to make it relative'
nl|'\n'
indent|'        '
name|'if'
name|'not'
...
name|'die'
op|'('
string|'"No prefix \'%s\' on clientFile \'%s\'"'
op|'%'
nl|'\n'
op|'('
name|'self'
...
newline|'\n'
nl|'\n'
DECL|member|update_client_spec_path_cache
dedent|''
name|'def'
...
newline|'\n'
indent|'        '
string|'""" Caching file paths by "p4 where" batch query """'
newline|'\n'
nl|'\n'
comment|'# List depot file paths exclude that already cached'
nl|'\n'
name|'fileArgs'
op|'='
op|'['
...
op|':'
newline|'\n'
comment|'# assume error is "... file(s) not in client view"'
nl|'\n'
indent|'                '
name|'continue'
newline|'\n'
...
name|'die'
op|'('
string|'"No clientFile in \'p4 where\' output"'
op|')'
newline|'\n'
dedent|''
name|'if'
...
op|':'
newline|'\n'
comment|'# it will list all of them, but only one not unmap-ped'
nl|'\n'
indent|'                '
name|'continue'
newline|'\n'
...
newline|'\n'
nl|'\n'
comment|'# not found files or unmap files set to ""'
nl|'\n'
dedent|''
name|'for'
name|'depotFile'
...
name|'die'
op|'('
string|'"Error: %s is not found in client spec path"'
op|'%'
name|'depot_path'
op|')'
newline|'\n'
...
name|'help'
op|'='
string|'"Maximum number of changes to import"'
op|')'
op|','
nl|'\n'
name|'optparse'
...
name|'make_option'
op|'('
string|'"--changes-block-size"'
op|','
name|'dest'
op|'='
string|'"changes_block_size"'
op|','
name|'type'
op|'='
string|'"int"'
...
name|'help'
op|'='
string|'"Internal block size to use when iteratively calling p4 changes"'
op|')'
op|','
nl|'\n'
name|'optparse'
...
newline|'\n'
indent|'            '
name|'largeFileSystemConstructor'
op|'='
name|'globals'
op|'('
op|')'
...
newline|'\n'
nl|'\n'
DECL|member|extractJobsFromCommit
dedent|''
name|'def'
name|'extractJobsFromCommit'
op|'('
name|'self'
op|','
name|'commit'
...
newline|'\n'
indent|'        '
string|'"""When streaming files, this is called to map a p4 depot path\n           to where it should go in git.  The prefixes are either\n           self.depotPaths, or self.branchPrefixes in the case of\n           branch detection."""'
newline|'\n'
nl|'\n'
name|'if'
name|'self'
...
op|':'
newline|'\n'
comment|'# branch detection moves files up a level (the branch name)'
nl|'\n'
comment|'# from what client spec interpretation gives'
nl|'\n'
indent|'            '
name|'path'
op|'='
...
op|':'
newline|'\n'
comment|'# Preserve everything in relative path name except leading'
nl|'\n'
comment|'# //depot/; just look at first prefix as they all should'
nl|'\n'
comment|'# be in the same depot.'
nl|'\n'
indent|'            '
name|'depot'
op|'='
...
newline|'\n'
indent|'        '
string|'"""Look at each depotFile in the commit to figure out to what\n           branch it belongs."""'
newline|'\n'
nl|'\n'
name|'if'
name|'self'
...
newline|'\n'
nl|'\n'
comment|'# start with the full relative path where this file would'
nl|'\n'
comment|'# go in a p4 client'
nl|'\n'
name|'if'
name|'self'
op|'.'
...
op|':'
newline|'\n'
comment|'# add a trailing slash so that a commit into qt/4.2foo'
nl|'\n'
comment|"# doesn't end up in qt/4.2, e.g."
nl|'\n'
indent|'                '
name|'if'
name|'relPath'
...
string|'"120000"'
newline|'\n'
comment|'# p4 print on a symlink sometimes contains "target\\n";'
nl|'\n'
comment|'# if it does, remove the newline'
nl|'\n'
name|'data'
op|'='
string|"''"
...
op|':'
newline|'\n'
comment|'# Some version of p4 allowed creating a symlink that pointed'
nl|'\n'
comment|'# to nothing.  This causes p4 errors when checking out such'
nl|'\n'
comment|'# a change, and errors here too.  Work around it by ignoring'
nl|'\n'
comment|'# the bad symlink; hopefully a future change fixes it.'
nl|'\n'
indent|'                '
name|'print'
string|'"\\nIgnoring empty symlink in %s"'
op|'%'
name|'file'
op|'['
string|"'depotFile'"
...
comment|'#'
nl|'\n'
comment|'# On windows, the newlines will always be mangled by print, so put'
nl|'\n'
comment|'# them back too.  This is not needed to the cygwin windows version,'
nl|'\n'
comment|'# just the native "NT" type.'
nl|'\n'
comment|'#'
nl|'\n'
indent|'            '
...
indent|'                '
name|'if'
string|"'Translation of file content failed'"
name|'in'
name|'str'
op|'('
name|'e'
...
indent|'                '
name|'print'
string|"'Path with non-ASCII characters detected. Used %s to encode: %s '"
op|'%'
op|'('
name|'encoding'
op|','
...
newline|'\n'
nl|'\n'
comment|'# catch p4 errors and complain'
nl|'\n'
indent|'        '
name|'err'
op|'='
...
name|'err'
op|'='
string|"'Not enough space left on %s! Free at least %i MB.'"
op|'%'
op|'('
nl|'\n'
name|'os'
...
op|']'
newline|'\n'
comment|'# force a failure in fast-import, else an empty'
nl|'\n'
comment|'# commit will be made'
nl|'\n'
dedent|''
dedent|''
name|'self'
...
op|')'
newline|'\n'
comment|'# ignore errors, but make sure it exits first'
nl|'\n'
name|'self'
op|'.'
name|'importProcess'
...
name|'die'
op|'('
string|'"Error from p4 print for %s: %s"'
op|'%'
op|'('
name|'f'
op|','
...
name|'print'
op|'('
string|"'Ignoring file outside of client spec: {0}'"
op|'.'
name|'format'
op|'('
name|'path'
...
newline|'\n'
nl|'\n'
DECL|member|hasBranchPrefix
dedent|''
name|'def'
name|'hasBranchPrefix'
op|'('
name|'self'
op|','
name|'path'
...
name|'print'
op|'('
string|"'Ignoring file outside of prefix: {0}'"
op|'.'
name|'format'
op|'('
name|'path'
...
name|'gitConfigBool'
op|'('
string|"'git-p4.keepEmptyCommits'"
op|')'
op|':'
newline|'\n'
indent|'            '
name|'print'
op|'('
string|"'Ignoring revision {0} as it would produce an empty commit.'"
nl|'\n'
op|'.'
name|'format'
op|'('
...
op|'%'
name|'changelist'
comment|'# use a fast-import mark'
newline|'\n'
name|'commitFound'
op|'='
name|'True'
...
indent|'                        '
name|'print'
string|'"importing label %s: could not find git commit for changelist %d"'
op|'%'
op|'('
name|'name'
op|','
...
op|')'
newline|'\n'
comment|'# only needed once, to connect to the previous commit'
nl|'\n'
name|'self'
op|'.'
name|'initialParent'
...
indent|'                    '
name|'print'
string|'\'Syncing with origin first, using "git fetch origin"\''
newline|'\n'
dedent|''
name|'system'
op|'('
...
newline|'\n'
nl|'\n'
comment|'# branches holds mapping from branch name to sha1'
nl|'\n'
dedent|''
name|'branches'
op|'='
...
newline|'\n'
nl|'\n'
comment|'# restrict to just this one, disabling detect-branches'
nl|'\n'
name|'if'
name|'branch_arg_given'
op|':'
...
newline|'\n'
nl|'\n'
comment|'# accept multiple ref name abbreviations:'
nl|'\n'
comment|'#    refs/foo/bar/branch -> use it exactly'
nl|'\n'
comment|'#    p4/branch -> prepend refs/remotes/ or refs/heads/'
nl|'\n'
comment|'#    branch -> prepend refs/remotes/p4/ or refs/heads/p4/'
nl|'\n'
dedent|''
dedent|''
dedent|''
...
newline|'\n'
nl|'\n'
comment|'# --detect-branches may change this for each branch'
nl|'\n'
name|'self'
op|'.'
name|'branchPrefixes'
...
newline|'\n'
nl|'\n'
comment|'# The default branch is master, unless --branch is used to'
nl|'\n'
comment|'# specify something else.  Make sure it exists, or complain'
nl|'\n'
comment|'# nicely about how to use --branch.'
nl|'\n'
dedent|''
name|'if'
name|'not'
...
name|'die'
op|'('
string|'"Error: branch %s does not exist."'
op|'%'
name|'self'
op|'.'
name|'branch'
...
name|'die'
op|'('
string|'"Error: no branch %s; perhaps specify one with --branch."'
op|'%'
nl|'\n'
name|'self'
op|'.'
...
op|':'
newline|'\n'
comment|'# build on a previous revision'
nl|'\n'
indent|'                        '
name|'self'
op|'.'
...
newline|'\n'
nl|'\n'
comment|'# Create a symbolic ref p4/HEAD pointing to p4/<branch> to allow'
nl|'\n'
comment|'# a convenient shortcut refname "p4".'
nl|'\n'
dedent|''
name|'if'
name|'self'
...
name|'die'
op|'('
string|'"You have uncommitted changes. Please commit them before rebasing or stash them away with git stash."'
op|')'
op|';'
newline|'\n'
nl|'\n'
...
name|'write'
op|'('
string|'\'Depot paths must start with "//": %s\\n\''
op|'%'
name|'p'
op|')'
newline|'\n'
...
newline|'\n'
nl|'\n'
comment|'# create a master branch and check out a work tree'
nl|'\n'
dedent|''
name|'if'
name|'gitBranchExists'
...
indent|'            '
name|'print'
string|"'Not checking out any branch, use '"
string|'\'"git checkout -q -b master <branch>"\''
newline|'\n'
nl|'\n'
