...
argument_list|)
expr_stmt|;
comment|/* 	 * NEEDSWORK: If the original index is crafted, this copy could 	 * go unchecked. 	 */
name|memcpy
argument_list|(
name|ce
operator|->
...
literal|7
decl_stmt|;
comment|/* 	 * Alignment can cause differences. This should be "alignof", but 	 * since that's a gcc'ism, just use the size of a pointer. 	 */
name|per_entry
operator|+=
sizeof|sizeof
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 	 * The disk format is actually larger than the in-memory format, 	 * due to space for nsec etc, so even though the in-memory one 	 * has room for a few  more flags, we can allocate using the same 	 * index size 	 */
name|istate
operator|->
name|alloc
operator|=
