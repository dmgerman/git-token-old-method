...
end_ifndef
begin_comment
comment|/*  * platform's underlying implementation of SHA-1; could be OpenSSL,  * blk_SHA, Apple CommonCrypto, etc...  Note that including  * SHA1_HEADER may have already defined platform_SHA_CTX for our  * own implementations like block-sha1 and ppc-sha1, so we list  * the default for OpenSSL compatible SHA-1 implementations here.  */
end_comment
begin_define
...
end_function_decl
begin_comment
comment|/* The length in bytes and in hex digits of an object name (SHA-1 value). */
end_comment
begin_define
...
end_define
begin_comment
comment|/*  * A "directory link" is a link to another git directory.  *  * The value 0160000 is not normally a valid mode, and  * also just happens to be S_IFDIR + S_IFLNK  */
end_comment
begin_define
...
name|namelen
parameter_list|)
value|index_dir_exists(&the_index, (name), (namelen))
end_define
begin_define
...
define|#
directive|define
name|READ_GITFILE_ERR_STAT_FAILED
value|1
end_define
begin_define
...
define|#
directive|define
name|READ_GITFILE_ERR_OPEN_FAILED
value|3
end_define
begin_define
...
define|#
directive|define
name|READ_GITFILE_ERR_READ_FAILED
value|4
end_define
begin_define
...
define|#
directive|define
name|READ_GITFILE_ERR_INVALID_FORMAT
value|5
end_define
begin_define
...
name|path
parameter_list|)
value|read_gitfile_gently((path), NULL)
end_define
begin_function_decl
specifier|extern
specifier|const
...
parameter_list|)
define|\
value|do { \ 		if ((nr)> alloc) { \ 			if (alloc_nr(alloc)< (nr)) \ 				alloc = (nr); \ 			else \ 				alloc = alloc_nr(alloc); \ 			REALLOC_ARRAY(x, alloc); \ 		} \ 	} while (0)
end_define
begin_comment
...
end_comment
begin_enum
DECL|enum|hide_dotfiles_type
enum|enum
name|hide_dotfiles_type
block|{
DECL|enumerator|HIDE_DOTFILES_FALSE
name|HIDE_DOTFILES_FALSE
init|=
literal|0
block|,
DECL|enumerator|HIDE_DOTFILES_TRUE
name|HIDE_DOTFILES_TRUE
block|,
DECL|enumerator|HIDE_DOTFILES_DOTGITONLY
...
name|version
decl_stmt|;
DECL|member|precious_objects
name|int
name|precious_objects
decl_stmt|;
DECL|member|is_bare
name|int
name|is_bare
decl_stmt|;
DECL|member|work_tree
name|char
modifier|*
name|work_tree
decl_stmt|;
DECL|member|unknown_extensions
name|struct
name|string_list
name|unknown_extensions
decl_stmt|;
block|}
struct|;
end_struct
begin_comment
comment|/*  * Read the repository format characteristics from the config file "path" into  * "format" struct. Returns the numeric version. On error, -1 is returned,  * format->version is set to -1, and all other fields in the struct are  * undefined.  */
end_comment
begin_function_decl
name|int
...
end_function_decl
begin_comment
comment|/*  * Verify that the repository described by repository_format is something we  * can read. If it is, return 0. Otherwise, return -1, and "err" will describe  * any errors encountered.  */
end_comment
begin_function_decl
name|int
...
end_function_decl
begin_comment
comment|/*  * You can define a static memoized git path like:  *  *    static GIT_PATH_FUNC(git_path_foo, "FOO");  *  * or use one of the global ones below.  */
end_comment
begin_define
DECL|macro|GIT_PATH_FUNC
define|#
directive|define
name|GIT_PATH_FUNC
parameter_list|(
...
parameter_list|)
define|\
value|const char *func(void) \ 	{ \ 		static char *ret; \ 		if (!ret) \ 			ret = git_pathdup(filename); \ 		return ret; \ 	}
end_define
begin_function_decl
specifier|const
name|char
...
end_function_decl
begin_comment
comment|/*  * Return an abbreviated sha1 unique within this repository's object database.  * The result will be at least `len` characters long, and will be NUL  * terminated.  *  * The non-`_r` version returns a static buffer which will be overwritten by  * subsequent calls.  *  * The `_r` variant writes to a buffer supplied by the caller, which must be at  * least `GIT_SHA1_HEXSZ + 1` bytes. The return value is the number of bytes  * written (excluding the NUL terminator).  *  * Note that while this version avoids the static buffer, it is not fully  * reentrant, as it calls into other non-reentrant git code.  */
end_comment
begin_function_decl
specifier|extern
specifier|const
...
end_function_decl
begin_comment
comment|/*  * Convert a binary sha1 to its hex equivalent. The `_r` variant is reentrant,  * and writes the NUL-terminated output to the buffer `out`, which must be at  * least `GIT_SHA1_HEXSZ + 1` bytes, and returns a pointer to out for  * convenience.  *  * The non-`_r` variant returns a static buffer, but uses a ring of 4  * buffers, making it safe to make multiple calls for a single statement, like:  *  *   printf("%s -> %s", sha1_to_hex(one), sha1_to_hex(two));  */
end_comment
begin_function_decl
specifier|extern
name|char
...
end_function_decl
begin_comment
comment|/* same static buffer as sha1_to_hex */
end_comment
begin_function_decl
specifier|extern
name|int
...
end_struct
begin_comment
comment|/*  * Convenience helper for passing a constant type, like:  *  *   show_date(t, tz, DATE_MODE(NORMAL));  */
end_comment
begin_define
...
name|t
parameter_list|)
value|date_mode_from_type(DATE_##t)
end_define
begin_function_decl
name|struct
name|date_mode
modifier|*
name|date_mode_from_type
parameter_list|(
name|enum
name|date_mode_type
name|type
...
end_function_decl
begin_enum
DECL|enum|config_scope
enum|enum
name|config_scope
block|{
DECL|enumerator|CONFIG_SCOPE_UNKNOWN
name|CONFIG_SCOPE_UNKNOWN
init|=
literal|0
block|,
DECL|enumerator|CONFIG_SCOPE_SYSTEM
name|CONFIG_SCOPE_SYSTEM
block|,
DECL|enumerator|CONFIG_SCOPE_GLOBAL
name|CONFIG_SCOPE_GLOBAL
block|,
DECL|enumerator|CONFIG_SCOPE_REPO
name|CONFIG_SCOPE_REPO
block|,
DECL|enumerator|CONFIG_SCOPE_CMDLINE
name|CONFIG_SCOPE_CMDLINE
block|, }
enum|;
end_enum
begin_function_decl
...
end_struct
begin_comment
comment|/*  * the contents of the list are ordered according to their  * position in the config files and order of parsing the files.  * (i.e. key-value pair at the last position of .git/config will  * be at the last item of the list)  */
end_comment
begin_struct
...
specifier|extern
name|int
name|git_configset_get_string_const
parameter_list|(
name|struct
name|config_set
modifier|*
...
specifier|extern
name|int
name|git_configset_get_bool_or_int
parameter_list|(
name|struct
name|config_set
modifier|*
...
specifier|extern
name|int
name|git_configset_get_maybe_bool
parameter_list|(
name|struct
name|config_set
modifier|*
...
specifier|extern
name|int
name|git_configset_get_pathname
parameter_list|(
name|struct
name|config_set
modifier|*
...
specifier|extern
name|int
name|git_config_get_bool_or_int
parameter_list|(
specifier|const
name|char
modifier|*
...
specifier|extern
name|int
name|git_config_get_maybe_bool
parameter_list|(
specifier|const
name|char
modifier|*
