...
end_function
begin_comment
comment|/*  * Sanitize a string from the client so that it's OK to be inserted into a  * filesystem path. Specifically, we disallow slashes, runs of "..", and  * trailing and leading dots, which means that the client cannot escape  * our base path via ".." traversal.  */
end_comment
begin_function
DECL|function|sanitize_client_strbuf
specifier|static
name|void
name|sanitize_client_strbuf
parameter_list|(
name|struct
name|strbuf
modifier|*
...
end_function
begin_comment
comment|/*  * Like sanitize_client, but we also perform any canonicalization  * to make life easier on the admin.  */
end_comment
begin_function
