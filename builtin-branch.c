...
index|[]
init|=
literal|"git-branch [-r] (-d | -D)<branchname> | [--track | --no-track] [-l] [-f]<branchname> [<start-point>] | (-m | -M) [<oldbranch>]<newbranch> | [--color | --no-color] [-r | -a] [-v [--abbrev=<length> | --no-abbrev]]"
decl_stmt|;
end_decl_stmt
begin_define
...
begin_comment
DECL|variable|branch_track
comment|/* 0 = none, 1 = remotes, 2 = all */
end_comment
begin_decl_stmt
...
end_decl_stmt
begin_function
DECL|function|get_remote_branch_name
specifier|static
name|int
name|get_remote_branch_name
parameter_list|(
specifier|const
name|char
modifier|*
...
argument_list|)
expr_stmt|;
comment|/* 	 * Try an exact match first.  I.e. handle the case where the 	 * value is "$anything:refs/foo/bar/baz" and start_ref is exactly 	 * "refs/foo/bar/baz". Then the name at the remote is $anything. 	 */
if|if
condition|(
operator|!
name|strcmp
...
condition|)
block|{
comment|/* Truncate the value before the colon. */
name|nfasprintf
argument_list|(
operator|&
name|config_repo
...
return|;
block|}
comment|/* 	 * Is this a wildcard match? 	 */
if|if
condition|(
operator|(
name|end
...
literal|0
return|;
comment|/* 	 * Value is "refs/foo/bar/<asterisk>:refs/baz/boa/<asterisk>" 	 * and start_ref begins with "refs/baz/boa/"; the name at the 	 * remote is refs/foo/bar/ with the remaining part of the 	 * start_ref.  The length of the prefix on the RHS is (end - 	 * colon - 2), including the slash immediately before the 	 * asterisk. 	 */
if|if
condition|(
operator|(
name|strlen
...
literal|0
return|;
comment|/* does not match prefix */
comment|/* Replace the asterisk with the remote branch name.  */
name|nfasprintf
argument_list|(
operator|&
name|config_repo
...
literal|0
return|;
comment|/* 	 * Ok, we are looking at key == "remote.$foo.fetch"; 	 */
if|if
condition|(
name|get_remote_branch_name
argument_list|(
...
name|die
argument_list|(
literal|"what a long branch name you have!"
argument_list|)
expr_stmt|;
name|git_config_set
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 	 * We do not have to check if we have enough space for 	 * the 'merge' key, since it's shorter than the 	 * previous 'remote' key, which we already checked. 	 */
name|snprintf
argument_list|(
name|key
argument_list|,
...
parameter_list|)
block|{
comment|/* 	 * name is the name of new branch under refs/heads; 	 * real_ref is typically refs/remotes/$foo/$bar, where 	 * $foo is the remote name (there typically are no slashes) 	 * and $bar is the branch name we map from the remote 	 * (it could have slashes). 	 */
name|start_ref
operator|=
name|real_ref
expr_stmt|;
