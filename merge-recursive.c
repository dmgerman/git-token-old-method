...
end_function
begin_comment
comment|/* Low-level merge functions */
end_comment
begin_typedef
...
decl_stmt|;
block|}
DECL|variable|ll_user_merge_fns
DECL|variable|ll_user_merge_fns_tail
modifier|*
name|ll_user_merge_fns
struct|,
modifier|*
modifier|*
name|ll_user_merge_fns_tail
struct|;
end_struct
begin_function
...
argument_list|)
return|;
comment|/* 	 * merge.driver is a multi-valued configuration, whose value is 	 * of form: 	 * 	 *	name command-line 	 * 	 * The command-line will be interpolated with the following 	 * tokens and is given to the shell: 	 * 	 *    %O - temporary file name for the merge base. 	 *    %A - temporary file name for our version. 	 *    %B - temporary file name for the other branches' version. 	 * 	 * The external merge driver should write the results in the file 	 * named by %A, and signal that it has done with exit status 0. 	 */
for|for
control|(
name|nlen
operator|=
...
name|error
argument_list|(
literal|"%s '%s': lacks command line"
argument_list|,
name|var
argument_list|,
name|value
...
condition|)
block|{
comment|/* Make sure we end at 100% */
if|if
condition|(
operator|!
name|total_cnt
