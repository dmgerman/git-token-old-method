...
end_struct
begin_comment
comment|/*  * Currently there is only "diff.<drivername>.command" variable;  * because there are "diff.color.<slot>" variables, we are parsing  * this in a bit convoluted way to allow low level diff driver  * called "color".  */
end_comment
begin_function
DECL|function|parse_lldiff_command
specifier|static
name|int
name|parse_lldiff_command
parameter_list|(
specifier|const
name|char
modifier|*
...
end_function
begin_comment
comment|/*  * 'diff.<what>.funcname' attribute can be specified in the configuration  * to define a customized regexp to find the beginning of a function to  * be used for hunk header lines of "diff -p" style output.  */
end_comment
begin_struct
...
name|ident
condition|)
comment|/* 		 * If the config file has "funcname.default" defined, that 		 * regexp is used; otherwise NULL is returned and xemit uses 		 * the built-in default. 		 */
return|return
name|funcname_pattern
argument_list|(
literal|"default"
