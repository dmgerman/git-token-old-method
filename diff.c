...
argument_list|)
return|;
comment|/* diff-delta does not like to do delta with empty, so 	 * we do that by hand here.  Sigh... 	 */
if|if
condition|(
operator|!
name|src_size
condition|)
comment|/* literal copy can be done only 127-byte at a time. 		 */
name|bufsize
operator|=
name|dst_size
operator|+
...
name|die
argument_list|(
literal|"unable to generate binary diff"
argument_list|)
expr_stmt|;
comment|/* emit delta encoded in base85 */
name|cp
operator|=
name|delta
expr_stmt|;
