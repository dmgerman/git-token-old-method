...
name|var
argument_list|,
literal|"diff.compactionheuristic"
argument_list|)
condition|)
block|{
name|diff_compaction_heuristic
...
end_function
begin_comment
comment|/*  * Keep track of files used for diffing. Sometimes such an entry  * refers to a temporary file, sometimes to an existing file, and  * sometimes to "/dev/null".  */
end_comment
begin_struct
...
name|diff_tempfile
block|{
comment|/* 	 * filename external diff should read from, or NULL if this 	 * entry is currently not in use: 	 */
DECL|member|name
specifier|const
name|char
modifier|*
...
index|]
decl_stmt|;
comment|/* 	 * If this diff_tempfile instance refers to a temporary file, 	 * this tempfile object is used to manage its lifetime. 	 */
...
name|env
argument_list|,
literal|"GIT_DIFF_PATH_COUNTER=%d"
argument_list|,
operator|++
name|o
operator|->
...
name|_
argument_list|(
literal|"--follow requires exactly one pathspec"
argument_list|)
argument_list|)
expr_stmt|;
block|}
...
argument_list|()
expr_stmt|;
comment|/* 	 * If there is a negation e.g. 'd' in the input, and we haven't 	 * initialized the filter field with another --diff-filter, start 	 * from full set of bits, except for AON. 	 */
if|if
condition|(
operator|!
name|opt
...
name|error
argument_list|(
literal|"unknown value after ws-error-highlight=%.*s"
argument_list|,
call|(
name|int
call|)
...
name|arg
argument_list|,
literal|"--no-compaction-heuristic"
argument_list|)
condition|)
name|DIFF_XDL_CLR
argument_list|(
...
name|die
argument_list|(
literal|"unknown change class '%c' in --diff-filter=%s"
argument_list|,
name|offending
argument_list|,
name|optarg
...
specifier|static
name|int
name|diff_filespec_check_stat_unmatch
parameter_list|(
name|struct
name|diff_filepair
modifier|*
