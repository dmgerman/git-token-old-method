...
parameter_list|)
block|{
comment|/* 	 * Look at the line and see from which parents we have difference. 	 * Lower bits of sline->flag records if the parent had this line, 	 * so XOR with all_mask gives us on-bits for parents we have 	 * differences with. 	 */
name|unsigned
name|long
name|parents
init|=
...
condition|)
return|return;
comment|/* Look at each hunk, and if it contains changes from only 	 * one parent, mark that uninteresting. 	 */
name|i
operator|=
literal|0
expr_stmt|;
...
condition|)
break|break;
comment|/* [i..hunk_end) are interesting.  Now is it from 		 * only one parent? 		 * If lost lines are only from one parent and 		 * remaining lines existed in parents other than 		 * that parent, then the hunk is not that interesting. 		 */
name|parents
operator|=
literal|0
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|/* Now, how many bits from [0..num_parent) are on? */
for|for
control|(
name|j
operator|=
