...
condition|)
block|{
comment|/* 		 * Either we failed, or the format actually produces a 0-length 		 * output. There's not much we can do, so we leave it blank. 		 * However, the output array is left in an undefined state, so 		 * we must re-assert our NUL terminator. 		 */
name|sb
operator|->
name|buf
index|[
