begin_unit
begin_include
include|#
directive|include
file|"cache.h"
end_include
begin_include
include|#
directive|include
file|"lockfile.h"
end_include
begin_include
include|#
directive|include
file|"string-list.h"
end_include
begin_include
include|#
directive|include
file|"rerere.h"
end_include
begin_include
include|#
directive|include
file|"xdiff-interface.h"
end_include
begin_include
include|#
directive|include
file|"dir.h"
end_include
begin_include
include|#
directive|include
file|"resolve-undo.h"
end_include
begin_include
include|#
directive|include
file|"ll-merge.h"
end_include
begin_include
include|#
directive|include
file|"attr.h"
end_include
begin_include
include|#
directive|include
file|"pathspec.h"
end_include
begin_include
include|#
directive|include
file|"sha1-lookup.h"
end_include
begin_define
DECL|macro|RESOLVED
define|#
directive|define
name|RESOLVED
value|0
end_define
begin_define
DECL|macro|PUNTED
define|#
directive|define
name|PUNTED
value|1
end_define
begin_define
DECL|macro|THREE_STAGED
define|#
directive|define
name|THREE_STAGED
value|2
end_define
begin_decl_stmt
DECL|variable|RERERE_RESOLVED
name|void
modifier|*
name|RERERE_RESOLVED
init|=
operator|&
name|RERERE_RESOLVED
decl_stmt|;
end_decl_stmt
begin_comment
comment|/* if rerere_enabled == -1, fall back to detection of .git/rr-cache */
end_comment
begin_decl_stmt
DECL|variable|rerere_enabled
specifier|static
name|int
name|rerere_enabled
init|=
operator|-
literal|1
decl_stmt|;
end_decl_stmt
begin_comment
comment|/* automatically update cleanly resolved paths to the index */
end_comment
begin_decl_stmt
DECL|variable|rerere_autoupdate
specifier|static
name|int
name|rerere_autoupdate
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|rerere_dir_nr
specifier|static
name|int
name|rerere_dir_nr
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|rerere_dir_alloc
specifier|static
name|int
name|rerere_dir_alloc
decl_stmt|;
end_decl_stmt
begin_define
DECL|macro|RR_HAS_POSTIMAGE
define|#
directive|define
name|RR_HAS_POSTIMAGE
value|1
end_define
begin_define
DECL|macro|RR_HAS_PREIMAGE
define|#
directive|define
name|RR_HAS_PREIMAGE
value|2
end_define
begin_struct
DECL|struct|rerere_dir
specifier|static
struct|struct
name|rerere_dir
block|{
DECL|member|sha1
name|unsigned
name|char
name|sha1
index|[
literal|20
index|]
decl_stmt|;
DECL|member|status_alloc
DECL|member|status_nr
name|int
name|status_alloc
decl_stmt|,
name|status_nr
decl_stmt|;
DECL|member|status
name|unsigned
name|char
modifier|*
name|status
decl_stmt|;
block|}
DECL|variable|rerere_dir
modifier|*
modifier|*
name|rerere_dir
struct|;
end_struct
begin_function
DECL|function|free_rerere_dirs
specifier|static
name|void
name|free_rerere_dirs
parameter_list|(
name|void
parameter_list|)
block|{
name|int
name|i
decl_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|rerere_dir_nr
condition|;
name|i
operator|++
control|)
block|{
name|free
argument_list|(
name|rerere_dir
index|[
name|i
index|]
operator|->
name|status
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|rerere_dir
index|[
name|i
index|]
argument_list|)
expr_stmt|;
block|}
name|free
argument_list|(
name|rerere_dir
argument_list|)
expr_stmt|;
name|rerere_dir_nr
operator|=
name|rerere_dir_alloc
operator|=
literal|0
expr_stmt|;
name|rerere_dir
operator|=
name|NULL
expr_stmt|;
block|}
end_function
begin_function
DECL|function|free_rerere_id
specifier|static
name|void
name|free_rerere_id
parameter_list|(
name|struct
name|string_list_item
modifier|*
name|item
parameter_list|)
block|{
name|free
argument_list|(
name|item
operator|->
name|util
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|rerere_id_hex
specifier|static
specifier|const
name|char
modifier|*
name|rerere_id_hex
parameter_list|(
specifier|const
name|struct
name|rerere_id
modifier|*
name|id
parameter_list|)
block|{
return|return
name|sha1_to_hex
argument_list|(
name|id
operator|->
name|collection
operator|->
name|sha1
argument_list|)
return|;
block|}
end_function
begin_function
DECL|function|fit_variant
specifier|static
name|void
name|fit_variant
parameter_list|(
name|struct
name|rerere_dir
modifier|*
name|rr_dir
parameter_list|,
name|int
name|variant
parameter_list|)
block|{
name|variant
operator|++
expr_stmt|;
name|ALLOC_GROW
argument_list|(
name|rr_dir
operator|->
name|status
argument_list|,
name|variant
argument_list|,
name|rr_dir
operator|->
name|status_alloc
argument_list|)
expr_stmt|;
if|if
condition|(
name|rr_dir
operator|->
name|status_nr
operator|<
name|variant
condition|)
block|{
name|memset
argument_list|(
name|rr_dir
operator|->
name|status
operator|+
name|rr_dir
operator|->
name|status_nr
argument_list|,
literal|'\0'
argument_list|,
name|variant
operator|-
name|rr_dir
operator|->
name|status_nr
argument_list|)
expr_stmt|;
name|rr_dir
operator|->
name|status_nr
operator|=
name|variant
expr_stmt|;
block|}
block|}
end_function
begin_function
DECL|function|assign_variant
specifier|static
name|void
name|assign_variant
parameter_list|(
name|struct
name|rerere_id
modifier|*
name|id
parameter_list|)
block|{
name|int
name|variant
decl_stmt|;
name|struct
name|rerere_dir
modifier|*
name|rr_dir
init|=
name|id
operator|->
name|collection
decl_stmt|;
name|variant
operator|=
name|id
operator|->
name|variant
expr_stmt|;
if|if
condition|(
name|variant
operator|<
literal|0
condition|)
block|{
for|for
control|(
name|variant
operator|=
literal|0
init|;
name|variant
operator|<
name|rr_dir
operator|->
name|status_nr
condition|;
name|variant
operator|++
control|)
if|if
condition|(
operator|!
name|rr_dir
operator|->
name|status
index|[
name|variant
index|]
condition|)
break|break;
block|}
name|fit_variant
argument_list|(
name|rr_dir
argument_list|,
name|variant
argument_list|)
expr_stmt|;
name|id
operator|->
name|variant
operator|=
name|variant
expr_stmt|;
block|}
end_function
begin_function
DECL|function|rerere_path
specifier|const
name|char
modifier|*
name|rerere_path
parameter_list|(
specifier|const
name|struct
name|rerere_id
modifier|*
name|id
parameter_list|,
specifier|const
name|char
modifier|*
name|file
parameter_list|)
block|{
if|if
condition|(
operator|!
name|file
condition|)
return|return
name|git_path
argument_list|(
literal|"rr-cache/%s"
argument_list|,
name|rerere_id_hex
argument_list|(
name|id
argument_list|)
argument_list|)
return|;
if|if
condition|(
name|id
operator|->
name|variant
operator|<=
literal|0
condition|)
return|return
name|git_path
argument_list|(
literal|"rr-cache/%s/%s"
argument_list|,
name|rerere_id_hex
argument_list|(
name|id
argument_list|)
argument_list|,
name|file
argument_list|)
return|;
return|return
name|git_path
argument_list|(
literal|"rr-cache/%s/%s.%d"
argument_list|,
name|rerere_id_hex
argument_list|(
name|id
argument_list|)
argument_list|,
name|file
argument_list|,
name|id
operator|->
name|variant
argument_list|)
return|;
block|}
end_function
begin_function
DECL|function|is_rr_file
specifier|static
name|int
name|is_rr_file
parameter_list|(
specifier|const
name|char
modifier|*
name|name
parameter_list|,
specifier|const
name|char
modifier|*
name|filename
parameter_list|,
name|int
modifier|*
name|variant
parameter_list|)
block|{
specifier|const
name|char
modifier|*
name|suffix
decl_stmt|;
name|char
modifier|*
name|ep
decl_stmt|;
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|name
argument_list|,
name|filename
argument_list|)
condition|)
block|{
operator|*
name|variant
operator|=
literal|0
expr_stmt|;
return|return
literal|1
return|;
block|}
if|if
condition|(
operator|!
name|skip_prefix
argument_list|(
name|name
argument_list|,
name|filename
argument_list|,
operator|&
name|suffix
argument_list|)
operator|||
operator|*
name|suffix
operator|!=
literal|'.'
condition|)
return|return
literal|0
return|;
name|errno
operator|=
literal|0
expr_stmt|;
operator|*
name|variant
operator|=
name|strtol
argument_list|(
name|suffix
operator|+
literal|1
argument_list|,
operator|&
name|ep
argument_list|,
literal|10
argument_list|)
expr_stmt|;
if|if
condition|(
name|errno
operator|||
operator|*
name|ep
condition|)
return|return
literal|0
return|;
return|return
literal|1
return|;
block|}
end_function
begin_function
DECL|function|scan_rerere_dir
specifier|static
name|void
name|scan_rerere_dir
parameter_list|(
name|struct
name|rerere_dir
modifier|*
name|rr_dir
parameter_list|)
block|{
name|struct
name|dirent
modifier|*
name|de
decl_stmt|;
name|DIR
modifier|*
name|dir
init|=
name|opendir
argument_list|(
name|git_path
argument_list|(
literal|"rr-cache/%s"
argument_list|,
name|sha1_to_hex
argument_list|(
name|rr_dir
operator|->
name|sha1
argument_list|)
argument_list|)
argument_list|)
decl_stmt|;
if|if
condition|(
operator|!
name|dir
condition|)
return|return;
while|while
condition|(
operator|(
name|de
operator|=
name|readdir
argument_list|(
name|dir
argument_list|)
operator|)
operator|!=
name|NULL
condition|)
block|{
name|int
name|variant
decl_stmt|;
if|if
condition|(
name|is_rr_file
argument_list|(
name|de
operator|->
name|d_name
argument_list|,
literal|"postimage"
argument_list|,
operator|&
name|variant
argument_list|)
condition|)
block|{
name|fit_variant
argument_list|(
name|rr_dir
argument_list|,
name|variant
argument_list|)
expr_stmt|;
name|rr_dir
operator|->
name|status
index|[
name|variant
index|]
operator||=
name|RR_HAS_POSTIMAGE
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|is_rr_file
argument_list|(
name|de
operator|->
name|d_name
argument_list|,
literal|"preimage"
argument_list|,
operator|&
name|variant
argument_list|)
condition|)
block|{
name|fit_variant
argument_list|(
name|rr_dir
argument_list|,
name|variant
argument_list|)
expr_stmt|;
name|rr_dir
operator|->
name|status
index|[
name|variant
index|]
operator||=
name|RR_HAS_PREIMAGE
expr_stmt|;
block|}
block|}
name|closedir
argument_list|(
name|dir
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|rerere_dir_sha1
specifier|static
specifier|const
name|unsigned
name|char
modifier|*
name|rerere_dir_sha1
parameter_list|(
name|size_t
name|i
parameter_list|,
name|void
modifier|*
name|table
parameter_list|)
block|{
name|struct
name|rerere_dir
modifier|*
modifier|*
name|rr_dir
init|=
name|table
decl_stmt|;
return|return
name|rr_dir
index|[
name|i
index|]
operator|->
name|sha1
return|;
block|}
end_function
begin_function
DECL|function|find_rerere_dir
specifier|static
name|struct
name|rerere_dir
modifier|*
name|find_rerere_dir
parameter_list|(
specifier|const
name|char
modifier|*
name|hex
parameter_list|)
block|{
name|unsigned
name|char
name|sha1
index|[
literal|20
index|]
decl_stmt|;
name|struct
name|rerere_dir
modifier|*
name|rr_dir
decl_stmt|;
name|int
name|pos
decl_stmt|;
if|if
condition|(
name|get_sha1_hex
argument_list|(
name|hex
argument_list|,
name|sha1
argument_list|)
condition|)
return|return
name|NULL
return|;
comment|/* BUG */
name|pos
operator|=
name|sha1_pos
argument_list|(
name|sha1
argument_list|,
name|rerere_dir
argument_list|,
name|rerere_dir_nr
argument_list|,
name|rerere_dir_sha1
argument_list|)
expr_stmt|;
if|if
condition|(
name|pos
operator|<
literal|0
condition|)
block|{
name|rr_dir
operator|=
name|xmalloc
argument_list|(
sizeof|sizeof
argument_list|(
operator|*
name|rr_dir
argument_list|)
argument_list|)
expr_stmt|;
name|hashcpy
argument_list|(
name|rr_dir
operator|->
name|sha1
argument_list|,
name|sha1
argument_list|)
expr_stmt|;
name|rr_dir
operator|->
name|status
operator|=
name|NULL
expr_stmt|;
name|rr_dir
operator|->
name|status_nr
operator|=
literal|0
expr_stmt|;
name|rr_dir
operator|->
name|status_alloc
operator|=
literal|0
expr_stmt|;
name|pos
operator|=
operator|-
literal|1
operator|-
name|pos
expr_stmt|;
comment|/* Make sure the array is big enough ... */
name|ALLOC_GROW
argument_list|(
name|rerere_dir
argument_list|,
name|rerere_dir_nr
operator|+
literal|1
argument_list|,
name|rerere_dir_alloc
argument_list|)
expr_stmt|;
comment|/* ... and add it in. */
name|rerere_dir_nr
operator|++
expr_stmt|;
name|memmove
argument_list|(
name|rerere_dir
operator|+
name|pos
operator|+
literal|1
argument_list|,
name|rerere_dir
operator|+
name|pos
argument_list|,
operator|(
name|rerere_dir_nr
operator|-
name|pos
operator|-
literal|1
operator|)
operator|*
sizeof|sizeof
argument_list|(
operator|*
name|rerere_dir
argument_list|)
argument_list|)
expr_stmt|;
name|rerere_dir
index|[
name|pos
index|]
operator|=
name|rr_dir
expr_stmt|;
name|scan_rerere_dir
argument_list|(
name|rr_dir
argument_list|)
expr_stmt|;
block|}
return|return
name|rerere_dir
index|[
name|pos
index|]
return|;
block|}
end_function
begin_function
DECL|function|has_rerere_resolution
specifier|static
name|int
name|has_rerere_resolution
parameter_list|(
specifier|const
name|struct
name|rerere_id
modifier|*
name|id
parameter_list|)
block|{
specifier|const
name|int
name|both
init|=
name|RR_HAS_POSTIMAGE
operator||
name|RR_HAS_PREIMAGE
decl_stmt|;
name|int
name|variant
init|=
name|id
operator|->
name|variant
decl_stmt|;
if|if
condition|(
name|variant
operator|<
literal|0
condition|)
return|return
literal|0
return|;
return|return
operator|(
operator|(
name|id
operator|->
name|collection
operator|->
name|status
index|[
name|variant
index|]
operator|&
name|both
operator|)
operator|==
name|both
operator|)
return|;
block|}
end_function
begin_function
DECL|function|new_rerere_id_hex
specifier|static
name|struct
name|rerere_id
modifier|*
name|new_rerere_id_hex
parameter_list|(
name|char
modifier|*
name|hex
parameter_list|)
block|{
name|struct
name|rerere_id
modifier|*
name|id
init|=
name|xmalloc
argument_list|(
sizeof|sizeof
argument_list|(
operator|*
name|id
argument_list|)
argument_list|)
decl_stmt|;
name|id
operator|->
name|collection
operator|=
name|find_rerere_dir
argument_list|(
name|hex
argument_list|)
expr_stmt|;
name|id
operator|->
name|variant
operator|=
operator|-
literal|1
expr_stmt|;
comment|/* not known yet */
return|return
name|id
return|;
block|}
end_function
begin_function
DECL|function|new_rerere_id
specifier|static
name|struct
name|rerere_id
modifier|*
name|new_rerere_id
parameter_list|(
name|unsigned
name|char
modifier|*
name|sha1
parameter_list|)
block|{
return|return
name|new_rerere_id_hex
argument_list|(
name|sha1_to_hex
argument_list|(
name|sha1
argument_list|)
argument_list|)
return|;
block|}
end_function
begin_comment
comment|/*  * $GIT_DIR/MERGE_RR file is a collection of records, each of which is  * "conflict ID", a HT and pathname, terminated with a NUL, and is  * used to keep track of the set of paths that "rerere" may need to  * work on (i.e. what is left by the previous invocation of "git  * rerere" during the current conflict resolution session).  */
end_comment
begin_function
DECL|function|read_rr
specifier|static
name|void
name|read_rr
parameter_list|(
name|struct
name|string_list
modifier|*
name|rr
parameter_list|)
block|{
name|struct
name|strbuf
name|buf
init|=
name|STRBUF_INIT
decl_stmt|;
name|FILE
modifier|*
name|in
init|=
name|fopen
argument_list|(
name|git_path_merge_rr
argument_list|()
argument_list|,
literal|"r"
argument_list|)
decl_stmt|;
if|if
condition|(
operator|!
name|in
condition|)
return|return;
while|while
condition|(
operator|!
name|strbuf_getwholeline
argument_list|(
operator|&
name|buf
argument_list|,
name|in
argument_list|,
literal|'\0'
argument_list|)
condition|)
block|{
name|char
modifier|*
name|path
decl_stmt|;
name|unsigned
name|char
name|sha1
index|[
literal|20
index|]
decl_stmt|;
name|struct
name|rerere_id
modifier|*
name|id
decl_stmt|;
name|int
name|variant
decl_stmt|;
comment|/* There has to be the hash, tab, path and then NUL */
if|if
condition|(
name|buf
operator|.
name|len
operator|<
literal|42
operator|||
name|get_sha1_hex
argument_list|(
name|buf
operator|.
name|buf
argument_list|,
name|sha1
argument_list|)
condition|)
name|die
argument_list|(
literal|"corrupt MERGE_RR"
argument_list|)
expr_stmt|;
if|if
condition|(
name|buf
operator|.
name|buf
index|[
literal|40
index|]
operator|!=
literal|'.'
condition|)
block|{
name|variant
operator|=
literal|0
expr_stmt|;
name|path
operator|=
name|buf
operator|.
name|buf
operator|+
literal|40
expr_stmt|;
block|}
else|else
block|{
name|errno
operator|=
literal|0
expr_stmt|;
name|variant
operator|=
name|strtol
argument_list|(
name|buf
operator|.
name|buf
operator|+
literal|41
argument_list|,
operator|&
name|path
argument_list|,
literal|10
argument_list|)
expr_stmt|;
if|if
condition|(
name|errno
condition|)
name|die
argument_list|(
literal|"corrupt MERGE_RR"
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|*
operator|(
name|path
operator|++
operator|)
operator|!=
literal|'\t'
condition|)
name|die
argument_list|(
literal|"corrupt MERGE_RR"
argument_list|)
expr_stmt|;
name|buf
operator|.
name|buf
index|[
literal|40
index|]
operator|=
literal|'\0'
expr_stmt|;
name|id
operator|=
name|new_rerere_id_hex
argument_list|(
name|buf
operator|.
name|buf
argument_list|)
expr_stmt|;
name|id
operator|->
name|variant
operator|=
name|variant
expr_stmt|;
name|string_list_insert
argument_list|(
name|rr
argument_list|,
name|path
argument_list|)
operator|->
name|util
operator|=
name|id
expr_stmt|;
block|}
name|strbuf_release
argument_list|(
operator|&
name|buf
argument_list|)
expr_stmt|;
name|fclose
argument_list|(
name|in
argument_list|)
expr_stmt|;
block|}
end_function
begin_decl_stmt
DECL|variable|write_lock
specifier|static
name|struct
name|lock_file
name|write_lock
decl_stmt|;
end_decl_stmt
begin_function
DECL|function|write_rr
specifier|static
name|int
name|write_rr
parameter_list|(
name|struct
name|string_list
modifier|*
name|rr
parameter_list|,
name|int
name|out_fd
parameter_list|)
block|{
name|int
name|i
decl_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|rr
operator|->
name|nr
condition|;
name|i
operator|++
control|)
block|{
name|struct
name|strbuf
name|buf
init|=
name|STRBUF_INIT
decl_stmt|;
name|struct
name|rerere_id
modifier|*
name|id
decl_stmt|;
name|assert
argument_list|(
name|rr
operator|->
name|items
index|[
name|i
index|]
operator|.
name|util
operator|!=
name|RERERE_RESOLVED
argument_list|)
expr_stmt|;
name|id
operator|=
name|rr
operator|->
name|items
index|[
name|i
index|]
operator|.
name|util
expr_stmt|;
if|if
condition|(
operator|!
name|id
condition|)
continue|continue;
name|assert
argument_list|(
name|id
operator|->
name|variant
operator|>=
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
literal|0
operator|<
name|id
operator|->
name|variant
condition|)
name|strbuf_addf
argument_list|(
operator|&
name|buf
argument_list|,
literal|"%s.%d\t%s%c"
argument_list|,
name|rerere_id_hex
argument_list|(
name|id
argument_list|)
argument_list|,
name|id
operator|->
name|variant
argument_list|,
name|rr
operator|->
name|items
index|[
name|i
index|]
operator|.
name|string
argument_list|,
literal|0
argument_list|)
expr_stmt|;
else|else
name|strbuf_addf
argument_list|(
operator|&
name|buf
argument_list|,
literal|"%s\t%s%c"
argument_list|,
name|rerere_id_hex
argument_list|(
name|id
argument_list|)
argument_list|,
name|rr
operator|->
name|items
index|[
name|i
index|]
operator|.
name|string
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|write_in_full
argument_list|(
name|out_fd
argument_list|,
name|buf
operator|.
name|buf
argument_list|,
name|buf
operator|.
name|len
argument_list|)
operator|!=
name|buf
operator|.
name|len
condition|)
name|die
argument_list|(
literal|"unable to write rerere record"
argument_list|)
expr_stmt|;
name|strbuf_release
argument_list|(
operator|&
name|buf
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|commit_lock_file
argument_list|(
operator|&
name|write_lock
argument_list|)
operator|!=
literal|0
condition|)
name|die
argument_list|(
literal|"unable to write rerere record"
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
end_function
begin_comment
comment|/*  * "rerere" interacts with conflicted file contents using this I/O  * abstraction.  It reads a conflicted contents from one place via  * "getline()" method, and optionally can write it out after  * normalizing the conflicted hunks to the "output".  Subclasses of  * rerere_io embed this structure at the beginning of their own  * rerere_io object.  */
end_comment
begin_struct
DECL|struct|rerere_io
struct|struct
name|rerere_io
block|{
DECL|member|getline
name|int
function_decl|(
modifier|*
name|getline
function_decl|)
parameter_list|(
name|struct
name|strbuf
modifier|*
parameter_list|,
name|struct
name|rerere_io
modifier|*
parameter_list|)
function_decl|;
DECL|member|output
name|FILE
modifier|*
name|output
decl_stmt|;
DECL|member|wrerror
name|int
name|wrerror
decl_stmt|;
comment|/* some more stuff */
block|}
struct|;
end_struct
begin_function
DECL|function|ferr_write
specifier|static
name|void
name|ferr_write
parameter_list|(
specifier|const
name|void
modifier|*
name|p
parameter_list|,
name|size_t
name|count
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|,
name|int
modifier|*
name|err
parameter_list|)
block|{
if|if
condition|(
operator|!
name|count
operator|||
operator|*
name|err
condition|)
return|return;
if|if
condition|(
name|fwrite
argument_list|(
name|p
argument_list|,
name|count
argument_list|,
literal|1
argument_list|,
name|fp
argument_list|)
operator|!=
literal|1
condition|)
operator|*
name|err
operator|=
name|errno
expr_stmt|;
block|}
end_function
begin_function
DECL|function|ferr_puts
specifier|static
specifier|inline
name|void
name|ferr_puts
parameter_list|(
specifier|const
name|char
modifier|*
name|s
parameter_list|,
name|FILE
modifier|*
name|fp
parameter_list|,
name|int
modifier|*
name|err
parameter_list|)
block|{
name|ferr_write
argument_list|(
name|s
argument_list|,
name|strlen
argument_list|(
name|s
argument_list|)
argument_list|,
name|fp
argument_list|,
name|err
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|rerere_io_putstr
specifier|static
name|void
name|rerere_io_putstr
parameter_list|(
specifier|const
name|char
modifier|*
name|str
parameter_list|,
name|struct
name|rerere_io
modifier|*
name|io
parameter_list|)
block|{
if|if
condition|(
name|io
operator|->
name|output
condition|)
name|ferr_puts
argument_list|(
name|str
argument_list|,
name|io
operator|->
name|output
argument_list|,
operator|&
name|io
operator|->
name|wrerror
argument_list|)
expr_stmt|;
block|}
end_function
begin_comment
comment|/*  * Write a conflict marker to io->output (if defined).  */
end_comment
begin_function
DECL|function|rerere_io_putconflict
specifier|static
name|void
name|rerere_io_putconflict
parameter_list|(
name|int
name|ch
parameter_list|,
name|int
name|size
parameter_list|,
name|struct
name|rerere_io
modifier|*
name|io
parameter_list|)
block|{
name|char
name|buf
index|[
literal|64
index|]
decl_stmt|;
while|while
condition|(
name|size
condition|)
block|{
if|if
condition|(
name|size
operator|<=
sizeof|sizeof
argument_list|(
name|buf
argument_list|)
operator|-
literal|2
condition|)
block|{
name|memset
argument_list|(
name|buf
argument_list|,
name|ch
argument_list|,
name|size
argument_list|)
expr_stmt|;
name|buf
index|[
name|size
index|]
operator|=
literal|'\n'
expr_stmt|;
name|buf
index|[
name|size
operator|+
literal|1
index|]
operator|=
literal|'\0'
expr_stmt|;
name|size
operator|=
literal|0
expr_stmt|;
block|}
else|else
block|{
name|int
name|sz
init|=
sizeof|sizeof
argument_list|(
name|buf
argument_list|)
operator|-
literal|1
decl_stmt|;
comment|/* 			 * Make sure we will not write everything out 			 * in this round by leaving at least 1 byte 			 * for the next round, giving the next round 			 * a chance to add the terminating LF.  Yuck. 			 */
if|if
condition|(
name|size
operator|<=
name|sz
condition|)
name|sz
operator|-=
operator|(
name|sz
operator|-
name|size
operator|)
operator|+
literal|1
expr_stmt|;
name|memset
argument_list|(
name|buf
argument_list|,
name|ch
argument_list|,
name|sz
argument_list|)
expr_stmt|;
name|buf
index|[
name|sz
index|]
operator|=
literal|'\0'
expr_stmt|;
name|size
operator|-=
name|sz
expr_stmt|;
block|}
name|rerere_io_putstr
argument_list|(
name|buf
argument_list|,
name|io
argument_list|)
expr_stmt|;
block|}
block|}
end_function
begin_function
DECL|function|rerere_io_putmem
specifier|static
name|void
name|rerere_io_putmem
parameter_list|(
specifier|const
name|char
modifier|*
name|mem
parameter_list|,
name|size_t
name|sz
parameter_list|,
name|struct
name|rerere_io
modifier|*
name|io
parameter_list|)
block|{
if|if
condition|(
name|io
operator|->
name|output
condition|)
name|ferr_write
argument_list|(
name|mem
argument_list|,
name|sz
argument_list|,
name|io
operator|->
name|output
argument_list|,
operator|&
name|io
operator|->
name|wrerror
argument_list|)
expr_stmt|;
block|}
end_function
begin_comment
comment|/*  * Subclass of rerere_io that reads from an on-disk file  */
end_comment
begin_struct
DECL|struct|rerere_io_file
struct|struct
name|rerere_io_file
block|{
DECL|member|io
name|struct
name|rerere_io
name|io
decl_stmt|;
DECL|member|input
name|FILE
modifier|*
name|input
decl_stmt|;
block|}
struct|;
end_struct
begin_comment
comment|/*  * ... and its getline() method implementation  */
end_comment
begin_function
DECL|function|rerere_file_getline
specifier|static
name|int
name|rerere_file_getline
parameter_list|(
name|struct
name|strbuf
modifier|*
name|sb
parameter_list|,
name|struct
name|rerere_io
modifier|*
name|io_
parameter_list|)
block|{
name|struct
name|rerere_io_file
modifier|*
name|io
init|=
operator|(
expr|struct
name|rerere_io_file
operator|*
operator|)
name|io_
decl_stmt|;
return|return
name|strbuf_getwholeline
argument_list|(
name|sb
argument_list|,
name|io
operator|->
name|input
argument_list|,
literal|'\n'
argument_list|)
return|;
block|}
end_function
begin_comment
comment|/*  * Require the exact number of conflict marker letters, no more, no  * less, followed by SP or any whitespace  * (including LF).  */
end_comment
begin_function
DECL|function|is_cmarker
specifier|static
name|int
name|is_cmarker
parameter_list|(
name|char
modifier|*
name|buf
parameter_list|,
name|int
name|marker_char
parameter_list|,
name|int
name|marker_size
parameter_list|)
block|{
name|int
name|want_sp
decl_stmt|;
comment|/* 	 * The beginning of our version and the end of their version 	 * always are labeled like "<<<<< ours" or ">>>>> theirs", 	 * hence we set want_sp for them.  Note that the version from 	 * the common ancestor in diff3-style output is not always 	 * labelled (e.g. "||||| common" is often seen but "|||||" 	 * alone is also valid), so we do not set want_sp. 	 */
name|want_sp
operator|=
operator|(
name|marker_char
operator|==
literal|'<'
operator|)
operator|||
operator|(
name|marker_char
operator|==
literal|'>'
operator|)
expr_stmt|;
while|while
condition|(
name|marker_size
operator|--
condition|)
if|if
condition|(
operator|*
name|buf
operator|++
operator|!=
name|marker_char
condition|)
return|return
literal|0
return|;
if|if
condition|(
name|want_sp
operator|&&
operator|*
name|buf
operator|!=
literal|' '
condition|)
return|return
literal|0
return|;
return|return
name|isspace
argument_list|(
operator|*
name|buf
argument_list|)
return|;
block|}
end_function
begin_comment
comment|/*  * Read contents a file with conflicts, normalize the conflicts  * by (1) discarding the common ancestor version in diff3-style,  * (2) reordering our side and their side so that whichever sorts  * alphabetically earlier comes before the other one, while  * computing the "conflict ID", which is just an SHA-1 hash of  * one side of the conflict, NUL, the other side of the conflict,  * and NUL concatenated together.  *  * Return the number of conflict hunks found.  *  * NEEDSWORK: the logic and theory of operation behind this conflict  * normalization may deserve to be documented somewhere, perhaps in  * Documentation/technical/rerere.txt.  */
end_comment
begin_function
DECL|function|handle_path
specifier|static
name|int
name|handle_path
parameter_list|(
name|unsigned
name|char
modifier|*
name|sha1
parameter_list|,
name|struct
name|rerere_io
modifier|*
name|io
parameter_list|,
name|int
name|marker_size
parameter_list|)
block|{
name|git_SHA_CTX
name|ctx
decl_stmt|;
name|int
name|hunk_no
init|=
literal|0
decl_stmt|;
enum|enum
block|{
name|RR_CONTEXT
init|=
literal|0
block|,
name|RR_SIDE_1
block|,
name|RR_SIDE_2
block|,
name|RR_ORIGINAL
block|}
name|hunk
init|=
name|RR_CONTEXT
enum|;
name|struct
name|strbuf
name|one
init|=
name|STRBUF_INIT
decl_stmt|,
name|two
init|=
name|STRBUF_INIT
decl_stmt|;
name|struct
name|strbuf
name|buf
init|=
name|STRBUF_INIT
decl_stmt|;
if|if
condition|(
name|sha1
condition|)
name|git_SHA1_Init
argument_list|(
operator|&
name|ctx
argument_list|)
expr_stmt|;
while|while
condition|(
operator|!
name|io
operator|->
name|getline
argument_list|(
operator|&
name|buf
argument_list|,
name|io
argument_list|)
condition|)
block|{
if|if
condition|(
name|is_cmarker
argument_list|(
name|buf
operator|.
name|buf
argument_list|,
literal|'<'
argument_list|,
name|marker_size
argument_list|)
condition|)
block|{
if|if
condition|(
name|hunk
operator|!=
name|RR_CONTEXT
condition|)
goto|goto
name|bad
goto|;
name|hunk
operator|=
name|RR_SIDE_1
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|is_cmarker
argument_list|(
name|buf
operator|.
name|buf
argument_list|,
literal|'|'
argument_list|,
name|marker_size
argument_list|)
condition|)
block|{
if|if
condition|(
name|hunk
operator|!=
name|RR_SIDE_1
condition|)
goto|goto
name|bad
goto|;
name|hunk
operator|=
name|RR_ORIGINAL
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|is_cmarker
argument_list|(
name|buf
operator|.
name|buf
argument_list|,
literal|'='
argument_list|,
name|marker_size
argument_list|)
condition|)
block|{
if|if
condition|(
name|hunk
operator|!=
name|RR_SIDE_1
operator|&&
name|hunk
operator|!=
name|RR_ORIGINAL
condition|)
goto|goto
name|bad
goto|;
name|hunk
operator|=
name|RR_SIDE_2
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|is_cmarker
argument_list|(
name|buf
operator|.
name|buf
argument_list|,
literal|'>'
argument_list|,
name|marker_size
argument_list|)
condition|)
block|{
if|if
condition|(
name|hunk
operator|!=
name|RR_SIDE_2
condition|)
goto|goto
name|bad
goto|;
if|if
condition|(
name|strbuf_cmp
argument_list|(
operator|&
name|one
argument_list|,
operator|&
name|two
argument_list|)
operator|>
literal|0
condition|)
name|strbuf_swap
argument_list|(
operator|&
name|one
argument_list|,
operator|&
name|two
argument_list|)
expr_stmt|;
name|hunk_no
operator|++
expr_stmt|;
name|hunk
operator|=
name|RR_CONTEXT
expr_stmt|;
name|rerere_io_putconflict
argument_list|(
literal|'<'
argument_list|,
name|marker_size
argument_list|,
name|io
argument_list|)
expr_stmt|;
name|rerere_io_putmem
argument_list|(
name|one
operator|.
name|buf
argument_list|,
name|one
operator|.
name|len
argument_list|,
name|io
argument_list|)
expr_stmt|;
name|rerere_io_putconflict
argument_list|(
literal|'='
argument_list|,
name|marker_size
argument_list|,
name|io
argument_list|)
expr_stmt|;
name|rerere_io_putmem
argument_list|(
name|two
operator|.
name|buf
argument_list|,
name|two
operator|.
name|len
argument_list|,
name|io
argument_list|)
expr_stmt|;
name|rerere_io_putconflict
argument_list|(
literal|'>'
argument_list|,
name|marker_size
argument_list|,
name|io
argument_list|)
expr_stmt|;
if|if
condition|(
name|sha1
condition|)
block|{
name|git_SHA1_Update
argument_list|(
operator|&
name|ctx
argument_list|,
name|one
operator|.
name|buf
condition|?
name|one
operator|.
name|buf
else|:
literal|""
argument_list|,
name|one
operator|.
name|len
operator|+
literal|1
argument_list|)
expr_stmt|;
name|git_SHA1_Update
argument_list|(
operator|&
name|ctx
argument_list|,
name|two
operator|.
name|buf
condition|?
name|two
operator|.
name|buf
else|:
literal|""
argument_list|,
name|two
operator|.
name|len
operator|+
literal|1
argument_list|)
expr_stmt|;
block|}
name|strbuf_reset
argument_list|(
operator|&
name|one
argument_list|)
expr_stmt|;
name|strbuf_reset
argument_list|(
operator|&
name|two
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|hunk
operator|==
name|RR_SIDE_1
condition|)
name|strbuf_addbuf
argument_list|(
operator|&
name|one
argument_list|,
operator|&
name|buf
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|hunk
operator|==
name|RR_ORIGINAL
condition|)
empty_stmt|;
comment|/* discard */
elseif|else
if|if
condition|(
name|hunk
operator|==
name|RR_SIDE_2
condition|)
name|strbuf_addbuf
argument_list|(
operator|&
name|two
argument_list|,
operator|&
name|buf
argument_list|)
expr_stmt|;
else|else
name|rerere_io_putstr
argument_list|(
name|buf
operator|.
name|buf
argument_list|,
name|io
argument_list|)
expr_stmt|;
continue|continue;
name|bad
label|:
name|hunk
operator|=
literal|99
expr_stmt|;
comment|/* force error exit */
break|break;
block|}
name|strbuf_release
argument_list|(
operator|&
name|one
argument_list|)
expr_stmt|;
name|strbuf_release
argument_list|(
operator|&
name|two
argument_list|)
expr_stmt|;
name|strbuf_release
argument_list|(
operator|&
name|buf
argument_list|)
expr_stmt|;
if|if
condition|(
name|sha1
condition|)
name|git_SHA1_Final
argument_list|(
name|sha1
argument_list|,
operator|&
name|ctx
argument_list|)
expr_stmt|;
if|if
condition|(
name|hunk
operator|!=
name|RR_CONTEXT
condition|)
return|return
operator|-
literal|1
return|;
return|return
name|hunk_no
return|;
block|}
end_function
begin_comment
comment|/*  * Scan the path for conflicts, do the "handle_path()" thing above, and  * return the number of conflict hunks found.  */
end_comment
begin_function
DECL|function|handle_file
specifier|static
name|int
name|handle_file
parameter_list|(
specifier|const
name|char
modifier|*
name|path
parameter_list|,
name|unsigned
name|char
modifier|*
name|sha1
parameter_list|,
specifier|const
name|char
modifier|*
name|output
parameter_list|)
block|{
name|int
name|hunk_no
init|=
literal|0
decl_stmt|;
name|struct
name|rerere_io_file
name|io
decl_stmt|;
name|int
name|marker_size
init|=
name|ll_merge_marker_size
argument_list|(
name|path
argument_list|)
decl_stmt|;
name|memset
argument_list|(
operator|&
name|io
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
name|io
argument_list|)
argument_list|)
expr_stmt|;
name|io
operator|.
name|io
operator|.
name|getline
operator|=
name|rerere_file_getline
expr_stmt|;
name|io
operator|.
name|input
operator|=
name|fopen
argument_list|(
name|path
argument_list|,
literal|"r"
argument_list|)
expr_stmt|;
name|io
operator|.
name|io
operator|.
name|wrerror
operator|=
literal|0
expr_stmt|;
if|if
condition|(
operator|!
name|io
operator|.
name|input
condition|)
return|return
name|error
argument_list|(
literal|"Could not open %s"
argument_list|,
name|path
argument_list|)
return|;
if|if
condition|(
name|output
condition|)
block|{
name|io
operator|.
name|io
operator|.
name|output
operator|=
name|fopen
argument_list|(
name|output
argument_list|,
literal|"w"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|io
operator|.
name|io
operator|.
name|output
condition|)
block|{
name|fclose
argument_list|(
name|io
operator|.
name|input
argument_list|)
expr_stmt|;
return|return
name|error
argument_list|(
literal|"Could not write %s"
argument_list|,
name|output
argument_list|)
return|;
block|}
block|}
name|hunk_no
operator|=
name|handle_path
argument_list|(
name|sha1
argument_list|,
operator|(
expr|struct
name|rerere_io
operator|*
operator|)
operator|&
name|io
argument_list|,
name|marker_size
argument_list|)
expr_stmt|;
name|fclose
argument_list|(
name|io
operator|.
name|input
argument_list|)
expr_stmt|;
if|if
condition|(
name|io
operator|.
name|io
operator|.
name|wrerror
condition|)
name|error
argument_list|(
literal|"There were errors while writing %s (%s)"
argument_list|,
name|path
argument_list|,
name|strerror
argument_list|(
name|io
operator|.
name|io
operator|.
name|wrerror
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|io
operator|.
name|io
operator|.
name|output
operator|&&
name|fclose
argument_list|(
name|io
operator|.
name|io
operator|.
name|output
argument_list|)
condition|)
name|io
operator|.
name|io
operator|.
name|wrerror
operator|=
name|error_errno
argument_list|(
literal|"Failed to flush %s"
argument_list|,
name|path
argument_list|)
expr_stmt|;
if|if
condition|(
name|hunk_no
operator|<
literal|0
condition|)
block|{
if|if
condition|(
name|output
condition|)
name|unlink_or_warn
argument_list|(
name|output
argument_list|)
expr_stmt|;
return|return
name|error
argument_list|(
literal|"Could not parse conflict hunks in %s"
argument_list|,
name|path
argument_list|)
return|;
block|}
if|if
condition|(
name|io
operator|.
name|io
operator|.
name|wrerror
condition|)
return|return
operator|-
literal|1
return|;
return|return
name|hunk_no
return|;
block|}
end_function
begin_comment
comment|/*  * Look at a cache entry at "i" and see if it is not conflicting,  * conflicting and we are willing to handle, or conflicting and  * we are unable to handle, and return the determination in *type.  * Return the cache index to be looked at next, by skipping the  * stages we have already looked at in this invocation of this  * function.  */
end_comment
begin_function
DECL|function|check_one_conflict
specifier|static
name|int
name|check_one_conflict
parameter_list|(
name|int
name|i
parameter_list|,
name|int
modifier|*
name|type
parameter_list|)
block|{
specifier|const
name|struct
name|cache_entry
modifier|*
name|e
init|=
name|active_cache
index|[
name|i
index|]
decl_stmt|;
if|if
condition|(
operator|!
name|ce_stage
argument_list|(
name|e
argument_list|)
condition|)
block|{
operator|*
name|type
operator|=
name|RESOLVED
expr_stmt|;
return|return
name|i
operator|+
literal|1
return|;
block|}
operator|*
name|type
operator|=
name|PUNTED
expr_stmt|;
while|while
condition|(
name|ce_stage
argument_list|(
name|active_cache
index|[
name|i
index|]
argument_list|)
operator|==
literal|1
condition|)
name|i
operator|++
expr_stmt|;
comment|/* Only handle regular files with both stages #2 and #3 */
if|if
condition|(
name|i
operator|+
literal|1
operator|<
name|active_nr
condition|)
block|{
specifier|const
name|struct
name|cache_entry
modifier|*
name|e2
init|=
name|active_cache
index|[
name|i
index|]
decl_stmt|;
specifier|const
name|struct
name|cache_entry
modifier|*
name|e3
init|=
name|active_cache
index|[
name|i
operator|+
literal|1
index|]
decl_stmt|;
if|if
condition|(
name|ce_stage
argument_list|(
name|e2
argument_list|)
operator|==
literal|2
operator|&&
name|ce_stage
argument_list|(
name|e3
argument_list|)
operator|==
literal|3
operator|&&
name|ce_same_name
argument_list|(
name|e
argument_list|,
name|e3
argument_list|)
operator|&&
name|S_ISREG
argument_list|(
name|e2
operator|->
name|ce_mode
argument_list|)
operator|&&
name|S_ISREG
argument_list|(
name|e3
operator|->
name|ce_mode
argument_list|)
condition|)
operator|*
name|type
operator|=
name|THREE_STAGED
expr_stmt|;
block|}
comment|/* Skip the entries with the same name */
while|while
condition|(
name|i
operator|<
name|active_nr
operator|&&
name|ce_same_name
argument_list|(
name|e
argument_list|,
name|active_cache
index|[
name|i
index|]
argument_list|)
condition|)
name|i
operator|++
expr_stmt|;
return|return
name|i
return|;
block|}
end_function
begin_comment
comment|/*  * Scan the index and find paths that have conflicts that rerere can  * handle, i.e. the ones that has both stages #2 and #3.  *  * NEEDSWORK: we do not record or replay a previous "resolve by  * deletion" for a delete-modify conflict, as that is inherently risky  * without knowing what modification is being discarded.  The only  * safe case, i.e. both side doing the deletion and modification that  * are identical to the previous round, might want to be handled,  * though.  */
end_comment
begin_function
DECL|function|find_conflict
specifier|static
name|int
name|find_conflict
parameter_list|(
name|struct
name|string_list
modifier|*
name|conflict
parameter_list|)
block|{
name|int
name|i
decl_stmt|;
if|if
condition|(
name|read_cache
argument_list|()
operator|<
literal|0
condition|)
return|return
name|error
argument_list|(
literal|"Could not read index"
argument_list|)
return|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|active_nr
condition|;
control|)
block|{
name|int
name|conflict_type
decl_stmt|;
specifier|const
name|struct
name|cache_entry
modifier|*
name|e
init|=
name|active_cache
index|[
name|i
index|]
decl_stmt|;
name|i
operator|=
name|check_one_conflict
argument_list|(
name|i
argument_list|,
operator|&
name|conflict_type
argument_list|)
expr_stmt|;
if|if
condition|(
name|conflict_type
operator|==
name|THREE_STAGED
condition|)
name|string_list_insert
argument_list|(
name|conflict
argument_list|,
operator|(
specifier|const
name|char
operator|*
operator|)
name|e
operator|->
name|name
argument_list|)
expr_stmt|;
block|}
return|return
literal|0
return|;
block|}
end_function
begin_comment
comment|/*  * The merge_rr list is meant to hold outstanding conflicted paths  * that rerere could handle.  Abuse the list by adding other types of  * entries to allow the caller to show "rerere remaining".  *  * - Conflicted paths that rerere does not handle are added  * - Conflicted paths that have been resolved are marked as such  *   by storing RERERE_RESOLVED to .util field (where conflict ID  *   is expected to be stored).  *  * Do *not* write MERGE_RR file out after calling this function.  *  * NEEDSWORK: we may want to fix the caller that implements "rerere  * remaining" to do this without abusing merge_rr.  */
end_comment
begin_function
DECL|function|rerere_remaining
name|int
name|rerere_remaining
parameter_list|(
name|struct
name|string_list
modifier|*
name|merge_rr
parameter_list|)
block|{
name|int
name|i
decl_stmt|;
if|if
condition|(
name|setup_rerere
argument_list|(
name|merge_rr
argument_list|,
name|RERERE_READONLY
argument_list|)
condition|)
return|return
literal|0
return|;
if|if
condition|(
name|read_cache
argument_list|()
operator|<
literal|0
condition|)
return|return
name|error
argument_list|(
literal|"Could not read index"
argument_list|)
return|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|active_nr
condition|;
control|)
block|{
name|int
name|conflict_type
decl_stmt|;
specifier|const
name|struct
name|cache_entry
modifier|*
name|e
init|=
name|active_cache
index|[
name|i
index|]
decl_stmt|;
name|i
operator|=
name|check_one_conflict
argument_list|(
name|i
argument_list|,
operator|&
name|conflict_type
argument_list|)
expr_stmt|;
if|if
condition|(
name|conflict_type
operator|==
name|PUNTED
condition|)
name|string_list_insert
argument_list|(
name|merge_rr
argument_list|,
operator|(
specifier|const
name|char
operator|*
operator|)
name|e
operator|->
name|name
argument_list|)
expr_stmt|;
elseif|else
if|if
condition|(
name|conflict_type
operator|==
name|RESOLVED
condition|)
block|{
name|struct
name|string_list_item
modifier|*
name|it
decl_stmt|;
name|it
operator|=
name|string_list_lookup
argument_list|(
name|merge_rr
argument_list|,
operator|(
specifier|const
name|char
operator|*
operator|)
name|e
operator|->
name|name
argument_list|)
expr_stmt|;
if|if
condition|(
name|it
operator|!=
name|NULL
condition|)
block|{
name|free_rerere_id
argument_list|(
name|it
argument_list|)
expr_stmt|;
name|it
operator|->
name|util
operator|=
name|RERERE_RESOLVED
expr_stmt|;
block|}
block|}
block|}
return|return
literal|0
return|;
block|}
end_function
begin_comment
comment|/*  * Try using the given conflict resolution "ID" to see  * if that recorded conflict resolves cleanly what we  * got in the "cur".  */
end_comment
begin_function
DECL|function|try_merge
specifier|static
name|int
name|try_merge
parameter_list|(
specifier|const
name|struct
name|rerere_id
modifier|*
name|id
parameter_list|,
specifier|const
name|char
modifier|*
name|path
parameter_list|,
name|mmfile_t
modifier|*
name|cur
parameter_list|,
name|mmbuffer_t
modifier|*
name|result
parameter_list|)
block|{
name|int
name|ret
decl_stmt|;
name|mmfile_t
name|base
init|=
block|{
name|NULL
block|,
literal|0
block|}
decl_stmt|,
name|other
init|=
block|{
name|NULL
block|,
literal|0
block|}
decl_stmt|;
if|if
condition|(
name|read_mmfile
argument_list|(
operator|&
name|base
argument_list|,
name|rerere_path
argument_list|(
name|id
argument_list|,
literal|"preimage"
argument_list|)
argument_list|)
operator|||
name|read_mmfile
argument_list|(
operator|&
name|other
argument_list|,
name|rerere_path
argument_list|(
name|id
argument_list|,
literal|"postimage"
argument_list|)
argument_list|)
condition|)
name|ret
operator|=
literal|1
expr_stmt|;
else|else
comment|/* 		 * A three-way merge. Note that this honors user-customizable 		 * low-level merge driver settings. 		 */
name|ret
operator|=
name|ll_merge
argument_list|(
name|result
argument_list|,
name|path
argument_list|,
operator|&
name|base
argument_list|,
name|NULL
argument_list|,
name|cur
argument_list|,
literal|""
argument_list|,
operator|&
name|other
argument_list|,
literal|""
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|base
operator|.
name|ptr
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|other
operator|.
name|ptr
argument_list|)
expr_stmt|;
return|return
name|ret
return|;
block|}
end_function
begin_comment
comment|/*  * Find the conflict identified by "id"; the change between its  * "preimage" (i.e. a previous contents with conflict markers) and its  * "postimage" (i.e. the corresponding contents with conflicts  * resolved) may apply cleanly to the contents stored in "path", i.e.  * the conflict this time around.  *  * Returns 0 for successful replay of recorded resolution, or non-zero  * for failure.  */
end_comment
begin_function
DECL|function|merge
specifier|static
name|int
name|merge
parameter_list|(
specifier|const
name|struct
name|rerere_id
modifier|*
name|id
parameter_list|,
specifier|const
name|char
modifier|*
name|path
parameter_list|)
block|{
name|FILE
modifier|*
name|f
decl_stmt|;
name|int
name|ret
decl_stmt|;
name|mmfile_t
name|cur
init|=
block|{
name|NULL
block|,
literal|0
block|}
decl_stmt|;
name|mmbuffer_t
name|result
init|=
block|{
name|NULL
block|,
literal|0
block|}
decl_stmt|;
comment|/* 	 * Normalize the conflicts in path and write it out to 	 * "thisimage" temporary file. 	 */
if|if
condition|(
operator|(
name|handle_file
argument_list|(
name|path
argument_list|,
name|NULL
argument_list|,
name|rerere_path
argument_list|(
name|id
argument_list|,
literal|"thisimage"
argument_list|)
argument_list|)
operator|<
literal|0
operator|)
operator|||
name|read_mmfile
argument_list|(
operator|&
name|cur
argument_list|,
name|rerere_path
argument_list|(
name|id
argument_list|,
literal|"thisimage"
argument_list|)
argument_list|)
condition|)
block|{
name|ret
operator|=
literal|1
expr_stmt|;
goto|goto
name|out
goto|;
block|}
name|ret
operator|=
name|try_merge
argument_list|(
name|id
argument_list|,
name|path
argument_list|,
operator|&
name|cur
argument_list|,
operator|&
name|result
argument_list|)
expr_stmt|;
if|if
condition|(
name|ret
condition|)
goto|goto
name|out
goto|;
comment|/* 	 * A successful replay of recorded resolution. 	 * Mark that "postimage" was used to help gc. 	 */
if|if
condition|(
name|utime
argument_list|(
name|rerere_path
argument_list|(
name|id
argument_list|,
literal|"postimage"
argument_list|)
argument_list|,
name|NULL
argument_list|)
operator|<
literal|0
condition|)
name|warning_errno
argument_list|(
literal|"failed utime() on %s"
argument_list|,
name|rerere_path
argument_list|(
name|id
argument_list|,
literal|"postimage"
argument_list|)
argument_list|)
expr_stmt|;
comment|/* Update "path" with the resolution */
name|f
operator|=
name|fopen
argument_list|(
name|path
argument_list|,
literal|"w"
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|f
condition|)
return|return
name|error_errno
argument_list|(
literal|"Could not open %s"
argument_list|,
name|path
argument_list|)
return|;
if|if
condition|(
name|fwrite
argument_list|(
name|result
operator|.
name|ptr
argument_list|,
name|result
operator|.
name|size
argument_list|,
literal|1
argument_list|,
name|f
argument_list|)
operator|!=
literal|1
condition|)
name|error_errno
argument_list|(
literal|"Could not write %s"
argument_list|,
name|path
argument_list|)
expr_stmt|;
if|if
condition|(
name|fclose
argument_list|(
name|f
argument_list|)
condition|)
return|return
name|error_errno
argument_list|(
literal|"Writing %s failed"
argument_list|,
name|path
argument_list|)
return|;
name|out
label|:
name|free
argument_list|(
name|cur
operator|.
name|ptr
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|result
operator|.
name|ptr
argument_list|)
expr_stmt|;
return|return
name|ret
return|;
block|}
end_function
begin_decl_stmt
DECL|variable|index_lock
specifier|static
name|struct
name|lock_file
name|index_lock
decl_stmt|;
end_decl_stmt
begin_function
DECL|function|update_paths
specifier|static
name|void
name|update_paths
parameter_list|(
name|struct
name|string_list
modifier|*
name|update
parameter_list|)
block|{
name|int
name|i
decl_stmt|;
name|hold_locked_index
argument_list|(
operator|&
name|index_lock
argument_list|,
literal|1
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|update
operator|->
name|nr
condition|;
name|i
operator|++
control|)
block|{
name|struct
name|string_list_item
modifier|*
name|item
init|=
operator|&
name|update
operator|->
name|items
index|[
name|i
index|]
decl_stmt|;
if|if
condition|(
name|add_file_to_cache
argument_list|(
name|item
operator|->
name|string
argument_list|,
literal|0
argument_list|)
condition|)
name|exit
argument_list|(
literal|128
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Staged '%s' using previous resolution.\n"
argument_list|,
name|item
operator|->
name|string
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|active_cache_changed
condition|)
block|{
if|if
condition|(
name|write_locked_index
argument_list|(
operator|&
name|the_index
argument_list|,
operator|&
name|index_lock
argument_list|,
name|COMMIT_LOCK
argument_list|)
condition|)
name|die
argument_list|(
literal|"Unable to write new index file"
argument_list|)
expr_stmt|;
block|}
else|else
name|rollback_lock_file
argument_list|(
operator|&
name|index_lock
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|remove_variant
specifier|static
name|void
name|remove_variant
parameter_list|(
name|struct
name|rerere_id
modifier|*
name|id
parameter_list|)
block|{
name|unlink_or_warn
argument_list|(
name|rerere_path
argument_list|(
name|id
argument_list|,
literal|"postimage"
argument_list|)
argument_list|)
expr_stmt|;
name|unlink_or_warn
argument_list|(
name|rerere_path
argument_list|(
name|id
argument_list|,
literal|"preimage"
argument_list|)
argument_list|)
expr_stmt|;
name|id
operator|->
name|collection
operator|->
name|status
index|[
name|id
operator|->
name|variant
index|]
operator|=
literal|0
expr_stmt|;
block|}
end_function
begin_comment
comment|/*  * The path indicated by rr_item may still have conflict for which we  * have a recorded resolution, in which case replay it and optionally  * update it.  Or it may have been resolved by the user and we may  * only have the preimage for that conflict, in which case the result  * needs to be recorded as a resolution in a postimage file.  */
end_comment
begin_function
DECL|function|do_rerere_one_path
specifier|static
name|void
name|do_rerere_one_path
parameter_list|(
name|struct
name|string_list_item
modifier|*
name|rr_item
parameter_list|,
name|struct
name|string_list
modifier|*
name|update
parameter_list|)
block|{
specifier|const
name|char
modifier|*
name|path
init|=
name|rr_item
operator|->
name|string
decl_stmt|;
name|struct
name|rerere_id
modifier|*
name|id
init|=
name|rr_item
operator|->
name|util
decl_stmt|;
name|struct
name|rerere_dir
modifier|*
name|rr_dir
init|=
name|id
operator|->
name|collection
decl_stmt|;
name|int
name|variant
decl_stmt|;
name|variant
operator|=
name|id
operator|->
name|variant
expr_stmt|;
comment|/* Has the user resolved it already? */
if|if
condition|(
name|variant
operator|>=
literal|0
condition|)
block|{
if|if
condition|(
operator|!
name|handle_file
argument_list|(
name|path
argument_list|,
name|NULL
argument_list|,
name|NULL
argument_list|)
condition|)
block|{
name|copy_file
argument_list|(
name|rerere_path
argument_list|(
name|id
argument_list|,
literal|"postimage"
argument_list|)
argument_list|,
name|path
argument_list|,
literal|0666
argument_list|)
expr_stmt|;
name|id
operator|->
name|collection
operator|->
name|status
index|[
name|variant
index|]
operator||=
name|RR_HAS_POSTIMAGE
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Recorded resolution for '%s'.\n"
argument_list|,
name|path
argument_list|)
expr_stmt|;
name|free_rerere_id
argument_list|(
name|rr_item
argument_list|)
expr_stmt|;
name|rr_item
operator|->
name|util
operator|=
name|NULL
expr_stmt|;
return|return;
block|}
comment|/* 		 * There may be other variants that can cleanly 		 * replay.  Try them and update the variant number for 		 * this one. 		 */
block|}
comment|/* Does any existing resolution apply cleanly? */
for|for
control|(
name|variant
operator|=
literal|0
init|;
name|variant
operator|<
name|rr_dir
operator|->
name|status_nr
condition|;
name|variant
operator|++
control|)
block|{
specifier|const
name|int
name|both
init|=
name|RR_HAS_PREIMAGE
operator||
name|RR_HAS_POSTIMAGE
decl_stmt|;
name|struct
name|rerere_id
name|vid
init|=
operator|*
name|id
decl_stmt|;
if|if
condition|(
operator|(
name|rr_dir
operator|->
name|status
index|[
name|variant
index|]
operator|&
name|both
operator|)
operator|!=
name|both
condition|)
continue|continue;
name|vid
operator|.
name|variant
operator|=
name|variant
expr_stmt|;
if|if
condition|(
name|merge
argument_list|(
operator|&
name|vid
argument_list|,
name|path
argument_list|)
condition|)
continue|continue;
comment|/* failed to replay */
comment|/* 		 * If there already is a different variant that applies 		 * cleanly, there is no point maintaining our own variant. 		 */
if|if
condition|(
literal|0
operator|<=
name|id
operator|->
name|variant
operator|&&
name|id
operator|->
name|variant
operator|!=
name|variant
condition|)
name|remove_variant
argument_list|(
name|id
argument_list|)
expr_stmt|;
if|if
condition|(
name|rerere_autoupdate
condition|)
name|string_list_insert
argument_list|(
name|update
argument_list|,
name|path
argument_list|)
expr_stmt|;
else|else
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Resolved '%s' using previous resolution.\n"
argument_list|,
name|path
argument_list|)
expr_stmt|;
name|free_rerere_id
argument_list|(
name|rr_item
argument_list|)
expr_stmt|;
name|rr_item
operator|->
name|util
operator|=
name|NULL
expr_stmt|;
return|return;
block|}
comment|/* None of the existing one applies; we need a new variant */
name|assign_variant
argument_list|(
name|id
argument_list|)
expr_stmt|;
name|variant
operator|=
name|id
operator|->
name|variant
expr_stmt|;
name|handle_file
argument_list|(
name|path
argument_list|,
name|NULL
argument_list|,
name|rerere_path
argument_list|(
name|id
argument_list|,
literal|"preimage"
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|id
operator|->
name|collection
operator|->
name|status
index|[
name|variant
index|]
operator|&
name|RR_HAS_POSTIMAGE
condition|)
block|{
specifier|const
name|char
modifier|*
name|path
init|=
name|rerere_path
argument_list|(
name|id
argument_list|,
literal|"postimage"
argument_list|)
decl_stmt|;
if|if
condition|(
name|unlink
argument_list|(
name|path
argument_list|)
condition|)
name|die_errno
argument_list|(
literal|"cannot unlink stray '%s'"
argument_list|,
name|path
argument_list|)
expr_stmt|;
name|id
operator|->
name|collection
operator|->
name|status
index|[
name|variant
index|]
operator|&=
operator|~
name|RR_HAS_POSTIMAGE
expr_stmt|;
block|}
name|id
operator|->
name|collection
operator|->
name|status
index|[
name|variant
index|]
operator||=
name|RR_HAS_PREIMAGE
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Recorded preimage for '%s'\n"
argument_list|,
name|path
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|do_plain_rerere
specifier|static
name|int
name|do_plain_rerere
parameter_list|(
name|struct
name|string_list
modifier|*
name|rr
parameter_list|,
name|int
name|fd
parameter_list|)
block|{
name|struct
name|string_list
name|conflict
init|=
name|STRING_LIST_INIT_DUP
decl_stmt|;
name|struct
name|string_list
name|update
init|=
name|STRING_LIST_INIT_DUP
decl_stmt|;
name|int
name|i
decl_stmt|;
name|find_conflict
argument_list|(
operator|&
name|conflict
argument_list|)
expr_stmt|;
comment|/* 	 * MERGE_RR records paths with conflicts immediately after 	 * merge failed.  Some of the conflicted paths might have been 	 * hand resolved in the working tree since then, but the 	 * initial run would catch all and register their preimages. 	 */
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|conflict
operator|.
name|nr
condition|;
name|i
operator|++
control|)
block|{
name|struct
name|rerere_id
modifier|*
name|id
decl_stmt|;
name|unsigned
name|char
name|sha1
index|[
literal|20
index|]
decl_stmt|;
specifier|const
name|char
modifier|*
name|path
init|=
name|conflict
operator|.
name|items
index|[
name|i
index|]
operator|.
name|string
decl_stmt|;
name|int
name|ret
decl_stmt|;
if|if
condition|(
name|string_list_has_string
argument_list|(
name|rr
argument_list|,
name|path
argument_list|)
condition|)
continue|continue;
comment|/* 		 * Ask handle_file() to scan and assign a 		 * conflict ID.  No need to write anything out 		 * yet. 		 */
name|ret
operator|=
name|handle_file
argument_list|(
name|path
argument_list|,
name|sha1
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
if|if
condition|(
name|ret
operator|<
literal|1
condition|)
continue|continue;
name|id
operator|=
name|new_rerere_id
argument_list|(
name|sha1
argument_list|)
expr_stmt|;
name|string_list_insert
argument_list|(
name|rr
argument_list|,
name|path
argument_list|)
operator|->
name|util
operator|=
name|id
expr_stmt|;
comment|/* Ensure that the directory exists. */
name|mkdir_in_gitdir
argument_list|(
name|rerere_path
argument_list|(
name|id
argument_list|,
name|NULL
argument_list|)
argument_list|)
expr_stmt|;
block|}
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|rr
operator|->
name|nr
condition|;
name|i
operator|++
control|)
name|do_rerere_one_path
argument_list|(
operator|&
name|rr
operator|->
name|items
index|[
name|i
index|]
argument_list|,
operator|&
name|update
argument_list|)
expr_stmt|;
if|if
condition|(
name|update
operator|.
name|nr
condition|)
name|update_paths
argument_list|(
operator|&
name|update
argument_list|)
expr_stmt|;
return|return
name|write_rr
argument_list|(
name|rr
argument_list|,
name|fd
argument_list|)
return|;
block|}
end_function
begin_function
DECL|function|git_rerere_config
specifier|static
name|void
name|git_rerere_config
parameter_list|(
name|void
parameter_list|)
block|{
name|git_config_get_bool
argument_list|(
literal|"rerere.enabled"
argument_list|,
operator|&
name|rerere_enabled
argument_list|)
expr_stmt|;
name|git_config_get_bool
argument_list|(
literal|"rerere.autoupdate"
argument_list|,
operator|&
name|rerere_autoupdate
argument_list|)
expr_stmt|;
name|git_config
argument_list|(
name|git_default_config
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
block|}
end_function
begin_expr_stmt
specifier|static
name|GIT_PATH_FUNC
argument_list|(
argument|git_path_rr_cache
argument_list|,
literal|"rr-cache"
argument_list|)
DECL|function|is_rerere_enabled
specifier|static
name|int
name|is_rerere_enabled
argument_list|(
argument|void
argument_list|)
block|{
name|int
name|rr_cache_exists
block|;
if|if
condition|(
operator|!
name|rerere_enabled
condition|)
return|return
literal|0
return|;
name|rr_cache_exists
operator|=
name|is_directory
argument_list|(
name|git_path_rr_cache
argument_list|()
argument_list|)
expr_stmt|;
end_expr_stmt
begin_if
if|if
condition|(
name|rerere_enabled
operator|<
literal|0
condition|)
return|return
name|rr_cache_exists
return|;
end_if
begin_if
if|if
condition|(
operator|!
name|rr_cache_exists
operator|&&
name|mkdir_in_gitdir
argument_list|(
name|git_path_rr_cache
argument_list|()
argument_list|)
condition|)
name|die
argument_list|(
literal|"Could not create directory %s"
argument_list|,
name|git_path_rr_cache
argument_list|()
argument_list|)
expr_stmt|;
end_if
begin_return
return|return
literal|1
return|;
end_return
begin_macro
unit|}  int
DECL|function|setup_rerere
name|setup_rerere
argument_list|(
argument|struct string_list *merge_rr
argument_list|,
argument|int flags
argument_list|)
end_macro
begin_block
block|{
name|int
name|fd
decl_stmt|;
name|git_rerere_config
argument_list|()
expr_stmt|;
if|if
condition|(
operator|!
name|is_rerere_enabled
argument_list|()
condition|)
return|return
operator|-
literal|1
return|;
if|if
condition|(
name|flags
operator|&
operator|(
name|RERERE_AUTOUPDATE
operator||
name|RERERE_NOAUTOUPDATE
operator|)
condition|)
name|rerere_autoupdate
operator|=
operator|!
operator|!
operator|(
name|flags
operator|&
name|RERERE_AUTOUPDATE
operator|)
expr_stmt|;
if|if
condition|(
name|flags
operator|&
name|RERERE_READONLY
condition|)
name|fd
operator|=
literal|0
expr_stmt|;
else|else
name|fd
operator|=
name|hold_lock_file_for_update
argument_list|(
operator|&
name|write_lock
argument_list|,
name|git_path_merge_rr
argument_list|()
argument_list|,
name|LOCK_DIE_ON_ERROR
argument_list|)
expr_stmt|;
name|read_rr
argument_list|(
name|merge_rr
argument_list|)
expr_stmt|;
return|return
name|fd
return|;
block|}
end_block
begin_comment
comment|/*  * The main entry point that is called internally from codepaths that  * perform mergy operations, possibly leaving conflicted index entries  * and working tree files.  */
end_comment
begin_function
DECL|function|rerere
name|int
name|rerere
parameter_list|(
name|int
name|flags
parameter_list|)
block|{
name|struct
name|string_list
name|merge_rr
init|=
name|STRING_LIST_INIT_DUP
decl_stmt|;
name|int
name|fd
decl_stmt|,
name|status
decl_stmt|;
name|fd
operator|=
name|setup_rerere
argument_list|(
operator|&
name|merge_rr
argument_list|,
name|flags
argument_list|)
expr_stmt|;
if|if
condition|(
name|fd
operator|<
literal|0
condition|)
return|return
literal|0
return|;
name|status
operator|=
name|do_plain_rerere
argument_list|(
operator|&
name|merge_rr
argument_list|,
name|fd
argument_list|)
expr_stmt|;
name|free_rerere_dirs
argument_list|()
expr_stmt|;
return|return
name|status
return|;
block|}
end_function
begin_comment
comment|/*  * Subclass of rerere_io that reads from an in-core buffer that is a  * strbuf  */
end_comment
begin_struct
DECL|struct|rerere_io_mem
struct|struct
name|rerere_io_mem
block|{
DECL|member|io
name|struct
name|rerere_io
name|io
decl_stmt|;
DECL|member|input
name|struct
name|strbuf
name|input
decl_stmt|;
block|}
struct|;
end_struct
begin_comment
comment|/*  * ... and its getline() method implementation  */
end_comment
begin_function
DECL|function|rerere_mem_getline
specifier|static
name|int
name|rerere_mem_getline
parameter_list|(
name|struct
name|strbuf
modifier|*
name|sb
parameter_list|,
name|struct
name|rerere_io
modifier|*
name|io_
parameter_list|)
block|{
name|struct
name|rerere_io_mem
modifier|*
name|io
init|=
operator|(
expr|struct
name|rerere_io_mem
operator|*
operator|)
name|io_
decl_stmt|;
name|char
modifier|*
name|ep
decl_stmt|;
name|size_t
name|len
decl_stmt|;
name|strbuf_release
argument_list|(
name|sb
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|io
operator|->
name|input
operator|.
name|len
condition|)
return|return
operator|-
literal|1
return|;
name|ep
operator|=
name|memchr
argument_list|(
name|io
operator|->
name|input
operator|.
name|buf
argument_list|,
literal|'\n'
argument_list|,
name|io
operator|->
name|input
operator|.
name|len
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|ep
condition|)
name|ep
operator|=
name|io
operator|->
name|input
operator|.
name|buf
operator|+
name|io
operator|->
name|input
operator|.
name|len
expr_stmt|;
elseif|else
if|if
condition|(
operator|*
name|ep
operator|==
literal|'\n'
condition|)
name|ep
operator|++
expr_stmt|;
name|len
operator|=
name|ep
operator|-
name|io
operator|->
name|input
operator|.
name|buf
expr_stmt|;
name|strbuf_add
argument_list|(
name|sb
argument_list|,
name|io
operator|->
name|input
operator|.
name|buf
argument_list|,
name|len
argument_list|)
expr_stmt|;
name|strbuf_remove
argument_list|(
operator|&
name|io
operator|->
name|input
argument_list|,
literal|0
argument_list|,
name|len
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
end_function
begin_function
DECL|function|handle_cache
specifier|static
name|int
name|handle_cache
parameter_list|(
specifier|const
name|char
modifier|*
name|path
parameter_list|,
name|unsigned
name|char
modifier|*
name|sha1
parameter_list|,
specifier|const
name|char
modifier|*
name|output
parameter_list|)
block|{
name|mmfile_t
name|mmfile
index|[
literal|3
index|]
init|=
block|{
block|{
name|NULL
block|}
block|}
decl_stmt|;
name|mmbuffer_t
name|result
init|=
block|{
name|NULL
block|,
literal|0
block|}
decl_stmt|;
specifier|const
name|struct
name|cache_entry
modifier|*
name|ce
decl_stmt|;
name|int
name|pos
decl_stmt|,
name|len
decl_stmt|,
name|i
decl_stmt|,
name|hunk_no
decl_stmt|;
name|struct
name|rerere_io_mem
name|io
decl_stmt|;
name|int
name|marker_size
init|=
name|ll_merge_marker_size
argument_list|(
name|path
argument_list|)
decl_stmt|;
comment|/* 	 * Reproduce the conflicted merge in-core 	 */
name|len
operator|=
name|strlen
argument_list|(
name|path
argument_list|)
expr_stmt|;
name|pos
operator|=
name|cache_name_pos
argument_list|(
name|path
argument_list|,
name|len
argument_list|)
expr_stmt|;
if|if
condition|(
literal|0
operator|<=
name|pos
condition|)
return|return
operator|-
literal|1
return|;
name|pos
operator|=
operator|-
name|pos
operator|-
literal|1
expr_stmt|;
while|while
condition|(
name|pos
operator|<
name|active_nr
condition|)
block|{
name|enum
name|object_type
name|type
decl_stmt|;
name|unsigned
name|long
name|size
decl_stmt|;
name|ce
operator|=
name|active_cache
index|[
name|pos
operator|++
index|]
expr_stmt|;
if|if
condition|(
name|ce_namelen
argument_list|(
name|ce
argument_list|)
operator|!=
name|len
operator|||
name|memcmp
argument_list|(
name|ce
operator|->
name|name
argument_list|,
name|path
argument_list|,
name|len
argument_list|)
condition|)
break|break;
name|i
operator|=
name|ce_stage
argument_list|(
name|ce
argument_list|)
operator|-
literal|1
expr_stmt|;
if|if
condition|(
operator|!
name|mmfile
index|[
name|i
index|]
operator|.
name|ptr
condition|)
block|{
name|mmfile
index|[
name|i
index|]
operator|.
name|ptr
operator|=
name|read_sha1_file
argument_list|(
name|ce
operator|->
name|sha1
argument_list|,
operator|&
name|type
argument_list|,
operator|&
name|size
argument_list|)
expr_stmt|;
name|mmfile
index|[
name|i
index|]
operator|.
name|size
operator|=
name|size
expr_stmt|;
block|}
block|}
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
literal|3
condition|;
name|i
operator|++
control|)
if|if
condition|(
operator|!
name|mmfile
index|[
name|i
index|]
operator|.
name|ptr
operator|&&
operator|!
name|mmfile
index|[
name|i
index|]
operator|.
name|size
condition|)
name|mmfile
index|[
name|i
index|]
operator|.
name|ptr
operator|=
name|xstrdup
argument_list|(
literal|""
argument_list|)
expr_stmt|;
comment|/* 	 * NEEDSWORK: handle conflicts from merges with 	 * merge.renormalize set, too? 	 */
name|ll_merge
argument_list|(
operator|&
name|result
argument_list|,
name|path
argument_list|,
operator|&
name|mmfile
index|[
literal|0
index|]
argument_list|,
name|NULL
argument_list|,
operator|&
name|mmfile
index|[
literal|1
index|]
argument_list|,
literal|"ours"
argument_list|,
operator|&
name|mmfile
index|[
literal|2
index|]
argument_list|,
literal|"theirs"
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
literal|3
condition|;
name|i
operator|++
control|)
name|free
argument_list|(
name|mmfile
index|[
name|i
index|]
operator|.
name|ptr
argument_list|)
expr_stmt|;
name|memset
argument_list|(
operator|&
name|io
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
name|io
argument_list|)
argument_list|)
expr_stmt|;
name|io
operator|.
name|io
operator|.
name|getline
operator|=
name|rerere_mem_getline
expr_stmt|;
if|if
condition|(
name|output
condition|)
name|io
operator|.
name|io
operator|.
name|output
operator|=
name|fopen
argument_list|(
name|output
argument_list|,
literal|"w"
argument_list|)
expr_stmt|;
else|else
name|io
operator|.
name|io
operator|.
name|output
operator|=
name|NULL
expr_stmt|;
name|strbuf_init
argument_list|(
operator|&
name|io
operator|.
name|input
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|strbuf_attach
argument_list|(
operator|&
name|io
operator|.
name|input
argument_list|,
name|result
operator|.
name|ptr
argument_list|,
name|result
operator|.
name|size
argument_list|,
name|result
operator|.
name|size
argument_list|)
expr_stmt|;
comment|/* 	 * Grab the conflict ID and optionally write the original 	 * contents with conflict markers out. 	 */
name|hunk_no
operator|=
name|handle_path
argument_list|(
name|sha1
argument_list|,
operator|(
expr|struct
name|rerere_io
operator|*
operator|)
operator|&
name|io
argument_list|,
name|marker_size
argument_list|)
expr_stmt|;
name|strbuf_release
argument_list|(
operator|&
name|io
operator|.
name|input
argument_list|)
expr_stmt|;
if|if
condition|(
name|io
operator|.
name|io
operator|.
name|output
condition|)
name|fclose
argument_list|(
name|io
operator|.
name|io
operator|.
name|output
argument_list|)
expr_stmt|;
return|return
name|hunk_no
return|;
block|}
end_function
begin_function
DECL|function|rerere_forget_one_path
specifier|static
name|int
name|rerere_forget_one_path
parameter_list|(
specifier|const
name|char
modifier|*
name|path
parameter_list|,
name|struct
name|string_list
modifier|*
name|rr
parameter_list|)
block|{
specifier|const
name|char
modifier|*
name|filename
decl_stmt|;
name|struct
name|rerere_id
modifier|*
name|id
decl_stmt|;
name|unsigned
name|char
name|sha1
index|[
literal|20
index|]
decl_stmt|;
name|int
name|ret
decl_stmt|;
name|struct
name|string_list_item
modifier|*
name|item
decl_stmt|;
comment|/* 	 * Recreate the original conflict from the stages in the 	 * index and compute the conflict ID 	 */
name|ret
operator|=
name|handle_cache
argument_list|(
name|path
argument_list|,
name|sha1
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
if|if
condition|(
name|ret
operator|<
literal|1
condition|)
return|return
name|error
argument_list|(
literal|"Could not parse conflict hunks in '%s'"
argument_list|,
name|path
argument_list|)
return|;
comment|/* Nuke the recorded resolution for the conflict */
name|id
operator|=
name|new_rerere_id
argument_list|(
name|sha1
argument_list|)
expr_stmt|;
for|for
control|(
name|id
operator|->
name|variant
operator|=
literal|0
init|;
name|id
operator|->
name|variant
operator|<
name|id
operator|->
name|collection
operator|->
name|status_nr
condition|;
name|id
operator|->
name|variant
operator|++
control|)
block|{
name|mmfile_t
name|cur
init|=
block|{
name|NULL
block|,
literal|0
block|}
decl_stmt|;
name|mmbuffer_t
name|result
init|=
block|{
name|NULL
block|,
literal|0
block|}
decl_stmt|;
name|int
name|cleanly_resolved
decl_stmt|;
if|if
condition|(
operator|!
name|has_rerere_resolution
argument_list|(
name|id
argument_list|)
condition|)
continue|continue;
name|handle_cache
argument_list|(
name|path
argument_list|,
name|sha1
argument_list|,
name|rerere_path
argument_list|(
name|id
argument_list|,
literal|"thisimage"
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|read_mmfile
argument_list|(
operator|&
name|cur
argument_list|,
name|rerere_path
argument_list|(
name|id
argument_list|,
literal|"thisimage"
argument_list|)
argument_list|)
condition|)
block|{
name|free
argument_list|(
name|cur
operator|.
name|ptr
argument_list|)
expr_stmt|;
return|return
name|error
argument_list|(
literal|"Failed to update conflicted state in '%s'"
argument_list|,
name|path
argument_list|)
return|;
block|}
name|cleanly_resolved
operator|=
operator|!
name|try_merge
argument_list|(
name|id
argument_list|,
name|path
argument_list|,
operator|&
name|cur
argument_list|,
operator|&
name|result
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|result
operator|.
name|ptr
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cur
operator|.
name|ptr
argument_list|)
expr_stmt|;
if|if
condition|(
name|cleanly_resolved
condition|)
break|break;
block|}
if|if
condition|(
name|id
operator|->
name|collection
operator|->
name|status_nr
operator|<=
name|id
operator|->
name|variant
condition|)
return|return
name|error
argument_list|(
literal|"no remembered resolution for '%s'"
argument_list|,
name|path
argument_list|)
return|;
name|filename
operator|=
name|rerere_path
argument_list|(
name|id
argument_list|,
literal|"postimage"
argument_list|)
expr_stmt|;
if|if
condition|(
name|unlink
argument_list|(
name|filename
argument_list|)
condition|)
return|return
operator|(
name|errno
operator|==
name|ENOENT
condition|?
name|error
argument_list|(
literal|"no remembered resolution for %s"
argument_list|,
name|path
argument_list|)
else|:
name|error_errno
argument_list|(
literal|"cannot unlink %s"
argument_list|,
name|filename
argument_list|)
operator|)
return|;
comment|/* 	 * Update the preimage so that the user can resolve the 	 * conflict in the working tree, run us again to record 	 * the postimage. 	 */
name|handle_cache
argument_list|(
name|path
argument_list|,
name|sha1
argument_list|,
name|rerere_path
argument_list|(
name|id
argument_list|,
literal|"preimage"
argument_list|)
argument_list|)
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Updated preimage for '%s'\n"
argument_list|,
name|path
argument_list|)
expr_stmt|;
comment|/* 	 * And remember that we can record resolution for this 	 * conflict when the user is done. 	 */
name|item
operator|=
name|string_list_insert
argument_list|(
name|rr
argument_list|,
name|path
argument_list|)
expr_stmt|;
name|free_rerere_id
argument_list|(
name|item
argument_list|)
expr_stmt|;
name|item
operator|->
name|util
operator|=
name|id
expr_stmt|;
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Forgot resolution for %s\n"
argument_list|,
name|path
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
end_function
begin_function
DECL|function|rerere_forget
name|int
name|rerere_forget
parameter_list|(
name|struct
name|pathspec
modifier|*
name|pathspec
parameter_list|)
block|{
name|int
name|i
decl_stmt|,
name|fd
decl_stmt|;
name|struct
name|string_list
name|conflict
init|=
name|STRING_LIST_INIT_DUP
decl_stmt|;
name|struct
name|string_list
name|merge_rr
init|=
name|STRING_LIST_INIT_DUP
decl_stmt|;
if|if
condition|(
name|read_cache
argument_list|()
operator|<
literal|0
condition|)
return|return
name|error
argument_list|(
literal|"Could not read index"
argument_list|)
return|;
name|fd
operator|=
name|setup_rerere
argument_list|(
operator|&
name|merge_rr
argument_list|,
name|RERERE_NOAUTOUPDATE
argument_list|)
expr_stmt|;
if|if
condition|(
name|fd
operator|<
literal|0
condition|)
return|return
literal|0
return|;
comment|/* 	 * The paths may have been resolved (incorrectly); 	 * recover the original conflicted state and then 	 * find the conflicted paths. 	 */
name|unmerge_cache
argument_list|(
name|pathspec
argument_list|)
expr_stmt|;
name|find_conflict
argument_list|(
operator|&
name|conflict
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|conflict
operator|.
name|nr
condition|;
name|i
operator|++
control|)
block|{
name|struct
name|string_list_item
modifier|*
name|it
init|=
operator|&
name|conflict
operator|.
name|items
index|[
name|i
index|]
decl_stmt|;
if|if
condition|(
operator|!
name|match_pathspec
argument_list|(
name|pathspec
argument_list|,
name|it
operator|->
name|string
argument_list|,
name|strlen
argument_list|(
name|it
operator|->
name|string
argument_list|)
argument_list|,
literal|0
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
condition|)
continue|continue;
name|rerere_forget_one_path
argument_list|(
name|it
operator|->
name|string
argument_list|,
operator|&
name|merge_rr
argument_list|)
expr_stmt|;
block|}
return|return
name|write_rr
argument_list|(
operator|&
name|merge_rr
argument_list|,
name|fd
argument_list|)
return|;
block|}
end_function
begin_comment
comment|/*  * Garbage collection support  */
end_comment
begin_function
DECL|function|rerere_created_at
specifier|static
name|time_t
name|rerere_created_at
parameter_list|(
name|struct
name|rerere_id
modifier|*
name|id
parameter_list|)
block|{
name|struct
name|stat
name|st
decl_stmt|;
return|return
name|stat
argument_list|(
name|rerere_path
argument_list|(
name|id
argument_list|,
literal|"preimage"
argument_list|)
argument_list|,
operator|&
name|st
argument_list|)
condition|?
operator|(
name|time_t
operator|)
literal|0
else|:
name|st
operator|.
name|st_mtime
return|;
block|}
end_function
begin_function
DECL|function|rerere_last_used_at
specifier|static
name|time_t
name|rerere_last_used_at
parameter_list|(
name|struct
name|rerere_id
modifier|*
name|id
parameter_list|)
block|{
name|struct
name|stat
name|st
decl_stmt|;
return|return
name|stat
argument_list|(
name|rerere_path
argument_list|(
name|id
argument_list|,
literal|"postimage"
argument_list|)
argument_list|,
operator|&
name|st
argument_list|)
condition|?
operator|(
name|time_t
operator|)
literal|0
else|:
name|st
operator|.
name|st_mtime
return|;
block|}
end_function
begin_comment
comment|/*  * Remove the recorded resolution for a given conflict ID  */
end_comment
begin_function
DECL|function|unlink_rr_item
specifier|static
name|void
name|unlink_rr_item
parameter_list|(
name|struct
name|rerere_id
modifier|*
name|id
parameter_list|)
block|{
name|unlink_or_warn
argument_list|(
name|rerere_path
argument_list|(
name|id
argument_list|,
literal|"thisimage"
argument_list|)
argument_list|)
expr_stmt|;
name|remove_variant
argument_list|(
name|id
argument_list|)
expr_stmt|;
name|id
operator|->
name|collection
operator|->
name|status
index|[
name|id
operator|->
name|variant
index|]
operator|=
literal|0
expr_stmt|;
block|}
end_function
begin_function
DECL|function|prune_one
specifier|static
name|void
name|prune_one
parameter_list|(
name|struct
name|rerere_id
modifier|*
name|id
parameter_list|,
name|time_t
name|now
parameter_list|,
name|int
name|cutoff_resolve
parameter_list|,
name|int
name|cutoff_noresolve
parameter_list|)
block|{
name|time_t
name|then
decl_stmt|;
name|int
name|cutoff
decl_stmt|;
name|then
operator|=
name|rerere_last_used_at
argument_list|(
name|id
argument_list|)
expr_stmt|;
if|if
condition|(
name|then
condition|)
name|cutoff
operator|=
name|cutoff_resolve
expr_stmt|;
else|else
block|{
name|then
operator|=
name|rerere_created_at
argument_list|(
name|id
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|then
condition|)
return|return;
name|cutoff
operator|=
name|cutoff_noresolve
expr_stmt|;
block|}
if|if
condition|(
name|then
operator|<
name|now
operator|-
name|cutoff
operator|*
literal|86400
condition|)
name|unlink_rr_item
argument_list|(
name|id
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|rerere_gc
name|void
name|rerere_gc
parameter_list|(
name|struct
name|string_list
modifier|*
name|rr
parameter_list|)
block|{
name|struct
name|string_list
name|to_remove
init|=
name|STRING_LIST_INIT_DUP
decl_stmt|;
name|DIR
modifier|*
name|dir
decl_stmt|;
name|struct
name|dirent
modifier|*
name|e
decl_stmt|;
name|int
name|i
decl_stmt|;
name|time_t
name|now
init|=
name|time
argument_list|(
name|NULL
argument_list|)
decl_stmt|;
name|int
name|cutoff_noresolve
init|=
literal|15
decl_stmt|;
name|int
name|cutoff_resolve
init|=
literal|60
decl_stmt|;
if|if
condition|(
name|setup_rerere
argument_list|(
name|rr
argument_list|,
literal|0
argument_list|)
operator|<
literal|0
condition|)
return|return;
name|git_config_get_int
argument_list|(
literal|"gc.rerereresolved"
argument_list|,
operator|&
name|cutoff_resolve
argument_list|)
expr_stmt|;
name|git_config_get_int
argument_list|(
literal|"gc.rerereunresolved"
argument_list|,
operator|&
name|cutoff_noresolve
argument_list|)
expr_stmt|;
name|git_config
argument_list|(
name|git_default_config
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
name|dir
operator|=
name|opendir
argument_list|(
name|git_path
argument_list|(
literal|"rr-cache"
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|dir
condition|)
name|die_errno
argument_list|(
literal|"unable to open rr-cache directory"
argument_list|)
expr_stmt|;
comment|/* Collect stale conflict IDs ... */
while|while
condition|(
operator|(
name|e
operator|=
name|readdir
argument_list|(
name|dir
argument_list|)
operator|)
condition|)
block|{
name|struct
name|rerere_dir
modifier|*
name|rr_dir
decl_stmt|;
name|struct
name|rerere_id
name|id
decl_stmt|;
name|int
name|now_empty
decl_stmt|;
if|if
condition|(
name|is_dot_or_dotdot
argument_list|(
name|e
operator|->
name|d_name
argument_list|)
condition|)
continue|continue;
name|rr_dir
operator|=
name|find_rerere_dir
argument_list|(
name|e
operator|->
name|d_name
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|rr_dir
condition|)
continue|continue;
comment|/* or should we remove e->d_name? */
name|now_empty
operator|=
literal|1
expr_stmt|;
for|for
control|(
name|id
operator|.
name|variant
operator|=
literal|0
operator|,
name|id
operator|.
name|collection
operator|=
name|rr_dir
init|;
name|id
operator|.
name|variant
operator|<
name|id
operator|.
name|collection
operator|->
name|status_nr
condition|;
name|id
operator|.
name|variant
operator|++
control|)
block|{
name|prune_one
argument_list|(
operator|&
name|id
argument_list|,
name|now
argument_list|,
name|cutoff_resolve
argument_list|,
name|cutoff_noresolve
argument_list|)
expr_stmt|;
if|if
condition|(
name|id
operator|.
name|collection
operator|->
name|status
index|[
name|id
operator|.
name|variant
index|]
condition|)
name|now_empty
operator|=
literal|0
expr_stmt|;
block|}
if|if
condition|(
name|now_empty
condition|)
name|string_list_append
argument_list|(
operator|&
name|to_remove
argument_list|,
name|e
operator|->
name|d_name
argument_list|)
expr_stmt|;
block|}
name|closedir
argument_list|(
name|dir
argument_list|)
expr_stmt|;
comment|/* ... and then remove the empty directories */
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|to_remove
operator|.
name|nr
condition|;
name|i
operator|++
control|)
name|rmdir
argument_list|(
name|git_path
argument_list|(
literal|"rr-cache/%s"
argument_list|,
name|to_remove
operator|.
name|items
index|[
name|i
index|]
operator|.
name|string
argument_list|)
argument_list|)
expr_stmt|;
name|string_list_clear
argument_list|(
operator|&
name|to_remove
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|rollback_lock_file
argument_list|(
operator|&
name|write_lock
argument_list|)
expr_stmt|;
block|}
end_function
begin_comment
comment|/*  * During a conflict resolution, after "rerere" recorded the  * preimages, abandon them if the user did not resolve them or  * record their resolutions.  And drop $GIT_DIR/MERGE_RR.  *  * NEEDSWORK: shouldn't we be calling this from "reset --hard"?  */
end_comment
begin_function
DECL|function|rerere_clear
name|void
name|rerere_clear
parameter_list|(
name|struct
name|string_list
modifier|*
name|merge_rr
parameter_list|)
block|{
name|int
name|i
decl_stmt|;
if|if
condition|(
name|setup_rerere
argument_list|(
name|merge_rr
argument_list|,
literal|0
argument_list|)
operator|<
literal|0
condition|)
return|return;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|merge_rr
operator|->
name|nr
condition|;
name|i
operator|++
control|)
block|{
name|struct
name|rerere_id
modifier|*
name|id
init|=
name|merge_rr
operator|->
name|items
index|[
name|i
index|]
operator|.
name|util
decl_stmt|;
if|if
condition|(
operator|!
name|has_rerere_resolution
argument_list|(
name|id
argument_list|)
condition|)
block|{
name|unlink_rr_item
argument_list|(
name|id
argument_list|)
expr_stmt|;
name|rmdir
argument_list|(
name|rerere_path
argument_list|(
name|id
argument_list|,
name|NULL
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
name|unlink_or_warn
argument_list|(
name|git_path_merge_rr
argument_list|()
argument_list|)
expr_stmt|;
name|rollback_lock_file
argument_list|(
operator|&
name|write_lock
argument_list|)
expr_stmt|;
block|}
end_function
end_unit
