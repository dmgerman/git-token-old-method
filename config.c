...
end_decl_stmt
begin_decl_stmt
DECL|variable|current_config_kvi
specifier|static
name|struct
name|key_value_info
modifier|*
name|current_config_kvi
decl_stmt|;
end_decl_stmt
begin_comment
comment|/*  * Similar to the variables above, this gives access to the "scope" of the  * current value (repo, global, etc). For cached values, it can be found via  * the current_config_kvi as above. During parsing, the current value can be  * found in this variable. It's not part of "cf" because it transcends a single  * file (i.e., a file included from .git/config is still in "repo" scope).  */
end_comment
begin_decl_stmt
DECL|variable|current_parsing_scope
specifier|static
name|enum
name|config_scope
name|current_parsing_scope
decl_stmt|;
end_decl_stmt
begin_decl_stmt
...
end_decl_stmt
begin_comment
comment|/*  * Default config_set that contains key-value pairs from the usual set of config  * config files (i.e repo specific .git/config, user wide ~/.gitconfig, XDG  * config file and the global /etc/gitconfig)  */
end_comment
begin_decl_stmt
...
name|_
argument_list|(
literal|"bad config line %d in %s %s"
argument_list|)
argument_list|,
name|cf
operator|->
...
name|_
argument_list|(
literal|"bad numeric config value '%s' for '%s' in %s %s: %s"
argument_list|)
argument_list|,
name|value
argument_list|,
...
name|error
argument_list|(
literal|"core.commentChar should only be one character"
argument_list|)
return|;
return|return
literal|0
...
specifier|static
name|int
name|git_config_from_blob_sha1
parameter_list|(
name|config_fn_t
name|fn
parameter_list|,
...
name|error
argument_list|(
literal|"unable to load config blob object '%s'"
argument_list|,
name|name
argument_list|)
return|;
...
name|error
argument_list|(
literal|"reference '%s' does not point to a blob"
argument_list|,
name|name
argument_list|)
return|;
...
name|error
argument_list|(
literal|"unable to resolve config blob '%s'"
argument_list|,
name|name
argument_list|)
return|;
...
end_function
begin_comment
comment|/*  * Parse environment variable 'k' as a boolean (in various  * possible spellings); if missing, use the default value 'def'.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*  * Parse environment variable 'k' as ulong with possibly a unit  * suffix; if missing, use the default value 'val'.  */
end_comment
begin_function
...
end_function
begin_function
DECL|function|do_git_config_sequence
specifier|static
name|int
name|do_git_config_sequence
parameter_list|(
name|config_fn_t
name|fn
parameter_list|,
...
argument_list|)
expr_stmt|;
comment|/* 	 * Since the keys are being fed by git_config*() callback mechanism, they 	 * are already normalized. So simply add them without any further munging. 	 */
if|if
condition|(
operator|!
name|e
...
name|die
argument_list|(
literal|"BUG: configset_add_value has no source"
argument_list|)
expr_stmt|;
if|if
condition|(
...
block|{
comment|/* for values read from `git_config_from_parameters()` */
name|kv_info
operator|->
name|filename
operator|=
...
name|NULL
decl_stmt|;
comment|/* 	 * Follows "last one wins" semantic, i.e., if there are multiple matches for the 	 * queried key in the files of the configset, the value returned will be the last 	 * value in the value list for that key. 	 */
name|values
operator|=
...
end_function
begin_function
DECL|function|git_config_get_untracked_cache
name|int
...
name|error
argument_list|(
literal|"unknown core.untrackedCache value '%s'; "
literal|"using 'keep' default value"
argument_list|,
name|v
argument_list|)
expr_stmt|;
...
name|_
argument_list|(
literal|"unable to parse '%s' from command-line config"
argument_list|)
argument_list|,
name|key
argument_list|)
...
name|_
argument_list|(
literal|"bad config variable '%s' in file '%s' at line %d"
argument_list|)
argument_list|,
name|key
argument_list|,
...
literal|0
expr_stmt|;
comment|/* Invalidate the config cache */
name|git_config_clear
argument_list|()
expr_stmt|;
name|out_free
...
name|dot
decl_stmt|;
comment|/* Does it start with "section." ? */
if|if
condition|(
operator|!
name|starts_with
...
literal|1
return|;
comment|/* 	 * Find the key; we don't know yet if we have a subsection, but we must 	 * parse backwards from the end, since the subsection may have dots in 	 * it, too. 	 */
name|dot
operator|=
name|strrchr
argument_list|(
...
literal|1
expr_stmt|;
comment|/* Did we have a subsection at all? */
if|if
condition|(
name|dot
operator|==
...
end_function
begin_function
DECL|function|current_config_origin_type
specifier|const
name|char
modifier|*
...
name|die
argument_list|(
literal|"BUG: current_config_origin_type called outside config callback"
argument_list|)
expr_stmt|;
return|return
name|type
...
end_function
begin_function
DECL|function|current_config_name
specifier|const
name|char
modifier|*
...
name|die
argument_list|(
literal|"BUG: current_config_name called outside config callback"
argument_list|)
expr_stmt|;
return|return
name|name
...
end_function
begin_function
DECL|function|current_config_scope
name|enum
name|config_scope
