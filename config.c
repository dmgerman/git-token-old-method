...
argument_list|)
decl_stmt|;
comment|/* Forward-compatibility cue: $GIT_CONFIG makes git read _only_ 	 * the given config file, $GIT_CONFIG_LOCAL will make it process 	 * it in addition to the global config file, the same way it would 	 * the per-repository config file otherwise. */
if|if
condition|(
name|getenv
argument_list|(
