...
argument_list|)
expr_stmt|;
comment|/* pack+1 is the base sha1, unless we have it, we need to 	 * unpack it first. 	 */
if|if
condition|(
operator|!
name|has_sha1_file
...
expr_stmt|;
block|}
comment|/* pack+1 thru pack+20 is the base sha1 and 	 * pack+21 thru unknown number is the delta data. 	 * we do not even have size of the delta data uncompressed. 	 * sheesh! 	 */
name|delta_alloc
operator|=
literal|1024
expr_stmt|;
...
name|die
argument_list|(
literal|"failed to read delta-pack base object"
argument_list|)
expr_stmt|;
name|result
operator|=
