...
end_enum
begin_comment
comment|/*  * Support data structure for our opendir/readdir/closedir wrappers  */
end_comment
begin_struct
...
end_function
begin_function
DECL|function|different_decisions
specifier|static
specifier|inline
name|int
name|different_decisions
parameter_list|(
specifier|const
name|struct
name|exclude
...
end_function
begin_comment
comment|/*  * Return non-zero if pathname is a directory and an ancestor of the  * literal path in a pattern.  */
end_comment
begin_function
DECL|function|match_directory_part
specifier|static
name|int
name|match_directory_part
parameter_list|(
specifier|const
name|char
modifier|*
...
return|;
block|}
comment|/* 	 * We have found a matching pattern "exc" that may exclude whole 	 * directory. We also found that there may be a pattern that matches 	 * something inside the directory and reincludes stuff. 	 * 	 * Go through the patterns again, find that pattern and double check. 	 * If it's true, return "undecided" and keep descending in. "exc" is 	 * marked sticky so that it continues to match inside the directory. 	 */
if|if
condition|(
operator|!
operator|(
...
name|trace_exclude
argument_list|,
literal|"exclude: %.*s vs %s at line %d => %s,"
literal|" forced open by %s at line %d => n/a\n"
argument_list|,
name|pathlen
argument_list|,
name|pathname
...
name|trace_exclude
argument_list|,
literal|"exclude: %.*s vs %s at line %d => %s%s\n"
argument_list|,
name|pathlen
argument_list|,
name|pathname
...
expr_stmt|;
block|}
comment|/* 		 * NEEDSWORK: when untracked cache is enabled, prep_exclude() 		 * will first be called in valid_cached_dir() then maybe many 		 * times more in last_exclude_matching(). When the cache is 		 * used, last_exclude_matching() will not be called and 		 * reading .gitignore content will be a waste. 		 * 		 * So when it's called by valid_cached_dir() and we can get 		 * .gitignore SHA-1 from the index (i.e. .gitignore is not 		 * modified on work tree), we could delay reading the 		 * .gitignore content until we absolutely need it in 		 * last_exclude_matching(). Be careful about ignore rule 		 * order, though, if you do that. 		 */
if|if
condition|(
name|untracked
operator|&&
...
argument_list|)
expr_stmt|;
comment|/* treat_one_path() does this before it calls treat_directory() */
...
name|check_only
condition|)
comment|/* 		 * check_only is set as a result of treat_directory() getting 		 * to its bottom. Verify again the same set of directories 		 * with check_only set. 		 */
return|return
name|read_directory_recursive
argument_list|(
name|dir
...
argument_list|)
return|;
comment|/* 	 * We get path_recurse in the first run when 	 * directory_exists_in_index() returns index_nonexistent. We 	 * are sure that new changes in the index does not impact the 	 * outcome. Return now. 	 */
return|return
name|path_recurse
return|;
block|}
...
return|;
block|}
comment|/* 	 * prep_exclude will be called eventually on this directory, 	 * but it's called much later in last_exclude_matching(). We 	 * need it now to determine the validity of the cache for this 	 * path. The next calls will be nearly no-op, the way 	 * prep_exclude() is designed. 	 */
if|if
condition|(
name|path
operator|->
...
argument_list|)
expr_stmt|;
comment|/* hopefully prep_exclude() haven't invalidated this entry... */
return|return
name|untracked
operator|->
name|valid
...
argument_list|)
expr_stmt|;
comment|/* 	 * We have gone through this directory and found no untracked 	 * entries. Mark it valid. 	 */
if|if
condition|(
name|cdir
operator|->
...
name|_
argument_list|(
literal|"failed to get kernel name and information"
argument_list|)
argument_list|)
expr_stmt|;
name|strbuf_addf
...
parameter_list|)
block|{
comment|/* 	 * Previous git versions may have saved many NUL separated 	 * strings in the "ident" field, but it is insane to manage 	 * many locations, so just take care of the first one. 	 */
return|return
operator|!
name|strcmp
argument_list|(
...
end_function
begin_function
DECL|function|set_untracked_ident
specifier|static
name|void
name|set_untracked_ident
parameter_list|(
name|struct
name|untracked_cache
modifier|*
...
argument_list|)
expr_stmt|;
comment|/* 	 * This strbuf used to contain a list of NUL separated 	 * strings, so save NUL too for backward compatibility. 	 */
name|strbuf_addch
argument_list|(
operator|&
name|uc
...
end_function
begin_function
DECL|function|remove_untracked_cache
name|void
...
name|_
argument_list|(
literal|"Untracked cache is disabled on this system or location."
argument_list|)
argument_list|)
expr_stmt|;
return|return
...
expr_stmt|;
block|}
comment|/* Make sure this directory is not dropped out at saving phase */
name|root
operator|->
name|recurse
operator|=
...
name|nr
return|;
comment|/* 	 * Stay on the safe side. if read_directory() has run once on 	 * "dir", some sticky flag may have been left. Clear them all. 	 */
name|clear_sticky
argument_list|(
name|dir
argument_list|)
...
name|struct
name|trace_key
name|trace_untracked_stats
init|=
name|TRACE_KEY_INIT
argument_list|(
name|UNTRACKED_STATS
argument_list|)
decl_stmt|;
name|trace_printf_key
argument_list|(
...
name|trace_untracked_stats
argument_list|,
literal|"node creation: %u\n"
literal|"gitignore invalidation: %u\n"
literal|"directory invalidation: %u\n"
literal|"opendir: %u\n"
argument_list|,
name|dir
operator|->
...
name|len
parameter_list|)
value|(offsetof(struct ondisk_untracked_cache, exclude_per_dir) + len + 1)
end_define
begin_struct
...
operator|++
decl_stmt|;
comment|/* 	 * untracked_nr should be reset whenever valid is clear, but 	 * for safety.. 	 */
if|if
condition|(
operator|!
name|untracked
...
argument_list|)
expr_stmt|;
comment|/* skip non-recurse directories */
for|for
control|(
name|i
operator|=
...
argument_list|)
expr_stmt|;
comment|/* safe guard for string lists */
name|ewah_free
argument_list|(
name|wd
operator|.
...
argument_list|)
expr_stmt|;
comment|/* NUL after exclude_per_dir is covered by sizeof(*ouc) */
name|next
operator|+=
name|ouc_size
argument_list|(
