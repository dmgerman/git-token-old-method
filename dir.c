...
literal|1
return|;
comment|/* 		 * This can happen when we ignore some exclude rules 		 * on directories in other to see if negative rules 		 * may match. E.g. 		 * 		 * /abc 		 * !/abc/def/ghi 		 * 		 * The pattern of interest is "/abc". On the first 		 * try, we should match path "abc" with this pattern 		 * in the "if" statement right above, but the caller 		 * ignores it. 		 * 		 * On the second try with paths within "abc", 		 * e.g. "abc/xyz", we come here and try to match it 		 * with "/abc". 		 */
if|if
condition|(
operator|!
name|patternlen
...
end_function
begin_comment
comment|/*  * Return non-zero if pathname is a directory and an ancestor of the  * literal path in a (negative) pattern. This is used to keep  * descending in "foo" and "foo/bar" when the pattern is  * "!foo/bar/.gitignore". "foo/notbar" will not be descended however.  */
end_comment
begin_function
