begin_unit
begin_include
include|#
directive|include
file|"cache.h"
end_include
begin_include
include|#
directive|include
file|"pkt-line.h"
end_include
begin_decl_stmt
DECL|variable|packet_buffer
name|char
name|packet_buffer
index|[
name|LARGE_PACKET_MAX
index|]
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|packet_trace_prefix
specifier|static
specifier|const
name|char
modifier|*
name|packet_trace_prefix
init|=
literal|"git"
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|trace_packet
specifier|static
name|struct
name|trace_key
name|trace_packet
init|=
name|TRACE_KEY_INIT
argument_list|(
name|PACKET
argument_list|)
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|trace_pack
specifier|static
name|struct
name|trace_key
name|trace_pack
init|=
name|TRACE_KEY_INIT
argument_list|(
name|PACKFILE
argument_list|)
decl_stmt|;
end_decl_stmt
begin_function
DECL|function|packet_trace_identity
name|void
name|packet_trace_identity
parameter_list|(
specifier|const
name|char
modifier|*
name|prog
parameter_list|)
block|{
name|packet_trace_prefix
operator|=
name|xstrdup
argument_list|(
name|prog
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|packet_trace_pack
specifier|static
name|int
name|packet_trace_pack
parameter_list|(
specifier|const
name|char
modifier|*
name|buf
parameter_list|,
name|unsigned
name|int
name|len
parameter_list|,
name|int
name|sideband
parameter_list|)
block|{
if|if
condition|(
operator|!
name|sideband
condition|)
block|{
name|trace_verbatim
argument_list|(
operator|&
name|trace_pack
argument_list|,
name|buf
argument_list|,
name|len
argument_list|)
expr_stmt|;
return|return
literal|1
return|;
block|}
elseif|else
if|if
condition|(
name|len
operator|&&
operator|*
name|buf
operator|==
literal|'\1'
condition|)
block|{
name|trace_verbatim
argument_list|(
operator|&
name|trace_pack
argument_list|,
name|buf
operator|+
literal|1
argument_list|,
name|len
operator|-
literal|1
argument_list|)
expr_stmt|;
return|return
literal|1
return|;
block|}
else|else
block|{
comment|/* it's another non-pack sideband */
return|return
literal|0
return|;
block|}
block|}
end_function
begin_function
DECL|function|packet_trace
specifier|static
name|void
name|packet_trace
parameter_list|(
specifier|const
name|char
modifier|*
name|buf
parameter_list|,
name|unsigned
name|int
name|len
parameter_list|,
name|int
name|write
parameter_list|)
block|{
name|int
name|i
decl_stmt|;
name|struct
name|strbuf
name|out
decl_stmt|;
specifier|static
name|int
name|in_pack
decl_stmt|,
name|sideband
decl_stmt|;
if|if
condition|(
operator|!
name|trace_want
argument_list|(
operator|&
name|trace_packet
argument_list|)
operator|&&
operator|!
name|trace_want
argument_list|(
operator|&
name|trace_pack
argument_list|)
condition|)
return|return;
if|if
condition|(
name|in_pack
condition|)
block|{
if|if
condition|(
name|packet_trace_pack
argument_list|(
name|buf
argument_list|,
name|len
argument_list|,
name|sideband
argument_list|)
condition|)
return|return;
block|}
elseif|else
if|if
condition|(
name|starts_with
argument_list|(
name|buf
argument_list|,
literal|"PACK"
argument_list|)
operator|||
name|starts_with
argument_list|(
name|buf
argument_list|,
literal|"\1PACK"
argument_list|)
condition|)
block|{
name|in_pack
operator|=
literal|1
expr_stmt|;
name|sideband
operator|=
operator|*
name|buf
operator|==
literal|'\1'
expr_stmt|;
name|packet_trace_pack
argument_list|(
name|buf
argument_list|,
name|len
argument_list|,
name|sideband
argument_list|)
expr_stmt|;
comment|/* 		 * Make a note in the human-readable trace that the pack data 		 * started. 		 */
name|buf
operator|=
literal|"PACK ..."
expr_stmt|;
name|len
operator|=
name|strlen
argument_list|(
name|buf
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|trace_want
argument_list|(
operator|&
name|trace_packet
argument_list|)
condition|)
return|return;
comment|/* +32 is just a guess for header + quoting */
name|strbuf_init
argument_list|(
operator|&
name|out
argument_list|,
name|len
operator|+
literal|32
argument_list|)
expr_stmt|;
name|strbuf_addf
argument_list|(
operator|&
name|out
argument_list|,
literal|"packet: %12s%c "
argument_list|,
name|packet_trace_prefix
argument_list|,
name|write
condition|?
literal|'>'
else|:
literal|'<'
argument_list|)
expr_stmt|;
comment|/* XXX we should really handle printable utf8 */
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|len
condition|;
name|i
operator|++
control|)
block|{
comment|/* suppress newlines */
if|if
condition|(
name|buf
index|[
name|i
index|]
operator|==
literal|'\n'
condition|)
continue|continue;
if|if
condition|(
name|buf
index|[
name|i
index|]
operator|>=
literal|0x20
operator|&&
name|buf
index|[
name|i
index|]
operator|<=
literal|0x7e
condition|)
name|strbuf_addch
argument_list|(
operator|&
name|out
argument_list|,
name|buf
index|[
name|i
index|]
argument_list|)
expr_stmt|;
else|else
name|strbuf_addf
argument_list|(
operator|&
name|out
argument_list|,
literal|"\\%o"
argument_list|,
name|buf
index|[
name|i
index|]
argument_list|)
expr_stmt|;
block|}
name|strbuf_addch
argument_list|(
operator|&
name|out
argument_list|,
literal|'\n'
argument_list|)
expr_stmt|;
name|trace_strbuf
argument_list|(
operator|&
name|trace_packet
argument_list|,
operator|&
name|out
argument_list|)
expr_stmt|;
name|strbuf_release
argument_list|(
operator|&
name|out
argument_list|)
expr_stmt|;
block|}
end_function
begin_comment
comment|/*  * If we buffered things up above (we don't, but we should),  * we'd flush it here  */
end_comment
begin_function
DECL|function|packet_flush
name|void
name|packet_flush
parameter_list|(
name|int
name|fd
parameter_list|)
block|{
name|packet_trace
argument_list|(
literal|"0000"
argument_list|,
literal|4
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|write_or_die
argument_list|(
name|fd
argument_list|,
literal|"0000"
argument_list|,
literal|4
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|packet_buf_flush
name|void
name|packet_buf_flush
parameter_list|(
name|struct
name|strbuf
modifier|*
name|buf
parameter_list|)
block|{
name|packet_trace
argument_list|(
literal|"0000"
argument_list|,
literal|4
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|strbuf_add
argument_list|(
name|buf
argument_list|,
literal|"0000"
argument_list|,
literal|4
argument_list|)
expr_stmt|;
block|}
end_function
begin_define
DECL|macro|hex
define|#
directive|define
name|hex
parameter_list|(
name|a
parameter_list|)
value|(hexchar[(a)& 15])
end_define
begin_function
DECL|function|format_packet
specifier|static
name|void
name|format_packet
parameter_list|(
name|struct
name|strbuf
modifier|*
name|out
parameter_list|,
specifier|const
name|char
modifier|*
name|fmt
parameter_list|,
name|va_list
name|args
parameter_list|)
block|{
specifier|static
name|char
name|hexchar
index|[]
init|=
literal|"0123456789abcdef"
decl_stmt|;
name|size_t
name|orig_len
decl_stmt|,
name|n
decl_stmt|;
name|orig_len
operator|=
name|out
operator|->
name|len
expr_stmt|;
name|strbuf_addstr
argument_list|(
name|out
argument_list|,
literal|"0000"
argument_list|)
expr_stmt|;
name|strbuf_vaddf
argument_list|(
name|out
argument_list|,
name|fmt
argument_list|,
name|args
argument_list|)
expr_stmt|;
name|n
operator|=
name|out
operator|->
name|len
operator|-
name|orig_len
expr_stmt|;
if|if
condition|(
name|n
operator|>
name|LARGE_PACKET_MAX
condition|)
name|die
argument_list|(
literal|"protocol error: impossibly long line"
argument_list|)
expr_stmt|;
name|out
operator|->
name|buf
index|[
name|orig_len
operator|+
literal|0
index|]
operator|=
name|hex
argument_list|(
name|n
operator|>>
literal|12
argument_list|)
expr_stmt|;
name|out
operator|->
name|buf
index|[
name|orig_len
operator|+
literal|1
index|]
operator|=
name|hex
argument_list|(
name|n
operator|>>
literal|8
argument_list|)
expr_stmt|;
name|out
operator|->
name|buf
index|[
name|orig_len
operator|+
literal|2
index|]
operator|=
name|hex
argument_list|(
name|n
operator|>>
literal|4
argument_list|)
expr_stmt|;
name|out
operator|->
name|buf
index|[
name|orig_len
operator|+
literal|3
index|]
operator|=
name|hex
argument_list|(
name|n
argument_list|)
expr_stmt|;
name|packet_trace
argument_list|(
name|out
operator|->
name|buf
operator|+
name|orig_len
operator|+
literal|4
argument_list|,
name|n
operator|-
literal|4
argument_list|,
literal|1
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|packet_write
name|void
name|packet_write
parameter_list|(
name|int
name|fd
parameter_list|,
specifier|const
name|char
modifier|*
name|fmt
parameter_list|,
modifier|...
parameter_list|)
block|{
specifier|static
name|struct
name|strbuf
name|buf
init|=
name|STRBUF_INIT
decl_stmt|;
name|va_list
name|args
decl_stmt|;
name|strbuf_reset
argument_list|(
operator|&
name|buf
argument_list|)
expr_stmt|;
name|va_start
argument_list|(
name|args
argument_list|,
name|fmt
argument_list|)
expr_stmt|;
name|format_packet
argument_list|(
operator|&
name|buf
argument_list|,
name|fmt
argument_list|,
name|args
argument_list|)
expr_stmt|;
name|va_end
argument_list|(
name|args
argument_list|)
expr_stmt|;
name|write_or_die
argument_list|(
name|fd
argument_list|,
name|buf
operator|.
name|buf
argument_list|,
name|buf
operator|.
name|len
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|packet_buf_write
name|void
name|packet_buf_write
parameter_list|(
name|struct
name|strbuf
modifier|*
name|buf
parameter_list|,
specifier|const
name|char
modifier|*
name|fmt
parameter_list|,
modifier|...
parameter_list|)
block|{
name|va_list
name|args
decl_stmt|;
name|va_start
argument_list|(
name|args
argument_list|,
name|fmt
argument_list|)
expr_stmt|;
name|format_packet
argument_list|(
name|buf
argument_list|,
name|fmt
argument_list|,
name|args
argument_list|)
expr_stmt|;
name|va_end
argument_list|(
name|args
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|get_packet_data
specifier|static
name|int
name|get_packet_data
parameter_list|(
name|int
name|fd
parameter_list|,
name|char
modifier|*
modifier|*
name|src_buf
parameter_list|,
name|size_t
modifier|*
name|src_size
parameter_list|,
name|void
modifier|*
name|dst
parameter_list|,
name|unsigned
name|size
parameter_list|,
name|int
name|options
parameter_list|)
block|{
name|ssize_t
name|ret
decl_stmt|;
if|if
condition|(
name|fd
operator|>=
literal|0
operator|&&
name|src_buf
operator|&&
operator|*
name|src_buf
condition|)
name|die
argument_list|(
literal|"BUG: multiple sources given to packet_read"
argument_list|)
expr_stmt|;
comment|/* Read up to "size" bytes from our source, whatever it is. */
if|if
condition|(
name|src_buf
operator|&&
operator|*
name|src_buf
condition|)
block|{
name|ret
operator|=
name|size
operator|<
operator|*
name|src_size
condition|?
name|size
else|:
operator|*
name|src_size
expr_stmt|;
name|memcpy
argument_list|(
name|dst
argument_list|,
operator|*
name|src_buf
argument_list|,
name|ret
argument_list|)
expr_stmt|;
operator|*
name|src_buf
operator|+=
name|ret
expr_stmt|;
operator|*
name|src_size
operator|-=
name|ret
expr_stmt|;
block|}
else|else
block|{
name|ret
operator|=
name|read_in_full
argument_list|(
name|fd
argument_list|,
name|dst
argument_list|,
name|size
argument_list|)
expr_stmt|;
if|if
condition|(
name|ret
operator|<
literal|0
condition|)
name|die_errno
argument_list|(
literal|"read error"
argument_list|)
expr_stmt|;
block|}
comment|/* And complain if we didn't get enough bytes to satisfy the read. */
if|if
condition|(
name|ret
operator|<
name|size
condition|)
block|{
if|if
condition|(
name|options
operator|&
name|PACKET_READ_GENTLE_ON_EOF
condition|)
return|return
operator|-
literal|1
return|;
name|die
argument_list|(
literal|"The remote end hung up unexpectedly"
argument_list|)
expr_stmt|;
block|}
return|return
name|ret
return|;
block|}
end_function
begin_function
DECL|function|packet_length
specifier|static
name|int
name|packet_length
parameter_list|(
specifier|const
name|char
modifier|*
name|linelen
parameter_list|)
block|{
name|int
name|n
decl_stmt|;
name|int
name|len
init|=
literal|0
decl_stmt|;
for|for
control|(
name|n
operator|=
literal|0
init|;
name|n
operator|<
literal|4
condition|;
name|n
operator|++
control|)
block|{
name|unsigned
name|char
name|c
init|=
name|linelen
index|[
name|n
index|]
decl_stmt|;
name|len
operator|<<=
literal|4
expr_stmt|;
if|if
condition|(
name|c
operator|>=
literal|'0'
operator|&&
name|c
operator|<=
literal|'9'
condition|)
block|{
name|len
operator|+=
name|c
operator|-
literal|'0'
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
name|c
operator|>=
literal|'a'
operator|&&
name|c
operator|<=
literal|'f'
condition|)
block|{
name|len
operator|+=
name|c
operator|-
literal|'a'
operator|+
literal|10
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
name|c
operator|>=
literal|'A'
operator|&&
name|c
operator|<=
literal|'F'
condition|)
block|{
name|len
operator|+=
name|c
operator|-
literal|'A'
operator|+
literal|10
expr_stmt|;
continue|continue;
block|}
return|return
operator|-
literal|1
return|;
block|}
return|return
name|len
return|;
block|}
end_function
begin_function
DECL|function|packet_read
name|int
name|packet_read
parameter_list|(
name|int
name|fd
parameter_list|,
name|char
modifier|*
modifier|*
name|src_buf
parameter_list|,
name|size_t
modifier|*
name|src_len
parameter_list|,
name|char
modifier|*
name|buffer
parameter_list|,
name|unsigned
name|size
parameter_list|,
name|int
name|options
parameter_list|)
block|{
name|int
name|len
decl_stmt|,
name|ret
decl_stmt|;
name|char
name|linelen
index|[
literal|4
index|]
decl_stmt|;
name|ret
operator|=
name|get_packet_data
argument_list|(
name|fd
argument_list|,
name|src_buf
argument_list|,
name|src_len
argument_list|,
name|linelen
argument_list|,
literal|4
argument_list|,
name|options
argument_list|)
expr_stmt|;
if|if
condition|(
name|ret
operator|<
literal|0
condition|)
return|return
name|ret
return|;
name|len
operator|=
name|packet_length
argument_list|(
name|linelen
argument_list|)
expr_stmt|;
if|if
condition|(
name|len
operator|<
literal|0
condition|)
name|die
argument_list|(
literal|"protocol error: bad line length character: %.4s"
argument_list|,
name|linelen
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|len
condition|)
block|{
name|packet_trace
argument_list|(
literal|"0000"
argument_list|,
literal|4
argument_list|,
literal|0
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
name|len
operator|-=
literal|4
expr_stmt|;
if|if
condition|(
name|len
operator|>=
name|size
condition|)
name|die
argument_list|(
literal|"protocol error: bad line length %d"
argument_list|,
name|len
argument_list|)
expr_stmt|;
name|ret
operator|=
name|get_packet_data
argument_list|(
name|fd
argument_list|,
name|src_buf
argument_list|,
name|src_len
argument_list|,
name|buffer
argument_list|,
name|len
argument_list|,
name|options
argument_list|)
expr_stmt|;
if|if
condition|(
name|ret
operator|<
literal|0
condition|)
return|return
name|ret
return|;
if|if
condition|(
operator|(
name|options
operator|&
name|PACKET_READ_CHOMP_NEWLINE
operator|)
operator|&&
name|len
operator|&&
name|buffer
index|[
name|len
operator|-
literal|1
index|]
operator|==
literal|'\n'
condition|)
name|len
operator|--
expr_stmt|;
name|buffer
index|[
name|len
index|]
operator|=
literal|0
expr_stmt|;
name|packet_trace
argument_list|(
name|buffer
argument_list|,
name|len
argument_list|,
literal|0
argument_list|)
expr_stmt|;
return|return
name|len
return|;
block|}
end_function
begin_function
DECL|function|packet_read_line_generic
specifier|static
name|char
modifier|*
name|packet_read_line_generic
parameter_list|(
name|int
name|fd
parameter_list|,
name|char
modifier|*
modifier|*
name|src
parameter_list|,
name|size_t
modifier|*
name|src_len
parameter_list|,
name|int
modifier|*
name|dst_len
parameter_list|)
block|{
name|int
name|len
init|=
name|packet_read
argument_list|(
name|fd
argument_list|,
name|src
argument_list|,
name|src_len
argument_list|,
name|packet_buffer
argument_list|,
sizeof|sizeof
argument_list|(
name|packet_buffer
argument_list|)
argument_list|,
name|PACKET_READ_CHOMP_NEWLINE
argument_list|)
decl_stmt|;
if|if
condition|(
name|dst_len
condition|)
operator|*
name|dst_len
operator|=
name|len
expr_stmt|;
return|return
name|len
condition|?
name|packet_buffer
else|:
name|NULL
return|;
block|}
end_function
begin_function
DECL|function|packet_read_line
name|char
modifier|*
name|packet_read_line
parameter_list|(
name|int
name|fd
parameter_list|,
name|int
modifier|*
name|len_p
parameter_list|)
block|{
return|return
name|packet_read_line_generic
argument_list|(
name|fd
argument_list|,
name|NULL
argument_list|,
name|NULL
argument_list|,
name|len_p
argument_list|)
return|;
block|}
end_function
begin_function
DECL|function|packet_read_line_buf
name|char
modifier|*
name|packet_read_line_buf
parameter_list|(
name|char
modifier|*
modifier|*
name|src
parameter_list|,
name|size_t
modifier|*
name|src_len
parameter_list|,
name|int
modifier|*
name|dst_len
parameter_list|)
block|{
return|return
name|packet_read_line_generic
argument_list|(
operator|-
literal|1
argument_list|,
name|src
argument_list|,
name|src_len
argument_list|,
name|dst_len
argument_list|)
return|;
block|}
end_function
end_unit
