...
name|porigin
return|;
comment|/* Otherwise we would look for a rename */
name|diff_setup
argument_list|(
operator|&
name|diff_opts
...
argument_list|)
expr_stmt|;
comment|/* 	 * NEEDSWORK: This loop is wasteful in that it opens the same 	 * blob in the parent number of times.  We should swap the 	 * loop inside out, which would require keeping track of 	 * "best blame so far" for blame entries that the current 	 * "target" is being suspected. 	 */
for|for
control|(
name|e
operator|=
