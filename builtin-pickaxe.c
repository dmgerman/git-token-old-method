...
index|[]
init|=
literal|"git-pickaxe [-c] [-l] [-t] [-f] [-n] [-p] [-L n,m] [-S<revs-file>] [-M] [commit] [--] file\n"
...
literal|"  -M                  Find line movements within the file\n"
...
name|stderr
argument_list|,
literal|"plno = %d, tlno = %d, "
literal|"same as parent up to %d, resync %d and %d\n"
argument_list|,
name|plno
argument_list|,
name|tlno
...
index|]
decl_stmt|;
comment|/* tlno to chunk->same are the same as ent */
if|if
condition|(
name|ent
operator|->
...
name|porigin
expr_stmt|;
comment|/* now everything blamed for origin is blamed for 			 * porigin, we do not need to keep it anymore. 			 * Do not free porigin (or the ones we got from 			 * earlier round); they may still be used elsewhere. 			 */
name|free_origin
argument_list|(
name|origin
argument_list|)
