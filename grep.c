...
name|var
argument_list|,
literal|"color.grep.matchcontext"
argument_list|)
condition|)
name|color
operator|=
...
name|var
argument_list|,
literal|"color.grep.matchselected"
argument_list|)
condition|)
name|color
operator|=
...
argument_list|)
return|;
comment|/* 	 * The textconv interface is intimately tied to diff_filespecs, so we 	 * have to pretend to be one. If we could unify the grep_source 	 * and diff_filespec structs, this mess could just go away. 	 */
name|df
operator|=
name|alloc_filespec
argument_list|(
...
name|die
argument_list|(
literal|"BUG: attempt to textconv something without a path?"
argument_list|)
expr_stmt|;
block|}
comment|/* 	 * fill_textconv is not remotely thread-safe; it may load objects 	 * behind the scenes, and it modifies the global diff tempfile 	 * structure. 	 */
name|grep_read_lock
argument_list|()
expr_stmt|;
name|size
...
argument_list|)
expr_stmt|;
comment|/* 	 * The normal fill_textconv usage by the diff machinery would just keep 	 * the textconv'd buf separate from the diff_filespec. But much of the 	 * grep code passes around a grep_source and assumes that its "buf" 	 * pointer is the beginning of the thing we are searching. So let's 	 * install our textconv'd version into the grep_source, taking care not 	 * to leak any existing buffer. 	 */
...
argument_list|)
expr_stmt|;
comment|/* 		 * We might set up the shared textconv cache data here, which 		 * is not thread-safe. 		 */
name|grep_attr_lock
argument_list|()
expr_stmt|;
name|textconv
...
expr_stmt|;
block|}
comment|/* 	 * We know the result of a textconv is text, so we only have to care 	 * about binary handling if we are not using it. 	 */
if|if
condition|(
operator|!
name|textconv
...
name|eol
decl_stmt|;
comment|/* 			 * Trailing empty lines are not interesting. 			 * Peek past them to see if they belong to the 			 * body of the current function. 			 */
name|peek_bol
operator|=
name|bol
expr_stmt|;
