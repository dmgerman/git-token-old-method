...
end_function
begin_comment
comment|/*  * Pick the next char from the stream, ignoring codepoints an HFS+ would.  * Note that this is _not_ complete by any means. It's just enough  * to make is_hfs_dotgit() work, and should not be used otherwise.  */
end_comment
begin_function
...
argument_list|)
expr_stmt|;
comment|/* 	 * there's a great deal of other case-folding that occurs 	 * in HFS+, but this is enough to catch anything that will 	 * convert to ".git" 	 */
if|if
condition|(
name|c
operator|!=
