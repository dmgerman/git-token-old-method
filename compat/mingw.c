...
name|hide_dotfiles
operator|==
name|HIDE_DOTFILES_FALSE
condition|)
return|return
literal|0
return|;
comment|/* We cannot use basename(), as it would remove trailing slashes */
...
end_function
begin_function
DECL|function|set_hidden_flag
specifier|static
name|int
name|set_hidden_flag
parameter_list|(
specifier|const
name|wchar_t
modifier|*
...
condition|)
block|{
comment|/* 		 * Internally, _wopen() uses the CreateFile() API which errors 		 * out with an ERROR_ACCESS_DENIED if CREATE_ALWAYS was 		 * specified and an already existing file's attributes do not 		 * match *exactly*. As there is no mode or flag we can set that 		 * would correspond to FILE_ATTRIBUTE_HIDDEN, let's just try 		 * again *without* the O_CREAT flag (that corresponds to the 		 * CREATE_ALWAYS flag of CreateFile()). 		 */
if|if
condition|(
name|fd
operator|<
...
name|warning
argument_list|(
literal|"could not mark '%s' as hidden."
argument_list|,
name|filename
argument_list|)
expr_stmt|;
...
end_function
begin_comment
comment|/**  * Verifies that safe_create_leading_directories() would succeed.  */
end_comment
begin_function
DECL|function|has_valid_directory_prefix
specifier|static
name|int
name|has_valid_directory_prefix
parameter_list|(
name|wchar_t
modifier|*
name|wfilename
...
name|ERROR_FILE_NOT_FOUND
case|:
comment|/* This implies parent directory exists. */
return|return
literal|1
return|;
block|}
...
end_function
begin_function
DECL|function|mingw_skip_dos_drive_prefix
name|int
name|mingw_skip_dos_drive_prefix
parameter_list|(
name|char
...
end_function
begin_function
DECL|function|setup_windows_environment
specifier|static
name|void
...
condition|)
block|{
comment|/* 		 * Convert all dir separators to forward slashes, 		 * to help shell commands called from the Git 		 * executable (by not mistaking the dir separators 		 * for escape characters). 		 */
for|for
control|(
init|;
operator|*
...
argument_list|)
expr_stmt|;
comment|/* assuming NT variants only.. */
name|xsnprintf
argument_list|(
name|buf
operator|->
