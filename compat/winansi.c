...
name|warning
argument_list|(
literal|"Your console font probably doesn\'t support Unicode. If "
literal|"you experience strange characters in the output, consider "
literal|"switching to a TrueType font such as Lucida Console!"
argument_list|)
expr_stmt|;
block|}
end_function
...
literal|0
decl_stmt|;
comment|/* use cached value if stream hasn't changed */
if|if
condition|(
name|stream
operator|==
...
name|NULL
expr_stmt|;
comment|/* get OS handle of the stream */
name|hcon
operator|=
operator|(
name|HANDLE
...
parameter_list|)
block|{
comment|/* convert utf-8 to utf-16, write directly to console */
name|int
name|wlen
init|=
...
argument_list|)
expr_stmt|;
comment|/* 	 * if non-ascii characters are printed, check that the current console 	 * font supports this 	 */
if|if
condition|(
name|wlen
operator|!=
...
argument_list|()
expr_stmt|;
comment|/* return original (utf-8 encoded) length */
return|return
name|len
return|;
block|}
