...
end_ifdef
begin_comment
comment|/* For ZOS USS we must define btowc */
end_comment
begin_function
name|wchar_t
DECL|function|btowc
...
directive|endif
comment|/* strcasecmp isn't a standard interface. brute force check */
if|#
directive|if
literal|0
block|if (strcasecmp (codeset_name, "UTF-8") == 0       || strcasecmp (codeset_name, "UTF8") == 0)     dfa->is_utf8 = 1;
else|#
directive|else
if|if
condition|(
...
directive|ifdef
name|GAWK
comment|/*      * Fedora Core 2, maybe others, have broken `btowc' that returns -1      * for any value> 127. Sigh. Note that `start_ch' and `end_ch' are      * unsigned, so we don't have sign extension problems.      */
name|start_wc
operator|=
operator|(
operator|(
