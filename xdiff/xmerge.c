...
name|mode
decl_stmt|;
comment|/* 	 * These point at the respective postimages.  E.g.<i1,chg1> is 	 * how side #1 wants to change the common ancestor; if there is no 	 * overlap, lines before i1 in the postimage of side #1 appear 	 * in the merge result as a region touched by neither side. 	 */
DECL|member|i1
DECL|member|i2
name|long
name|i1
...
name|chg2
decl_stmt|;
comment|/* 	 * These point at the preimage; of course there is just one 	 * preimage, that is from the shared common ancestor. 	 */
DECL|member|i0
name|long
name|i0
decl_stmt|;
...
end_function
begin_comment
comment|/*  * Returns 1 if the i'th line ends in CR/LF (if it is the last line and  * has no eol, the preceding line, if any), 0 if it ends in LF-only, and  * -1 if the line ending cannot be determined.  */
end_comment
begin_function
DECL|function|is_eol_crlf
specifier|static
name|int
name|is_eol_crlf
parameter_list|(
...
literal|1
condition|)
comment|/* All lines before the last *must* end in LF */
return|return
operator|(
name|size
operator|=
...
name|nrec
condition|)
comment|/* Cannot determine eol style from empty file */
return|return
operator|-
literal|1
return|;
...
literal|'\n'
condition|)
comment|/* Last line; ends in LF; Is it CR/LF? */
return|return
name|size
operator|>
literal|1
...
name|i
condition|)
comment|/* The only line has no eol */
return|return
operator|-
literal|1
return|;
comment|/* Determine eol from second-to-last line */
return|return
operator|(
name|size
operator|=
...
name|needs_cr
decl_stmt|;
comment|/* Match post-images' preceding, or first, lines' end-of-line style */
name|needs_cr
operator|=
name|is_eol_crlf
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Look at pre-image's first line, unless we already settled on LF */
if|if
condition|(
name|needs_cr
condition|)
...
argument_list|)
expr_stmt|;
comment|/* If still undecided, use LF-only */
return|return
name|needs_cr
operator|<
literal|0
