...
end_function
begin_comment
comment|/*  * Return true iff a reference named refname could be created without  * conflicting with the name of an existing reference in dir.  If  * oldrefname is non-NULL, ignore potential conflicts with oldrefname  * (e.g., because oldrefname is scheduled for deletion in the same  * operation).  *  * Two reference names conflict if one of them exactly matches the  * leading components of the other; e.g., "foo/bar" conflicts with  * both "foo" and with "foo/bar/baz" but not with "foo/bar" or  * "foo/barbados".  */
end_comment
begin_function
...
control|)
block|{
comment|/* 		 * We are still at a leading dir of the refname; we are 		 * looking for a conflict with a leaf entry. 		 * 		 * If we find one, we still must make sure it is 		 * not "oldrefname". 		 */
name|pos
operator|=
name|search_ref_dir
argument_list|(
...
condition|)
block|{
comment|/* 		 * We found a directory named "refname". It is a 		 * problem iff it contains any ref that is not 		 * "oldrefname". 		 */
name|struct
name|ref_entry
modifier|*
name|entry
