...
end_function
begin_comment
comment|/*  * Call fn for each reference in dir that has index in the range  * offset<= index< dir->nr.  Recurse into subdirectories that are in  * that index range, sorting them before iterating.  This function  * does not sort dir itself; it should be sorted beforehand.  */
end_comment
begin_function
DECL|function|do_for_each_ref_in_dir
specifier|static
name|int
name|do_for_each_ref_in_dir
parameter_list|(
name|struct
name|ref_dir
modifier|*
...
end_function
begin_comment
comment|/*  * Call fn for each reference in the union of dir1 and dir2, in order  * by refname.  Recurse into subdirectories.  If a value entry appears  * in both dir1 and dir2, then only process the version that is in  * dir2.  The input dirs must already be sorted, but subdirs will be  * sorted as needed.  */
end_comment
begin_function
DECL|function|do_for_each_ref_in_dirs
specifier|static
name|int
name|do_for_each_ref_in_dirs
parameter_list|(
name|struct
name|ref_dir
modifier|*
...
end_function
begin_comment
comment|/*  * Return true iff a reference named refname could be created without  * conflicting with the name of an existing reference in array.  If  * oldrefname is non-NULL, ignore potential conflicts with oldrefname  * (e.g., because oldrefname is scheduled for deletion in the same  * operation).  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*  * Try to read ref from the packed references.  On success, set sha1  * and return 0; otherwise, return -1.  */
end_comment
begin_function
