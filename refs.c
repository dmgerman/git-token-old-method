...
end_include
begin_comment
comment|/*  * How to handle various characters in refnames:  * This table is used by both the SIMD and non-SIMD code.  It has  * some cases that are only useful for the SIMD; these are handled  * equivalently to the listed disposition in the non-SIMD code.  * 0: An acceptable character for refs  * 1: @, look for a following { to reject @{ in refs (SIMD or = 0)  * 2: \0: End-of-component and string  * 3: /: End-of-component (SIMD or = 2)  * 4: ., look for a preceding . to reject .. in refs  * 5: {, look for a preceding @ to reject @{ in refs  * 6: *, usually a bad character except, once as a wildcard (SIMD or = 7)  * 7: A bad character except * (see check_refname_component below)  */
end_comment
begin_decl_stmt
...
specifier|static
name|int
name|check_refname_format_bytewise
parameter_list|(
specifier|const
name|char
modifier|*
...
define|#
directive|define
name|SSE_VECTOR_BYTES
value|16
end_define
begin_comment
comment|/* Vectorized version of check_refname_format. */
end_comment
begin_function
...
argument_list|)
decl_stmt|;
comment|/* below '*', all characters are forbidden or rare */
specifier|const
name|__m128i
name|star_ub
init|=
...
argument_list|)
decl_stmt|;
comment|/* '['..'^' contains 4 characters: 3 forbidden and 1 rare */
specifier|const
name|__m128i
name|bracket_lb
init|=
...
argument_list|)
decl_stmt|;
comment|/* '~' and above are forbidden */
specifier|const
name|__m128i
name|tilde_lb
init|=
...
condition|)
block|{
comment|/* entirely empty ref or initial ref component */
return|return
operator|-
literal|1
return|;
block|}
comment|/* 	 * Initial ref component of '.'; below we look for /. so we'll 	 * miss this. 	 */
if|if
condition|(
name|refname
index|[
...
literal|1
condition|)
comment|/* 			 * End-of-page; fall back to slow method for 			 * this entire ref. 			 */
return|return
name|check_refname_format_bytewise
argument_list|(
name|refname
...
argument_list|)
expr_stmt|;
comment|/* 		 * This range (note the lt) contains some 		 * permissible-but-rare characters (including all 		 * characters>= 128), which we handle later.  It also 		 * includes \000. 		 */
name|result
operator|=
name|_mm_cmplt_epi8
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* This range contains the permissible ] as bycatch */
name|result
operator|=
name|_mm_or_si128
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* 		 * Even though /. is not necessarily an error, we flag 		 * it anyway. If we find it, we'll check if it's valid 		 * and if so we'll advance just past it. 		 */
name|result
operator|=
name|_mm_or_si128
argument_list|(
...
condition|)
block|{
comment|/* 			 * We've found either end-of-string, or some 			 * probably-bad character or substring. 			 */
name|int
name|i
init|=
name|__builtin_ctz
...
literal|5
case|:
comment|/* 				 * bycatch: a good character that's in 				 * one of the ranges of mostly-forbidden 				 * characters 				 */
name|cp
operator|+=
name|i
operator|+
...
operator|++
expr_stmt|;
comment|/* 				 * Even if leading dots are allowed, don't 				 * allow "." as a component (".." is 				 * prevented by case 4 below). 				 */
if|if
condition|(
name|cp
index|[
...
condition|)
block|{
comment|/* skip to just after the /. */
name|cp
operator|+=
name|i
operator|+
...
literal|1
return|;
comment|/* .lock as end-of-component or end-of-string */
if|if
condition|(
operator|(
operator|!
...
end_function
begin_comment
comment|/* This function needs to return a meaningful errno on failure */
end_comment
begin_function
...
argument_list|)
expr_stmt|;
comment|/* Copy, sort, and reject duplicate refs */
name|qsort
argument_list|(
name|updates
argument_list|,
...
name|NULL
expr_stmt|;
comment|/* freed by update_ref_write */
if|if
condition|(
name|ret
condition|)
