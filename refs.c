...
end_function
begin_comment
comment|/* Add a ref_entry to the end of the ref_dir (unsorted). */
end_comment
begin_function
...
condition|)
block|{
comment|/* Duplicate name; make sure that the SHA1s match: */
if|if
condition|(
name|hashcmp
argument_list|(
...
end_function
begin_comment
comment|/*  * Sort the entries in dir (if they are not already sorted).  */
end_comment
begin_function
...
argument_list|)
expr_stmt|;
comment|/* Remove any duplicates from the ref_dir */
name|i
operator|=
literal|0
expr_stmt|;
...
end_function
begin_comment
comment|/*  * Call fn for each reference in dir that has index in the range  * offset<= index< dir->nr.  This function does not sort the dir;  * sorting should be done by the caller.  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*  * Call fn for each reference in the union of dir1 and dir2, in order  * by refname.  If an entry appears in both dir1 and dir2, then only  * process the version that is in dir2.  The input dirs must already  * be sorted.  */
end_comment
begin_function
...
condition|)
block|{
comment|/* 				 * There was a ref in array1 with the 				 * same name; ignore it. 				 */
name|i1
operator|++
expr_stmt|;
block|}
