...
end_function
begin_comment
comment|/*  * This is basically strbuf_read(), except that if we  * hit max_request_buffer we die (we'd rather reject a  * maliciously large request than chew up infinite memory).  */
end_comment
begin_function
...
return|;
block|}
comment|/* partial read from read_in_full means we hit EOF */
name|len
operator|+=
name|cnt
expr_stmt|;
...
return|;
block|}
comment|/* otherwise, grow and try again (if we can) */
if|if
condition|(
name|alloc
operator|==
...
name|die
argument_list|(
literal|"request was larger than our maximum size (%lu);"
literal|" try setting GIT_HTTP_MAX_REQUEST_BUFFER"
argument_list|,
name|max_request_buffer
argument_list|)
expr_stmt|;
...
name|die_errno
argument_list|(
literal|"error reading request body"
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|git_env_ulong
argument_list|(
literal|"GIT_HTTP_MAX_REQUEST_BUFFER"
argument_list|,
name|max_request_buffer
argument_list|)
expr_stmt|;
