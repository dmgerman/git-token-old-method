...
specifier|static
name|int
name|remove_duplicates_in_refs
parameter_list|(
name|struct
name|ref
modifier|*
...
condition|)
block|{
comment|/* 		 * The temporary shallow file is only useful for 		 * index-pack and unpack-objects because it may 		 * contain more roots than we want. Delete it. 		 */
if|if
condition|(
operator|*
name|alternate_shallow_file
...
condition|)
block|{
comment|/* 		 * remote is shallow, but this is a clone, there are 		 * no objects in repo to worry about. Accept any 		 * shallow points that exist in the pack (iow in repo 		 * after get_pack() and reprepare_packed_git()) 		 */
name|struct
name|sha1_array
name|extra
init|=
...
condition|)
block|{
comment|/* 		 * remote is also shallow, .git/shallow may be updated 		 * so all refs can be accepted. Make sure we only add 		 * shallow roots that are actually reachable from new 		 * refs. 		 */
name|struct
name|sha1_array
name|extra
init|=
...
return|return;
block|}
comment|/* 	 * remote is also shallow, check what ref is safe to update 	 * without updating .git/shallow 	 */
name|status
operator|=
name|xcalloc
argument_list|(
