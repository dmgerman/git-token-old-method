...
name|die_errno
argument_list|(
literal|"unable to fdopen alternates lockfile"
argument_list|)
expr_stmt|;
name|in
operator|=
...
name|die_errno
argument_list|(
literal|"unable to read alternates file"
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|die_errno
argument_list|(
literal|"unable to move new alternates file into place"
argument_list|)
expr_stmt|;
if|if
condition|(
...
specifier|static
name|int
name|check_and_freshen_nonlocal
parameter_list|(
specifier|const
name|unsigned
name|char
...
name|NULL
return|;
comment|/* 	 * ".pack" is long enough to hold any suffix we're adding (and 	 * the use xsnprintf double-checks that) 	 */
name|alloc
operator|=
name|st_add3
argument_list|(
...
specifier|static
name|int
name|unpack_sha1_header_to_strbuf
parameter_list|(
name|git_zstream
modifier|*
name|stream
...
argument_list|)
expr_stmt|;
comment|/* 	 * Check if entire header is unpacked in the first iteration. 	 */
if|if
condition|(
name|memchr
argument_list|(
...
literal|0
return|;
comment|/* 	 * buffer[0..bufsiz] was not large enough.  Copy the partial 	 * result out to header, and then append the result of further 	 * reading the stream. 	 */
name|strbuf_add
argument_list|(
name|header
argument_list|,
...
literal|0
decl_stmt|;
comment|/* 	 * The type can be of any size but is followed by 	 * a space. 	 */
for|for
control|(
init|;
condition|;
...
end_function
begin_comment
comment|/*  * Like get_delta_base above, but we return the sha1 instead of the pack  * offset. This means it is cheaper for REF deltas (we do not have to do  * the final object lookup), but more expensive for OFS deltas (we  * have to load the revidx to convert the offset back into a sha1).  */
end_comment
begin_function
...
end_function
begin_function
DECL|function|check_pack_index_ptr
name|void
...
argument_list|)
expr_stmt|;
comment|/* No need to check for underflow; .idx files must be at least 8 bytes */
if|if
condition|(
name|ptr
operator|>=
...
name|error
argument_list|(
literal|"unable to unpack %s header with --allow-unknown-type"
argument_list|,
name|sha1_to_hex
argument_list|(
name|sha1
...
name|error
argument_list|(
literal|"unable to parse %s header with --allow-unknown-type"
argument_list|,
name|sha1_to_hex
argument_list|(
name|sha1
...
return|;
block|}
comment|/* 	 * packed_object_info() does not follow the delta chain to 	 * find out the real type, unless it is given oi->typep. 	 */
if|if
condition|(
name|oi
operator|->
...
condition|)
block|{
comment|/* 		 * Make sure the directory exists; note that the contents 		 * of the buffer are undefined after mkstemp returns an 		 * error, so we have to rewrite the whole buffer from 		 * scratch. 		 */
name|strbuf_reset
argument_list|(
name|tmp
argument_list|)
...
specifier|static
name|int
name|index_stream_convert_blob
parameter_list|(
name|unsigned
name|char
modifier|*
...
specifier|static
name|int
name|for_each_file_in_obj_subdir
parameter_list|(
name|int
name|subdir_nr
parameter_list|,
...
name|r
decl_stmt|;
comment|/* copy base not including trailing '/' */
name|strbuf_add
argument_list|(
operator|&
name|buf
...
name|error
argument_list|(
literal|"unable to get sha1 of object %u in %s"
argument_list|,
name|i
argument_list|,
name|p
