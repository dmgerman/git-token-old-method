...
end_if
begin_decl_stmt
DECL|variable|git_curl_ipresolve
name|long
name|int
...
end_decl_stmt
begin_decl_stmt
DECL|variable|http_proxy_authmethod
specifier|static
specifier|const
name|char
modifier|*
...
name|name
decl_stmt|;
DECL|member|curlauth_param
name|long
name|curlauth_param
decl_stmt|;
block|}
DECL|variable|proxy_authmethods
name|proxy_authmethods
index|[]
init|=
block|{
block|{
...
directive|endif
comment|/* 	 * CURLAUTH_DIGEST_IE has no corresponding command-line option in 	 * curl(1) and is not included in CURLAUTH_ANY, so we leave it out 	 * here, too 	 */
block|}
struct|;
end_struct
begin_decl_stmt
DECL|variable|proxy_auth
specifier|static
name|struct
name|credential
name|proxy_auth
...
end_decl_stmt
begin_decl_stmt
DECL|variable|curl_proxyuserpwd
specifier|static
specifier|const
name|char
modifier|*
name|curl_proxyuserpwd
decl_stmt|;
end_decl_stmt
begin_decl_stmt
...
end_decl_stmt
begin_decl_stmt
DECL|variable|curl_empty_auth
specifier|static
name|int
name|curl_empty_auth
decl_stmt|;
end_decl_stmt
begin_if
if|#
...
end_decl_stmt
begin_decl_stmt
DECL|variable|extra_http_headers
specifier|static
name|struct
name|curl_slist
modifier|*
name|extra_http_headers
decl_stmt|;
end_decl_stmt
begin_decl_stmt
...
name|_
argument_list|(
literal|"Public key pinning not supported with cURL< 7.44.0"
argument_list|)
argument_list|)
expr_stmt|;
return|return
...
end_function
begin_comment
comment|/* *var must be free-able */
end_comment
begin_function
DECL|function|var_override
specifier|static
name|void
name|var_override
parameter_list|(
...
end_function
begin_function
DECL|function|set_proxyauth_name_password
specifier|static
name|void
name|set_proxyauth_name_password
parameter_list|(
name|CURL
modifier|*
name|result
...
end_function
begin_function
DECL|function|init_curl_proxy_auth
specifier|static
name|void
name|init_curl_proxy_auth
parameter_list|(
name|CURL
modifier|*
name|result
...
name|getenv
argument_list|(
literal|"GIT_HTTP_PROXY_AUTHMETHOD"
argument_list|)
argument_list|)
expr_stmt|;
if|#
...
operator|>=
literal|0x070a07
comment|/* CURLOPT_PROXYAUTH and CURLAUTH_ANY */
if|if
condition|(
name|http_proxy_authmethod
condition|)
...
name|warning
argument_list|(
literal|"unsupported proxy authentication method %s: using anyauth"
argument_list|,
name|http_proxy_authmethod
argument_list|)
expr_stmt|;
...
literal|0
return|;
comment|/* CURL_SOCKOPT_OK only exists since curl 7.21.5 */
block|}
end_function
begin_function
DECL|function|set_curl_keepalive
...
parameter_list|)
block|{
comment|/* not supported on older curl versions */
block|}
end_function
begin_endif
endif|#
...
end_endif
begin_function
DECL|function|redact_sensitive_header
specifier|static
name|void
name|redact_sensitive_header
parameter_list|(
name|struct
name|strbuf
modifier|*
...
condition|)
block|{
comment|/* The first token is the type, which is OK to log */
while|while
condition|(
name|isspace
argument_list|(
...
operator|++
expr_stmt|;
comment|/* Everything else is opaque and possibly sensitive */
name|strbuf_setlen
argument_list|(
name|header
argument_list|,
...
end_function
begin_function
DECL|function|curl_dump_header
specifier|static
name|void
name|curl_dump_header
parameter_list|(
specifier|const
name|char
modifier|*
...
name|out
argument_list|,
literal|"%s, %10.10ld bytes (0x%8.8lx)\n"
argument_list|,
name|text
argument_list|,
operator|(
...
expr_stmt|;
default|default:
comment|/* we ignore unknown types by default */
return|return
literal|0
return|;
case|case
...
end_function
begin_function
DECL|function|setup_curl_trace
name|void
...
name|warning
argument_list|(
literal|"unsupported ssl version %s: using default"
argument_list|,
name|ssl_version
argument_list|)
expr_stmt|;
...
name|warning
argument_list|(
literal|"protocol restrictions not applied to curl redirects because\n"
literal|"your curl version is too old (>= 7.19.4)"
argument_list|)
expr_stmt|;
endif|#
directive|endif
...
directive|endif
comment|/* 	 * CURL also examines these variables as a fallback; but we need to query 	 * them here in order to decide whether to prompt for missing password (cf. 	 * init_curl_proxy_auth()). 	 * 	 * Unlike many other common environment variables, these are historically 	 * lowercase only. It appears that CURL did not know this and implemented 	 * only uppercase variants, which was later corrected to take both - with 	 * the exception of http_proxy, which is lowercase only also in CURL. As 	 * the lowercase versions are the historical quasi-standard, they take 	 * precedence here, as in CURL. 	 */
if|if
condition|(
operator|!
name|curl_http_proxy
...
end_function
begin_function
DECL|function|http_copy_default_headers
name|struct
name|curl_slist
modifier|*
name|http_copy_default_headers
...
end_function
begin_comment
comment|/*  * Check for and extract a content-type parameter. "raw"  * should be positioned at the start of the potential  * parameter, with any whitespace already removed.  *  * "name" is the name of the parameter. The value is appended  * to "out".  */
end_comment
begin_function
...
end_function
begin_comment
comment|/*  * Extract a normalized version of the content type, with any  * spaces suppressed, all letters lowercased, and no trailing ";"  * or parameters.  *  * Note that we will silently remove even invalid whitespace. For  * example, "text / plain" is specifically forbidden by RFC 2616,  * but "text/plain" is the only reasonable output, and this keeps  * our code simple.  *  * If the "charset" argument is not NULL, store the value of any  * charset parameter there.  *  * Example:  *   "TEXT/PLAIN; charset=utf-8" -> "text/plain", "utf-8"  *   "text / plain" -> "text/plain"  */
end_comment
begin_function
...
parameter_list|)
block|{
comment|/* 	 * MAX_DECIMAL_PLACES must not be larger than 3. If it is larger than 	 * that, q-value will be smaller than 0.001, the minimum q-value the 	 * HTTP specification allows. See 	 * http://tools.ietf.org/html/rfc7231#section-5.3.1 for q-value. 	 */
specifier|const
name|int
...
specifier|const
name|int
name|MAX_LANGUAGE_TAGS
init|=
literal|1000
decl_stmt|;
specifier|const
name|int
name|MAX_ACCEPT_LANGUAGE_HEADER_SIZE
init|=
literal|4000
decl_stmt|;
name|char
...
name|STRBUF_INIT
decl_stmt|;
comment|/* Don't add Accept-Language header if no language is preferred. */
if|if
condition|(
operator|!
name|s
condition|)
return|return;
comment|/* 	 * Split the colon-separated string of preferred languages into 	 * language_tags array. 	 */
do|do
block|{
comment|/* collect language tag */
for|for
control|(
init|;
operator|*
...
argument_list|)
expr_stmt|;
comment|/* skip .codeset, @modifier and any other unnecessary parts */
while|while
condition|(
operator|*
name|s
...
condition|)
do|;
comment|/* write Accept-Language header into buf */
if|if
condition|(
name|num_langs
condition|)
...
literal|"*"
expr_stmt|;
comment|/* it's OK; this won't be freed */
comment|/* compute decimal_places */
for|for
control|(
name|max_q
operator|=
...
block|}
block|}
comment|/* free language tags -- last one is a static '*' */
for|for
control|(
name|i
operator|=
...
specifier|static
name|void
name|http_opt_request_remainder
parameter_list|(
name|CURL
modifier|*
name|curl
...
name|die
argument_list|(
literal|"BUG: update_url_from_redirect: %s is not a superset of %s"
argument_list|,
name|asked
argument_list|,
name|base
...
literal|0
return|;
comment|/* insane redirect scheme */
name|strbuf_reset
argument_list|(
name|base
argument_list|)
...
name|die
argument_list|(
literal|"BUG: pack tmpfile does not end in .pack.temp?"
argument_list|)
expr_stmt|;
name|tmp_idx
operator|=
