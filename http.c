...
name|char
modifier|*
name|git_getpass_with_description
parameter_list|(
specifier|const
name|char
modifier|*
...
argument_list|)
expr_stmt|;
comment|/* 	 * NEEDSWORK: for usernames, we should do something less magical that 	 * actually echoes the characters. However, we need to read from 	 * /dev/tty and not stdio, which is not portable (but getpass will do 	 * it for us). http.c uses the same workaround. 	 */
name|r
operator|=
name|git_getpass
argument_list|(
...
literal|0
return|;
comment|/* Only prompt the user once. */
name|ssl_cert_password_required
operator|=
operator|-
literal|1
...
condition|)
return|return;
comment|/* 	 * Ok, the URL looks like "proto://something".  Which one? 	 * "proto://<user>:<pass>@<host>/...", 	 * "proto://<user>@<host>/...", or just 	 * "proto://<host>/..."? 	 */
name|cp
operator|+=
literal|3
expr_stmt|;
...
condition|)
block|{
comment|/* No credentials, but we may have to ask for some later */
name|host
operator|=
name|cp
expr_stmt|;
