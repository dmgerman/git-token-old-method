...
argument_list|)
expr_stmt|;
comment|/* 	 * We can only do diffstat with a unique reference point, and 	 * log is a bit tricky, so just skip it. 	 */
if|if
condition|(
operator|!
name|origin
...
expr_stmt|;
block|}
comment|/* Multiple origins don't work for diffstat. */
if|if
condition|(
name|negative_count
operator|>
