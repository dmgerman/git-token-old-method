...
end_decl_stmt
begin_decl_stmt
DECL|variable|option_shallow_submodules
specifier|static
name|int
...
name|N_
argument_list|(
literal|"number of submodules cloned in parallel"
argument_list|)
argument_list|)
block|,
name|OPT_STRING
...
name|N_
argument_list|(
literal|"any cloned submodules will be shallow"
argument_list|)
argument_list|)
block|,
name|OPT_STRING
...
literal|3
expr_stmt|;
comment|/* 	 * Skip authentication data. The stripping does happen 	 * greedily, such that we strip up to the last '@' inside 	 * the host part. 	 */
for|for
control|(
name|ptr
operator|=
...
expr_stmt|;
block|}
comment|/* 	 * Strip trailing port number if we've got only a 	 * hostname (that is, there is no dir separator but a 	 * colon). This check is required such that we do not 	 * strip URI's like '/foo/bar:2222.git', which should 	 * result in a dir '2222' being guessed due to backwards 	 * compatibility. 	 */
if|if
condition|(
name|memchr
argument_list|(
...
expr_stmt|;
block|}
comment|/* 	 * Find last component. To remain backwards compatible we 	 * also regard colons as path separators, such that 	 * cloning a repository 'foo:bar.git' would result in a 	 * directory 'bar' being guessed. 	 */
name|ptr
operator|=
name|end
expr_stmt|;
...
name|_
argument_list|(
literal|"reference repository '%s' as a linked checkout is not supported yet."
argument_list|)
argument_list|,
name|item
operator|->
...
name|_
argument_list|(
literal|"reference repository '%s' is shallow"
argument_list|)
argument_list|,
name|item
operator|->
...
name|_
argument_list|(
literal|"reference repository '%s' is grafted"
argument_list|)
argument_list|,
name|item
operator|->
...
name|die
argument_list|(
literal|"BUG: remote HEAD points at non-head?"
argument_list|)
expr_stmt|;
name|strbuf_addf
argument_list|(
...
name|_
argument_list|(
literal|"cannot repack to clean up"
argument_list|)
argument_list|)
expr_stmt|;
if|if
...
name|_
argument_list|(
literal|"cannot unlink temporary alternates file"
argument_list|)
argument_list|)
expr_stmt|;
block|}
...
name|_
argument_list|(
literal|"source repository is shallow, ignoring --local"
argument_list|)
argument_list|)
expr_stmt|;
name|is_local
