...
name|NULL
expr_stmt|;
comment|/* 	 * mapped_refs may be updated if transport-helper is used so 	 * we need fetch it early because remote_head code below 	 * relies on it. 	 * 	 * for normal clones, transport_get_remote_refs() should 	 * return reliable ref set, we can delay cloning until after 	 * remote HEAD check. 	 */
if|if
condition|(
operator|!
name|is_local
