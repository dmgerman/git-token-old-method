begin_unit
begin_include
include|#
directive|include
file|"builtin.h"
end_include
begin_include
include|#
directive|include
file|"cache.h"
end_include
begin_include
include|#
directive|include
file|"attr.h"
end_include
begin_include
include|#
directive|include
file|"object.h"
end_include
begin_include
include|#
directive|include
file|"blob.h"
end_include
begin_include
include|#
directive|include
file|"commit.h"
end_include
begin_include
include|#
directive|include
file|"tag.h"
end_include
begin_include
include|#
directive|include
file|"tree.h"
end_include
begin_include
include|#
directive|include
file|"delta.h"
end_include
begin_include
include|#
directive|include
file|"pack.h"
end_include
begin_include
include|#
directive|include
file|"pack-revindex.h"
end_include
begin_include
include|#
directive|include
file|"csum-file.h"
end_include
begin_include
include|#
directive|include
file|"tree-walk.h"
end_include
begin_include
include|#
directive|include
file|"diff.h"
end_include
begin_include
include|#
directive|include
file|"revision.h"
end_include
begin_include
include|#
directive|include
file|"list-objects.h"
end_include
begin_include
include|#
directive|include
file|"pack-objects.h"
end_include
begin_include
include|#
directive|include
file|"progress.h"
end_include
begin_include
include|#
directive|include
file|"refs.h"
end_include
begin_include
include|#
directive|include
file|"streaming.h"
end_include
begin_include
include|#
directive|include
file|"thread-utils.h"
end_include
begin_include
include|#
directive|include
file|"pack-bitmap.h"
end_include
begin_include
include|#
directive|include
file|"reachable.h"
end_include
begin_include
include|#
directive|include
file|"sha1-array.h"
end_include
begin_include
include|#
directive|include
file|"argv-array.h"
end_include
begin_decl_stmt
DECL|variable|pack_usage
specifier|static
specifier|const
name|char
modifier|*
name|pack_usage
index|[]
init|=
block|{
name|N_
argument_list|(
literal|"git pack-objects --stdout [<options>...] [<<ref-list> |<<object-list>]"
argument_list|)
block|,
name|N_
argument_list|(
literal|"git pack-objects [<options>...]<base-name> [<<ref-list> |<<object-list>]"
argument_list|)
block|,
name|NULL
block|}
decl_stmt|;
end_decl_stmt
begin_comment
comment|/*  * Objects we are going to pack are collected in the `to_pack` structure.  * It contains an array (dynamically expanded) of the object data, and a map  * that can resolve SHA1s to their position in the array.  */
end_comment
begin_decl_stmt
DECL|variable|to_pack
specifier|static
name|struct
name|packing_data
name|to_pack
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|written_list
specifier|static
name|struct
name|pack_idx_entry
modifier|*
modifier|*
name|written_list
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|nr_result
DECL|variable|nr_written
specifier|static
name|uint32_t
name|nr_result
decl_stmt|,
name|nr_written
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|non_empty
specifier|static
name|int
name|non_empty
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|reuse_delta
DECL|variable|reuse_object
specifier|static
name|int
name|reuse_delta
init|=
literal|1
decl_stmt|,
name|reuse_object
init|=
literal|1
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|keep_unreachable
DECL|variable|unpack_unreachable
DECL|variable|include_tag
specifier|static
name|int
name|keep_unreachable
decl_stmt|,
name|unpack_unreachable
decl_stmt|,
name|include_tag
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|unpack_unreachable_expiration
specifier|static
name|unsigned
name|long
name|unpack_unreachable_expiration
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|pack_loose_unreachable
specifier|static
name|int
name|pack_loose_unreachable
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|local
specifier|static
name|int
name|local
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|incremental
specifier|static
name|int
name|incremental
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|ignore_packed_keep
specifier|static
name|int
name|ignore_packed_keep
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|allow_ofs_delta
specifier|static
name|int
name|allow_ofs_delta
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|pack_idx_opts
specifier|static
name|struct
name|pack_idx_option
name|pack_idx_opts
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|base_name
specifier|static
specifier|const
name|char
modifier|*
name|base_name
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|progress
specifier|static
name|int
name|progress
init|=
literal|1
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|window
specifier|static
name|int
name|window
init|=
literal|10
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|pack_size_limit
specifier|static
name|unsigned
name|long
name|pack_size_limit
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|depth
specifier|static
name|int
name|depth
init|=
literal|50
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|delta_search_threads
specifier|static
name|int
name|delta_search_threads
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|pack_to_stdout
specifier|static
name|int
name|pack_to_stdout
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|num_preferred_base
specifier|static
name|int
name|num_preferred_base
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|progress_state
specifier|static
name|struct
name|progress
modifier|*
name|progress_state
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|pack_compression_level
specifier|static
name|int
name|pack_compression_level
init|=
name|Z_DEFAULT_COMPRESSION
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|pack_compression_seen
specifier|static
name|int
name|pack_compression_seen
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|reuse_packfile
specifier|static
name|struct
name|packed_git
modifier|*
name|reuse_packfile
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|reuse_packfile_objects
specifier|static
name|uint32_t
name|reuse_packfile_objects
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|reuse_packfile_offset
specifier|static
name|off_t
name|reuse_packfile_offset
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|use_bitmap_index
specifier|static
name|int
name|use_bitmap_index
init|=
literal|1
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|write_bitmap_index
specifier|static
name|int
name|write_bitmap_index
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|write_bitmap_options
specifier|static
name|uint16_t
name|write_bitmap_options
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|delta_cache_size
specifier|static
name|unsigned
name|long
name|delta_cache_size
init|=
literal|0
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|max_delta_cache_size
specifier|static
name|unsigned
name|long
name|max_delta_cache_size
init|=
literal|256
operator|*
literal|1024
operator|*
literal|1024
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|cache_max_small_delta_size
specifier|static
name|unsigned
name|long
name|cache_max_small_delta_size
init|=
literal|1000
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|window_memory_limit
specifier|static
name|unsigned
name|long
name|window_memory_limit
init|=
literal|0
decl_stmt|;
end_decl_stmt
begin_comment
comment|/*  * stats  */
end_comment
begin_decl_stmt
DECL|variable|written
DECL|variable|written_delta
specifier|static
name|uint32_t
name|written
decl_stmt|,
name|written_delta
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|reused
DECL|variable|reused_delta
specifier|static
name|uint32_t
name|reused
decl_stmt|,
name|reused_delta
decl_stmt|;
end_decl_stmt
begin_comment
comment|/*  * Indexed commits  */
end_comment
begin_decl_stmt
DECL|variable|indexed_commits
specifier|static
name|struct
name|commit
modifier|*
modifier|*
name|indexed_commits
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|indexed_commits_nr
specifier|static
name|unsigned
name|int
name|indexed_commits_nr
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|indexed_commits_alloc
specifier|static
name|unsigned
name|int
name|indexed_commits_alloc
decl_stmt|;
end_decl_stmt
begin_function
DECL|function|index_commit_for_bitmap
specifier|static
name|void
name|index_commit_for_bitmap
parameter_list|(
name|struct
name|commit
modifier|*
name|commit
parameter_list|)
block|{
if|if
condition|(
name|indexed_commits_nr
operator|>=
name|indexed_commits_alloc
condition|)
block|{
name|indexed_commits_alloc
operator|=
operator|(
name|indexed_commits_alloc
operator|+
literal|32
operator|)
operator|*
literal|2
expr_stmt|;
name|REALLOC_ARRAY
argument_list|(
name|indexed_commits
argument_list|,
name|indexed_commits_alloc
argument_list|)
expr_stmt|;
block|}
name|indexed_commits
index|[
name|indexed_commits_nr
operator|++
index|]
operator|=
name|commit
expr_stmt|;
block|}
end_function
begin_function
DECL|function|get_delta
specifier|static
name|void
modifier|*
name|get_delta
parameter_list|(
name|struct
name|object_entry
modifier|*
name|entry
parameter_list|)
block|{
name|unsigned
name|long
name|size
decl_stmt|,
name|base_size
decl_stmt|,
name|delta_size
decl_stmt|;
name|void
modifier|*
name|buf
decl_stmt|,
modifier|*
name|base_buf
decl_stmt|,
modifier|*
name|delta_buf
decl_stmt|;
name|enum
name|object_type
name|type
decl_stmt|;
name|buf
operator|=
name|read_sha1_file
argument_list|(
name|entry
operator|->
name|idx
operator|.
name|sha1
argument_list|,
operator|&
name|type
argument_list|,
operator|&
name|size
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|buf
condition|)
name|die
argument_list|(
literal|"unable to read %s"
argument_list|,
name|sha1_to_hex
argument_list|(
name|entry
operator|->
name|idx
operator|.
name|sha1
argument_list|)
argument_list|)
expr_stmt|;
name|base_buf
operator|=
name|read_sha1_file
argument_list|(
name|entry
operator|->
name|delta
operator|->
name|idx
operator|.
name|sha1
argument_list|,
operator|&
name|type
argument_list|,
operator|&
name|base_size
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|base_buf
condition|)
name|die
argument_list|(
literal|"unable to read %s"
argument_list|,
name|sha1_to_hex
argument_list|(
name|entry
operator|->
name|delta
operator|->
name|idx
operator|.
name|sha1
argument_list|)
argument_list|)
expr_stmt|;
name|delta_buf
operator|=
name|diff_delta
argument_list|(
name|base_buf
argument_list|,
name|base_size
argument_list|,
name|buf
argument_list|,
name|size
argument_list|,
operator|&
name|delta_size
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|delta_buf
operator|||
name|delta_size
operator|!=
name|entry
operator|->
name|delta_size
condition|)
name|die
argument_list|(
literal|"delta size changed"
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|buf
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|base_buf
argument_list|)
expr_stmt|;
return|return
name|delta_buf
return|;
block|}
end_function
begin_function
DECL|function|do_compress
specifier|static
name|unsigned
name|long
name|do_compress
parameter_list|(
name|void
modifier|*
modifier|*
name|pptr
parameter_list|,
name|unsigned
name|long
name|size
parameter_list|)
block|{
name|git_zstream
name|stream
decl_stmt|;
name|void
modifier|*
name|in
decl_stmt|,
modifier|*
name|out
decl_stmt|;
name|unsigned
name|long
name|maxsize
decl_stmt|;
name|git_deflate_init
argument_list|(
operator|&
name|stream
argument_list|,
name|pack_compression_level
argument_list|)
expr_stmt|;
name|maxsize
operator|=
name|git_deflate_bound
argument_list|(
operator|&
name|stream
argument_list|,
name|size
argument_list|)
expr_stmt|;
name|in
operator|=
operator|*
name|pptr
expr_stmt|;
name|out
operator|=
name|xmalloc
argument_list|(
name|maxsize
argument_list|)
expr_stmt|;
operator|*
name|pptr
operator|=
name|out
expr_stmt|;
name|stream
operator|.
name|next_in
operator|=
name|in
expr_stmt|;
name|stream
operator|.
name|avail_in
operator|=
name|size
expr_stmt|;
name|stream
operator|.
name|next_out
operator|=
name|out
expr_stmt|;
name|stream
operator|.
name|avail_out
operator|=
name|maxsize
expr_stmt|;
while|while
condition|(
name|git_deflate
argument_list|(
operator|&
name|stream
argument_list|,
name|Z_FINISH
argument_list|)
operator|==
name|Z_OK
condition|)
empty_stmt|;
comment|/* nothing */
name|git_deflate_end
argument_list|(
operator|&
name|stream
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|in
argument_list|)
expr_stmt|;
return|return
name|stream
operator|.
name|total_out
return|;
block|}
end_function
begin_function
DECL|function|write_large_blob_data
specifier|static
name|unsigned
name|long
name|write_large_blob_data
parameter_list|(
name|struct
name|git_istream
modifier|*
name|st
parameter_list|,
name|struct
name|sha1file
modifier|*
name|f
parameter_list|,
specifier|const
name|unsigned
name|char
modifier|*
name|sha1
parameter_list|)
block|{
name|git_zstream
name|stream
decl_stmt|;
name|unsigned
name|char
name|ibuf
index|[
literal|1024
operator|*
literal|16
index|]
decl_stmt|;
name|unsigned
name|char
name|obuf
index|[
literal|1024
operator|*
literal|16
index|]
decl_stmt|;
name|unsigned
name|long
name|olen
init|=
literal|0
decl_stmt|;
name|git_deflate_init
argument_list|(
operator|&
name|stream
argument_list|,
name|pack_compression_level
argument_list|)
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
name|ssize_t
name|readlen
decl_stmt|;
name|int
name|zret
init|=
name|Z_OK
decl_stmt|;
name|readlen
operator|=
name|read_istream
argument_list|(
name|st
argument_list|,
name|ibuf
argument_list|,
sizeof|sizeof
argument_list|(
name|ibuf
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|readlen
operator|==
operator|-
literal|1
condition|)
name|die
argument_list|(
name|_
argument_list|(
literal|"unable to read %s"
argument_list|)
argument_list|,
name|sha1_to_hex
argument_list|(
name|sha1
argument_list|)
argument_list|)
expr_stmt|;
name|stream
operator|.
name|next_in
operator|=
name|ibuf
expr_stmt|;
name|stream
operator|.
name|avail_in
operator|=
name|readlen
expr_stmt|;
while|while
condition|(
operator|(
name|stream
operator|.
name|avail_in
operator|||
name|readlen
operator|==
literal|0
operator|)
operator|&&
operator|(
name|zret
operator|==
name|Z_OK
operator|||
name|zret
operator|==
name|Z_BUF_ERROR
operator|)
condition|)
block|{
name|stream
operator|.
name|next_out
operator|=
name|obuf
expr_stmt|;
name|stream
operator|.
name|avail_out
operator|=
sizeof|sizeof
argument_list|(
name|obuf
argument_list|)
expr_stmt|;
name|zret
operator|=
name|git_deflate
argument_list|(
operator|&
name|stream
argument_list|,
name|readlen
condition|?
literal|0
else|:
name|Z_FINISH
argument_list|)
expr_stmt|;
name|sha1write
argument_list|(
name|f
argument_list|,
name|obuf
argument_list|,
name|stream
operator|.
name|next_out
operator|-
name|obuf
argument_list|)
expr_stmt|;
name|olen
operator|+=
name|stream
operator|.
name|next_out
operator|-
name|obuf
expr_stmt|;
block|}
if|if
condition|(
name|stream
operator|.
name|avail_in
condition|)
name|die
argument_list|(
name|_
argument_list|(
literal|"deflate error (%d)"
argument_list|)
argument_list|,
name|zret
argument_list|)
expr_stmt|;
if|if
condition|(
name|readlen
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|zret
operator|!=
name|Z_STREAM_END
condition|)
name|die
argument_list|(
name|_
argument_list|(
literal|"deflate error (%d)"
argument_list|)
argument_list|,
name|zret
argument_list|)
expr_stmt|;
break|break;
block|}
block|}
name|git_deflate_end
argument_list|(
operator|&
name|stream
argument_list|)
expr_stmt|;
return|return
name|olen
return|;
block|}
end_function
begin_comment
comment|/*  * we are going to reuse the existing object data as is.  make  * sure it is not corrupt.  */
end_comment
begin_function
DECL|function|check_pack_inflate
specifier|static
name|int
name|check_pack_inflate
parameter_list|(
name|struct
name|packed_git
modifier|*
name|p
parameter_list|,
name|struct
name|pack_window
modifier|*
modifier|*
name|w_curs
parameter_list|,
name|off_t
name|offset
parameter_list|,
name|off_t
name|len
parameter_list|,
name|unsigned
name|long
name|expect
parameter_list|)
block|{
name|git_zstream
name|stream
decl_stmt|;
name|unsigned
name|char
name|fakebuf
index|[
literal|4096
index|]
decl_stmt|,
modifier|*
name|in
decl_stmt|;
name|int
name|st
decl_stmt|;
name|memset
argument_list|(
operator|&
name|stream
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
name|stream
argument_list|)
argument_list|)
expr_stmt|;
name|git_inflate_init
argument_list|(
operator|&
name|stream
argument_list|)
expr_stmt|;
do|do
block|{
name|in
operator|=
name|use_pack
argument_list|(
name|p
argument_list|,
name|w_curs
argument_list|,
name|offset
argument_list|,
operator|&
name|stream
operator|.
name|avail_in
argument_list|)
expr_stmt|;
name|stream
operator|.
name|next_in
operator|=
name|in
expr_stmt|;
name|stream
operator|.
name|next_out
operator|=
name|fakebuf
expr_stmt|;
name|stream
operator|.
name|avail_out
operator|=
sizeof|sizeof
argument_list|(
name|fakebuf
argument_list|)
expr_stmt|;
name|st
operator|=
name|git_inflate
argument_list|(
operator|&
name|stream
argument_list|,
name|Z_FINISH
argument_list|)
expr_stmt|;
name|offset
operator|+=
name|stream
operator|.
name|next_in
operator|-
name|in
expr_stmt|;
block|}
do|while
condition|(
name|st
operator|==
name|Z_OK
operator|||
name|st
operator|==
name|Z_BUF_ERROR
condition|)
do|;
name|git_inflate_end
argument_list|(
operator|&
name|stream
argument_list|)
expr_stmt|;
return|return
operator|(
name|st
operator|==
name|Z_STREAM_END
operator|&&
name|stream
operator|.
name|total_out
operator|==
name|expect
operator|&&
name|stream
operator|.
name|total_in
operator|==
name|len
operator|)
condition|?
literal|0
else|:
operator|-
literal|1
return|;
block|}
end_function
begin_function
DECL|function|copy_pack_data
specifier|static
name|void
name|copy_pack_data
parameter_list|(
name|struct
name|sha1file
modifier|*
name|f
parameter_list|,
name|struct
name|packed_git
modifier|*
name|p
parameter_list|,
name|struct
name|pack_window
modifier|*
modifier|*
name|w_curs
parameter_list|,
name|off_t
name|offset
parameter_list|,
name|off_t
name|len
parameter_list|)
block|{
name|unsigned
name|char
modifier|*
name|in
decl_stmt|;
name|unsigned
name|long
name|avail
decl_stmt|;
while|while
condition|(
name|len
condition|)
block|{
name|in
operator|=
name|use_pack
argument_list|(
name|p
argument_list|,
name|w_curs
argument_list|,
name|offset
argument_list|,
operator|&
name|avail
argument_list|)
expr_stmt|;
if|if
condition|(
name|avail
operator|>
name|len
condition|)
name|avail
operator|=
operator|(
name|unsigned
name|long
operator|)
name|len
expr_stmt|;
name|sha1write
argument_list|(
name|f
argument_list|,
name|in
argument_list|,
name|avail
argument_list|)
expr_stmt|;
name|offset
operator|+=
name|avail
expr_stmt|;
name|len
operator|-=
name|avail
expr_stmt|;
block|}
block|}
end_function
begin_comment
comment|/* Return 0 if we will bust the pack-size limit */
end_comment
begin_function
DECL|function|write_no_reuse_object
specifier|static
name|unsigned
name|long
name|write_no_reuse_object
parameter_list|(
name|struct
name|sha1file
modifier|*
name|f
parameter_list|,
name|struct
name|object_entry
modifier|*
name|entry
parameter_list|,
name|unsigned
name|long
name|limit
parameter_list|,
name|int
name|usable_delta
parameter_list|)
block|{
name|unsigned
name|long
name|size
decl_stmt|,
name|datalen
decl_stmt|;
name|unsigned
name|char
name|header
index|[
literal|10
index|]
decl_stmt|,
name|dheader
index|[
literal|10
index|]
decl_stmt|;
name|unsigned
name|hdrlen
decl_stmt|;
name|enum
name|object_type
name|type
decl_stmt|;
name|void
modifier|*
name|buf
decl_stmt|;
name|struct
name|git_istream
modifier|*
name|st
init|=
name|NULL
decl_stmt|;
if|if
condition|(
operator|!
name|usable_delta
condition|)
block|{
if|if
condition|(
name|entry
operator|->
name|type
operator|==
name|OBJ_BLOB
operator|&&
name|entry
operator|->
name|size
operator|>
name|big_file_threshold
operator|&&
operator|(
name|st
operator|=
name|open_istream
argument_list|(
name|entry
operator|->
name|idx
operator|.
name|sha1
argument_list|,
operator|&
name|type
argument_list|,
operator|&
name|size
argument_list|,
name|NULL
argument_list|)
operator|)
operator|!=
name|NULL
condition|)
name|buf
operator|=
name|NULL
expr_stmt|;
else|else
block|{
name|buf
operator|=
name|read_sha1_file
argument_list|(
name|entry
operator|->
name|idx
operator|.
name|sha1
argument_list|,
operator|&
name|type
argument_list|,
operator|&
name|size
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|buf
condition|)
name|die
argument_list|(
name|_
argument_list|(
literal|"unable to read %s"
argument_list|)
argument_list|,
name|sha1_to_hex
argument_list|(
name|entry
operator|->
name|idx
operator|.
name|sha1
argument_list|)
argument_list|)
expr_stmt|;
block|}
comment|/* 		 * make sure no cached delta data remains from a 		 * previous attempt before a pack split occurred. 		 */
name|free
argument_list|(
name|entry
operator|->
name|delta_data
argument_list|)
expr_stmt|;
name|entry
operator|->
name|delta_data
operator|=
name|NULL
expr_stmt|;
name|entry
operator|->
name|z_delta_size
operator|=
literal|0
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|entry
operator|->
name|delta_data
condition|)
block|{
name|size
operator|=
name|entry
operator|->
name|delta_size
expr_stmt|;
name|buf
operator|=
name|entry
operator|->
name|delta_data
expr_stmt|;
name|entry
operator|->
name|delta_data
operator|=
name|NULL
expr_stmt|;
name|type
operator|=
operator|(
name|allow_ofs_delta
operator|&&
name|entry
operator|->
name|delta
operator|->
name|idx
operator|.
name|offset
operator|)
condition|?
name|OBJ_OFS_DELTA
else|:
name|OBJ_REF_DELTA
expr_stmt|;
block|}
else|else
block|{
name|buf
operator|=
name|get_delta
argument_list|(
name|entry
argument_list|)
expr_stmt|;
name|size
operator|=
name|entry
operator|->
name|delta_size
expr_stmt|;
name|type
operator|=
operator|(
name|allow_ofs_delta
operator|&&
name|entry
operator|->
name|delta
operator|->
name|idx
operator|.
name|offset
operator|)
condition|?
name|OBJ_OFS_DELTA
else|:
name|OBJ_REF_DELTA
expr_stmt|;
block|}
if|if
condition|(
name|st
condition|)
comment|/* large blob case, just assume we don't compress well */
name|datalen
operator|=
name|size
expr_stmt|;
elseif|else
if|if
condition|(
name|entry
operator|->
name|z_delta_size
condition|)
name|datalen
operator|=
name|entry
operator|->
name|z_delta_size
expr_stmt|;
else|else
name|datalen
operator|=
name|do_compress
argument_list|(
operator|&
name|buf
argument_list|,
name|size
argument_list|)
expr_stmt|;
comment|/* 	 * The object header is a byte of 'type' followed by zero or 	 * more bytes of length. 	 */
name|hdrlen
operator|=
name|encode_in_pack_object_header
argument_list|(
name|type
argument_list|,
name|size
argument_list|,
name|header
argument_list|)
expr_stmt|;
if|if
condition|(
name|type
operator|==
name|OBJ_OFS_DELTA
condition|)
block|{
comment|/* 		 * Deltas with relative base contain an additional 		 * encoding of the relative offset for the delta 		 * base from this object's position in the pack. 		 */
name|off_t
name|ofs
init|=
name|entry
operator|->
name|idx
operator|.
name|offset
operator|-
name|entry
operator|->
name|delta
operator|->
name|idx
operator|.
name|offset
decl_stmt|;
name|unsigned
name|pos
init|=
sizeof|sizeof
argument_list|(
name|dheader
argument_list|)
operator|-
literal|1
decl_stmt|;
name|dheader
index|[
name|pos
index|]
operator|=
name|ofs
operator|&
literal|127
expr_stmt|;
while|while
condition|(
name|ofs
operator|>>=
literal|7
condition|)
name|dheader
index|[
operator|--
name|pos
index|]
operator|=
literal|128
operator||
operator|(
operator|--
name|ofs
operator|&
literal|127
operator|)
expr_stmt|;
if|if
condition|(
name|limit
operator|&&
name|hdrlen
operator|+
sizeof|sizeof
argument_list|(
name|dheader
argument_list|)
operator|-
name|pos
operator|+
name|datalen
operator|+
literal|20
operator|>=
name|limit
condition|)
block|{
if|if
condition|(
name|st
condition|)
name|close_istream
argument_list|(
name|st
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|buf
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
name|sha1write
argument_list|(
name|f
argument_list|,
name|header
argument_list|,
name|hdrlen
argument_list|)
expr_stmt|;
name|sha1write
argument_list|(
name|f
argument_list|,
name|dheader
operator|+
name|pos
argument_list|,
sizeof|sizeof
argument_list|(
name|dheader
argument_list|)
operator|-
name|pos
argument_list|)
expr_stmt|;
name|hdrlen
operator|+=
sizeof|sizeof
argument_list|(
name|dheader
argument_list|)
operator|-
name|pos
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|type
operator|==
name|OBJ_REF_DELTA
condition|)
block|{
comment|/* 		 * Deltas with a base reference contain 		 * an additional 20 bytes for the base sha1. 		 */
if|if
condition|(
name|limit
operator|&&
name|hdrlen
operator|+
literal|20
operator|+
name|datalen
operator|+
literal|20
operator|>=
name|limit
condition|)
block|{
if|if
condition|(
name|st
condition|)
name|close_istream
argument_list|(
name|st
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|buf
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
name|sha1write
argument_list|(
name|f
argument_list|,
name|header
argument_list|,
name|hdrlen
argument_list|)
expr_stmt|;
name|sha1write
argument_list|(
name|f
argument_list|,
name|entry
operator|->
name|delta
operator|->
name|idx
operator|.
name|sha1
argument_list|,
literal|20
argument_list|)
expr_stmt|;
name|hdrlen
operator|+=
literal|20
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|limit
operator|&&
name|hdrlen
operator|+
name|datalen
operator|+
literal|20
operator|>=
name|limit
condition|)
block|{
if|if
condition|(
name|st
condition|)
name|close_istream
argument_list|(
name|st
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|buf
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
name|sha1write
argument_list|(
name|f
argument_list|,
name|header
argument_list|,
name|hdrlen
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|st
condition|)
block|{
name|datalen
operator|=
name|write_large_blob_data
argument_list|(
name|st
argument_list|,
name|f
argument_list|,
name|entry
operator|->
name|idx
operator|.
name|sha1
argument_list|)
expr_stmt|;
name|close_istream
argument_list|(
name|st
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|sha1write
argument_list|(
name|f
argument_list|,
name|buf
argument_list|,
name|datalen
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|buf
argument_list|)
expr_stmt|;
block|}
return|return
name|hdrlen
operator|+
name|datalen
return|;
block|}
end_function
begin_comment
comment|/* Return 0 if we will bust the pack-size limit */
end_comment
begin_function
DECL|function|write_reuse_object
specifier|static
name|unsigned
name|long
name|write_reuse_object
parameter_list|(
name|struct
name|sha1file
modifier|*
name|f
parameter_list|,
name|struct
name|object_entry
modifier|*
name|entry
parameter_list|,
name|unsigned
name|long
name|limit
parameter_list|,
name|int
name|usable_delta
parameter_list|)
block|{
name|struct
name|packed_git
modifier|*
name|p
init|=
name|entry
operator|->
name|in_pack
decl_stmt|;
name|struct
name|pack_window
modifier|*
name|w_curs
init|=
name|NULL
decl_stmt|;
name|struct
name|revindex_entry
modifier|*
name|revidx
decl_stmt|;
name|off_t
name|offset
decl_stmt|;
name|enum
name|object_type
name|type
init|=
name|entry
operator|->
name|type
decl_stmt|;
name|unsigned
name|long
name|datalen
decl_stmt|;
name|unsigned
name|char
name|header
index|[
literal|10
index|]
decl_stmt|,
name|dheader
index|[
literal|10
index|]
decl_stmt|;
name|unsigned
name|hdrlen
decl_stmt|;
if|if
condition|(
name|entry
operator|->
name|delta
condition|)
name|type
operator|=
operator|(
name|allow_ofs_delta
operator|&&
name|entry
operator|->
name|delta
operator|->
name|idx
operator|.
name|offset
operator|)
condition|?
name|OBJ_OFS_DELTA
else|:
name|OBJ_REF_DELTA
expr_stmt|;
name|hdrlen
operator|=
name|encode_in_pack_object_header
argument_list|(
name|type
argument_list|,
name|entry
operator|->
name|size
argument_list|,
name|header
argument_list|)
expr_stmt|;
name|offset
operator|=
name|entry
operator|->
name|in_pack_offset
expr_stmt|;
name|revidx
operator|=
name|find_pack_revindex
argument_list|(
name|p
argument_list|,
name|offset
argument_list|)
expr_stmt|;
name|datalen
operator|=
name|revidx
index|[
literal|1
index|]
operator|.
name|offset
operator|-
name|offset
expr_stmt|;
if|if
condition|(
operator|!
name|pack_to_stdout
operator|&&
name|p
operator|->
name|index_version
operator|>
literal|1
operator|&&
name|check_pack_crc
argument_list|(
name|p
argument_list|,
operator|&
name|w_curs
argument_list|,
name|offset
argument_list|,
name|datalen
argument_list|,
name|revidx
operator|->
name|nr
argument_list|)
condition|)
block|{
name|error
argument_list|(
literal|"bad packed object CRC for %s"
argument_list|,
name|sha1_to_hex
argument_list|(
name|entry
operator|->
name|idx
operator|.
name|sha1
argument_list|)
argument_list|)
expr_stmt|;
name|unuse_pack
argument_list|(
operator|&
name|w_curs
argument_list|)
expr_stmt|;
return|return
name|write_no_reuse_object
argument_list|(
name|f
argument_list|,
name|entry
argument_list|,
name|limit
argument_list|,
name|usable_delta
argument_list|)
return|;
block|}
name|offset
operator|+=
name|entry
operator|->
name|in_pack_header_size
expr_stmt|;
name|datalen
operator|-=
name|entry
operator|->
name|in_pack_header_size
expr_stmt|;
if|if
condition|(
operator|!
name|pack_to_stdout
operator|&&
name|p
operator|->
name|index_version
operator|==
literal|1
operator|&&
name|check_pack_inflate
argument_list|(
name|p
argument_list|,
operator|&
name|w_curs
argument_list|,
name|offset
argument_list|,
name|datalen
argument_list|,
name|entry
operator|->
name|size
argument_list|)
condition|)
block|{
name|error
argument_list|(
literal|"corrupt packed object for %s"
argument_list|,
name|sha1_to_hex
argument_list|(
name|entry
operator|->
name|idx
operator|.
name|sha1
argument_list|)
argument_list|)
expr_stmt|;
name|unuse_pack
argument_list|(
operator|&
name|w_curs
argument_list|)
expr_stmt|;
return|return
name|write_no_reuse_object
argument_list|(
name|f
argument_list|,
name|entry
argument_list|,
name|limit
argument_list|,
name|usable_delta
argument_list|)
return|;
block|}
if|if
condition|(
name|type
operator|==
name|OBJ_OFS_DELTA
condition|)
block|{
name|off_t
name|ofs
init|=
name|entry
operator|->
name|idx
operator|.
name|offset
operator|-
name|entry
operator|->
name|delta
operator|->
name|idx
operator|.
name|offset
decl_stmt|;
name|unsigned
name|pos
init|=
sizeof|sizeof
argument_list|(
name|dheader
argument_list|)
operator|-
literal|1
decl_stmt|;
name|dheader
index|[
name|pos
index|]
operator|=
name|ofs
operator|&
literal|127
expr_stmt|;
while|while
condition|(
name|ofs
operator|>>=
literal|7
condition|)
name|dheader
index|[
operator|--
name|pos
index|]
operator|=
literal|128
operator||
operator|(
operator|--
name|ofs
operator|&
literal|127
operator|)
expr_stmt|;
if|if
condition|(
name|limit
operator|&&
name|hdrlen
operator|+
sizeof|sizeof
argument_list|(
name|dheader
argument_list|)
operator|-
name|pos
operator|+
name|datalen
operator|+
literal|20
operator|>=
name|limit
condition|)
block|{
name|unuse_pack
argument_list|(
operator|&
name|w_curs
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
name|sha1write
argument_list|(
name|f
argument_list|,
name|header
argument_list|,
name|hdrlen
argument_list|)
expr_stmt|;
name|sha1write
argument_list|(
name|f
argument_list|,
name|dheader
operator|+
name|pos
argument_list|,
sizeof|sizeof
argument_list|(
name|dheader
argument_list|)
operator|-
name|pos
argument_list|)
expr_stmt|;
name|hdrlen
operator|+=
sizeof|sizeof
argument_list|(
name|dheader
argument_list|)
operator|-
name|pos
expr_stmt|;
name|reused_delta
operator|++
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|type
operator|==
name|OBJ_REF_DELTA
condition|)
block|{
if|if
condition|(
name|limit
operator|&&
name|hdrlen
operator|+
literal|20
operator|+
name|datalen
operator|+
literal|20
operator|>=
name|limit
condition|)
block|{
name|unuse_pack
argument_list|(
operator|&
name|w_curs
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
name|sha1write
argument_list|(
name|f
argument_list|,
name|header
argument_list|,
name|hdrlen
argument_list|)
expr_stmt|;
name|sha1write
argument_list|(
name|f
argument_list|,
name|entry
operator|->
name|delta
operator|->
name|idx
operator|.
name|sha1
argument_list|,
literal|20
argument_list|)
expr_stmt|;
name|hdrlen
operator|+=
literal|20
expr_stmt|;
name|reused_delta
operator|++
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|limit
operator|&&
name|hdrlen
operator|+
name|datalen
operator|+
literal|20
operator|>=
name|limit
condition|)
block|{
name|unuse_pack
argument_list|(
operator|&
name|w_curs
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
name|sha1write
argument_list|(
name|f
argument_list|,
name|header
argument_list|,
name|hdrlen
argument_list|)
expr_stmt|;
block|}
name|copy_pack_data
argument_list|(
name|f
argument_list|,
name|p
argument_list|,
operator|&
name|w_curs
argument_list|,
name|offset
argument_list|,
name|datalen
argument_list|)
expr_stmt|;
name|unuse_pack
argument_list|(
operator|&
name|w_curs
argument_list|)
expr_stmt|;
name|reused
operator|++
expr_stmt|;
return|return
name|hdrlen
operator|+
name|datalen
return|;
block|}
end_function
begin_comment
comment|/* Return 0 if we will bust the pack-size limit */
end_comment
begin_function
DECL|function|write_object
specifier|static
name|unsigned
name|long
name|write_object
parameter_list|(
name|struct
name|sha1file
modifier|*
name|f
parameter_list|,
name|struct
name|object_entry
modifier|*
name|entry
parameter_list|,
name|off_t
name|write_offset
parameter_list|)
block|{
name|unsigned
name|long
name|limit
decl_stmt|,
name|len
decl_stmt|;
name|int
name|usable_delta
decl_stmt|,
name|to_reuse
decl_stmt|;
if|if
condition|(
operator|!
name|pack_to_stdout
condition|)
name|crc32_begin
argument_list|(
name|f
argument_list|)
expr_stmt|;
comment|/* apply size limit if limited packsize and not first object */
if|if
condition|(
operator|!
name|pack_size_limit
operator|||
operator|!
name|nr_written
condition|)
name|limit
operator|=
literal|0
expr_stmt|;
elseif|else
if|if
condition|(
name|pack_size_limit
operator|<=
name|write_offset
condition|)
comment|/* 		 * the earlier object did not fit the limit; avoid 		 * mistaking this with unlimited (i.e. limit = 0). 		 */
name|limit
operator|=
literal|1
expr_stmt|;
else|else
name|limit
operator|=
name|pack_size_limit
operator|-
name|write_offset
expr_stmt|;
if|if
condition|(
operator|!
name|entry
operator|->
name|delta
condition|)
name|usable_delta
operator|=
literal|0
expr_stmt|;
comment|/* no delta */
elseif|else
if|if
condition|(
operator|!
name|pack_size_limit
condition|)
name|usable_delta
operator|=
literal|1
expr_stmt|;
comment|/* unlimited packfile */
elseif|else
if|if
condition|(
name|entry
operator|->
name|delta
operator|->
name|idx
operator|.
name|offset
operator|==
operator|(
name|off_t
operator|)
operator|-
literal|1
condition|)
name|usable_delta
operator|=
literal|0
expr_stmt|;
comment|/* base was written to another pack */
elseif|else
if|if
condition|(
name|entry
operator|->
name|delta
operator|->
name|idx
operator|.
name|offset
condition|)
name|usable_delta
operator|=
literal|1
expr_stmt|;
comment|/* base already exists in this pack */
else|else
name|usable_delta
operator|=
literal|0
expr_stmt|;
comment|/* base could end up in another pack */
if|if
condition|(
operator|!
name|reuse_object
condition|)
name|to_reuse
operator|=
literal|0
expr_stmt|;
comment|/* explicit */
elseif|else
if|if
condition|(
operator|!
name|entry
operator|->
name|in_pack
condition|)
name|to_reuse
operator|=
literal|0
expr_stmt|;
comment|/* can't reuse what we don't have */
elseif|else
if|if
condition|(
name|entry
operator|->
name|type
operator|==
name|OBJ_REF_DELTA
operator|||
name|entry
operator|->
name|type
operator|==
name|OBJ_OFS_DELTA
condition|)
comment|/* check_object() decided it for us ... */
name|to_reuse
operator|=
name|usable_delta
expr_stmt|;
comment|/* ... but pack split may override that */
elseif|else
if|if
condition|(
name|entry
operator|->
name|type
operator|!=
name|entry
operator|->
name|in_pack_type
condition|)
name|to_reuse
operator|=
literal|0
expr_stmt|;
comment|/* pack has delta which is unusable */
elseif|else
if|if
condition|(
name|entry
operator|->
name|delta
condition|)
name|to_reuse
operator|=
literal|0
expr_stmt|;
comment|/* we want to pack afresh */
else|else
name|to_reuse
operator|=
literal|1
expr_stmt|;
comment|/* we have it in-pack undeltified, 				 * and we do not need to deltify it. 				 */
if|if
condition|(
operator|!
name|to_reuse
condition|)
name|len
operator|=
name|write_no_reuse_object
argument_list|(
name|f
argument_list|,
name|entry
argument_list|,
name|limit
argument_list|,
name|usable_delta
argument_list|)
expr_stmt|;
else|else
name|len
operator|=
name|write_reuse_object
argument_list|(
name|f
argument_list|,
name|entry
argument_list|,
name|limit
argument_list|,
name|usable_delta
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|len
condition|)
return|return
literal|0
return|;
if|if
condition|(
name|usable_delta
condition|)
name|written_delta
operator|++
expr_stmt|;
name|written
operator|++
expr_stmt|;
if|if
condition|(
operator|!
name|pack_to_stdout
condition|)
name|entry
operator|->
name|idx
operator|.
name|crc32
operator|=
name|crc32_end
argument_list|(
name|f
argument_list|)
expr_stmt|;
return|return
name|len
return|;
block|}
end_function
begin_enum
DECL|enum|write_one_status
enum|enum
name|write_one_status
block|{
DECL|enumerator|WRITE_ONE_SKIP
name|WRITE_ONE_SKIP
init|=
operator|-
literal|1
block|,
comment|/* already written */
DECL|enumerator|WRITE_ONE_BREAK
name|WRITE_ONE_BREAK
init|=
literal|0
block|,
comment|/* writing this will bust the limit; not written */
DECL|enumerator|WRITE_ONE_WRITTEN
name|WRITE_ONE_WRITTEN
init|=
literal|1
block|,
comment|/* normal */
DECL|enumerator|WRITE_ONE_RECURSIVE
name|WRITE_ONE_RECURSIVE
init|=
literal|2
comment|/* already scheduled to be written */
block|}
enum|;
end_enum
begin_function
DECL|function|write_one
specifier|static
name|enum
name|write_one_status
name|write_one
parameter_list|(
name|struct
name|sha1file
modifier|*
name|f
parameter_list|,
name|struct
name|object_entry
modifier|*
name|e
parameter_list|,
name|off_t
modifier|*
name|offset
parameter_list|)
block|{
name|unsigned
name|long
name|size
decl_stmt|;
name|int
name|recursing
decl_stmt|;
comment|/* 	 * we set offset to 1 (which is an impossible value) to mark 	 * the fact that this object is involved in "write its base 	 * first before writing a deltified object" recursion. 	 */
name|recursing
operator|=
operator|(
name|e
operator|->
name|idx
operator|.
name|offset
operator|==
literal|1
operator|)
expr_stmt|;
if|if
condition|(
name|recursing
condition|)
block|{
name|warning
argument_list|(
literal|"recursive delta detected for object %s"
argument_list|,
name|sha1_to_hex
argument_list|(
name|e
operator|->
name|idx
operator|.
name|sha1
argument_list|)
argument_list|)
expr_stmt|;
return|return
name|WRITE_ONE_RECURSIVE
return|;
block|}
elseif|else
if|if
condition|(
name|e
operator|->
name|idx
operator|.
name|offset
operator|||
name|e
operator|->
name|preferred_base
condition|)
block|{
comment|/* offset is non zero if object is written already. */
return|return
name|WRITE_ONE_SKIP
return|;
block|}
comment|/* if we are deltified, write out base object first. */
if|if
condition|(
name|e
operator|->
name|delta
condition|)
block|{
name|e
operator|->
name|idx
operator|.
name|offset
operator|=
literal|1
expr_stmt|;
comment|/* now recurse */
switch|switch
condition|(
name|write_one
argument_list|(
name|f
argument_list|,
name|e
operator|->
name|delta
argument_list|,
name|offset
argument_list|)
condition|)
block|{
case|case
name|WRITE_ONE_RECURSIVE
case|:
comment|/* we cannot depend on this one */
name|e
operator|->
name|delta
operator|=
name|NULL
expr_stmt|;
break|break;
default|default:
break|break;
case|case
name|WRITE_ONE_BREAK
case|:
name|e
operator|->
name|idx
operator|.
name|offset
operator|=
name|recursing
expr_stmt|;
return|return
name|WRITE_ONE_BREAK
return|;
block|}
block|}
name|e
operator|->
name|idx
operator|.
name|offset
operator|=
operator|*
name|offset
expr_stmt|;
name|size
operator|=
name|write_object
argument_list|(
name|f
argument_list|,
name|e
argument_list|,
operator|*
name|offset
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|size
condition|)
block|{
name|e
operator|->
name|idx
operator|.
name|offset
operator|=
name|recursing
expr_stmt|;
return|return
name|WRITE_ONE_BREAK
return|;
block|}
name|written_list
index|[
name|nr_written
operator|++
index|]
operator|=
operator|&
name|e
operator|->
name|idx
expr_stmt|;
comment|/* make sure off_t is sufficiently large not to wrap */
if|if
condition|(
name|signed_add_overflows
argument_list|(
operator|*
name|offset
argument_list|,
name|size
argument_list|)
condition|)
name|die
argument_list|(
literal|"pack too large for current definition of off_t"
argument_list|)
expr_stmt|;
operator|*
name|offset
operator|+=
name|size
expr_stmt|;
return|return
name|WRITE_ONE_WRITTEN
return|;
block|}
end_function
begin_function
DECL|function|mark_tagged
specifier|static
name|int
name|mark_tagged
parameter_list|(
specifier|const
name|char
modifier|*
name|path
parameter_list|,
specifier|const
name|struct
name|object_id
modifier|*
name|oid
parameter_list|,
name|int
name|flag
parameter_list|,
name|void
modifier|*
name|cb_data
parameter_list|)
block|{
name|unsigned
name|char
name|peeled
index|[
literal|20
index|]
decl_stmt|;
name|struct
name|object_entry
modifier|*
name|entry
init|=
name|packlist_find
argument_list|(
operator|&
name|to_pack
argument_list|,
name|oid
operator|->
name|hash
argument_list|,
name|NULL
argument_list|)
decl_stmt|;
if|if
condition|(
name|entry
condition|)
name|entry
operator|->
name|tagged
operator|=
literal|1
expr_stmt|;
if|if
condition|(
operator|!
name|peel_ref
argument_list|(
name|path
argument_list|,
name|peeled
argument_list|)
condition|)
block|{
name|entry
operator|=
name|packlist_find
argument_list|(
operator|&
name|to_pack
argument_list|,
name|peeled
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
if|if
condition|(
name|entry
condition|)
name|entry
operator|->
name|tagged
operator|=
literal|1
expr_stmt|;
block|}
return|return
literal|0
return|;
block|}
end_function
begin_function
DECL|function|add_to_write_order
specifier|static
specifier|inline
name|void
name|add_to_write_order
parameter_list|(
name|struct
name|object_entry
modifier|*
modifier|*
name|wo
parameter_list|,
name|unsigned
name|int
modifier|*
name|endp
parameter_list|,
name|struct
name|object_entry
modifier|*
name|e
parameter_list|)
block|{
if|if
condition|(
name|e
operator|->
name|filled
condition|)
return|return;
name|wo
index|[
operator|(
operator|*
name|endp
operator|)
operator|++
index|]
operator|=
name|e
expr_stmt|;
name|e
operator|->
name|filled
operator|=
literal|1
expr_stmt|;
block|}
end_function
begin_function
DECL|function|add_descendants_to_write_order
specifier|static
name|void
name|add_descendants_to_write_order
parameter_list|(
name|struct
name|object_entry
modifier|*
modifier|*
name|wo
parameter_list|,
name|unsigned
name|int
modifier|*
name|endp
parameter_list|,
name|struct
name|object_entry
modifier|*
name|e
parameter_list|)
block|{
name|int
name|add_to_order
init|=
literal|1
decl_stmt|;
while|while
condition|(
name|e
condition|)
block|{
if|if
condition|(
name|add_to_order
condition|)
block|{
name|struct
name|object_entry
modifier|*
name|s
decl_stmt|;
comment|/* add this node... */
name|add_to_write_order
argument_list|(
name|wo
argument_list|,
name|endp
argument_list|,
name|e
argument_list|)
expr_stmt|;
comment|/* all its siblings... */
for|for
control|(
name|s
operator|=
name|e
operator|->
name|delta_sibling
init|;
name|s
condition|;
name|s
operator|=
name|s
operator|->
name|delta_sibling
control|)
block|{
name|add_to_write_order
argument_list|(
name|wo
argument_list|,
name|endp
argument_list|,
name|s
argument_list|)
expr_stmt|;
block|}
block|}
comment|/* drop down a level to add left subtree nodes if possible */
if|if
condition|(
name|e
operator|->
name|delta_child
condition|)
block|{
name|add_to_order
operator|=
literal|1
expr_stmt|;
name|e
operator|=
name|e
operator|->
name|delta_child
expr_stmt|;
block|}
else|else
block|{
name|add_to_order
operator|=
literal|0
expr_stmt|;
comment|/* our sibling might have some children, it is next */
if|if
condition|(
name|e
operator|->
name|delta_sibling
condition|)
block|{
name|e
operator|=
name|e
operator|->
name|delta_sibling
expr_stmt|;
continue|continue;
block|}
comment|/* go back to our parent node */
name|e
operator|=
name|e
operator|->
name|delta
expr_stmt|;
while|while
condition|(
name|e
operator|&&
operator|!
name|e
operator|->
name|delta_sibling
condition|)
block|{
comment|/* we're on the right side of a subtree, keep 				 * going up until we can go right again */
name|e
operator|=
name|e
operator|->
name|delta
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|e
condition|)
block|{
comment|/* done- we hit our original root node */
return|return;
block|}
comment|/* pass it off to sibling at this level */
name|e
operator|=
name|e
operator|->
name|delta_sibling
expr_stmt|;
block|}
block|}
empty_stmt|;
block|}
end_function
begin_function
DECL|function|add_family_to_write_order
specifier|static
name|void
name|add_family_to_write_order
parameter_list|(
name|struct
name|object_entry
modifier|*
modifier|*
name|wo
parameter_list|,
name|unsigned
name|int
modifier|*
name|endp
parameter_list|,
name|struct
name|object_entry
modifier|*
name|e
parameter_list|)
block|{
name|struct
name|object_entry
modifier|*
name|root
decl_stmt|;
for|for
control|(
name|root
operator|=
name|e
init|;
name|root
operator|->
name|delta
condition|;
name|root
operator|=
name|root
operator|->
name|delta
control|)
empty_stmt|;
comment|/* nothing */
name|add_descendants_to_write_order
argument_list|(
name|wo
argument_list|,
name|endp
argument_list|,
name|root
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|compute_write_order
specifier|static
name|struct
name|object_entry
modifier|*
modifier|*
name|compute_write_order
parameter_list|(
name|void
parameter_list|)
block|{
name|unsigned
name|int
name|i
decl_stmt|,
name|wo_end
decl_stmt|,
name|last_untagged
decl_stmt|;
name|struct
name|object_entry
modifier|*
modifier|*
name|wo
decl_stmt|;
name|struct
name|object_entry
modifier|*
name|objects
init|=
name|to_pack
operator|.
name|objects
decl_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|to_pack
operator|.
name|nr_objects
condition|;
name|i
operator|++
control|)
block|{
name|objects
index|[
name|i
index|]
operator|.
name|tagged
operator|=
literal|0
expr_stmt|;
name|objects
index|[
name|i
index|]
operator|.
name|filled
operator|=
literal|0
expr_stmt|;
name|objects
index|[
name|i
index|]
operator|.
name|delta_child
operator|=
name|NULL
expr_stmt|;
name|objects
index|[
name|i
index|]
operator|.
name|delta_sibling
operator|=
name|NULL
expr_stmt|;
block|}
comment|/* 	 * Fully connect delta_child/delta_sibling network. 	 * Make sure delta_sibling is sorted in the original 	 * recency order. 	 */
for|for
control|(
name|i
operator|=
name|to_pack
operator|.
name|nr_objects
init|;
name|i
operator|>
literal|0
condition|;
control|)
block|{
name|struct
name|object_entry
modifier|*
name|e
init|=
operator|&
name|objects
index|[
operator|--
name|i
index|]
decl_stmt|;
if|if
condition|(
operator|!
name|e
operator|->
name|delta
condition|)
continue|continue;
comment|/* Mark me as the first child */
name|e
operator|->
name|delta_sibling
operator|=
name|e
operator|->
name|delta
operator|->
name|delta_child
expr_stmt|;
name|e
operator|->
name|delta
operator|->
name|delta_child
operator|=
name|e
expr_stmt|;
block|}
comment|/* 	 * Mark objects that are at the tip of tags. 	 */
name|for_each_tag_ref
argument_list|(
name|mark_tagged
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
comment|/* 	 * Give the objects in the original recency order until 	 * we see a tagged tip. 	 */
name|ALLOC_ARRAY
argument_list|(
name|wo
argument_list|,
name|to_pack
operator|.
name|nr_objects
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
name|wo_end
operator|=
literal|0
init|;
name|i
operator|<
name|to_pack
operator|.
name|nr_objects
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|objects
index|[
name|i
index|]
operator|.
name|tagged
condition|)
break|break;
name|add_to_write_order
argument_list|(
name|wo
argument_list|,
operator|&
name|wo_end
argument_list|,
operator|&
name|objects
index|[
name|i
index|]
argument_list|)
expr_stmt|;
block|}
name|last_untagged
operator|=
name|i
expr_stmt|;
comment|/* 	 * Then fill all the tagged tips. 	 */
for|for
control|(
init|;
name|i
operator|<
name|to_pack
operator|.
name|nr_objects
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|objects
index|[
name|i
index|]
operator|.
name|tagged
condition|)
name|add_to_write_order
argument_list|(
name|wo
argument_list|,
operator|&
name|wo_end
argument_list|,
operator|&
name|objects
index|[
name|i
index|]
argument_list|)
expr_stmt|;
block|}
comment|/* 	 * And then all remaining commits and tags. 	 */
for|for
control|(
name|i
operator|=
name|last_untagged
init|;
name|i
operator|<
name|to_pack
operator|.
name|nr_objects
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|objects
index|[
name|i
index|]
operator|.
name|type
operator|!=
name|OBJ_COMMIT
operator|&&
name|objects
index|[
name|i
index|]
operator|.
name|type
operator|!=
name|OBJ_TAG
condition|)
continue|continue;
name|add_to_write_order
argument_list|(
name|wo
argument_list|,
operator|&
name|wo_end
argument_list|,
operator|&
name|objects
index|[
name|i
index|]
argument_list|)
expr_stmt|;
block|}
comment|/* 	 * And then all the trees. 	 */
for|for
control|(
name|i
operator|=
name|last_untagged
init|;
name|i
operator|<
name|to_pack
operator|.
name|nr_objects
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
name|objects
index|[
name|i
index|]
operator|.
name|type
operator|!=
name|OBJ_TREE
condition|)
continue|continue;
name|add_to_write_order
argument_list|(
name|wo
argument_list|,
operator|&
name|wo_end
argument_list|,
operator|&
name|objects
index|[
name|i
index|]
argument_list|)
expr_stmt|;
block|}
comment|/* 	 * Finally all the rest in really tight order 	 */
for|for
control|(
name|i
operator|=
name|last_untagged
init|;
name|i
operator|<
name|to_pack
operator|.
name|nr_objects
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
operator|!
name|objects
index|[
name|i
index|]
operator|.
name|filled
condition|)
name|add_family_to_write_order
argument_list|(
name|wo
argument_list|,
operator|&
name|wo_end
argument_list|,
operator|&
name|objects
index|[
name|i
index|]
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|wo_end
operator|!=
name|to_pack
operator|.
name|nr_objects
condition|)
name|die
argument_list|(
literal|"ordered %u objects, expected %"
name|PRIu32
argument_list|,
name|wo_end
argument_list|,
name|to_pack
operator|.
name|nr_objects
argument_list|)
expr_stmt|;
return|return
name|wo
return|;
block|}
end_function
begin_function
DECL|function|write_reused_pack
specifier|static
name|off_t
name|write_reused_pack
parameter_list|(
name|struct
name|sha1file
modifier|*
name|f
parameter_list|)
block|{
name|unsigned
name|char
name|buffer
index|[
literal|8192
index|]
decl_stmt|;
name|off_t
name|to_write
decl_stmt|,
name|total
decl_stmt|;
name|int
name|fd
decl_stmt|;
if|if
condition|(
operator|!
name|is_pack_valid
argument_list|(
name|reuse_packfile
argument_list|)
condition|)
name|die
argument_list|(
literal|"packfile is invalid: %s"
argument_list|,
name|reuse_packfile
operator|->
name|pack_name
argument_list|)
expr_stmt|;
name|fd
operator|=
name|git_open_noatime
argument_list|(
name|reuse_packfile
operator|->
name|pack_name
argument_list|)
expr_stmt|;
if|if
condition|(
name|fd
operator|<
literal|0
condition|)
name|die_errno
argument_list|(
literal|"unable to open packfile for reuse: %s"
argument_list|,
name|reuse_packfile
operator|->
name|pack_name
argument_list|)
expr_stmt|;
if|if
condition|(
name|lseek
argument_list|(
name|fd
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|pack_header
argument_list|)
argument_list|,
name|SEEK_SET
argument_list|)
operator|==
operator|-
literal|1
condition|)
name|die_errno
argument_list|(
literal|"unable to seek in reused packfile"
argument_list|)
expr_stmt|;
if|if
condition|(
name|reuse_packfile_offset
operator|<
literal|0
condition|)
name|reuse_packfile_offset
operator|=
name|reuse_packfile
operator|->
name|pack_size
operator|-
literal|20
expr_stmt|;
name|total
operator|=
name|to_write
operator|=
name|reuse_packfile_offset
operator|-
sizeof|sizeof
argument_list|(
expr|struct
name|pack_header
argument_list|)
expr_stmt|;
while|while
condition|(
name|to_write
condition|)
block|{
name|int
name|read_pack
init|=
name|xread
argument_list|(
name|fd
argument_list|,
name|buffer
argument_list|,
sizeof|sizeof
argument_list|(
name|buffer
argument_list|)
argument_list|)
decl_stmt|;
if|if
condition|(
name|read_pack
operator|<=
literal|0
condition|)
name|die_errno
argument_list|(
literal|"unable to read from reused packfile"
argument_list|)
expr_stmt|;
if|if
condition|(
name|read_pack
operator|>
name|to_write
condition|)
name|read_pack
operator|=
name|to_write
expr_stmt|;
name|sha1write
argument_list|(
name|f
argument_list|,
name|buffer
argument_list|,
name|read_pack
argument_list|)
expr_stmt|;
name|to_write
operator|-=
name|read_pack
expr_stmt|;
comment|/* 		 * We don't know the actual number of objects written, 		 * only how many bytes written, how many bytes total, and 		 * how many objects total. So we can fake it by pretending all 		 * objects we are writing are the same size. This gives us a 		 * smooth progress meter, and at the end it matches the true 		 * answer. 		 */
name|written
operator|=
name|reuse_packfile_objects
operator|*
operator|(
operator|(
call|(
name|double
call|)
argument_list|(
name|total
operator|-
name|to_write
argument_list|)
operator|)
operator|/
name|total
operator|)
expr_stmt|;
name|display_progress
argument_list|(
name|progress_state
argument_list|,
name|written
argument_list|)
expr_stmt|;
block|}
name|close
argument_list|(
name|fd
argument_list|)
expr_stmt|;
name|written
operator|=
name|reuse_packfile_objects
expr_stmt|;
name|display_progress
argument_list|(
name|progress_state
argument_list|,
name|written
argument_list|)
expr_stmt|;
return|return
name|reuse_packfile_offset
operator|-
sizeof|sizeof
argument_list|(
expr|struct
name|pack_header
argument_list|)
return|;
block|}
end_function
begin_decl_stmt
DECL|variable|no_split_warning
specifier|static
specifier|const
name|char
name|no_split_warning
index|[]
init|=
name|N_
argument_list|(
literal|"disabling bitmap writing, packs are split due to pack.packSizeLimit"
argument_list|)
decl_stmt|;
end_decl_stmt
begin_function
DECL|function|write_pack_file
specifier|static
name|void
name|write_pack_file
parameter_list|(
name|void
parameter_list|)
block|{
name|uint32_t
name|i
init|=
literal|0
decl_stmt|,
name|j
decl_stmt|;
name|struct
name|sha1file
modifier|*
name|f
decl_stmt|;
name|off_t
name|offset
decl_stmt|;
name|uint32_t
name|nr_remaining
init|=
name|nr_result
decl_stmt|;
name|time_t
name|last_mtime
init|=
literal|0
decl_stmt|;
name|struct
name|object_entry
modifier|*
modifier|*
name|write_order
decl_stmt|;
if|if
condition|(
name|progress
operator|>
name|pack_to_stdout
condition|)
name|progress_state
operator|=
name|start_progress
argument_list|(
name|_
argument_list|(
literal|"Writing objects"
argument_list|)
argument_list|,
name|nr_result
argument_list|)
expr_stmt|;
name|ALLOC_ARRAY
argument_list|(
name|written_list
argument_list|,
name|to_pack
operator|.
name|nr_objects
argument_list|)
expr_stmt|;
name|write_order
operator|=
name|compute_write_order
argument_list|()
expr_stmt|;
do|do
block|{
name|unsigned
name|char
name|sha1
index|[
literal|20
index|]
decl_stmt|;
name|char
modifier|*
name|pack_tmp_name
init|=
name|NULL
decl_stmt|;
if|if
condition|(
name|pack_to_stdout
condition|)
name|f
operator|=
name|sha1fd_throughput
argument_list|(
literal|1
argument_list|,
literal|"<stdout>"
argument_list|,
name|progress_state
argument_list|)
expr_stmt|;
else|else
name|f
operator|=
name|create_tmp_packfile
argument_list|(
operator|&
name|pack_tmp_name
argument_list|)
expr_stmt|;
name|offset
operator|=
name|write_pack_header
argument_list|(
name|f
argument_list|,
name|nr_remaining
argument_list|)
expr_stmt|;
if|if
condition|(
name|reuse_packfile
condition|)
block|{
name|off_t
name|packfile_size
decl_stmt|;
name|assert
argument_list|(
name|pack_to_stdout
argument_list|)
expr_stmt|;
name|packfile_size
operator|=
name|write_reused_pack
argument_list|(
name|f
argument_list|)
expr_stmt|;
name|offset
operator|+=
name|packfile_size
expr_stmt|;
block|}
name|nr_written
operator|=
literal|0
expr_stmt|;
for|for
control|(
init|;
name|i
operator|<
name|to_pack
operator|.
name|nr_objects
condition|;
name|i
operator|++
control|)
block|{
name|struct
name|object_entry
modifier|*
name|e
init|=
name|write_order
index|[
name|i
index|]
decl_stmt|;
if|if
condition|(
name|write_one
argument_list|(
name|f
argument_list|,
name|e
argument_list|,
operator|&
name|offset
argument_list|)
operator|==
name|WRITE_ONE_BREAK
condition|)
break|break;
name|display_progress
argument_list|(
name|progress_state
argument_list|,
name|written
argument_list|)
expr_stmt|;
block|}
comment|/* 		 * Did we write the wrong # entries in the header? 		 * If so, rewrite it like in fast-import 		 */
if|if
condition|(
name|pack_to_stdout
condition|)
block|{
name|sha1close
argument_list|(
name|f
argument_list|,
name|sha1
argument_list|,
name|CSUM_CLOSE
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|nr_written
operator|==
name|nr_remaining
condition|)
block|{
name|sha1close
argument_list|(
name|f
argument_list|,
name|sha1
argument_list|,
name|CSUM_FSYNC
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|int
name|fd
init|=
name|sha1close
argument_list|(
name|f
argument_list|,
name|sha1
argument_list|,
literal|0
argument_list|)
decl_stmt|;
name|fixup_pack_header_footer
argument_list|(
name|fd
argument_list|,
name|sha1
argument_list|,
name|pack_tmp_name
argument_list|,
name|nr_written
argument_list|,
name|sha1
argument_list|,
name|offset
argument_list|)
expr_stmt|;
name|close
argument_list|(
name|fd
argument_list|)
expr_stmt|;
if|if
condition|(
name|write_bitmap_index
condition|)
block|{
name|warning
argument_list|(
name|_
argument_list|(
name|no_split_warning
argument_list|)
argument_list|)
expr_stmt|;
name|write_bitmap_index
operator|=
literal|0
expr_stmt|;
block|}
block|}
if|if
condition|(
operator|!
name|pack_to_stdout
condition|)
block|{
name|struct
name|stat
name|st
decl_stmt|;
name|struct
name|strbuf
name|tmpname
init|=
name|STRBUF_INIT
decl_stmt|;
comment|/* 			 * Packs are runtime accessed in their mtime 			 * order since newer packs are more likely to contain 			 * younger objects.  So if we are creating multiple 			 * packs then we should modify the mtime of later ones 			 * to preserve this property. 			 */
if|if
condition|(
name|stat
argument_list|(
name|pack_tmp_name
argument_list|,
operator|&
name|st
argument_list|)
operator|<
literal|0
condition|)
block|{
name|warning_errno
argument_list|(
literal|"failed to stat %s"
argument_list|,
name|pack_tmp_name
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
operator|!
name|last_mtime
condition|)
block|{
name|last_mtime
operator|=
name|st
operator|.
name|st_mtime
expr_stmt|;
block|}
else|else
block|{
name|struct
name|utimbuf
name|utb
decl_stmt|;
name|utb
operator|.
name|actime
operator|=
name|st
operator|.
name|st_atime
expr_stmt|;
name|utb
operator|.
name|modtime
operator|=
operator|--
name|last_mtime
expr_stmt|;
if|if
condition|(
name|utime
argument_list|(
name|pack_tmp_name
argument_list|,
operator|&
name|utb
argument_list|)
operator|<
literal|0
condition|)
name|warning_errno
argument_list|(
literal|"failed utime() on %s"
argument_list|,
name|pack_tmp_name
argument_list|)
expr_stmt|;
block|}
name|strbuf_addf
argument_list|(
operator|&
name|tmpname
argument_list|,
literal|"%s-"
argument_list|,
name|base_name
argument_list|)
expr_stmt|;
if|if
condition|(
name|write_bitmap_index
condition|)
block|{
name|bitmap_writer_set_checksum
argument_list|(
name|sha1
argument_list|)
expr_stmt|;
name|bitmap_writer_build_type_index
argument_list|(
name|written_list
argument_list|,
name|nr_written
argument_list|)
expr_stmt|;
block|}
name|finish_tmp_packfile
argument_list|(
operator|&
name|tmpname
argument_list|,
name|pack_tmp_name
argument_list|,
name|written_list
argument_list|,
name|nr_written
argument_list|,
operator|&
name|pack_idx_opts
argument_list|,
name|sha1
argument_list|)
expr_stmt|;
if|if
condition|(
name|write_bitmap_index
condition|)
block|{
name|strbuf_addf
argument_list|(
operator|&
name|tmpname
argument_list|,
literal|"%s.bitmap"
argument_list|,
name|sha1_to_hex
argument_list|(
name|sha1
argument_list|)
argument_list|)
expr_stmt|;
name|stop_progress
argument_list|(
operator|&
name|progress_state
argument_list|)
expr_stmt|;
name|bitmap_writer_show_progress
argument_list|(
name|progress
argument_list|)
expr_stmt|;
name|bitmap_writer_reuse_bitmaps
argument_list|(
operator|&
name|to_pack
argument_list|)
expr_stmt|;
name|bitmap_writer_select_commits
argument_list|(
name|indexed_commits
argument_list|,
name|indexed_commits_nr
argument_list|,
operator|-
literal|1
argument_list|)
expr_stmt|;
name|bitmap_writer_build
argument_list|(
operator|&
name|to_pack
argument_list|)
expr_stmt|;
name|bitmap_writer_finish
argument_list|(
name|written_list
argument_list|,
name|nr_written
argument_list|,
name|tmpname
operator|.
name|buf
argument_list|,
name|write_bitmap_options
argument_list|)
expr_stmt|;
name|write_bitmap_index
operator|=
literal|0
expr_stmt|;
block|}
name|strbuf_release
argument_list|(
operator|&
name|tmpname
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|pack_tmp_name
argument_list|)
expr_stmt|;
name|puts
argument_list|(
name|sha1_to_hex
argument_list|(
name|sha1
argument_list|)
argument_list|)
expr_stmt|;
block|}
comment|/* mark written objects as written to previous pack */
for|for
control|(
name|j
operator|=
literal|0
init|;
name|j
operator|<
name|nr_written
condition|;
name|j
operator|++
control|)
block|{
name|written_list
index|[
name|j
index|]
operator|->
name|offset
operator|=
operator|(
name|off_t
operator|)
operator|-
literal|1
expr_stmt|;
block|}
name|nr_remaining
operator|-=
name|nr_written
expr_stmt|;
block|}
do|while
condition|(
name|nr_remaining
operator|&&
name|i
operator|<
name|to_pack
operator|.
name|nr_objects
condition|)
do|;
name|free
argument_list|(
name|written_list
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|write_order
argument_list|)
expr_stmt|;
name|stop_progress
argument_list|(
operator|&
name|progress_state
argument_list|)
expr_stmt|;
if|if
condition|(
name|written
operator|!=
name|nr_result
condition|)
name|die
argument_list|(
literal|"wrote %"
name|PRIu32
literal|" objects while expecting %"
name|PRIu32
argument_list|,
name|written
argument_list|,
name|nr_result
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|setup_delta_attr_check
specifier|static
name|void
name|setup_delta_attr_check
parameter_list|(
name|struct
name|git_attr_check
modifier|*
name|check
parameter_list|)
block|{
specifier|static
name|struct
name|git_attr
modifier|*
name|attr_delta
decl_stmt|;
if|if
condition|(
operator|!
name|attr_delta
condition|)
name|attr_delta
operator|=
name|git_attr
argument_list|(
literal|"delta"
argument_list|)
expr_stmt|;
name|check
index|[
literal|0
index|]
operator|.
name|attr
operator|=
name|attr_delta
expr_stmt|;
block|}
end_function
begin_function
DECL|function|no_try_delta
specifier|static
name|int
name|no_try_delta
parameter_list|(
specifier|const
name|char
modifier|*
name|path
parameter_list|)
block|{
name|struct
name|git_attr_check
name|check
index|[
literal|1
index|]
decl_stmt|;
name|setup_delta_attr_check
argument_list|(
name|check
argument_list|)
expr_stmt|;
if|if
condition|(
name|git_check_attr
argument_list|(
name|path
argument_list|,
name|ARRAY_SIZE
argument_list|(
name|check
argument_list|)
argument_list|,
name|check
argument_list|)
condition|)
return|return
literal|0
return|;
if|if
condition|(
name|ATTR_FALSE
argument_list|(
name|check
operator|->
name|value
argument_list|)
condition|)
return|return
literal|1
return|;
return|return
literal|0
return|;
block|}
end_function
begin_comment
comment|/*  * When adding an object, check whether we have already added it  * to our packing list. If so, we can skip. However, if we are  * being asked to excludei t, but the previous mention was to include  * it, make sure to adjust its flags and tweak our numbers accordingly.  *  * As an optimization, we pass out the index position where we would have  * found the item, since that saves us from having to look it up again a  * few lines later when we want to add the new entry.  */
end_comment
begin_function
DECL|function|have_duplicate_entry
specifier|static
name|int
name|have_duplicate_entry
parameter_list|(
specifier|const
name|unsigned
name|char
modifier|*
name|sha1
parameter_list|,
name|int
name|exclude
parameter_list|,
name|uint32_t
modifier|*
name|index_pos
parameter_list|)
block|{
name|struct
name|object_entry
modifier|*
name|entry
decl_stmt|;
name|entry
operator|=
name|packlist_find
argument_list|(
operator|&
name|to_pack
argument_list|,
name|sha1
argument_list|,
name|index_pos
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|entry
condition|)
return|return
literal|0
return|;
if|if
condition|(
name|exclude
condition|)
block|{
if|if
condition|(
operator|!
name|entry
operator|->
name|preferred_base
condition|)
name|nr_result
operator|--
expr_stmt|;
name|entry
operator|->
name|preferred_base
operator|=
literal|1
expr_stmt|;
block|}
return|return
literal|1
return|;
block|}
end_function
begin_comment
comment|/*  * Check whether we want the object in the pack (e.g., we do not want  * objects found in non-local stores if the "--local" option was used).  *  * As a side effect of this check, we will find the packed version of this  * object, if any. We therefore pass out the pack information to avoid having  * to look it up again later.  */
end_comment
begin_function
DECL|function|want_object_in_pack
specifier|static
name|int
name|want_object_in_pack
parameter_list|(
specifier|const
name|unsigned
name|char
modifier|*
name|sha1
parameter_list|,
name|int
name|exclude
parameter_list|,
name|struct
name|packed_git
modifier|*
modifier|*
name|found_pack
parameter_list|,
name|off_t
modifier|*
name|found_offset
parameter_list|)
block|{
name|struct
name|packed_git
modifier|*
name|p
decl_stmt|;
if|if
condition|(
operator|!
name|exclude
operator|&&
name|local
operator|&&
name|has_loose_object_nonlocal
argument_list|(
name|sha1
argument_list|)
condition|)
return|return
literal|0
return|;
operator|*
name|found_pack
operator|=
name|NULL
expr_stmt|;
operator|*
name|found_offset
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|p
operator|=
name|packed_git
init|;
name|p
condition|;
name|p
operator|=
name|p
operator|->
name|next
control|)
block|{
name|off_t
name|offset
init|=
name|find_pack_entry_one
argument_list|(
name|sha1
argument_list|,
name|p
argument_list|)
decl_stmt|;
if|if
condition|(
name|offset
condition|)
block|{
if|if
condition|(
operator|!
operator|*
name|found_pack
condition|)
block|{
if|if
condition|(
operator|!
name|is_pack_valid
argument_list|(
name|p
argument_list|)
condition|)
continue|continue;
operator|*
name|found_offset
operator|=
name|offset
expr_stmt|;
operator|*
name|found_pack
operator|=
name|p
expr_stmt|;
block|}
if|if
condition|(
name|exclude
condition|)
return|return
literal|1
return|;
if|if
condition|(
name|incremental
condition|)
return|return
literal|0
return|;
if|if
condition|(
name|local
operator|&&
operator|!
name|p
operator|->
name|pack_local
condition|)
return|return
literal|0
return|;
if|if
condition|(
name|ignore_packed_keep
operator|&&
name|p
operator|->
name|pack_local
operator|&&
name|p
operator|->
name|pack_keep
condition|)
return|return
literal|0
return|;
block|}
block|}
return|return
literal|1
return|;
block|}
end_function
begin_function
DECL|function|create_object_entry
specifier|static
name|void
name|create_object_entry
parameter_list|(
specifier|const
name|unsigned
name|char
modifier|*
name|sha1
parameter_list|,
name|enum
name|object_type
name|type
parameter_list|,
name|uint32_t
name|hash
parameter_list|,
name|int
name|exclude
parameter_list|,
name|int
name|no_try_delta
parameter_list|,
name|uint32_t
name|index_pos
parameter_list|,
name|struct
name|packed_git
modifier|*
name|found_pack
parameter_list|,
name|off_t
name|found_offset
parameter_list|)
block|{
name|struct
name|object_entry
modifier|*
name|entry
decl_stmt|;
name|entry
operator|=
name|packlist_alloc
argument_list|(
operator|&
name|to_pack
argument_list|,
name|sha1
argument_list|,
name|index_pos
argument_list|)
expr_stmt|;
name|entry
operator|->
name|hash
operator|=
name|hash
expr_stmt|;
if|if
condition|(
name|type
condition|)
name|entry
operator|->
name|type
operator|=
name|type
expr_stmt|;
if|if
condition|(
name|exclude
condition|)
name|entry
operator|->
name|preferred_base
operator|=
literal|1
expr_stmt|;
else|else
name|nr_result
operator|++
expr_stmt|;
if|if
condition|(
name|found_pack
condition|)
block|{
name|entry
operator|->
name|in_pack
operator|=
name|found_pack
expr_stmt|;
name|entry
operator|->
name|in_pack_offset
operator|=
name|found_offset
expr_stmt|;
block|}
name|entry
operator|->
name|no_try_delta
operator|=
name|no_try_delta
expr_stmt|;
block|}
end_function
begin_decl_stmt
DECL|variable|no_closure_warning
specifier|static
specifier|const
name|char
name|no_closure_warning
index|[]
init|=
name|N_
argument_list|(
literal|"disabling bitmap writing, as some objects are not being packed"
argument_list|)
decl_stmt|;
end_decl_stmt
begin_function
DECL|function|add_object_entry
specifier|static
name|int
name|add_object_entry
parameter_list|(
specifier|const
name|unsigned
name|char
modifier|*
name|sha1
parameter_list|,
name|enum
name|object_type
name|type
parameter_list|,
specifier|const
name|char
modifier|*
name|name
parameter_list|,
name|int
name|exclude
parameter_list|)
block|{
name|struct
name|packed_git
modifier|*
name|found_pack
decl_stmt|;
name|off_t
name|found_offset
decl_stmt|;
name|uint32_t
name|index_pos
decl_stmt|;
if|if
condition|(
name|have_duplicate_entry
argument_list|(
name|sha1
argument_list|,
name|exclude
argument_list|,
operator|&
name|index_pos
argument_list|)
condition|)
return|return
literal|0
return|;
if|if
condition|(
operator|!
name|want_object_in_pack
argument_list|(
name|sha1
argument_list|,
name|exclude
argument_list|,
operator|&
name|found_pack
argument_list|,
operator|&
name|found_offset
argument_list|)
condition|)
block|{
comment|/* The pack is missing an object, so it will not have closure */
if|if
condition|(
name|write_bitmap_index
condition|)
block|{
name|warning
argument_list|(
name|_
argument_list|(
name|no_closure_warning
argument_list|)
argument_list|)
expr_stmt|;
name|write_bitmap_index
operator|=
literal|0
expr_stmt|;
block|}
return|return
literal|0
return|;
block|}
name|create_object_entry
argument_list|(
name|sha1
argument_list|,
name|type
argument_list|,
name|pack_name_hash
argument_list|(
name|name
argument_list|)
argument_list|,
name|exclude
argument_list|,
name|name
operator|&&
name|no_try_delta
argument_list|(
name|name
argument_list|)
argument_list|,
name|index_pos
argument_list|,
name|found_pack
argument_list|,
name|found_offset
argument_list|)
expr_stmt|;
name|display_progress
argument_list|(
name|progress_state
argument_list|,
name|nr_result
argument_list|)
expr_stmt|;
return|return
literal|1
return|;
block|}
end_function
begin_function
DECL|function|add_object_entry_from_bitmap
specifier|static
name|int
name|add_object_entry_from_bitmap
parameter_list|(
specifier|const
name|unsigned
name|char
modifier|*
name|sha1
parameter_list|,
name|enum
name|object_type
name|type
parameter_list|,
name|int
name|flags
parameter_list|,
name|uint32_t
name|name_hash
parameter_list|,
name|struct
name|packed_git
modifier|*
name|pack
parameter_list|,
name|off_t
name|offset
parameter_list|)
block|{
name|uint32_t
name|index_pos
decl_stmt|;
if|if
condition|(
name|have_duplicate_entry
argument_list|(
name|sha1
argument_list|,
literal|0
argument_list|,
operator|&
name|index_pos
argument_list|)
condition|)
return|return
literal|0
return|;
name|create_object_entry
argument_list|(
name|sha1
argument_list|,
name|type
argument_list|,
name|name_hash
argument_list|,
literal|0
argument_list|,
literal|0
argument_list|,
name|index_pos
argument_list|,
name|pack
argument_list|,
name|offset
argument_list|)
expr_stmt|;
name|display_progress
argument_list|(
name|progress_state
argument_list|,
name|nr_result
argument_list|)
expr_stmt|;
return|return
literal|1
return|;
block|}
end_function
begin_struct
DECL|struct|pbase_tree_cache
struct|struct
name|pbase_tree_cache
block|{
DECL|member|sha1
name|unsigned
name|char
name|sha1
index|[
literal|20
index|]
decl_stmt|;
DECL|member|ref
name|int
name|ref
decl_stmt|;
DECL|member|temporary
name|int
name|temporary
decl_stmt|;
DECL|member|tree_data
name|void
modifier|*
name|tree_data
decl_stmt|;
DECL|member|tree_size
name|unsigned
name|long
name|tree_size
decl_stmt|;
block|}
struct|;
end_struct
begin_function_decl
DECL|variable|pbase_tree_cache
specifier|static
name|struct
name|pbase_tree_cache
modifier|*
function_decl|(
name|pbase_tree_cache
function_decl|[256]
end_function_decl
begin_empty_stmt
DECL|variable|pbase_tree_cache
unit|)
empty_stmt|;
end_empty_stmt
begin_function
DECL|function|pbase_tree_cache_ix
specifier|static
name|int
name|pbase_tree_cache_ix
parameter_list|(
specifier|const
name|unsigned
name|char
modifier|*
name|sha1
parameter_list|)
block|{
return|return
name|sha1
index|[
literal|0
index|]
operator|%
name|ARRAY_SIZE
argument_list|(
name|pbase_tree_cache
argument_list|)
return|;
block|}
end_function
begin_function
DECL|function|pbase_tree_cache_ix_incr
specifier|static
name|int
name|pbase_tree_cache_ix_incr
parameter_list|(
name|int
name|ix
parameter_list|)
block|{
return|return
operator|(
name|ix
operator|+
literal|1
operator|)
operator|%
name|ARRAY_SIZE
argument_list|(
name|pbase_tree_cache
argument_list|)
return|;
block|}
end_function
begin_struct
DECL|struct|pbase_tree
specifier|static
struct|struct
name|pbase_tree
block|{
DECL|member|next
name|struct
name|pbase_tree
modifier|*
name|next
decl_stmt|;
comment|/* This is a phony "cache" entry; we are not 	 * going to evict it or find it through _get() 	 * mechanism -- this is for the toplevel node that 	 * would almost always change with any commit. 	 */
DECL|member|pcache
name|struct
name|pbase_tree_cache
name|pcache
decl_stmt|;
block|}
DECL|variable|pbase_tree
modifier|*
name|pbase_tree
struct|;
end_struct
begin_function
DECL|function|pbase_tree_get
specifier|static
name|struct
name|pbase_tree_cache
modifier|*
name|pbase_tree_get
parameter_list|(
specifier|const
name|unsigned
name|char
modifier|*
name|sha1
parameter_list|)
block|{
name|struct
name|pbase_tree_cache
modifier|*
name|ent
decl_stmt|,
modifier|*
name|nent
decl_stmt|;
name|void
modifier|*
name|data
decl_stmt|;
name|unsigned
name|long
name|size
decl_stmt|;
name|enum
name|object_type
name|type
decl_stmt|;
name|int
name|neigh
decl_stmt|;
name|int
name|my_ix
init|=
name|pbase_tree_cache_ix
argument_list|(
name|sha1
argument_list|)
decl_stmt|;
name|int
name|available_ix
init|=
operator|-
literal|1
decl_stmt|;
comment|/* pbase-tree-cache acts as a limited hashtable. 	 * your object will be found at your index or within a few 	 * slots after that slot if it is cached. 	 */
for|for
control|(
name|neigh
operator|=
literal|0
init|;
name|neigh
operator|<
literal|8
condition|;
name|neigh
operator|++
control|)
block|{
name|ent
operator|=
name|pbase_tree_cache
index|[
name|my_ix
index|]
expr_stmt|;
if|if
condition|(
name|ent
operator|&&
operator|!
name|hashcmp
argument_list|(
name|ent
operator|->
name|sha1
argument_list|,
name|sha1
argument_list|)
condition|)
block|{
name|ent
operator|->
name|ref
operator|++
expr_stmt|;
return|return
name|ent
return|;
block|}
elseif|else
if|if
condition|(
operator|(
operator|(
name|available_ix
operator|<
literal|0
operator|)
operator|&&
operator|(
operator|!
name|ent
operator|||
operator|!
name|ent
operator|->
name|ref
operator|)
operator|)
operator|||
operator|(
operator|(
literal|0
operator|<=
name|available_ix
operator|)
operator|&&
operator|(
operator|!
name|ent
operator|&&
name|pbase_tree_cache
index|[
name|available_ix
index|]
operator|)
operator|)
condition|)
name|available_ix
operator|=
name|my_ix
expr_stmt|;
if|if
condition|(
operator|!
name|ent
condition|)
break|break;
name|my_ix
operator|=
name|pbase_tree_cache_ix_incr
argument_list|(
name|my_ix
argument_list|)
expr_stmt|;
block|}
comment|/* Did not find one.  Either we got a bogus request or 	 * we need to read and perhaps cache. 	 */
name|data
operator|=
name|read_sha1_file
argument_list|(
name|sha1
argument_list|,
operator|&
name|type
argument_list|,
operator|&
name|size
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|data
condition|)
return|return
name|NULL
return|;
if|if
condition|(
name|type
operator|!=
name|OBJ_TREE
condition|)
block|{
name|free
argument_list|(
name|data
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
comment|/* We need to either cache or return a throwaway copy */
if|if
condition|(
name|available_ix
operator|<
literal|0
condition|)
name|ent
operator|=
name|NULL
expr_stmt|;
else|else
block|{
name|ent
operator|=
name|pbase_tree_cache
index|[
name|available_ix
index|]
expr_stmt|;
name|my_ix
operator|=
name|available_ix
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|ent
condition|)
block|{
name|nent
operator|=
name|xmalloc
argument_list|(
sizeof|sizeof
argument_list|(
operator|*
name|nent
argument_list|)
argument_list|)
expr_stmt|;
name|nent
operator|->
name|temporary
operator|=
operator|(
name|available_ix
operator|<
literal|0
operator|)
expr_stmt|;
block|}
else|else
block|{
comment|/* evict and reuse */
name|free
argument_list|(
name|ent
operator|->
name|tree_data
argument_list|)
expr_stmt|;
name|nent
operator|=
name|ent
expr_stmt|;
block|}
name|hashcpy
argument_list|(
name|nent
operator|->
name|sha1
argument_list|,
name|sha1
argument_list|)
expr_stmt|;
name|nent
operator|->
name|tree_data
operator|=
name|data
expr_stmt|;
name|nent
operator|->
name|tree_size
operator|=
name|size
expr_stmt|;
name|nent
operator|->
name|ref
operator|=
literal|1
expr_stmt|;
if|if
condition|(
operator|!
name|nent
operator|->
name|temporary
condition|)
name|pbase_tree_cache
index|[
name|my_ix
index|]
operator|=
name|nent
expr_stmt|;
return|return
name|nent
return|;
block|}
end_function
begin_function
DECL|function|pbase_tree_put
specifier|static
name|void
name|pbase_tree_put
parameter_list|(
name|struct
name|pbase_tree_cache
modifier|*
name|cache
parameter_list|)
block|{
if|if
condition|(
operator|!
name|cache
operator|->
name|temporary
condition|)
block|{
name|cache
operator|->
name|ref
operator|--
expr_stmt|;
return|return;
block|}
name|free
argument_list|(
name|cache
operator|->
name|tree_data
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|cache
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|name_cmp_len
specifier|static
name|int
name|name_cmp_len
parameter_list|(
specifier|const
name|char
modifier|*
name|name
parameter_list|)
block|{
name|int
name|i
decl_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|name
index|[
name|i
index|]
operator|&&
name|name
index|[
name|i
index|]
operator|!=
literal|'\n'
operator|&&
name|name
index|[
name|i
index|]
operator|!=
literal|'/'
condition|;
name|i
operator|++
control|)
empty_stmt|;
return|return
name|i
return|;
block|}
end_function
begin_function
DECL|function|add_pbase_object
specifier|static
name|void
name|add_pbase_object
parameter_list|(
name|struct
name|tree_desc
modifier|*
name|tree
parameter_list|,
specifier|const
name|char
modifier|*
name|name
parameter_list|,
name|int
name|cmplen
parameter_list|,
specifier|const
name|char
modifier|*
name|fullname
parameter_list|)
block|{
name|struct
name|name_entry
name|entry
decl_stmt|;
name|int
name|cmp
decl_stmt|;
while|while
condition|(
name|tree_entry
argument_list|(
name|tree
argument_list|,
operator|&
name|entry
argument_list|)
condition|)
block|{
if|if
condition|(
name|S_ISGITLINK
argument_list|(
name|entry
operator|.
name|mode
argument_list|)
condition|)
continue|continue;
name|cmp
operator|=
name|tree_entry_len
argument_list|(
operator|&
name|entry
argument_list|)
operator|!=
name|cmplen
condition|?
literal|1
else|:
name|memcmp
argument_list|(
name|name
argument_list|,
name|entry
operator|.
name|path
argument_list|,
name|cmplen
argument_list|)
expr_stmt|;
if|if
condition|(
name|cmp
operator|>
literal|0
condition|)
continue|continue;
if|if
condition|(
name|cmp
operator|<
literal|0
condition|)
return|return;
if|if
condition|(
name|name
index|[
name|cmplen
index|]
operator|!=
literal|'/'
condition|)
block|{
name|add_object_entry
argument_list|(
name|entry
operator|.
name|oid
operator|->
name|hash
argument_list|,
name|object_type
argument_list|(
name|entry
operator|.
name|mode
argument_list|)
argument_list|,
name|fullname
argument_list|,
literal|1
argument_list|)
expr_stmt|;
return|return;
block|}
if|if
condition|(
name|S_ISDIR
argument_list|(
name|entry
operator|.
name|mode
argument_list|)
condition|)
block|{
name|struct
name|tree_desc
name|sub
decl_stmt|;
name|struct
name|pbase_tree_cache
modifier|*
name|tree
decl_stmt|;
specifier|const
name|char
modifier|*
name|down
init|=
name|name
operator|+
name|cmplen
operator|+
literal|1
decl_stmt|;
name|int
name|downlen
init|=
name|name_cmp_len
argument_list|(
name|down
argument_list|)
decl_stmt|;
name|tree
operator|=
name|pbase_tree_get
argument_list|(
name|entry
operator|.
name|oid
operator|->
name|hash
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|tree
condition|)
return|return;
name|init_tree_desc
argument_list|(
operator|&
name|sub
argument_list|,
name|tree
operator|->
name|tree_data
argument_list|,
name|tree
operator|->
name|tree_size
argument_list|)
expr_stmt|;
name|add_pbase_object
argument_list|(
operator|&
name|sub
argument_list|,
name|down
argument_list|,
name|downlen
argument_list|,
name|fullname
argument_list|)
expr_stmt|;
name|pbase_tree_put
argument_list|(
name|tree
argument_list|)
expr_stmt|;
block|}
block|}
block|}
end_function
begin_decl_stmt
DECL|variable|done_pbase_paths
specifier|static
name|unsigned
modifier|*
name|done_pbase_paths
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|done_pbase_paths_num
specifier|static
name|int
name|done_pbase_paths_num
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|done_pbase_paths_alloc
specifier|static
name|int
name|done_pbase_paths_alloc
decl_stmt|;
end_decl_stmt
begin_function
DECL|function|done_pbase_path_pos
specifier|static
name|int
name|done_pbase_path_pos
parameter_list|(
name|unsigned
name|hash
parameter_list|)
block|{
name|int
name|lo
init|=
literal|0
decl_stmt|;
name|int
name|hi
init|=
name|done_pbase_paths_num
decl_stmt|;
while|while
condition|(
name|lo
operator|<
name|hi
condition|)
block|{
name|int
name|mi
init|=
operator|(
name|hi
operator|+
name|lo
operator|)
operator|/
literal|2
decl_stmt|;
if|if
condition|(
name|done_pbase_paths
index|[
name|mi
index|]
operator|==
name|hash
condition|)
return|return
name|mi
return|;
if|if
condition|(
name|done_pbase_paths
index|[
name|mi
index|]
operator|<
name|hash
condition|)
name|hi
operator|=
name|mi
expr_stmt|;
else|else
name|lo
operator|=
name|mi
operator|+
literal|1
expr_stmt|;
block|}
return|return
operator|-
name|lo
operator|-
literal|1
return|;
block|}
end_function
begin_function
DECL|function|check_pbase_path
specifier|static
name|int
name|check_pbase_path
parameter_list|(
name|unsigned
name|hash
parameter_list|)
block|{
name|int
name|pos
init|=
operator|(
operator|!
name|done_pbase_paths
operator|)
condition|?
operator|-
literal|1
else|:
name|done_pbase_path_pos
argument_list|(
name|hash
argument_list|)
decl_stmt|;
if|if
condition|(
literal|0
operator|<=
name|pos
condition|)
return|return
literal|1
return|;
name|pos
operator|=
operator|-
name|pos
operator|-
literal|1
expr_stmt|;
name|ALLOC_GROW
argument_list|(
name|done_pbase_paths
argument_list|,
name|done_pbase_paths_num
operator|+
literal|1
argument_list|,
name|done_pbase_paths_alloc
argument_list|)
expr_stmt|;
name|done_pbase_paths_num
operator|++
expr_stmt|;
if|if
condition|(
name|pos
operator|<
name|done_pbase_paths_num
condition|)
name|memmove
argument_list|(
name|done_pbase_paths
operator|+
name|pos
operator|+
literal|1
argument_list|,
name|done_pbase_paths
operator|+
name|pos
argument_list|,
operator|(
name|done_pbase_paths_num
operator|-
name|pos
operator|-
literal|1
operator|)
operator|*
sizeof|sizeof
argument_list|(
name|unsigned
argument_list|)
argument_list|)
expr_stmt|;
name|done_pbase_paths
index|[
name|pos
index|]
operator|=
name|hash
expr_stmt|;
return|return
literal|0
return|;
block|}
end_function
begin_function
DECL|function|add_preferred_base_object
specifier|static
name|void
name|add_preferred_base_object
parameter_list|(
specifier|const
name|char
modifier|*
name|name
parameter_list|)
block|{
name|struct
name|pbase_tree
modifier|*
name|it
decl_stmt|;
name|int
name|cmplen
decl_stmt|;
name|unsigned
name|hash
init|=
name|pack_name_hash
argument_list|(
name|name
argument_list|)
decl_stmt|;
if|if
condition|(
operator|!
name|num_preferred_base
operator|||
name|check_pbase_path
argument_list|(
name|hash
argument_list|)
condition|)
return|return;
name|cmplen
operator|=
name|name_cmp_len
argument_list|(
name|name
argument_list|)
expr_stmt|;
for|for
control|(
name|it
operator|=
name|pbase_tree
init|;
name|it
condition|;
name|it
operator|=
name|it
operator|->
name|next
control|)
block|{
if|if
condition|(
name|cmplen
operator|==
literal|0
condition|)
block|{
name|add_object_entry
argument_list|(
name|it
operator|->
name|pcache
operator|.
name|sha1
argument_list|,
name|OBJ_TREE
argument_list|,
name|NULL
argument_list|,
literal|1
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|struct
name|tree_desc
name|tree
decl_stmt|;
name|init_tree_desc
argument_list|(
operator|&
name|tree
argument_list|,
name|it
operator|->
name|pcache
operator|.
name|tree_data
argument_list|,
name|it
operator|->
name|pcache
operator|.
name|tree_size
argument_list|)
expr_stmt|;
name|add_pbase_object
argument_list|(
operator|&
name|tree
argument_list|,
name|name
argument_list|,
name|cmplen
argument_list|,
name|name
argument_list|)
expr_stmt|;
block|}
block|}
block|}
end_function
begin_function
DECL|function|add_preferred_base
specifier|static
name|void
name|add_preferred_base
parameter_list|(
name|unsigned
name|char
modifier|*
name|sha1
parameter_list|)
block|{
name|struct
name|pbase_tree
modifier|*
name|it
decl_stmt|;
name|void
modifier|*
name|data
decl_stmt|;
name|unsigned
name|long
name|size
decl_stmt|;
name|unsigned
name|char
name|tree_sha1
index|[
literal|20
index|]
decl_stmt|;
if|if
condition|(
name|window
operator|<=
name|num_preferred_base
operator|++
condition|)
return|return;
name|data
operator|=
name|read_object_with_reference
argument_list|(
name|sha1
argument_list|,
name|tree_type
argument_list|,
operator|&
name|size
argument_list|,
name|tree_sha1
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|data
condition|)
return|return;
for|for
control|(
name|it
operator|=
name|pbase_tree
init|;
name|it
condition|;
name|it
operator|=
name|it
operator|->
name|next
control|)
block|{
if|if
condition|(
operator|!
name|hashcmp
argument_list|(
name|it
operator|->
name|pcache
operator|.
name|sha1
argument_list|,
name|tree_sha1
argument_list|)
condition|)
block|{
name|free
argument_list|(
name|data
argument_list|)
expr_stmt|;
return|return;
block|}
block|}
name|it
operator|=
name|xcalloc
argument_list|(
literal|1
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|it
argument_list|)
argument_list|)
expr_stmt|;
name|it
operator|->
name|next
operator|=
name|pbase_tree
expr_stmt|;
name|pbase_tree
operator|=
name|it
expr_stmt|;
name|hashcpy
argument_list|(
name|it
operator|->
name|pcache
operator|.
name|sha1
argument_list|,
name|tree_sha1
argument_list|)
expr_stmt|;
name|it
operator|->
name|pcache
operator|.
name|tree_data
operator|=
name|data
expr_stmt|;
name|it
operator|->
name|pcache
operator|.
name|tree_size
operator|=
name|size
expr_stmt|;
block|}
end_function
begin_function
DECL|function|cleanup_preferred_base
specifier|static
name|void
name|cleanup_preferred_base
parameter_list|(
name|void
parameter_list|)
block|{
name|struct
name|pbase_tree
modifier|*
name|it
decl_stmt|;
name|unsigned
name|i
decl_stmt|;
name|it
operator|=
name|pbase_tree
expr_stmt|;
name|pbase_tree
operator|=
name|NULL
expr_stmt|;
while|while
condition|(
name|it
condition|)
block|{
name|struct
name|pbase_tree
modifier|*
name|this
init|=
name|it
decl_stmt|;
name|it
operator|=
name|this
operator|->
name|next
expr_stmt|;
name|free
argument_list|(
name|this
operator|->
name|pcache
operator|.
name|tree_data
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|this
argument_list|)
expr_stmt|;
block|}
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|ARRAY_SIZE
argument_list|(
name|pbase_tree_cache
argument_list|)
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
operator|!
name|pbase_tree_cache
index|[
name|i
index|]
condition|)
continue|continue;
name|free
argument_list|(
name|pbase_tree_cache
index|[
name|i
index|]
operator|->
name|tree_data
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|pbase_tree_cache
index|[
name|i
index|]
argument_list|)
expr_stmt|;
name|pbase_tree_cache
index|[
name|i
index|]
operator|=
name|NULL
expr_stmt|;
block|}
name|free
argument_list|(
name|done_pbase_paths
argument_list|)
expr_stmt|;
name|done_pbase_paths
operator|=
name|NULL
expr_stmt|;
name|done_pbase_paths_num
operator|=
name|done_pbase_paths_alloc
operator|=
literal|0
expr_stmt|;
block|}
end_function
begin_function
DECL|function|check_object
specifier|static
name|void
name|check_object
parameter_list|(
name|struct
name|object_entry
modifier|*
name|entry
parameter_list|)
block|{
if|if
condition|(
name|entry
operator|->
name|in_pack
condition|)
block|{
name|struct
name|packed_git
modifier|*
name|p
init|=
name|entry
operator|->
name|in_pack
decl_stmt|;
name|struct
name|pack_window
modifier|*
name|w_curs
init|=
name|NULL
decl_stmt|;
specifier|const
name|unsigned
name|char
modifier|*
name|base_ref
init|=
name|NULL
decl_stmt|;
name|struct
name|object_entry
modifier|*
name|base_entry
decl_stmt|;
name|unsigned
name|long
name|used
decl_stmt|,
name|used_0
decl_stmt|;
name|unsigned
name|long
name|avail
decl_stmt|;
name|off_t
name|ofs
decl_stmt|;
name|unsigned
name|char
modifier|*
name|buf
decl_stmt|,
name|c
decl_stmt|;
name|buf
operator|=
name|use_pack
argument_list|(
name|p
argument_list|,
operator|&
name|w_curs
argument_list|,
name|entry
operator|->
name|in_pack_offset
argument_list|,
operator|&
name|avail
argument_list|)
expr_stmt|;
comment|/* 		 * We want in_pack_type even if we do not reuse delta 		 * since non-delta representations could still be reused. 		 */
name|used
operator|=
name|unpack_object_header_buffer
argument_list|(
name|buf
argument_list|,
name|avail
argument_list|,
operator|&
name|entry
operator|->
name|in_pack_type
argument_list|,
operator|&
name|entry
operator|->
name|size
argument_list|)
expr_stmt|;
if|if
condition|(
name|used
operator|==
literal|0
condition|)
goto|goto
name|give_up
goto|;
comment|/* 		 * Determine if this is a delta and if so whether we can 		 * reuse it or not.  Otherwise let's find out as cheaply as 		 * possible what the actual type and size for this object is. 		 */
switch|switch
condition|(
name|entry
operator|->
name|in_pack_type
condition|)
block|{
default|default:
comment|/* Not a delta hence we've already got all we need. */
name|entry
operator|->
name|type
operator|=
name|entry
operator|->
name|in_pack_type
expr_stmt|;
name|entry
operator|->
name|in_pack_header_size
operator|=
name|used
expr_stmt|;
if|if
condition|(
name|entry
operator|->
name|type
operator|<
name|OBJ_COMMIT
operator|||
name|entry
operator|->
name|type
operator|>
name|OBJ_BLOB
condition|)
goto|goto
name|give_up
goto|;
name|unuse_pack
argument_list|(
operator|&
name|w_curs
argument_list|)
expr_stmt|;
return|return;
case|case
name|OBJ_REF_DELTA
case|:
if|if
condition|(
name|reuse_delta
operator|&&
operator|!
name|entry
operator|->
name|preferred_base
condition|)
name|base_ref
operator|=
name|use_pack
argument_list|(
name|p
argument_list|,
operator|&
name|w_curs
argument_list|,
name|entry
operator|->
name|in_pack_offset
operator|+
name|used
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
name|entry
operator|->
name|in_pack_header_size
operator|=
name|used
operator|+
literal|20
expr_stmt|;
break|break;
case|case
name|OBJ_OFS_DELTA
case|:
name|buf
operator|=
name|use_pack
argument_list|(
name|p
argument_list|,
operator|&
name|w_curs
argument_list|,
name|entry
operator|->
name|in_pack_offset
operator|+
name|used
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
name|used_0
operator|=
literal|0
expr_stmt|;
name|c
operator|=
name|buf
index|[
name|used_0
operator|++
index|]
expr_stmt|;
name|ofs
operator|=
name|c
operator|&
literal|127
expr_stmt|;
while|while
condition|(
name|c
operator|&
literal|128
condition|)
block|{
name|ofs
operator|+=
literal|1
expr_stmt|;
if|if
condition|(
operator|!
name|ofs
operator|||
name|MSB
argument_list|(
name|ofs
argument_list|,
literal|7
argument_list|)
condition|)
block|{
name|error
argument_list|(
literal|"delta base offset overflow in pack for %s"
argument_list|,
name|sha1_to_hex
argument_list|(
name|entry
operator|->
name|idx
operator|.
name|sha1
argument_list|)
argument_list|)
expr_stmt|;
goto|goto
name|give_up
goto|;
block|}
name|c
operator|=
name|buf
index|[
name|used_0
operator|++
index|]
expr_stmt|;
name|ofs
operator|=
operator|(
name|ofs
operator|<<
literal|7
operator|)
operator|+
operator|(
name|c
operator|&
literal|127
operator|)
expr_stmt|;
block|}
name|ofs
operator|=
name|entry
operator|->
name|in_pack_offset
operator|-
name|ofs
expr_stmt|;
if|if
condition|(
name|ofs
operator|<=
literal|0
operator|||
name|ofs
operator|>=
name|entry
operator|->
name|in_pack_offset
condition|)
block|{
name|error
argument_list|(
literal|"delta base offset out of bound for %s"
argument_list|,
name|sha1_to_hex
argument_list|(
name|entry
operator|->
name|idx
operator|.
name|sha1
argument_list|)
argument_list|)
expr_stmt|;
goto|goto
name|give_up
goto|;
block|}
if|if
condition|(
name|reuse_delta
operator|&&
operator|!
name|entry
operator|->
name|preferred_base
condition|)
block|{
name|struct
name|revindex_entry
modifier|*
name|revidx
decl_stmt|;
name|revidx
operator|=
name|find_pack_revindex
argument_list|(
name|p
argument_list|,
name|ofs
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|revidx
condition|)
goto|goto
name|give_up
goto|;
name|base_ref
operator|=
name|nth_packed_object_sha1
argument_list|(
name|p
argument_list|,
name|revidx
operator|->
name|nr
argument_list|)
expr_stmt|;
block|}
name|entry
operator|->
name|in_pack_header_size
operator|=
name|used
operator|+
name|used_0
expr_stmt|;
break|break;
block|}
if|if
condition|(
name|base_ref
operator|&&
operator|(
name|base_entry
operator|=
name|packlist_find
argument_list|(
operator|&
name|to_pack
argument_list|,
name|base_ref
argument_list|,
name|NULL
argument_list|)
operator|)
condition|)
block|{
comment|/* 			 * If base_ref was set above that means we wish to 			 * reuse delta data, and we even found that base 			 * in the list of objects we want to pack. Goodie! 			 * 			 * Depth value does not matter - find_deltas() will 			 * never consider reused delta as the base object to 			 * deltify other objects against, in order to avoid 			 * circular deltas. 			 */
name|entry
operator|->
name|type
operator|=
name|entry
operator|->
name|in_pack_type
expr_stmt|;
name|entry
operator|->
name|delta
operator|=
name|base_entry
expr_stmt|;
name|entry
operator|->
name|delta_size
operator|=
name|entry
operator|->
name|size
expr_stmt|;
name|entry
operator|->
name|delta_sibling
operator|=
name|base_entry
operator|->
name|delta_child
expr_stmt|;
name|base_entry
operator|->
name|delta_child
operator|=
name|entry
expr_stmt|;
name|unuse_pack
argument_list|(
operator|&
name|w_curs
argument_list|)
expr_stmt|;
return|return;
block|}
if|if
condition|(
name|entry
operator|->
name|type
condition|)
block|{
comment|/* 			 * This must be a delta and we already know what the 			 * final object type is.  Let's extract the actual 			 * object size from the delta header. 			 */
name|entry
operator|->
name|size
operator|=
name|get_size_from_delta
argument_list|(
name|p
argument_list|,
operator|&
name|w_curs
argument_list|,
name|entry
operator|->
name|in_pack_offset
operator|+
name|entry
operator|->
name|in_pack_header_size
argument_list|)
expr_stmt|;
if|if
condition|(
name|entry
operator|->
name|size
operator|==
literal|0
condition|)
goto|goto
name|give_up
goto|;
name|unuse_pack
argument_list|(
operator|&
name|w_curs
argument_list|)
expr_stmt|;
return|return;
block|}
comment|/* 		 * No choice but to fall back to the recursive delta walk 		 * with sha1_object_info() to find about the object type 		 * at this point... 		 */
name|give_up
label|:
name|unuse_pack
argument_list|(
operator|&
name|w_curs
argument_list|)
expr_stmt|;
block|}
name|entry
operator|->
name|type
operator|=
name|sha1_object_info
argument_list|(
name|entry
operator|->
name|idx
operator|.
name|sha1
argument_list|,
operator|&
name|entry
operator|->
name|size
argument_list|)
expr_stmt|;
comment|/* 	 * The error condition is checked in prepare_pack().  This is 	 * to permit a missing preferred base object to be ignored 	 * as a preferred base.  Doing so can result in a larger 	 * pack file, but the transfer will still take place. 	 */
block|}
end_function
begin_function
DECL|function|pack_offset_sort
specifier|static
name|int
name|pack_offset_sort
parameter_list|(
specifier|const
name|void
modifier|*
name|_a
parameter_list|,
specifier|const
name|void
modifier|*
name|_b
parameter_list|)
block|{
specifier|const
name|struct
name|object_entry
modifier|*
name|a
init|=
operator|*
operator|(
expr|struct
name|object_entry
operator|*
operator|*
operator|)
name|_a
decl_stmt|;
specifier|const
name|struct
name|object_entry
modifier|*
name|b
init|=
operator|*
operator|(
expr|struct
name|object_entry
operator|*
operator|*
operator|)
name|_b
decl_stmt|;
comment|/* avoid filesystem trashing with loose objects */
if|if
condition|(
operator|!
name|a
operator|->
name|in_pack
operator|&&
operator|!
name|b
operator|->
name|in_pack
condition|)
return|return
name|hashcmp
argument_list|(
name|a
operator|->
name|idx
operator|.
name|sha1
argument_list|,
name|b
operator|->
name|idx
operator|.
name|sha1
argument_list|)
return|;
if|if
condition|(
name|a
operator|->
name|in_pack
operator|<
name|b
operator|->
name|in_pack
condition|)
return|return
operator|-
literal|1
return|;
if|if
condition|(
name|a
operator|->
name|in_pack
operator|>
name|b
operator|->
name|in_pack
condition|)
return|return
literal|1
return|;
return|return
name|a
operator|->
name|in_pack_offset
operator|<
name|b
operator|->
name|in_pack_offset
condition|?
operator|-
literal|1
else|:
operator|(
name|a
operator|->
name|in_pack_offset
operator|>
name|b
operator|->
name|in_pack_offset
operator|)
return|;
block|}
end_function
begin_function
DECL|function|get_object_details
specifier|static
name|void
name|get_object_details
parameter_list|(
name|void
parameter_list|)
block|{
name|uint32_t
name|i
decl_stmt|;
name|struct
name|object_entry
modifier|*
modifier|*
name|sorted_by_offset
decl_stmt|;
name|sorted_by_offset
operator|=
name|xcalloc
argument_list|(
name|to_pack
operator|.
name|nr_objects
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|object_entry
operator|*
argument_list|)
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|to_pack
operator|.
name|nr_objects
condition|;
name|i
operator|++
control|)
name|sorted_by_offset
index|[
name|i
index|]
operator|=
name|to_pack
operator|.
name|objects
operator|+
name|i
expr_stmt|;
name|qsort
argument_list|(
name|sorted_by_offset
argument_list|,
name|to_pack
operator|.
name|nr_objects
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|sorted_by_offset
argument_list|)
argument_list|,
name|pack_offset_sort
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|to_pack
operator|.
name|nr_objects
condition|;
name|i
operator|++
control|)
block|{
name|struct
name|object_entry
modifier|*
name|entry
init|=
name|sorted_by_offset
index|[
name|i
index|]
decl_stmt|;
name|check_object
argument_list|(
name|entry
argument_list|)
expr_stmt|;
if|if
condition|(
name|big_file_threshold
operator|<
name|entry
operator|->
name|size
condition|)
name|entry
operator|->
name|no_try_delta
operator|=
literal|1
expr_stmt|;
block|}
name|free
argument_list|(
name|sorted_by_offset
argument_list|)
expr_stmt|;
block|}
end_function
begin_comment
comment|/*  * We search for deltas in a list sorted by type, by filename hash, and then  * by size, so that we see progressively smaller and smaller files.  * That's because we prefer deltas to be from the bigger file  * to the smaller -- deletes are potentially cheaper, but perhaps  * more importantly, the bigger file is likely the more recent  * one.  The deepest deltas are therefore the oldest objects which are  * less susceptible to be accessed often.  */
end_comment
begin_function
DECL|function|type_size_sort
specifier|static
name|int
name|type_size_sort
parameter_list|(
specifier|const
name|void
modifier|*
name|_a
parameter_list|,
specifier|const
name|void
modifier|*
name|_b
parameter_list|)
block|{
specifier|const
name|struct
name|object_entry
modifier|*
name|a
init|=
operator|*
operator|(
expr|struct
name|object_entry
operator|*
operator|*
operator|)
name|_a
decl_stmt|;
specifier|const
name|struct
name|object_entry
modifier|*
name|b
init|=
operator|*
operator|(
expr|struct
name|object_entry
operator|*
operator|*
operator|)
name|_b
decl_stmt|;
if|if
condition|(
name|a
operator|->
name|type
operator|>
name|b
operator|->
name|type
condition|)
return|return
operator|-
literal|1
return|;
if|if
condition|(
name|a
operator|->
name|type
operator|<
name|b
operator|->
name|type
condition|)
return|return
literal|1
return|;
if|if
condition|(
name|a
operator|->
name|hash
operator|>
name|b
operator|->
name|hash
condition|)
return|return
operator|-
literal|1
return|;
if|if
condition|(
name|a
operator|->
name|hash
operator|<
name|b
operator|->
name|hash
condition|)
return|return
literal|1
return|;
if|if
condition|(
name|a
operator|->
name|preferred_base
operator|>
name|b
operator|->
name|preferred_base
condition|)
return|return
operator|-
literal|1
return|;
if|if
condition|(
name|a
operator|->
name|preferred_base
operator|<
name|b
operator|->
name|preferred_base
condition|)
return|return
literal|1
return|;
if|if
condition|(
name|a
operator|->
name|size
operator|>
name|b
operator|->
name|size
condition|)
return|return
operator|-
literal|1
return|;
if|if
condition|(
name|a
operator|->
name|size
operator|<
name|b
operator|->
name|size
condition|)
return|return
literal|1
return|;
return|return
name|a
operator|<
name|b
condition|?
operator|-
literal|1
else|:
operator|(
name|a
operator|>
name|b
operator|)
return|;
comment|/* newest first */
block|}
end_function
begin_struct
DECL|struct|unpacked
struct|struct
name|unpacked
block|{
DECL|member|entry
name|struct
name|object_entry
modifier|*
name|entry
decl_stmt|;
DECL|member|data
name|void
modifier|*
name|data
decl_stmt|;
DECL|member|index
name|struct
name|delta_index
modifier|*
name|index
decl_stmt|;
DECL|member|depth
name|unsigned
name|depth
decl_stmt|;
block|}
struct|;
end_struct
begin_function
DECL|function|delta_cacheable
specifier|static
name|int
name|delta_cacheable
parameter_list|(
name|unsigned
name|long
name|src_size
parameter_list|,
name|unsigned
name|long
name|trg_size
parameter_list|,
name|unsigned
name|long
name|delta_size
parameter_list|)
block|{
if|if
condition|(
name|max_delta_cache_size
operator|&&
name|delta_cache_size
operator|+
name|delta_size
operator|>
name|max_delta_cache_size
condition|)
return|return
literal|0
return|;
if|if
condition|(
name|delta_size
operator|<
name|cache_max_small_delta_size
condition|)
return|return
literal|1
return|;
comment|/* cache delta, if objects are large enough compared to delta size */
if|if
condition|(
operator|(
name|src_size
operator|>>
literal|20
operator|)
operator|+
operator|(
name|trg_size
operator|>>
literal|21
operator|)
operator|>
operator|(
name|delta_size
operator|>>
literal|10
operator|)
condition|)
return|return
literal|1
return|;
return|return
literal|0
return|;
block|}
end_function
begin_ifndef
ifndef|#
directive|ifndef
name|NO_PTHREADS
end_ifndef
begin_decl_stmt
DECL|variable|read_mutex
specifier|static
name|pthread_mutex_t
name|read_mutex
decl_stmt|;
end_decl_stmt
begin_define
DECL|macro|read_lock
define|#
directive|define
name|read_lock
parameter_list|()
value|pthread_mutex_lock(&read_mutex)
end_define
begin_define
DECL|macro|read_unlock
define|#
directive|define
name|read_unlock
parameter_list|()
value|pthread_mutex_unlock(&read_mutex)
end_define
begin_decl_stmt
DECL|variable|cache_mutex
specifier|static
name|pthread_mutex_t
name|cache_mutex
decl_stmt|;
end_decl_stmt
begin_define
DECL|macro|cache_lock
define|#
directive|define
name|cache_lock
parameter_list|()
value|pthread_mutex_lock(&cache_mutex)
end_define
begin_define
DECL|macro|cache_unlock
define|#
directive|define
name|cache_unlock
parameter_list|()
value|pthread_mutex_unlock(&cache_mutex)
end_define
begin_decl_stmt
DECL|variable|progress_mutex
specifier|static
name|pthread_mutex_t
name|progress_mutex
decl_stmt|;
end_decl_stmt
begin_define
DECL|macro|progress_lock
define|#
directive|define
name|progress_lock
parameter_list|()
value|pthread_mutex_lock(&progress_mutex)
end_define
begin_define
DECL|macro|progress_unlock
define|#
directive|define
name|progress_unlock
parameter_list|()
value|pthread_mutex_unlock(&progress_mutex)
end_define
begin_else
else|#
directive|else
end_else
begin_define
DECL|macro|read_lock
define|#
directive|define
name|read_lock
parameter_list|()
value|(void)0
end_define
begin_define
DECL|macro|read_unlock
define|#
directive|define
name|read_unlock
parameter_list|()
value|(void)0
end_define
begin_define
DECL|macro|cache_lock
define|#
directive|define
name|cache_lock
parameter_list|()
value|(void)0
end_define
begin_define
DECL|macro|cache_unlock
define|#
directive|define
name|cache_unlock
parameter_list|()
value|(void)0
end_define
begin_define
DECL|macro|progress_lock
define|#
directive|define
name|progress_lock
parameter_list|()
value|(void)0
end_define
begin_define
DECL|macro|progress_unlock
define|#
directive|define
name|progress_unlock
parameter_list|()
value|(void)0
end_define
begin_endif
endif|#
directive|endif
end_endif
begin_function
DECL|function|try_delta
specifier|static
name|int
name|try_delta
parameter_list|(
name|struct
name|unpacked
modifier|*
name|trg
parameter_list|,
name|struct
name|unpacked
modifier|*
name|src
parameter_list|,
name|unsigned
name|max_depth
parameter_list|,
name|unsigned
name|long
modifier|*
name|mem_usage
parameter_list|)
block|{
name|struct
name|object_entry
modifier|*
name|trg_entry
init|=
name|trg
operator|->
name|entry
decl_stmt|;
name|struct
name|object_entry
modifier|*
name|src_entry
init|=
name|src
operator|->
name|entry
decl_stmt|;
name|unsigned
name|long
name|trg_size
decl_stmt|,
name|src_size
decl_stmt|,
name|delta_size
decl_stmt|,
name|sizediff
decl_stmt|,
name|max_size
decl_stmt|,
name|sz
decl_stmt|;
name|unsigned
name|ref_depth
decl_stmt|;
name|enum
name|object_type
name|type
decl_stmt|;
name|void
modifier|*
name|delta_buf
decl_stmt|;
comment|/* Don't bother doing diffs between different types */
if|if
condition|(
name|trg_entry
operator|->
name|type
operator|!=
name|src_entry
operator|->
name|type
condition|)
return|return
operator|-
literal|1
return|;
comment|/* 	 * We do not bother to try a delta that we discarded on an 	 * earlier try, but only when reusing delta data.  Note that 	 * src_entry that is marked as the preferred_base should always 	 * be considered, as even if we produce a suboptimal delta against 	 * it, we will still save the transfer cost, as we already know 	 * the other side has it and we won't send src_entry at all. 	 */
if|if
condition|(
name|reuse_delta
operator|&&
name|trg_entry
operator|->
name|in_pack
operator|&&
name|trg_entry
operator|->
name|in_pack
operator|==
name|src_entry
operator|->
name|in_pack
operator|&&
operator|!
name|src_entry
operator|->
name|preferred_base
operator|&&
name|trg_entry
operator|->
name|in_pack_type
operator|!=
name|OBJ_REF_DELTA
operator|&&
name|trg_entry
operator|->
name|in_pack_type
operator|!=
name|OBJ_OFS_DELTA
condition|)
return|return
literal|0
return|;
comment|/* Let's not bust the allowed depth. */
if|if
condition|(
name|src
operator|->
name|depth
operator|>=
name|max_depth
condition|)
return|return
literal|0
return|;
comment|/* Now some size filtering heuristics. */
name|trg_size
operator|=
name|trg_entry
operator|->
name|size
expr_stmt|;
if|if
condition|(
operator|!
name|trg_entry
operator|->
name|delta
condition|)
block|{
name|max_size
operator|=
name|trg_size
operator|/
literal|2
operator|-
literal|20
expr_stmt|;
name|ref_depth
operator|=
literal|1
expr_stmt|;
block|}
else|else
block|{
name|max_size
operator|=
name|trg_entry
operator|->
name|delta_size
expr_stmt|;
name|ref_depth
operator|=
name|trg
operator|->
name|depth
expr_stmt|;
block|}
name|max_size
operator|=
operator|(
name|uint64_t
operator|)
name|max_size
operator|*
operator|(
name|max_depth
operator|-
name|src
operator|->
name|depth
operator|)
operator|/
operator|(
name|max_depth
operator|-
name|ref_depth
operator|+
literal|1
operator|)
expr_stmt|;
if|if
condition|(
name|max_size
operator|==
literal|0
condition|)
return|return
literal|0
return|;
name|src_size
operator|=
name|src_entry
operator|->
name|size
expr_stmt|;
name|sizediff
operator|=
name|src_size
operator|<
name|trg_size
condition|?
name|trg_size
operator|-
name|src_size
else|:
literal|0
expr_stmt|;
if|if
condition|(
name|sizediff
operator|>=
name|max_size
condition|)
return|return
literal|0
return|;
if|if
condition|(
name|trg_size
operator|<
name|src_size
operator|/
literal|32
condition|)
return|return
literal|0
return|;
comment|/* Load data if not already done */
if|if
condition|(
operator|!
name|trg
operator|->
name|data
condition|)
block|{
name|read_lock
argument_list|()
expr_stmt|;
name|trg
operator|->
name|data
operator|=
name|read_sha1_file
argument_list|(
name|trg_entry
operator|->
name|idx
operator|.
name|sha1
argument_list|,
operator|&
name|type
argument_list|,
operator|&
name|sz
argument_list|)
expr_stmt|;
name|read_unlock
argument_list|()
expr_stmt|;
if|if
condition|(
operator|!
name|trg
operator|->
name|data
condition|)
name|die
argument_list|(
literal|"object %s cannot be read"
argument_list|,
name|sha1_to_hex
argument_list|(
name|trg_entry
operator|->
name|idx
operator|.
name|sha1
argument_list|)
argument_list|)
expr_stmt|;
if|if
condition|(
name|sz
operator|!=
name|trg_size
condition|)
name|die
argument_list|(
literal|"object %s inconsistent object length (%lu vs %lu)"
argument_list|,
name|sha1_to_hex
argument_list|(
name|trg_entry
operator|->
name|idx
operator|.
name|sha1
argument_list|)
argument_list|,
name|sz
argument_list|,
name|trg_size
argument_list|)
expr_stmt|;
operator|*
name|mem_usage
operator|+=
name|sz
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|src
operator|->
name|data
condition|)
block|{
name|read_lock
argument_list|()
expr_stmt|;
name|src
operator|->
name|data
operator|=
name|read_sha1_file
argument_list|(
name|src_entry
operator|->
name|idx
operator|.
name|sha1
argument_list|,
operator|&
name|type
argument_list|,
operator|&
name|sz
argument_list|)
expr_stmt|;
name|read_unlock
argument_list|()
expr_stmt|;
if|if
condition|(
operator|!
name|src
operator|->
name|data
condition|)
block|{
if|if
condition|(
name|src_entry
operator|->
name|preferred_base
condition|)
block|{
specifier|static
name|int
name|warned
init|=
literal|0
decl_stmt|;
if|if
condition|(
operator|!
name|warned
operator|++
condition|)
name|warning
argument_list|(
literal|"object %s cannot be read"
argument_list|,
name|sha1_to_hex
argument_list|(
name|src_entry
operator|->
name|idx
operator|.
name|sha1
argument_list|)
argument_list|)
expr_stmt|;
comment|/* 				 * Those objects are not included in the 				 * resulting pack.  Be resilient and ignore 				 * them if they can't be read, in case the 				 * pack could be created nevertheless. 				 */
return|return
literal|0
return|;
block|}
name|die
argument_list|(
literal|"object %s cannot be read"
argument_list|,
name|sha1_to_hex
argument_list|(
name|src_entry
operator|->
name|idx
operator|.
name|sha1
argument_list|)
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|sz
operator|!=
name|src_size
condition|)
name|die
argument_list|(
literal|"object %s inconsistent object length (%lu vs %lu)"
argument_list|,
name|sha1_to_hex
argument_list|(
name|src_entry
operator|->
name|idx
operator|.
name|sha1
argument_list|)
argument_list|,
name|sz
argument_list|,
name|src_size
argument_list|)
expr_stmt|;
operator|*
name|mem_usage
operator|+=
name|sz
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|src
operator|->
name|index
condition|)
block|{
name|src
operator|->
name|index
operator|=
name|create_delta_index
argument_list|(
name|src
operator|->
name|data
argument_list|,
name|src_size
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|src
operator|->
name|index
condition|)
block|{
specifier|static
name|int
name|warned
init|=
literal|0
decl_stmt|;
if|if
condition|(
operator|!
name|warned
operator|++
condition|)
name|warning
argument_list|(
literal|"suboptimal pack - out of memory"
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
operator|*
name|mem_usage
operator|+=
name|sizeof_delta_index
argument_list|(
name|src
operator|->
name|index
argument_list|)
expr_stmt|;
block|}
name|delta_buf
operator|=
name|create_delta
argument_list|(
name|src
operator|->
name|index
argument_list|,
name|trg
operator|->
name|data
argument_list|,
name|trg_size
argument_list|,
operator|&
name|delta_size
argument_list|,
name|max_size
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|delta_buf
condition|)
return|return
literal|0
return|;
if|if
condition|(
name|trg_entry
operator|->
name|delta
condition|)
block|{
comment|/* Prefer only shallower same-sized deltas. */
if|if
condition|(
name|delta_size
operator|==
name|trg_entry
operator|->
name|delta_size
operator|&&
name|src
operator|->
name|depth
operator|+
literal|1
operator|>=
name|trg
operator|->
name|depth
condition|)
block|{
name|free
argument_list|(
name|delta_buf
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
block|}
comment|/* 	 * Handle memory allocation outside of the cache 	 * accounting lock.  Compiler will optimize the strangeness 	 * away when NO_PTHREADS is defined. 	 */
name|free
argument_list|(
name|trg_entry
operator|->
name|delta_data
argument_list|)
expr_stmt|;
name|cache_lock
argument_list|()
expr_stmt|;
if|if
condition|(
name|trg_entry
operator|->
name|delta_data
condition|)
block|{
name|delta_cache_size
operator|-=
name|trg_entry
operator|->
name|delta_size
expr_stmt|;
name|trg_entry
operator|->
name|delta_data
operator|=
name|NULL
expr_stmt|;
block|}
if|if
condition|(
name|delta_cacheable
argument_list|(
name|src_size
argument_list|,
name|trg_size
argument_list|,
name|delta_size
argument_list|)
condition|)
block|{
name|delta_cache_size
operator|+=
name|delta_size
expr_stmt|;
name|cache_unlock
argument_list|()
expr_stmt|;
name|trg_entry
operator|->
name|delta_data
operator|=
name|xrealloc
argument_list|(
name|delta_buf
argument_list|,
name|delta_size
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|cache_unlock
argument_list|()
expr_stmt|;
name|free
argument_list|(
name|delta_buf
argument_list|)
expr_stmt|;
block|}
name|trg_entry
operator|->
name|delta
operator|=
name|src_entry
expr_stmt|;
name|trg_entry
operator|->
name|delta_size
operator|=
name|delta_size
expr_stmt|;
name|trg
operator|->
name|depth
operator|=
name|src
operator|->
name|depth
operator|+
literal|1
expr_stmt|;
return|return
literal|1
return|;
block|}
end_function
begin_function
DECL|function|check_delta_limit
specifier|static
name|unsigned
name|int
name|check_delta_limit
parameter_list|(
name|struct
name|object_entry
modifier|*
name|me
parameter_list|,
name|unsigned
name|int
name|n
parameter_list|)
block|{
name|struct
name|object_entry
modifier|*
name|child
init|=
name|me
operator|->
name|delta_child
decl_stmt|;
name|unsigned
name|int
name|m
init|=
name|n
decl_stmt|;
while|while
condition|(
name|child
condition|)
block|{
name|unsigned
name|int
name|c
init|=
name|check_delta_limit
argument_list|(
name|child
argument_list|,
name|n
operator|+
literal|1
argument_list|)
decl_stmt|;
if|if
condition|(
name|m
operator|<
name|c
condition|)
name|m
operator|=
name|c
expr_stmt|;
name|child
operator|=
name|child
operator|->
name|delta_sibling
expr_stmt|;
block|}
return|return
name|m
return|;
block|}
end_function
begin_function
DECL|function|free_unpacked
specifier|static
name|unsigned
name|long
name|free_unpacked
parameter_list|(
name|struct
name|unpacked
modifier|*
name|n
parameter_list|)
block|{
name|unsigned
name|long
name|freed_mem
init|=
name|sizeof_delta_index
argument_list|(
name|n
operator|->
name|index
argument_list|)
decl_stmt|;
name|free_delta_index
argument_list|(
name|n
operator|->
name|index
argument_list|)
expr_stmt|;
name|n
operator|->
name|index
operator|=
name|NULL
expr_stmt|;
if|if
condition|(
name|n
operator|->
name|data
condition|)
block|{
name|freed_mem
operator|+=
name|n
operator|->
name|entry
operator|->
name|size
expr_stmt|;
name|free
argument_list|(
name|n
operator|->
name|data
argument_list|)
expr_stmt|;
name|n
operator|->
name|data
operator|=
name|NULL
expr_stmt|;
block|}
name|n
operator|->
name|entry
operator|=
name|NULL
expr_stmt|;
name|n
operator|->
name|depth
operator|=
literal|0
expr_stmt|;
return|return
name|freed_mem
return|;
block|}
end_function
begin_function
DECL|function|find_deltas
specifier|static
name|void
name|find_deltas
parameter_list|(
name|struct
name|object_entry
modifier|*
modifier|*
name|list
parameter_list|,
name|unsigned
modifier|*
name|list_size
parameter_list|,
name|int
name|window
parameter_list|,
name|int
name|depth
parameter_list|,
name|unsigned
modifier|*
name|processed
parameter_list|)
block|{
name|uint32_t
name|i
decl_stmt|,
name|idx
init|=
literal|0
decl_stmt|,
name|count
init|=
literal|0
decl_stmt|;
name|struct
name|unpacked
modifier|*
name|array
decl_stmt|;
name|unsigned
name|long
name|mem_usage
init|=
literal|0
decl_stmt|;
name|array
operator|=
name|xcalloc
argument_list|(
name|window
argument_list|,
sizeof|sizeof
argument_list|(
expr|struct
name|unpacked
argument_list|)
argument_list|)
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
name|struct
name|object_entry
modifier|*
name|entry
decl_stmt|;
name|struct
name|unpacked
modifier|*
name|n
init|=
name|array
operator|+
name|idx
decl_stmt|;
name|int
name|j
decl_stmt|,
name|max_depth
decl_stmt|,
name|best_base
init|=
operator|-
literal|1
decl_stmt|;
name|progress_lock
argument_list|()
expr_stmt|;
if|if
condition|(
operator|!
operator|*
name|list_size
condition|)
block|{
name|progress_unlock
argument_list|()
expr_stmt|;
break|break;
block|}
name|entry
operator|=
operator|*
name|list
operator|++
expr_stmt|;
operator|(
operator|*
name|list_size
operator|)
operator|--
expr_stmt|;
if|if
condition|(
operator|!
name|entry
operator|->
name|preferred_base
condition|)
block|{
operator|(
operator|*
name|processed
operator|)
operator|++
expr_stmt|;
name|display_progress
argument_list|(
name|progress_state
argument_list|,
operator|*
name|processed
argument_list|)
expr_stmt|;
block|}
name|progress_unlock
argument_list|()
expr_stmt|;
name|mem_usage
operator|-=
name|free_unpacked
argument_list|(
name|n
argument_list|)
expr_stmt|;
name|n
operator|->
name|entry
operator|=
name|entry
expr_stmt|;
while|while
condition|(
name|window_memory_limit
operator|&&
name|mem_usage
operator|>
name|window_memory_limit
operator|&&
name|count
operator|>
literal|1
condition|)
block|{
name|uint32_t
name|tail
init|=
operator|(
name|idx
operator|+
name|window
operator|-
name|count
operator|)
operator|%
name|window
decl_stmt|;
name|mem_usage
operator|-=
name|free_unpacked
argument_list|(
name|array
operator|+
name|tail
argument_list|)
expr_stmt|;
name|count
operator|--
expr_stmt|;
block|}
comment|/* We do not compute delta to *create* objects we are not 		 * going to pack. 		 */
if|if
condition|(
name|entry
operator|->
name|preferred_base
condition|)
goto|goto
name|next
goto|;
comment|/* 		 * If the current object is at pack edge, take the depth the 		 * objects that depend on the current object into account 		 * otherwise they would become too deep. 		 */
name|max_depth
operator|=
name|depth
expr_stmt|;
if|if
condition|(
name|entry
operator|->
name|delta_child
condition|)
block|{
name|max_depth
operator|-=
name|check_delta_limit
argument_list|(
name|entry
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|max_depth
operator|<=
literal|0
condition|)
goto|goto
name|next
goto|;
block|}
name|j
operator|=
name|window
expr_stmt|;
while|while
condition|(
operator|--
name|j
operator|>
literal|0
condition|)
block|{
name|int
name|ret
decl_stmt|;
name|uint32_t
name|other_idx
init|=
name|idx
operator|+
name|j
decl_stmt|;
name|struct
name|unpacked
modifier|*
name|m
decl_stmt|;
if|if
condition|(
name|other_idx
operator|>=
name|window
condition|)
name|other_idx
operator|-=
name|window
expr_stmt|;
name|m
operator|=
name|array
operator|+
name|other_idx
expr_stmt|;
if|if
condition|(
operator|!
name|m
operator|->
name|entry
condition|)
break|break;
name|ret
operator|=
name|try_delta
argument_list|(
name|n
argument_list|,
name|m
argument_list|,
name|max_depth
argument_list|,
operator|&
name|mem_usage
argument_list|)
expr_stmt|;
if|if
condition|(
name|ret
operator|<
literal|0
condition|)
break|break;
elseif|else
if|if
condition|(
name|ret
operator|>
literal|0
condition|)
name|best_base
operator|=
name|other_idx
expr_stmt|;
block|}
comment|/* 		 * If we decided to cache the delta data, then it is best 		 * to compress it right away.  First because we have to do 		 * it anyway, and doing it here while we're threaded will 		 * save a lot of time in the non threaded write phase, 		 * as well as allow for caching more deltas within 		 * the same cache size limit. 		 * ... 		 * But only if not writing to stdout, since in that case 		 * the network is most likely throttling writes anyway, 		 * and therefore it is best to go to the write phase ASAP 		 * instead, as we can afford spending more time compressing 		 * between writes at that moment. 		 */
if|if
condition|(
name|entry
operator|->
name|delta_data
operator|&&
operator|!
name|pack_to_stdout
condition|)
block|{
name|entry
operator|->
name|z_delta_size
operator|=
name|do_compress
argument_list|(
operator|&
name|entry
operator|->
name|delta_data
argument_list|,
name|entry
operator|->
name|delta_size
argument_list|)
expr_stmt|;
name|cache_lock
argument_list|()
expr_stmt|;
name|delta_cache_size
operator|-=
name|entry
operator|->
name|delta_size
expr_stmt|;
name|delta_cache_size
operator|+=
name|entry
operator|->
name|z_delta_size
expr_stmt|;
name|cache_unlock
argument_list|()
expr_stmt|;
block|}
comment|/* if we made n a delta, and if n is already at max 		 * depth, leaving it in the window is pointless.  we 		 * should evict it first. 		 */
if|if
condition|(
name|entry
operator|->
name|delta
operator|&&
name|max_depth
operator|<=
name|n
operator|->
name|depth
condition|)
continue|continue;
comment|/* 		 * Move the best delta base up in the window, after the 		 * currently deltified object, to keep it longer.  It will 		 * be the first base object to be attempted next. 		 */
if|if
condition|(
name|entry
operator|->
name|delta
condition|)
block|{
name|struct
name|unpacked
name|swap
init|=
name|array
index|[
name|best_base
index|]
decl_stmt|;
name|int
name|dist
init|=
operator|(
name|window
operator|+
name|idx
operator|-
name|best_base
operator|)
operator|%
name|window
decl_stmt|;
name|int
name|dst
init|=
name|best_base
decl_stmt|;
while|while
condition|(
name|dist
operator|--
condition|)
block|{
name|int
name|src
init|=
operator|(
name|dst
operator|+
literal|1
operator|)
operator|%
name|window
decl_stmt|;
name|array
index|[
name|dst
index|]
operator|=
name|array
index|[
name|src
index|]
expr_stmt|;
name|dst
operator|=
name|src
expr_stmt|;
block|}
name|array
index|[
name|dst
index|]
operator|=
name|swap
expr_stmt|;
block|}
name|next
label|:
name|idx
operator|++
expr_stmt|;
if|if
condition|(
name|count
operator|+
literal|1
operator|<
name|window
condition|)
name|count
operator|++
expr_stmt|;
if|if
condition|(
name|idx
operator|>=
name|window
condition|)
name|idx
operator|=
literal|0
expr_stmt|;
block|}
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|window
condition|;
operator|++
name|i
control|)
block|{
name|free_delta_index
argument_list|(
name|array
index|[
name|i
index|]
operator|.
name|index
argument_list|)
expr_stmt|;
name|free
argument_list|(
name|array
index|[
name|i
index|]
operator|.
name|data
argument_list|)
expr_stmt|;
block|}
name|free
argument_list|(
name|array
argument_list|)
expr_stmt|;
block|}
end_function
begin_ifndef
ifndef|#
directive|ifndef
name|NO_PTHREADS
end_ifndef
begin_function
DECL|function|try_to_free_from_threads
specifier|static
name|void
name|try_to_free_from_threads
parameter_list|(
name|size_t
name|size
parameter_list|)
block|{
name|read_lock
argument_list|()
expr_stmt|;
name|release_pack_memory
argument_list|(
name|size
argument_list|)
expr_stmt|;
name|read_unlock
argument_list|()
expr_stmt|;
block|}
end_function
begin_decl_stmt
DECL|variable|old_try_to_free_routine
specifier|static
name|try_to_free_t
name|old_try_to_free_routine
decl_stmt|;
end_decl_stmt
begin_comment
comment|/*  * The main thread waits on the condition that (at least) one of the workers  * has stopped working (which is indicated in the .working member of  * struct thread_params).  * When a work thread has completed its work, it sets .working to 0 and  * signals the main thread and waits on the condition that .data_ready  * becomes 1.  */
end_comment
begin_struct
DECL|struct|thread_params
struct|struct
name|thread_params
block|{
DECL|member|thread
name|pthread_t
name|thread
decl_stmt|;
DECL|member|list
name|struct
name|object_entry
modifier|*
modifier|*
name|list
decl_stmt|;
DECL|member|list_size
name|unsigned
name|list_size
decl_stmt|;
DECL|member|remaining
name|unsigned
name|remaining
decl_stmt|;
DECL|member|window
name|int
name|window
decl_stmt|;
DECL|member|depth
name|int
name|depth
decl_stmt|;
DECL|member|working
name|int
name|working
decl_stmt|;
DECL|member|data_ready
name|int
name|data_ready
decl_stmt|;
DECL|member|mutex
name|pthread_mutex_t
name|mutex
decl_stmt|;
DECL|member|cond
name|pthread_cond_t
name|cond
decl_stmt|;
DECL|member|processed
name|unsigned
modifier|*
name|processed
decl_stmt|;
block|}
struct|;
end_struct
begin_decl_stmt
DECL|variable|progress_cond
specifier|static
name|pthread_cond_t
name|progress_cond
decl_stmt|;
end_decl_stmt
begin_comment
comment|/*  * Mutex and conditional variable can't be statically-initialized on Windows.  */
end_comment
begin_function
DECL|function|init_threaded_search
specifier|static
name|void
name|init_threaded_search
parameter_list|(
name|void
parameter_list|)
block|{
name|init_recursive_mutex
argument_list|(
operator|&
name|read_mutex
argument_list|)
expr_stmt|;
name|pthread_mutex_init
argument_list|(
operator|&
name|cache_mutex
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
name|pthread_mutex_init
argument_list|(
operator|&
name|progress_mutex
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
name|pthread_cond_init
argument_list|(
operator|&
name|progress_cond
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
name|old_try_to_free_routine
operator|=
name|set_try_to_free_routine
argument_list|(
name|try_to_free_from_threads
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|cleanup_threaded_search
specifier|static
name|void
name|cleanup_threaded_search
parameter_list|(
name|void
parameter_list|)
block|{
name|set_try_to_free_routine
argument_list|(
name|old_try_to_free_routine
argument_list|)
expr_stmt|;
name|pthread_cond_destroy
argument_list|(
operator|&
name|progress_cond
argument_list|)
expr_stmt|;
name|pthread_mutex_destroy
argument_list|(
operator|&
name|read_mutex
argument_list|)
expr_stmt|;
name|pthread_mutex_destroy
argument_list|(
operator|&
name|cache_mutex
argument_list|)
expr_stmt|;
name|pthread_mutex_destroy
argument_list|(
operator|&
name|progress_mutex
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|threaded_find_deltas
specifier|static
name|void
modifier|*
name|threaded_find_deltas
parameter_list|(
name|void
modifier|*
name|arg
parameter_list|)
block|{
name|struct
name|thread_params
modifier|*
name|me
init|=
name|arg
decl_stmt|;
while|while
condition|(
name|me
operator|->
name|remaining
condition|)
block|{
name|find_deltas
argument_list|(
name|me
operator|->
name|list
argument_list|,
operator|&
name|me
operator|->
name|remaining
argument_list|,
name|me
operator|->
name|window
argument_list|,
name|me
operator|->
name|depth
argument_list|,
name|me
operator|->
name|processed
argument_list|)
expr_stmt|;
name|progress_lock
argument_list|()
expr_stmt|;
name|me
operator|->
name|working
operator|=
literal|0
expr_stmt|;
name|pthread_cond_signal
argument_list|(
operator|&
name|progress_cond
argument_list|)
expr_stmt|;
name|progress_unlock
argument_list|()
expr_stmt|;
comment|/* 		 * We must not set ->data_ready before we wait on the 		 * condition because the main thread may have set it to 1 		 * before we get here. In order to be sure that new 		 * work is available if we see 1 in ->data_ready, it 		 * was initialized to 0 before this thread was spawned 		 * and we reset it to 0 right away. 		 */
name|pthread_mutex_lock
argument_list|(
operator|&
name|me
operator|->
name|mutex
argument_list|)
expr_stmt|;
while|while
condition|(
operator|!
name|me
operator|->
name|data_ready
condition|)
name|pthread_cond_wait
argument_list|(
operator|&
name|me
operator|->
name|cond
argument_list|,
operator|&
name|me
operator|->
name|mutex
argument_list|)
expr_stmt|;
name|me
operator|->
name|data_ready
operator|=
literal|0
expr_stmt|;
name|pthread_mutex_unlock
argument_list|(
operator|&
name|me
operator|->
name|mutex
argument_list|)
expr_stmt|;
block|}
comment|/* leave ->working 1 so that this doesn't get more work assigned */
return|return
name|NULL
return|;
block|}
end_function
begin_function
DECL|function|ll_find_deltas
specifier|static
name|void
name|ll_find_deltas
parameter_list|(
name|struct
name|object_entry
modifier|*
modifier|*
name|list
parameter_list|,
name|unsigned
name|list_size
parameter_list|,
name|int
name|window
parameter_list|,
name|int
name|depth
parameter_list|,
name|unsigned
modifier|*
name|processed
parameter_list|)
block|{
name|struct
name|thread_params
modifier|*
name|p
decl_stmt|;
name|int
name|i
decl_stmt|,
name|ret
decl_stmt|,
name|active_threads
init|=
literal|0
decl_stmt|;
name|init_threaded_search
argument_list|()
expr_stmt|;
if|if
condition|(
name|delta_search_threads
operator|<=
literal|1
condition|)
block|{
name|find_deltas
argument_list|(
name|list
argument_list|,
operator|&
name|list_size
argument_list|,
name|window
argument_list|,
name|depth
argument_list|,
name|processed
argument_list|)
expr_stmt|;
name|cleanup_threaded_search
argument_list|()
expr_stmt|;
return|return;
block|}
if|if
condition|(
name|progress
operator|>
name|pack_to_stdout
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Delta compression using up to %d threads.\n"
argument_list|,
name|delta_search_threads
argument_list|)
expr_stmt|;
name|p
operator|=
name|xcalloc
argument_list|(
name|delta_search_threads
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|p
argument_list|)
argument_list|)
expr_stmt|;
comment|/* Partition the work amongst work threads. */
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|delta_search_threads
condition|;
name|i
operator|++
control|)
block|{
name|unsigned
name|sub_size
init|=
name|list_size
operator|/
operator|(
name|delta_search_threads
operator|-
name|i
operator|)
decl_stmt|;
comment|/* don't use too small segments or no deltas will be found */
if|if
condition|(
name|sub_size
operator|<
literal|2
operator|*
name|window
operator|&&
name|i
operator|+
literal|1
operator|<
name|delta_search_threads
condition|)
name|sub_size
operator|=
literal|0
expr_stmt|;
name|p
index|[
name|i
index|]
operator|.
name|window
operator|=
name|window
expr_stmt|;
name|p
index|[
name|i
index|]
operator|.
name|depth
operator|=
name|depth
expr_stmt|;
name|p
index|[
name|i
index|]
operator|.
name|processed
operator|=
name|processed
expr_stmt|;
name|p
index|[
name|i
index|]
operator|.
name|working
operator|=
literal|1
expr_stmt|;
name|p
index|[
name|i
index|]
operator|.
name|data_ready
operator|=
literal|0
expr_stmt|;
comment|/* try to split chunks on "path" boundaries */
while|while
condition|(
name|sub_size
operator|&&
name|sub_size
operator|<
name|list_size
operator|&&
name|list
index|[
name|sub_size
index|]
operator|->
name|hash
operator|&&
name|list
index|[
name|sub_size
index|]
operator|->
name|hash
operator|==
name|list
index|[
name|sub_size
operator|-
literal|1
index|]
operator|->
name|hash
condition|)
name|sub_size
operator|++
expr_stmt|;
name|p
index|[
name|i
index|]
operator|.
name|list
operator|=
name|list
expr_stmt|;
name|p
index|[
name|i
index|]
operator|.
name|list_size
operator|=
name|sub_size
expr_stmt|;
name|p
index|[
name|i
index|]
operator|.
name|remaining
operator|=
name|sub_size
expr_stmt|;
name|list
operator|+=
name|sub_size
expr_stmt|;
name|list_size
operator|-=
name|sub_size
expr_stmt|;
block|}
comment|/* Start work threads. */
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|delta_search_threads
condition|;
name|i
operator|++
control|)
block|{
if|if
condition|(
operator|!
name|p
index|[
name|i
index|]
operator|.
name|list_size
condition|)
continue|continue;
name|pthread_mutex_init
argument_list|(
operator|&
name|p
index|[
name|i
index|]
operator|.
name|mutex
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
name|pthread_cond_init
argument_list|(
operator|&
name|p
index|[
name|i
index|]
operator|.
name|cond
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
name|ret
operator|=
name|pthread_create
argument_list|(
operator|&
name|p
index|[
name|i
index|]
operator|.
name|thread
argument_list|,
name|NULL
argument_list|,
name|threaded_find_deltas
argument_list|,
operator|&
name|p
index|[
name|i
index|]
argument_list|)
expr_stmt|;
if|if
condition|(
name|ret
condition|)
name|die
argument_list|(
literal|"unable to create thread: %s"
argument_list|,
name|strerror
argument_list|(
name|ret
argument_list|)
argument_list|)
expr_stmt|;
name|active_threads
operator|++
expr_stmt|;
block|}
comment|/* 	 * Now let's wait for work completion.  Each time a thread is done 	 * with its work, we steal half of the remaining work from the 	 * thread with the largest number of unprocessed objects and give 	 * it to that newly idle thread.  This ensure good load balancing 	 * until the remaining object list segments are simply too short 	 * to be worth splitting anymore. 	 */
while|while
condition|(
name|active_threads
condition|)
block|{
name|struct
name|thread_params
modifier|*
name|target
init|=
name|NULL
decl_stmt|;
name|struct
name|thread_params
modifier|*
name|victim
init|=
name|NULL
decl_stmt|;
name|unsigned
name|sub_size
init|=
literal|0
decl_stmt|;
name|progress_lock
argument_list|()
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
for|for
control|(
name|i
operator|=
literal|0
init|;
operator|!
name|target
operator|&&
name|i
operator|<
name|delta_search_threads
condition|;
name|i
operator|++
control|)
if|if
condition|(
operator|!
name|p
index|[
name|i
index|]
operator|.
name|working
condition|)
name|target
operator|=
operator|&
name|p
index|[
name|i
index|]
expr_stmt|;
if|if
condition|(
name|target
condition|)
break|break;
name|pthread_cond_wait
argument_list|(
operator|&
name|progress_cond
argument_list|,
operator|&
name|progress_mutex
argument_list|)
expr_stmt|;
block|}
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|delta_search_threads
condition|;
name|i
operator|++
control|)
if|if
condition|(
name|p
index|[
name|i
index|]
operator|.
name|remaining
operator|>
literal|2
operator|*
name|window
operator|&&
operator|(
operator|!
name|victim
operator|||
name|victim
operator|->
name|remaining
operator|<
name|p
index|[
name|i
index|]
operator|.
name|remaining
operator|)
condition|)
name|victim
operator|=
operator|&
name|p
index|[
name|i
index|]
expr_stmt|;
if|if
condition|(
name|victim
condition|)
block|{
name|sub_size
operator|=
name|victim
operator|->
name|remaining
operator|/
literal|2
expr_stmt|;
name|list
operator|=
name|victim
operator|->
name|list
operator|+
name|victim
operator|->
name|list_size
operator|-
name|sub_size
expr_stmt|;
while|while
condition|(
name|sub_size
operator|&&
name|list
index|[
literal|0
index|]
operator|->
name|hash
operator|&&
name|list
index|[
literal|0
index|]
operator|->
name|hash
operator|==
name|list
index|[
operator|-
literal|1
index|]
operator|->
name|hash
condition|)
block|{
name|list
operator|++
expr_stmt|;
name|sub_size
operator|--
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|sub_size
condition|)
block|{
comment|/* 				 * It is possible for some "paths" to have 				 * so many objects that no hash boundary 				 * might be found.  Let's just steal the 				 * exact half in that case. 				 */
name|sub_size
operator|=
name|victim
operator|->
name|remaining
operator|/
literal|2
expr_stmt|;
name|list
operator|-=
name|sub_size
expr_stmt|;
block|}
name|target
operator|->
name|list
operator|=
name|list
expr_stmt|;
name|victim
operator|->
name|list_size
operator|-=
name|sub_size
expr_stmt|;
name|victim
operator|->
name|remaining
operator|-=
name|sub_size
expr_stmt|;
block|}
name|target
operator|->
name|list_size
operator|=
name|sub_size
expr_stmt|;
name|target
operator|->
name|remaining
operator|=
name|sub_size
expr_stmt|;
name|target
operator|->
name|working
operator|=
literal|1
expr_stmt|;
name|progress_unlock
argument_list|()
expr_stmt|;
name|pthread_mutex_lock
argument_list|(
operator|&
name|target
operator|->
name|mutex
argument_list|)
expr_stmt|;
name|target
operator|->
name|data_ready
operator|=
literal|1
expr_stmt|;
name|pthread_cond_signal
argument_list|(
operator|&
name|target
operator|->
name|cond
argument_list|)
expr_stmt|;
name|pthread_mutex_unlock
argument_list|(
operator|&
name|target
operator|->
name|mutex
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|sub_size
condition|)
block|{
name|pthread_join
argument_list|(
name|target
operator|->
name|thread
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
name|pthread_cond_destroy
argument_list|(
operator|&
name|target
operator|->
name|cond
argument_list|)
expr_stmt|;
name|pthread_mutex_destroy
argument_list|(
operator|&
name|target
operator|->
name|mutex
argument_list|)
expr_stmt|;
name|active_threads
operator|--
expr_stmt|;
block|}
block|}
name|cleanup_threaded_search
argument_list|()
expr_stmt|;
name|free
argument_list|(
name|p
argument_list|)
expr_stmt|;
block|}
end_function
begin_else
else|#
directive|else
end_else
begin_define
DECL|macro|ll_find_deltas
define|#
directive|define
name|ll_find_deltas
parameter_list|(
name|l
parameter_list|,
name|s
parameter_list|,
name|w
parameter_list|,
name|d
parameter_list|,
name|p
parameter_list|)
value|find_deltas(l,&s, w, d, p)
end_define
begin_endif
endif|#
directive|endif
end_endif
begin_function
DECL|function|add_ref_tag
specifier|static
name|int
name|add_ref_tag
parameter_list|(
specifier|const
name|char
modifier|*
name|path
parameter_list|,
specifier|const
name|struct
name|object_id
modifier|*
name|oid
parameter_list|,
name|int
name|flag
parameter_list|,
name|void
modifier|*
name|cb_data
parameter_list|)
block|{
name|struct
name|object_id
name|peeled
decl_stmt|;
if|if
condition|(
name|starts_with
argument_list|(
name|path
argument_list|,
literal|"refs/tags/"
argument_list|)
operator|&&
comment|/* is a tag? */
operator|!
name|peel_ref
argument_list|(
name|path
argument_list|,
name|peeled
operator|.
name|hash
argument_list|)
operator|&&
comment|/* peelable? */
name|packlist_find
argument_list|(
operator|&
name|to_pack
argument_list|,
name|peeled
operator|.
name|hash
argument_list|,
name|NULL
argument_list|)
condition|)
comment|/* object packed? */
name|add_object_entry
argument_list|(
name|oid
operator|->
name|hash
argument_list|,
name|OBJ_TAG
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
end_function
begin_function
DECL|function|prepare_pack
specifier|static
name|void
name|prepare_pack
parameter_list|(
name|int
name|window
parameter_list|,
name|int
name|depth
parameter_list|)
block|{
name|struct
name|object_entry
modifier|*
modifier|*
name|delta_list
decl_stmt|;
name|uint32_t
name|i
decl_stmt|,
name|nr_deltas
decl_stmt|;
name|unsigned
name|n
decl_stmt|;
name|get_object_details
argument_list|()
expr_stmt|;
comment|/* 	 * If we're locally repacking then we need to be doubly careful 	 * from now on in order to make sure no stealth corruption gets 	 * propagated to the new pack.  Clients receiving streamed packs 	 * should validate everything they get anyway so no need to incur 	 * the additional cost here in that case. 	 */
if|if
condition|(
operator|!
name|pack_to_stdout
condition|)
name|do_check_packed_object_crc
operator|=
literal|1
expr_stmt|;
if|if
condition|(
operator|!
name|to_pack
operator|.
name|nr_objects
operator|||
operator|!
name|window
operator|||
operator|!
name|depth
condition|)
return|return;
name|ALLOC_ARRAY
argument_list|(
name|delta_list
argument_list|,
name|to_pack
operator|.
name|nr_objects
argument_list|)
expr_stmt|;
name|nr_deltas
operator|=
name|n
operator|=
literal|0
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|to_pack
operator|.
name|nr_objects
condition|;
name|i
operator|++
control|)
block|{
name|struct
name|object_entry
modifier|*
name|entry
init|=
name|to_pack
operator|.
name|objects
operator|+
name|i
decl_stmt|;
if|if
condition|(
name|entry
operator|->
name|delta
condition|)
comment|/* This happens if we decided to reuse existing 			 * delta from a pack.  "reuse_delta&&" is implied. 			 */
continue|continue;
if|if
condition|(
name|entry
operator|->
name|size
operator|<
literal|50
condition|)
continue|continue;
if|if
condition|(
name|entry
operator|->
name|no_try_delta
condition|)
continue|continue;
if|if
condition|(
operator|!
name|entry
operator|->
name|preferred_base
condition|)
block|{
name|nr_deltas
operator|++
expr_stmt|;
if|if
condition|(
name|entry
operator|->
name|type
operator|<
literal|0
condition|)
name|die
argument_list|(
literal|"unable to get type of object %s"
argument_list|,
name|sha1_to_hex
argument_list|(
name|entry
operator|->
name|idx
operator|.
name|sha1
argument_list|)
argument_list|)
expr_stmt|;
block|}
else|else
block|{
if|if
condition|(
name|entry
operator|->
name|type
operator|<
literal|0
condition|)
block|{
comment|/* 				 * This object is not found, but we 				 * don't have to include it anyway. 				 */
continue|continue;
block|}
block|}
name|delta_list
index|[
name|n
operator|++
index|]
operator|=
name|entry
expr_stmt|;
block|}
if|if
condition|(
name|nr_deltas
operator|&&
name|n
operator|>
literal|1
condition|)
block|{
name|unsigned
name|nr_done
init|=
literal|0
decl_stmt|;
if|if
condition|(
name|progress
condition|)
name|progress_state
operator|=
name|start_progress
argument_list|(
name|_
argument_list|(
literal|"Compressing objects"
argument_list|)
argument_list|,
name|nr_deltas
argument_list|)
expr_stmt|;
name|qsort
argument_list|(
name|delta_list
argument_list|,
name|n
argument_list|,
sizeof|sizeof
argument_list|(
operator|*
name|delta_list
argument_list|)
argument_list|,
name|type_size_sort
argument_list|)
expr_stmt|;
name|ll_find_deltas
argument_list|(
name|delta_list
argument_list|,
name|n
argument_list|,
name|window
operator|+
literal|1
argument_list|,
name|depth
argument_list|,
operator|&
name|nr_done
argument_list|)
expr_stmt|;
name|stop_progress
argument_list|(
operator|&
name|progress_state
argument_list|)
expr_stmt|;
if|if
condition|(
name|nr_done
operator|!=
name|nr_deltas
condition|)
name|die
argument_list|(
literal|"inconsistency with delta count"
argument_list|)
expr_stmt|;
block|}
name|free
argument_list|(
name|delta_list
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|git_pack_config
specifier|static
name|int
name|git_pack_config
parameter_list|(
specifier|const
name|char
modifier|*
name|k
parameter_list|,
specifier|const
name|char
modifier|*
name|v
parameter_list|,
name|void
modifier|*
name|cb
parameter_list|)
block|{
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|k
argument_list|,
literal|"pack.window"
argument_list|)
condition|)
block|{
name|window
operator|=
name|git_config_int
argument_list|(
name|k
argument_list|,
name|v
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|k
argument_list|,
literal|"pack.windowmemory"
argument_list|)
condition|)
block|{
name|window_memory_limit
operator|=
name|git_config_ulong
argument_list|(
name|k
argument_list|,
name|v
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|k
argument_list|,
literal|"pack.depth"
argument_list|)
condition|)
block|{
name|depth
operator|=
name|git_config_int
argument_list|(
name|k
argument_list|,
name|v
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|k
argument_list|,
literal|"pack.compression"
argument_list|)
condition|)
block|{
name|int
name|level
init|=
name|git_config_int
argument_list|(
name|k
argument_list|,
name|v
argument_list|)
decl_stmt|;
if|if
condition|(
name|level
operator|==
operator|-
literal|1
condition|)
name|level
operator|=
name|Z_DEFAULT_COMPRESSION
expr_stmt|;
elseif|else
if|if
condition|(
name|level
operator|<
literal|0
operator|||
name|level
operator|>
name|Z_BEST_COMPRESSION
condition|)
name|die
argument_list|(
literal|"bad pack compression level %d"
argument_list|,
name|level
argument_list|)
expr_stmt|;
name|pack_compression_level
operator|=
name|level
expr_stmt|;
name|pack_compression_seen
operator|=
literal|1
expr_stmt|;
return|return
literal|0
return|;
block|}
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|k
argument_list|,
literal|"pack.deltacachesize"
argument_list|)
condition|)
block|{
name|max_delta_cache_size
operator|=
name|git_config_int
argument_list|(
name|k
argument_list|,
name|v
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|k
argument_list|,
literal|"pack.deltacachelimit"
argument_list|)
condition|)
block|{
name|cache_max_small_delta_size
operator|=
name|git_config_int
argument_list|(
name|k
argument_list|,
name|v
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|k
argument_list|,
literal|"pack.writebitmaphashcache"
argument_list|)
condition|)
block|{
if|if
condition|(
name|git_config_bool
argument_list|(
name|k
argument_list|,
name|v
argument_list|)
condition|)
name|write_bitmap_options
operator||=
name|BITMAP_OPT_HASH_CACHE
expr_stmt|;
else|else
name|write_bitmap_options
operator|&=
operator|~
name|BITMAP_OPT_HASH_CACHE
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|k
argument_list|,
literal|"pack.usebitmaps"
argument_list|)
condition|)
block|{
name|use_bitmap_index
operator|=
name|git_config_bool
argument_list|(
name|k
argument_list|,
name|v
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|k
argument_list|,
literal|"pack.threads"
argument_list|)
condition|)
block|{
name|delta_search_threads
operator|=
name|git_config_int
argument_list|(
name|k
argument_list|,
name|v
argument_list|)
expr_stmt|;
if|if
condition|(
name|delta_search_threads
operator|<
literal|0
condition|)
name|die
argument_list|(
literal|"invalid number of threads specified (%d)"
argument_list|,
name|delta_search_threads
argument_list|)
expr_stmt|;
ifdef|#
directive|ifdef
name|NO_PTHREADS
if|if
condition|(
name|delta_search_threads
operator|!=
literal|1
condition|)
name|warning
argument_list|(
literal|"no threads support, ignoring %s"
argument_list|,
name|k
argument_list|)
expr_stmt|;
endif|#
directive|endif
return|return
literal|0
return|;
block|}
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|k
argument_list|,
literal|"pack.indexversion"
argument_list|)
condition|)
block|{
name|pack_idx_opts
operator|.
name|version
operator|=
name|git_config_int
argument_list|(
name|k
argument_list|,
name|v
argument_list|)
expr_stmt|;
if|if
condition|(
name|pack_idx_opts
operator|.
name|version
operator|>
literal|2
condition|)
name|die
argument_list|(
literal|"bad pack.indexversion=%"
name|PRIu32
argument_list|,
name|pack_idx_opts
operator|.
name|version
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
return|return
name|git_default_config
argument_list|(
name|k
argument_list|,
name|v
argument_list|,
name|cb
argument_list|)
return|;
block|}
end_function
begin_function
DECL|function|read_object_list_from_stdin
specifier|static
name|void
name|read_object_list_from_stdin
parameter_list|(
name|void
parameter_list|)
block|{
name|char
name|line
index|[
literal|40
operator|+
literal|1
operator|+
name|PATH_MAX
operator|+
literal|2
index|]
decl_stmt|;
name|unsigned
name|char
name|sha1
index|[
literal|20
index|]
decl_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
if|if
condition|(
operator|!
name|fgets
argument_list|(
name|line
argument_list|,
sizeof|sizeof
argument_list|(
name|line
argument_list|)
argument_list|,
name|stdin
argument_list|)
condition|)
block|{
if|if
condition|(
name|feof
argument_list|(
name|stdin
argument_list|)
condition|)
break|break;
if|if
condition|(
operator|!
name|ferror
argument_list|(
name|stdin
argument_list|)
condition|)
name|die
argument_list|(
literal|"fgets returned NULL, not EOF, not error!"
argument_list|)
expr_stmt|;
if|if
condition|(
name|errno
operator|!=
name|EINTR
condition|)
name|die_errno
argument_list|(
literal|"fgets"
argument_list|)
expr_stmt|;
name|clearerr
argument_list|(
name|stdin
argument_list|)
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
name|line
index|[
literal|0
index|]
operator|==
literal|'-'
condition|)
block|{
if|if
condition|(
name|get_sha1_hex
argument_list|(
name|line
operator|+
literal|1
argument_list|,
name|sha1
argument_list|)
condition|)
name|die
argument_list|(
literal|"expected edge sha1, got garbage:\n %s"
argument_list|,
name|line
argument_list|)
expr_stmt|;
name|add_preferred_base
argument_list|(
name|sha1
argument_list|)
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
name|get_sha1_hex
argument_list|(
name|line
argument_list|,
name|sha1
argument_list|)
condition|)
name|die
argument_list|(
literal|"expected sha1, got garbage:\n %s"
argument_list|,
name|line
argument_list|)
expr_stmt|;
name|add_preferred_base_object
argument_list|(
name|line
operator|+
literal|41
argument_list|)
expr_stmt|;
name|add_object_entry
argument_list|(
name|sha1
argument_list|,
literal|0
argument_list|,
name|line
operator|+
literal|41
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
block|}
end_function
begin_define
DECL|macro|OBJECT_ADDED
define|#
directive|define
name|OBJECT_ADDED
value|(1u<<20)
end_define
begin_function
DECL|function|show_commit
specifier|static
name|void
name|show_commit
parameter_list|(
name|struct
name|commit
modifier|*
name|commit
parameter_list|,
name|void
modifier|*
name|data
parameter_list|)
block|{
name|add_object_entry
argument_list|(
name|commit
operator|->
name|object
operator|.
name|oid
operator|.
name|hash
argument_list|,
name|OBJ_COMMIT
argument_list|,
name|NULL
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|commit
operator|->
name|object
operator|.
name|flags
operator||=
name|OBJECT_ADDED
expr_stmt|;
if|if
condition|(
name|write_bitmap_index
condition|)
name|index_commit_for_bitmap
argument_list|(
name|commit
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|show_object
specifier|static
name|void
name|show_object
parameter_list|(
name|struct
name|object
modifier|*
name|obj
parameter_list|,
specifier|const
name|char
modifier|*
name|name
parameter_list|,
name|void
modifier|*
name|data
parameter_list|)
block|{
name|add_preferred_base_object
argument_list|(
name|name
argument_list|)
expr_stmt|;
name|add_object_entry
argument_list|(
name|obj
operator|->
name|oid
operator|.
name|hash
argument_list|,
name|obj
operator|->
name|type
argument_list|,
name|name
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|obj
operator|->
name|flags
operator||=
name|OBJECT_ADDED
expr_stmt|;
block|}
end_function
begin_function
DECL|function|show_edge
specifier|static
name|void
name|show_edge
parameter_list|(
name|struct
name|commit
modifier|*
name|commit
parameter_list|)
block|{
name|add_preferred_base
argument_list|(
name|commit
operator|->
name|object
operator|.
name|oid
operator|.
name|hash
argument_list|)
expr_stmt|;
block|}
end_function
begin_struct
DECL|struct|in_pack_object
struct|struct
name|in_pack_object
block|{
DECL|member|offset
name|off_t
name|offset
decl_stmt|;
DECL|member|object
name|struct
name|object
modifier|*
name|object
decl_stmt|;
block|}
struct|;
end_struct
begin_struct
DECL|struct|in_pack
struct|struct
name|in_pack
block|{
DECL|member|alloc
name|int
name|alloc
decl_stmt|;
DECL|member|nr
name|int
name|nr
decl_stmt|;
DECL|member|array
name|struct
name|in_pack_object
modifier|*
name|array
decl_stmt|;
block|}
struct|;
end_struct
begin_function
DECL|function|mark_in_pack_object
specifier|static
name|void
name|mark_in_pack_object
parameter_list|(
name|struct
name|object
modifier|*
name|object
parameter_list|,
name|struct
name|packed_git
modifier|*
name|p
parameter_list|,
name|struct
name|in_pack
modifier|*
name|in_pack
parameter_list|)
block|{
name|in_pack
operator|->
name|array
index|[
name|in_pack
operator|->
name|nr
index|]
operator|.
name|offset
operator|=
name|find_pack_entry_one
argument_list|(
name|object
operator|->
name|oid
operator|.
name|hash
argument_list|,
name|p
argument_list|)
expr_stmt|;
name|in_pack
operator|->
name|array
index|[
name|in_pack
operator|->
name|nr
index|]
operator|.
name|object
operator|=
name|object
expr_stmt|;
name|in_pack
operator|->
name|nr
operator|++
expr_stmt|;
block|}
end_function
begin_comment
comment|/*  * Compare the objects in the offset order, in order to emulate the  * "git rev-list --objects" output that produced the pack originally.  */
end_comment
begin_function
DECL|function|ofscmp
specifier|static
name|int
name|ofscmp
parameter_list|(
specifier|const
name|void
modifier|*
name|a_
parameter_list|,
specifier|const
name|void
modifier|*
name|b_
parameter_list|)
block|{
name|struct
name|in_pack_object
modifier|*
name|a
init|=
operator|(
expr|struct
name|in_pack_object
operator|*
operator|)
name|a_
decl_stmt|;
name|struct
name|in_pack_object
modifier|*
name|b
init|=
operator|(
expr|struct
name|in_pack_object
operator|*
operator|)
name|b_
decl_stmt|;
if|if
condition|(
name|a
operator|->
name|offset
operator|<
name|b
operator|->
name|offset
condition|)
return|return
operator|-
literal|1
return|;
elseif|else
if|if
condition|(
name|a
operator|->
name|offset
operator|>
name|b
operator|->
name|offset
condition|)
return|return
literal|1
return|;
else|else
return|return
name|oidcmp
argument_list|(
operator|&
name|a
operator|->
name|object
operator|->
name|oid
argument_list|,
operator|&
name|b
operator|->
name|object
operator|->
name|oid
argument_list|)
return|;
block|}
end_function
begin_function
DECL|function|add_objects_in_unpacked_packs
specifier|static
name|void
name|add_objects_in_unpacked_packs
parameter_list|(
name|struct
name|rev_info
modifier|*
name|revs
parameter_list|)
block|{
name|struct
name|packed_git
modifier|*
name|p
decl_stmt|;
name|struct
name|in_pack
name|in_pack
decl_stmt|;
name|uint32_t
name|i
decl_stmt|;
name|memset
argument_list|(
operator|&
name|in_pack
argument_list|,
literal|0
argument_list|,
sizeof|sizeof
argument_list|(
name|in_pack
argument_list|)
argument_list|)
expr_stmt|;
for|for
control|(
name|p
operator|=
name|packed_git
init|;
name|p
condition|;
name|p
operator|=
name|p
operator|->
name|next
control|)
block|{
specifier|const
name|unsigned
name|char
modifier|*
name|sha1
decl_stmt|;
name|struct
name|object
modifier|*
name|o
decl_stmt|;
if|if
condition|(
operator|!
name|p
operator|->
name|pack_local
operator|||
name|p
operator|->
name|pack_keep
condition|)
continue|continue;
if|if
condition|(
name|open_pack_index
argument_list|(
name|p
argument_list|)
condition|)
name|die
argument_list|(
literal|"cannot open pack index"
argument_list|)
expr_stmt|;
name|ALLOC_GROW
argument_list|(
name|in_pack
operator|.
name|array
argument_list|,
name|in_pack
operator|.
name|nr
operator|+
name|p
operator|->
name|num_objects
argument_list|,
name|in_pack
operator|.
name|alloc
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|p
operator|->
name|num_objects
condition|;
name|i
operator|++
control|)
block|{
name|sha1
operator|=
name|nth_packed_object_sha1
argument_list|(
name|p
argument_list|,
name|i
argument_list|)
expr_stmt|;
name|o
operator|=
name|lookup_unknown_object
argument_list|(
name|sha1
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
operator|(
name|o
operator|->
name|flags
operator|&
name|OBJECT_ADDED
operator|)
condition|)
name|mark_in_pack_object
argument_list|(
name|o
argument_list|,
name|p
argument_list|,
operator|&
name|in_pack
argument_list|)
expr_stmt|;
name|o
operator|->
name|flags
operator||=
name|OBJECT_ADDED
expr_stmt|;
block|}
block|}
if|if
condition|(
name|in_pack
operator|.
name|nr
condition|)
block|{
name|qsort
argument_list|(
name|in_pack
operator|.
name|array
argument_list|,
name|in_pack
operator|.
name|nr
argument_list|,
sizeof|sizeof
argument_list|(
name|in_pack
operator|.
name|array
index|[
literal|0
index|]
argument_list|)
argument_list|,
name|ofscmp
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|in_pack
operator|.
name|nr
condition|;
name|i
operator|++
control|)
block|{
name|struct
name|object
modifier|*
name|o
init|=
name|in_pack
operator|.
name|array
index|[
name|i
index|]
operator|.
name|object
decl_stmt|;
name|add_object_entry
argument_list|(
name|o
operator|->
name|oid
operator|.
name|hash
argument_list|,
name|o
operator|->
name|type
argument_list|,
literal|""
argument_list|,
literal|0
argument_list|)
expr_stmt|;
block|}
block|}
name|free
argument_list|(
name|in_pack
operator|.
name|array
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|add_loose_object
specifier|static
name|int
name|add_loose_object
parameter_list|(
specifier|const
name|unsigned
name|char
modifier|*
name|sha1
parameter_list|,
specifier|const
name|char
modifier|*
name|path
parameter_list|,
name|void
modifier|*
name|data
parameter_list|)
block|{
name|enum
name|object_type
name|type
init|=
name|sha1_object_info
argument_list|(
name|sha1
argument_list|,
name|NULL
argument_list|)
decl_stmt|;
if|if
condition|(
name|type
operator|<
literal|0
condition|)
block|{
name|warning
argument_list|(
literal|"loose object at %s could not be examined"
argument_list|,
name|path
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
name|add_object_entry
argument_list|(
name|sha1
argument_list|,
name|type
argument_list|,
literal|""
argument_list|,
literal|0
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
end_function
begin_comment
comment|/*  * We actually don't even have to worry about reachability here.  * add_object_entry will weed out duplicates, so we just add every  * loose object we find.  */
end_comment
begin_function
DECL|function|add_unreachable_loose_objects
specifier|static
name|void
name|add_unreachable_loose_objects
parameter_list|(
name|void
parameter_list|)
block|{
name|for_each_loose_file_in_objdir
argument_list|(
name|get_object_directory
argument_list|()
argument_list|,
name|add_loose_object
argument_list|,
name|NULL
argument_list|,
name|NULL
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|has_sha1_pack_kept_or_nonlocal
specifier|static
name|int
name|has_sha1_pack_kept_or_nonlocal
parameter_list|(
specifier|const
name|unsigned
name|char
modifier|*
name|sha1
parameter_list|)
block|{
specifier|static
name|struct
name|packed_git
modifier|*
name|last_found
init|=
operator|(
name|void
operator|*
operator|)
literal|1
decl_stmt|;
name|struct
name|packed_git
modifier|*
name|p
decl_stmt|;
name|p
operator|=
operator|(
name|last_found
operator|!=
operator|(
name|void
operator|*
operator|)
literal|1
operator|)
condition|?
name|last_found
else|:
name|packed_git
expr_stmt|;
while|while
condition|(
name|p
condition|)
block|{
if|if
condition|(
operator|(
operator|!
name|p
operator|->
name|pack_local
operator|||
name|p
operator|->
name|pack_keep
operator|)
operator|&&
name|find_pack_entry_one
argument_list|(
name|sha1
argument_list|,
name|p
argument_list|)
condition|)
block|{
name|last_found
operator|=
name|p
expr_stmt|;
return|return
literal|1
return|;
block|}
if|if
condition|(
name|p
operator|==
name|last_found
condition|)
name|p
operator|=
name|packed_git
expr_stmt|;
else|else
name|p
operator|=
name|p
operator|->
name|next
expr_stmt|;
if|if
condition|(
name|p
operator|==
name|last_found
condition|)
name|p
operator|=
name|p
operator|->
name|next
expr_stmt|;
block|}
return|return
literal|0
return|;
block|}
end_function
begin_comment
comment|/*  * Store a list of sha1s that are should not be discarded  * because they are either written too recently, or are  * reachable from another object that was.  *  * This is filled by get_object_list.  */
end_comment
begin_decl_stmt
DECL|variable|recent_objects
specifier|static
name|struct
name|sha1_array
name|recent_objects
decl_stmt|;
end_decl_stmt
begin_function
DECL|function|loosened_object_can_be_discarded
specifier|static
name|int
name|loosened_object_can_be_discarded
parameter_list|(
specifier|const
name|unsigned
name|char
modifier|*
name|sha1
parameter_list|,
name|unsigned
name|long
name|mtime
parameter_list|)
block|{
if|if
condition|(
operator|!
name|unpack_unreachable_expiration
condition|)
return|return
literal|0
return|;
if|if
condition|(
name|mtime
operator|>
name|unpack_unreachable_expiration
condition|)
return|return
literal|0
return|;
if|if
condition|(
name|sha1_array_lookup
argument_list|(
operator|&
name|recent_objects
argument_list|,
name|sha1
argument_list|)
operator|>=
literal|0
condition|)
return|return
literal|0
return|;
return|return
literal|1
return|;
block|}
end_function
begin_function
DECL|function|loosen_unused_packed_objects
specifier|static
name|void
name|loosen_unused_packed_objects
parameter_list|(
name|struct
name|rev_info
modifier|*
name|revs
parameter_list|)
block|{
name|struct
name|packed_git
modifier|*
name|p
decl_stmt|;
name|uint32_t
name|i
decl_stmt|;
specifier|const
name|unsigned
name|char
modifier|*
name|sha1
decl_stmt|;
for|for
control|(
name|p
operator|=
name|packed_git
init|;
name|p
condition|;
name|p
operator|=
name|p
operator|->
name|next
control|)
block|{
if|if
condition|(
operator|!
name|p
operator|->
name|pack_local
operator|||
name|p
operator|->
name|pack_keep
condition|)
continue|continue;
if|if
condition|(
name|open_pack_index
argument_list|(
name|p
argument_list|)
condition|)
name|die
argument_list|(
literal|"cannot open pack index"
argument_list|)
expr_stmt|;
for|for
control|(
name|i
operator|=
literal|0
init|;
name|i
operator|<
name|p
operator|->
name|num_objects
condition|;
name|i
operator|++
control|)
block|{
name|sha1
operator|=
name|nth_packed_object_sha1
argument_list|(
name|p
argument_list|,
name|i
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|packlist_find
argument_list|(
operator|&
name|to_pack
argument_list|,
name|sha1
argument_list|,
name|NULL
argument_list|)
operator|&&
operator|!
name|has_sha1_pack_kept_or_nonlocal
argument_list|(
name|sha1
argument_list|)
operator|&&
operator|!
name|loosened_object_can_be_discarded
argument_list|(
name|sha1
argument_list|,
name|p
operator|->
name|mtime
argument_list|)
condition|)
if|if
condition|(
name|force_object_loose
argument_list|(
name|sha1
argument_list|,
name|p
operator|->
name|mtime
argument_list|)
condition|)
name|die
argument_list|(
literal|"unable to force loose object"
argument_list|)
expr_stmt|;
block|}
block|}
block|}
end_function
begin_comment
comment|/*  * This tracks any options which a reader of the pack might  * not understand, and which would therefore prevent blind reuse  * of what we have on disk.  */
end_comment
begin_function
DECL|function|pack_options_allow_reuse
specifier|static
name|int
name|pack_options_allow_reuse
parameter_list|(
name|void
parameter_list|)
block|{
return|return
name|allow_ofs_delta
return|;
block|}
end_function
begin_function
DECL|function|get_object_list_from_bitmap
specifier|static
name|int
name|get_object_list_from_bitmap
parameter_list|(
name|struct
name|rev_info
modifier|*
name|revs
parameter_list|)
block|{
if|if
condition|(
name|prepare_bitmap_walk
argument_list|(
name|revs
argument_list|)
operator|<
literal|0
condition|)
return|return
operator|-
literal|1
return|;
if|if
condition|(
name|pack_options_allow_reuse
argument_list|()
operator|&&
operator|!
name|reuse_partial_packfile_from_bitmap
argument_list|(
operator|&
name|reuse_packfile
argument_list|,
operator|&
name|reuse_packfile_objects
argument_list|,
operator|&
name|reuse_packfile_offset
argument_list|)
condition|)
block|{
name|assert
argument_list|(
name|reuse_packfile_objects
argument_list|)
expr_stmt|;
name|nr_result
operator|+=
name|reuse_packfile_objects
expr_stmt|;
name|display_progress
argument_list|(
name|progress_state
argument_list|,
name|nr_result
argument_list|)
expr_stmt|;
block|}
name|traverse_bitmap_commit_list
argument_list|(
operator|&
name|add_object_entry_from_bitmap
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
end_function
begin_function
DECL|function|record_recent_object
specifier|static
name|void
name|record_recent_object
parameter_list|(
name|struct
name|object
modifier|*
name|obj
parameter_list|,
specifier|const
name|char
modifier|*
name|name
parameter_list|,
name|void
modifier|*
name|data
parameter_list|)
block|{
name|sha1_array_append
argument_list|(
operator|&
name|recent_objects
argument_list|,
name|obj
operator|->
name|oid
operator|.
name|hash
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|record_recent_commit
specifier|static
name|void
name|record_recent_commit
parameter_list|(
name|struct
name|commit
modifier|*
name|commit
parameter_list|,
name|void
modifier|*
name|data
parameter_list|)
block|{
name|sha1_array_append
argument_list|(
operator|&
name|recent_objects
argument_list|,
name|commit
operator|->
name|object
operator|.
name|oid
operator|.
name|hash
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|get_object_list
specifier|static
name|void
name|get_object_list
parameter_list|(
name|int
name|ac
parameter_list|,
specifier|const
name|char
modifier|*
modifier|*
name|av
parameter_list|)
block|{
name|struct
name|rev_info
name|revs
decl_stmt|;
name|char
name|line
index|[
literal|1000
index|]
decl_stmt|;
name|int
name|flags
init|=
literal|0
decl_stmt|;
name|init_revisions
argument_list|(
operator|&
name|revs
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
name|save_commit_buffer
operator|=
literal|0
expr_stmt|;
name|setup_revisions
argument_list|(
name|ac
argument_list|,
name|av
argument_list|,
operator|&
name|revs
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
comment|/* make sure shallows are read */
name|is_repository_shallow
argument_list|()
expr_stmt|;
while|while
condition|(
name|fgets
argument_list|(
name|line
argument_list|,
sizeof|sizeof
argument_list|(
name|line
argument_list|)
argument_list|,
name|stdin
argument_list|)
operator|!=
name|NULL
condition|)
block|{
name|int
name|len
init|=
name|strlen
argument_list|(
name|line
argument_list|)
decl_stmt|;
if|if
condition|(
name|len
operator|&&
name|line
index|[
name|len
operator|-
literal|1
index|]
operator|==
literal|'\n'
condition|)
name|line
index|[
operator|--
name|len
index|]
operator|=
literal|0
expr_stmt|;
if|if
condition|(
operator|!
name|len
condition|)
break|break;
if|if
condition|(
operator|*
name|line
operator|==
literal|'-'
condition|)
block|{
if|if
condition|(
operator|!
name|strcmp
argument_list|(
name|line
argument_list|,
literal|"--not"
argument_list|)
condition|)
block|{
name|flags
operator|^=
name|UNINTERESTING
expr_stmt|;
name|write_bitmap_index
operator|=
literal|0
expr_stmt|;
continue|continue;
block|}
if|if
condition|(
name|starts_with
argument_list|(
name|line
argument_list|,
literal|"--shallow "
argument_list|)
condition|)
block|{
name|unsigned
name|char
name|sha1
index|[
literal|20
index|]
decl_stmt|;
if|if
condition|(
name|get_sha1_hex
argument_list|(
name|line
operator|+
literal|10
argument_list|,
name|sha1
argument_list|)
condition|)
name|die
argument_list|(
literal|"not an SHA-1 '%s'"
argument_list|,
name|line
operator|+
literal|10
argument_list|)
expr_stmt|;
name|register_shallow
argument_list|(
name|sha1
argument_list|)
expr_stmt|;
name|use_bitmap_index
operator|=
literal|0
expr_stmt|;
continue|continue;
block|}
name|die
argument_list|(
literal|"not a rev '%s'"
argument_list|,
name|line
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|handle_revision_arg
argument_list|(
name|line
argument_list|,
operator|&
name|revs
argument_list|,
name|flags
argument_list|,
name|REVARG_CANNOT_BE_FILENAME
argument_list|)
condition|)
name|die
argument_list|(
literal|"bad revision '%s'"
argument_list|,
name|line
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|use_bitmap_index
operator|&&
operator|!
name|get_object_list_from_bitmap
argument_list|(
operator|&
name|revs
argument_list|)
condition|)
return|return;
if|if
condition|(
name|prepare_revision_walk
argument_list|(
operator|&
name|revs
argument_list|)
condition|)
name|die
argument_list|(
literal|"revision walk setup failed"
argument_list|)
expr_stmt|;
name|mark_edges_uninteresting
argument_list|(
operator|&
name|revs
argument_list|,
name|show_edge
argument_list|)
expr_stmt|;
name|traverse_commit_list
argument_list|(
operator|&
name|revs
argument_list|,
name|show_commit
argument_list|,
name|show_object
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
if|if
condition|(
name|unpack_unreachable_expiration
condition|)
block|{
name|revs
operator|.
name|ignore_missing_links
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|add_unseen_recent_objects_to_traversal
argument_list|(
operator|&
name|revs
argument_list|,
name|unpack_unreachable_expiration
argument_list|)
condition|)
name|die
argument_list|(
literal|"unable to add recent objects"
argument_list|)
expr_stmt|;
if|if
condition|(
name|prepare_revision_walk
argument_list|(
operator|&
name|revs
argument_list|)
condition|)
name|die
argument_list|(
literal|"revision walk setup failed"
argument_list|)
expr_stmt|;
name|traverse_commit_list
argument_list|(
operator|&
name|revs
argument_list|,
name|record_recent_commit
argument_list|,
name|record_recent_object
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|keep_unreachable
condition|)
name|add_objects_in_unpacked_packs
argument_list|(
operator|&
name|revs
argument_list|)
expr_stmt|;
if|if
condition|(
name|pack_loose_unreachable
condition|)
name|add_unreachable_loose_objects
argument_list|()
expr_stmt|;
if|if
condition|(
name|unpack_unreachable
condition|)
name|loosen_unused_packed_objects
argument_list|(
operator|&
name|revs
argument_list|)
expr_stmt|;
name|sha1_array_clear
argument_list|(
operator|&
name|recent_objects
argument_list|)
expr_stmt|;
block|}
end_function
begin_function
DECL|function|option_parse_index_version
specifier|static
name|int
name|option_parse_index_version
parameter_list|(
specifier|const
name|struct
name|option
modifier|*
name|opt
parameter_list|,
specifier|const
name|char
modifier|*
name|arg
parameter_list|,
name|int
name|unset
parameter_list|)
block|{
name|char
modifier|*
name|c
decl_stmt|;
specifier|const
name|char
modifier|*
name|val
init|=
name|arg
decl_stmt|;
name|pack_idx_opts
operator|.
name|version
operator|=
name|strtoul
argument_list|(
name|val
argument_list|,
operator|&
name|c
argument_list|,
literal|10
argument_list|)
expr_stmt|;
if|if
condition|(
name|pack_idx_opts
operator|.
name|version
operator|>
literal|2
condition|)
name|die
argument_list|(
name|_
argument_list|(
literal|"unsupported index version %s"
argument_list|)
argument_list|,
name|val
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|c
operator|==
literal|','
operator|&&
name|c
index|[
literal|1
index|]
condition|)
name|pack_idx_opts
operator|.
name|off32_limit
operator|=
name|strtoul
argument_list|(
name|c
operator|+
literal|1
argument_list|,
operator|&
name|c
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
operator|*
name|c
operator|||
name|pack_idx_opts
operator|.
name|off32_limit
operator|&
literal|0x80000000
condition|)
name|die
argument_list|(
name|_
argument_list|(
literal|"bad index version '%s'"
argument_list|)
argument_list|,
name|val
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
end_function
begin_function
DECL|function|option_parse_unpack_unreachable
specifier|static
name|int
name|option_parse_unpack_unreachable
parameter_list|(
specifier|const
name|struct
name|option
modifier|*
name|opt
parameter_list|,
specifier|const
name|char
modifier|*
name|arg
parameter_list|,
name|int
name|unset
parameter_list|)
block|{
if|if
condition|(
name|unset
condition|)
block|{
name|unpack_unreachable
operator|=
literal|0
expr_stmt|;
name|unpack_unreachable_expiration
operator|=
literal|0
expr_stmt|;
block|}
else|else
block|{
name|unpack_unreachable
operator|=
literal|1
expr_stmt|;
if|if
condition|(
name|arg
condition|)
name|unpack_unreachable_expiration
operator|=
name|approxidate
argument_list|(
name|arg
argument_list|)
expr_stmt|;
block|}
return|return
literal|0
return|;
block|}
end_function
begin_function
DECL|function|cmd_pack_objects
name|int
name|cmd_pack_objects
parameter_list|(
name|int
name|argc
parameter_list|,
specifier|const
name|char
modifier|*
modifier|*
name|argv
parameter_list|,
specifier|const
name|char
modifier|*
name|prefix
parameter_list|)
block|{
name|int
name|use_internal_rev_list
init|=
literal|0
decl_stmt|;
name|int
name|thin
init|=
literal|0
decl_stmt|;
name|int
name|shallow
init|=
literal|0
decl_stmt|;
name|int
name|all_progress_implied
init|=
literal|0
decl_stmt|;
name|struct
name|argv_array
name|rp
init|=
name|ARGV_ARRAY_INIT
decl_stmt|;
name|int
name|rev_list_unpacked
init|=
literal|0
decl_stmt|,
name|rev_list_all
init|=
literal|0
decl_stmt|,
name|rev_list_reflog
init|=
literal|0
decl_stmt|;
name|int
name|rev_list_index
init|=
literal|0
decl_stmt|;
name|struct
name|option
name|pack_objects_options
index|[]
init|=
block|{
name|OPT_SET_INT
argument_list|(
literal|'q'
argument_list|,
literal|"quiet"
argument_list|,
operator|&
name|progress
argument_list|,
name|N_
argument_list|(
literal|"do not show progress meter"
argument_list|)
argument_list|,
literal|0
argument_list|)
block|,
name|OPT_SET_INT
argument_list|(
literal|0
argument_list|,
literal|"progress"
argument_list|,
operator|&
name|progress
argument_list|,
name|N_
argument_list|(
literal|"show progress meter"
argument_list|)
argument_list|,
literal|1
argument_list|)
block|,
name|OPT_SET_INT
argument_list|(
literal|0
argument_list|,
literal|"all-progress"
argument_list|,
operator|&
name|progress
argument_list|,
name|N_
argument_list|(
literal|"show progress meter during object writing phase"
argument_list|)
argument_list|,
literal|2
argument_list|)
block|,
name|OPT_BOOL
argument_list|(
literal|0
argument_list|,
literal|"all-progress-implied"
argument_list|,
operator|&
name|all_progress_implied
argument_list|,
name|N_
argument_list|(
literal|"similar to --all-progress when progress meter is shown"
argument_list|)
argument_list|)
block|,
block|{
name|OPTION_CALLBACK
block|,
literal|0
block|,
literal|"index-version"
block|,
name|NULL
block|,
name|N_
argument_list|(
literal|"version[,offset]"
argument_list|)
block|,
name|N_
argument_list|(
literal|"write the pack index file in the specified idx format version"
argument_list|)
block|,
literal|0
block|,
name|option_parse_index_version
block|}
block|,
name|OPT_MAGNITUDE
argument_list|(
literal|0
argument_list|,
literal|"max-pack-size"
argument_list|,
operator|&
name|pack_size_limit
argument_list|,
name|N_
argument_list|(
literal|"maximum size of each output pack file"
argument_list|)
argument_list|)
block|,
name|OPT_BOOL
argument_list|(
literal|0
argument_list|,
literal|"local"
argument_list|,
operator|&
name|local
argument_list|,
name|N_
argument_list|(
literal|"ignore borrowed objects from alternate object store"
argument_list|)
argument_list|)
block|,
name|OPT_BOOL
argument_list|(
literal|0
argument_list|,
literal|"incremental"
argument_list|,
operator|&
name|incremental
argument_list|,
name|N_
argument_list|(
literal|"ignore packed objects"
argument_list|)
argument_list|)
block|,
name|OPT_INTEGER
argument_list|(
literal|0
argument_list|,
literal|"window"
argument_list|,
operator|&
name|window
argument_list|,
name|N_
argument_list|(
literal|"limit pack window by objects"
argument_list|)
argument_list|)
block|,
name|OPT_MAGNITUDE
argument_list|(
literal|0
argument_list|,
literal|"window-memory"
argument_list|,
operator|&
name|window_memory_limit
argument_list|,
name|N_
argument_list|(
literal|"limit pack window by memory in addition to object limit"
argument_list|)
argument_list|)
block|,
name|OPT_INTEGER
argument_list|(
literal|0
argument_list|,
literal|"depth"
argument_list|,
operator|&
name|depth
argument_list|,
name|N_
argument_list|(
literal|"maximum length of delta chain allowed in the resulting pack"
argument_list|)
argument_list|)
block|,
name|OPT_BOOL
argument_list|(
literal|0
argument_list|,
literal|"reuse-delta"
argument_list|,
operator|&
name|reuse_delta
argument_list|,
name|N_
argument_list|(
literal|"reuse existing deltas"
argument_list|)
argument_list|)
block|,
name|OPT_BOOL
argument_list|(
literal|0
argument_list|,
literal|"reuse-object"
argument_list|,
operator|&
name|reuse_object
argument_list|,
name|N_
argument_list|(
literal|"reuse existing objects"
argument_list|)
argument_list|)
block|,
name|OPT_BOOL
argument_list|(
literal|0
argument_list|,
literal|"delta-base-offset"
argument_list|,
operator|&
name|allow_ofs_delta
argument_list|,
name|N_
argument_list|(
literal|"use OFS_DELTA objects"
argument_list|)
argument_list|)
block|,
name|OPT_INTEGER
argument_list|(
literal|0
argument_list|,
literal|"threads"
argument_list|,
operator|&
name|delta_search_threads
argument_list|,
name|N_
argument_list|(
literal|"use threads when searching for best delta matches"
argument_list|)
argument_list|)
block|,
name|OPT_BOOL
argument_list|(
literal|0
argument_list|,
literal|"non-empty"
argument_list|,
operator|&
name|non_empty
argument_list|,
name|N_
argument_list|(
literal|"do not create an empty pack output"
argument_list|)
argument_list|)
block|,
name|OPT_BOOL
argument_list|(
literal|0
argument_list|,
literal|"revs"
argument_list|,
operator|&
name|use_internal_rev_list
argument_list|,
name|N_
argument_list|(
literal|"read revision arguments from standard input"
argument_list|)
argument_list|)
block|,
block|{
name|OPTION_SET_INT
block|,
literal|0
block|,
literal|"unpacked"
block|,
operator|&
name|rev_list_unpacked
block|,
name|NULL
block|,
name|N_
argument_list|(
literal|"limit the objects to those that are not yet packed"
argument_list|)
block|,
name|PARSE_OPT_NOARG
operator||
name|PARSE_OPT_NONEG
block|,
name|NULL
block|,
literal|1
block|}
block|,
block|{
name|OPTION_SET_INT
block|,
literal|0
block|,
literal|"all"
block|,
operator|&
name|rev_list_all
block|,
name|NULL
block|,
name|N_
argument_list|(
literal|"include objects reachable from any reference"
argument_list|)
block|,
name|PARSE_OPT_NOARG
operator||
name|PARSE_OPT_NONEG
block|,
name|NULL
block|,
literal|1
block|}
block|,
block|{
name|OPTION_SET_INT
block|,
literal|0
block|,
literal|"reflog"
block|,
operator|&
name|rev_list_reflog
block|,
name|NULL
block|,
name|N_
argument_list|(
literal|"include objects referred by reflog entries"
argument_list|)
block|,
name|PARSE_OPT_NOARG
operator||
name|PARSE_OPT_NONEG
block|,
name|NULL
block|,
literal|1
block|}
block|,
block|{
name|OPTION_SET_INT
block|,
literal|0
block|,
literal|"indexed-objects"
block|,
operator|&
name|rev_list_index
block|,
name|NULL
block|,
name|N_
argument_list|(
literal|"include objects referred to by the index"
argument_list|)
block|,
name|PARSE_OPT_NOARG
operator||
name|PARSE_OPT_NONEG
block|,
name|NULL
block|,
literal|1
block|}
block|,
name|OPT_BOOL
argument_list|(
literal|0
argument_list|,
literal|"stdout"
argument_list|,
operator|&
name|pack_to_stdout
argument_list|,
name|N_
argument_list|(
literal|"output pack to stdout"
argument_list|)
argument_list|)
block|,
name|OPT_BOOL
argument_list|(
literal|0
argument_list|,
literal|"include-tag"
argument_list|,
operator|&
name|include_tag
argument_list|,
name|N_
argument_list|(
literal|"include tag objects that refer to objects to be packed"
argument_list|)
argument_list|)
block|,
name|OPT_BOOL
argument_list|(
literal|0
argument_list|,
literal|"keep-unreachable"
argument_list|,
operator|&
name|keep_unreachable
argument_list|,
name|N_
argument_list|(
literal|"keep unreachable objects"
argument_list|)
argument_list|)
block|,
name|OPT_BOOL
argument_list|(
literal|0
argument_list|,
literal|"pack-loose-unreachable"
argument_list|,
operator|&
name|pack_loose_unreachable
argument_list|,
name|N_
argument_list|(
literal|"pack loose unreachable objects"
argument_list|)
argument_list|)
block|,
block|{
name|OPTION_CALLBACK
block|,
literal|0
block|,
literal|"unpack-unreachable"
block|,
name|NULL
block|,
name|N_
argument_list|(
literal|"time"
argument_list|)
block|,
name|N_
argument_list|(
literal|"unpack unreachable objects newer than<time>"
argument_list|)
block|,
name|PARSE_OPT_OPTARG
block|,
name|option_parse_unpack_unreachable
block|}
block|,
name|OPT_BOOL
argument_list|(
literal|0
argument_list|,
literal|"thin"
argument_list|,
operator|&
name|thin
argument_list|,
name|N_
argument_list|(
literal|"create thin packs"
argument_list|)
argument_list|)
block|,
name|OPT_BOOL
argument_list|(
literal|0
argument_list|,
literal|"shallow"
argument_list|,
operator|&
name|shallow
argument_list|,
name|N_
argument_list|(
literal|"create packs suitable for shallow fetches"
argument_list|)
argument_list|)
block|,
name|OPT_BOOL
argument_list|(
literal|0
argument_list|,
literal|"honor-pack-keep"
argument_list|,
operator|&
name|ignore_packed_keep
argument_list|,
name|N_
argument_list|(
literal|"ignore packs that have companion .keep file"
argument_list|)
argument_list|)
block|,
name|OPT_INTEGER
argument_list|(
literal|0
argument_list|,
literal|"compression"
argument_list|,
operator|&
name|pack_compression_level
argument_list|,
name|N_
argument_list|(
literal|"pack compression level"
argument_list|)
argument_list|)
block|,
name|OPT_SET_INT
argument_list|(
literal|0
argument_list|,
literal|"keep-true-parents"
argument_list|,
operator|&
name|grafts_replace_parents
argument_list|,
name|N_
argument_list|(
literal|"do not hide commits by grafts"
argument_list|)
argument_list|,
literal|0
argument_list|)
block|,
name|OPT_BOOL
argument_list|(
literal|0
argument_list|,
literal|"use-bitmap-index"
argument_list|,
operator|&
name|use_bitmap_index
argument_list|,
name|N_
argument_list|(
literal|"use a bitmap index if available to speed up counting objects"
argument_list|)
argument_list|)
block|,
name|OPT_BOOL
argument_list|(
literal|0
argument_list|,
literal|"write-bitmap-index"
argument_list|,
operator|&
name|write_bitmap_index
argument_list|,
name|N_
argument_list|(
literal|"write a bitmap index together with the pack index"
argument_list|)
argument_list|)
block|,
name|OPT_END
argument_list|()
block|, 	}
decl_stmt|;
name|check_replace_refs
operator|=
literal|0
expr_stmt|;
name|reset_pack_idx_option
argument_list|(
operator|&
name|pack_idx_opts
argument_list|)
expr_stmt|;
name|git_config
argument_list|(
name|git_pack_config
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|pack_compression_seen
operator|&&
name|core_compression_seen
condition|)
name|pack_compression_level
operator|=
name|core_compression_level
expr_stmt|;
name|progress
operator|=
name|isatty
argument_list|(
literal|2
argument_list|)
expr_stmt|;
name|argc
operator|=
name|parse_options
argument_list|(
name|argc
argument_list|,
name|argv
argument_list|,
name|prefix
argument_list|,
name|pack_objects_options
argument_list|,
name|pack_usage
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
name|argc
condition|)
block|{
name|base_name
operator|=
name|argv
index|[
literal|0
index|]
expr_stmt|;
name|argc
operator|--
expr_stmt|;
block|}
if|if
condition|(
name|pack_to_stdout
operator|!=
operator|!
name|base_name
operator|||
name|argc
condition|)
name|usage_with_options
argument_list|(
name|pack_usage
argument_list|,
name|pack_objects_options
argument_list|)
expr_stmt|;
name|argv_array_push
argument_list|(
operator|&
name|rp
argument_list|,
literal|"pack-objects"
argument_list|)
expr_stmt|;
if|if
condition|(
name|thin
condition|)
block|{
name|use_internal_rev_list
operator|=
literal|1
expr_stmt|;
name|argv_array_push
argument_list|(
operator|&
name|rp
argument_list|,
name|shallow
condition|?
literal|"--objects-edge-aggressive"
else|:
literal|"--objects-edge"
argument_list|)
expr_stmt|;
block|}
else|else
name|argv_array_push
argument_list|(
operator|&
name|rp
argument_list|,
literal|"--objects"
argument_list|)
expr_stmt|;
if|if
condition|(
name|rev_list_all
condition|)
block|{
name|use_internal_rev_list
operator|=
literal|1
expr_stmt|;
name|argv_array_push
argument_list|(
operator|&
name|rp
argument_list|,
literal|"--all"
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|rev_list_reflog
condition|)
block|{
name|use_internal_rev_list
operator|=
literal|1
expr_stmt|;
name|argv_array_push
argument_list|(
operator|&
name|rp
argument_list|,
literal|"--reflog"
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|rev_list_index
condition|)
block|{
name|use_internal_rev_list
operator|=
literal|1
expr_stmt|;
name|argv_array_push
argument_list|(
operator|&
name|rp
argument_list|,
literal|"--indexed-objects"
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|rev_list_unpacked
condition|)
block|{
name|use_internal_rev_list
operator|=
literal|1
expr_stmt|;
name|argv_array_push
argument_list|(
operator|&
name|rp
argument_list|,
literal|"--unpacked"
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|reuse_object
condition|)
name|reuse_delta
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|pack_compression_level
operator|==
operator|-
literal|1
condition|)
name|pack_compression_level
operator|=
name|Z_DEFAULT_COMPRESSION
expr_stmt|;
elseif|else
if|if
condition|(
name|pack_compression_level
operator|<
literal|0
operator|||
name|pack_compression_level
operator|>
name|Z_BEST_COMPRESSION
condition|)
name|die
argument_list|(
literal|"bad pack compression level %d"
argument_list|,
name|pack_compression_level
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|delta_search_threads
condition|)
comment|/* --threads=0 means autodetect */
name|delta_search_threads
operator|=
name|online_cpus
argument_list|()
expr_stmt|;
ifdef|#
directive|ifdef
name|NO_PTHREADS
if|if
condition|(
name|delta_search_threads
operator|!=
literal|1
condition|)
name|warning
argument_list|(
literal|"no threads support, ignoring --threads"
argument_list|)
expr_stmt|;
endif|#
directive|endif
if|if
condition|(
operator|!
name|pack_to_stdout
operator|&&
operator|!
name|pack_size_limit
condition|)
name|pack_size_limit
operator|=
name|pack_size_limit_cfg
expr_stmt|;
if|if
condition|(
name|pack_to_stdout
operator|&&
name|pack_size_limit
condition|)
name|die
argument_list|(
literal|"--max-pack-size cannot be used to build a pack for transfer."
argument_list|)
expr_stmt|;
if|if
condition|(
name|pack_size_limit
operator|&&
name|pack_size_limit
operator|<
literal|1024
operator|*
literal|1024
condition|)
block|{
name|warning
argument_list|(
literal|"minimum pack size limit is 1 MiB"
argument_list|)
expr_stmt|;
name|pack_size_limit
operator|=
literal|1024
operator|*
literal|1024
expr_stmt|;
block|}
if|if
condition|(
operator|!
name|pack_to_stdout
operator|&&
name|thin
condition|)
name|die
argument_list|(
literal|"--thin cannot be used to build an indexable pack."
argument_list|)
expr_stmt|;
if|if
condition|(
name|keep_unreachable
operator|&&
name|unpack_unreachable
condition|)
name|die
argument_list|(
literal|"--keep-unreachable and --unpack-unreachable are incompatible."
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|rev_list_all
operator|||
operator|!
name|rev_list_reflog
operator|||
operator|!
name|rev_list_index
condition|)
name|unpack_unreachable_expiration
operator|=
literal|0
expr_stmt|;
if|if
condition|(
operator|!
name|use_internal_rev_list
operator|||
operator|!
name|pack_to_stdout
operator|||
name|is_repository_shallow
argument_list|()
condition|)
name|use_bitmap_index
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|pack_to_stdout
operator|||
operator|!
name|rev_list_all
condition|)
name|write_bitmap_index
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|progress
operator|&&
name|all_progress_implied
condition|)
name|progress
operator|=
literal|2
expr_stmt|;
name|prepare_packed_git
argument_list|()
expr_stmt|;
if|if
condition|(
name|progress
condition|)
name|progress_state
operator|=
name|start_progress
argument_list|(
name|_
argument_list|(
literal|"Counting objects"
argument_list|)
argument_list|,
literal|0
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|use_internal_rev_list
condition|)
name|read_object_list_from_stdin
argument_list|()
expr_stmt|;
else|else
block|{
name|get_object_list
argument_list|(
name|rp
operator|.
name|argc
argument_list|,
name|rp
operator|.
name|argv
argument_list|)
expr_stmt|;
name|argv_array_clear
argument_list|(
operator|&
name|rp
argument_list|)
expr_stmt|;
block|}
name|cleanup_preferred_base
argument_list|()
expr_stmt|;
if|if
condition|(
name|include_tag
operator|&&
name|nr_result
condition|)
name|for_each_ref
argument_list|(
name|add_ref_tag
argument_list|,
name|NULL
argument_list|)
expr_stmt|;
name|stop_progress
argument_list|(
operator|&
name|progress_state
argument_list|)
expr_stmt|;
if|if
condition|(
name|non_empty
operator|&&
operator|!
name|nr_result
condition|)
return|return
literal|0
return|;
if|if
condition|(
name|nr_result
condition|)
name|prepare_pack
argument_list|(
name|window
argument_list|,
name|depth
argument_list|)
expr_stmt|;
name|write_pack_file
argument_list|()
expr_stmt|;
if|if
condition|(
name|progress
condition|)
name|fprintf
argument_list|(
name|stderr
argument_list|,
literal|"Total %"
name|PRIu32
literal|" (delta %"
name|PRIu32
literal|"),"
literal|" reused %"
name|PRIu32
literal|" (delta %"
name|PRIu32
literal|")\n"
argument_list|,
name|written
argument_list|,
name|written_delta
argument_list|,
name|reused
argument_list|,
name|reused_delta
argument_list|)
expr_stmt|;
return|return
literal|0
return|;
block|}
end_function
end_unit
