...
name|die
argument_list|(
literal|"BUG: caller miscounted postlen: asked %d, orig = %d, used = %d"
argument_list|,
operator|(
name|int
operator|)
...
end_function
begin_function
DECL|function|line_by_line_fuzzy_match
specifier|static
name|int
name|line_by_line_fuzzy_match
parameter_list|(
name|struct
name|image
modifier|*
...
comment|/* First count added lines in postimage */
name|postlen
operator|=
literal|0
expr_stmt|;
...
expr_stmt|;
block|}
comment|/* 	 * The preimage may extend beyond the end of the file, 	 * but in this loop we will only handle the part of the 	 * preimage that falls within the file. 	 */
name|strbuf_init
argument_list|(
operator|&
name|fixed
...
operator|)
expr_stmt|;
comment|/* Add the length if this is common with the postimage */
if|if
condition|(
name|preimage
operator|->
...
name|_
argument_list|(
literal|"reading from '%s' beyond a symbolic link"
argument_list|)
argument_list|,
name|name
argument_list|)
...
argument_list|)
expr_stmt|;
comment|/* 	 * An attempt to read from or delete a path that is beyond a 	 * symbolic link will be prevented by load_patch_target() that 	 * is called at the beginning of apply_data() so we do not 	 * have to worry about a patch marked with "is_delete" bit 	 * here.  We however need to make sure that the patch result 	 * is not deposited to a path that is beyond a symbolic link 	 * here. 	 */
if|if
condition|(
operator|!
name|patch
...
name|_
argument_list|(
literal|"affected file '%s' is beyond a symbolic link"
argument_list|)
argument_list|,
name|patch
operator|->
...
end_function
begin_function
DECL|function|apply_all_patches
specifier|static
name|int
