...
argument_list|)
expr_stmt|;
comment|/* 	 * If the entry is the same as the current index, we can leave the old 	 * entry in place. Whether it is UPTODATE or not, checkout_entry will 	 * do the right thing. 	 */
name|pos
operator|=
name|cache_name_pos
argument_list|(
...
comment|/* 	 * if not null the branch is detached because it's already 	 * checked out in this checkout 	 */
...
name|wrapped_add_pending_uninteresting_ref
init|=
block|{
name|add_pending_uninteresting_ref
block|,
...
argument_list|)
expr_stmt|;
comment|/* recursive execution from checkout_new_worktree() */
name|opts
operator|.
name|new_worktree_mode
operator|=
