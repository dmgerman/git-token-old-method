...
end_function
begin_comment
comment|/*  * The ref_map records the tips of the refs we are fetching. If  *  *  $ git rev-list --verify-objects --stdin --not --all  *  * (feeding all the refs in ref_map on its standard input) does not  * error out, that means everything reachable from these updated refs  * locally exists and is connected to some of our existing refs.  *  * Returns 0 if everything is connected, non-zero otherwise.  */
end_comment
begin_function
