...
argument_list|)
expr_stmt|;
comment|/* 		 * For any refs that we happen to be fetching via command-line 		 * arguments, take the opportunity to update their configured 		 * counterparts. However, we do not want to mention these 		 * entries in FETCH_HEAD at all, as they would simply be 		 * duplicates of existing entries. 		 */
name|old_tail
operator|=
name|tail
expr_stmt|;
...
condition|)
block|{
comment|/* If --tags was specified, pretend the user gave us the canonical tags refspec */
if|if
condition|(
name|tags
operator|==
