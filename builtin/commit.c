...
end_decl_stmt
begin_decl_stmt
DECL|variable|config_commit_verbose
specifier|static
name|int
name|config_commit_verbose
init|=
operator|-
literal|1
decl_stmt|;
...
name|die
argument_list|(
literal|"BUG: unable to parse our own ident: %s"
argument_list|,
name|buf
operator|->
name|buf
...
condition|)
block|{
comment|/* 		 * prepend SQUASH_MSG here if it exists and a 		 * "merge --squash" was originally performed 		 */
if|if
condition|(
operator|!
name|stat
...
argument_list|)
expr_stmt|;
comment|/* 		 * These should never fail because they come from our own 		 * fmt_ident. They may fail the sane_ident test, but we know 		 * that the name and mail pointers will at least be valid, 		 * which is enough for our tests and printing here. 		 */
name|assert_split_ident
argument_list|(
operator|&
name|ai
