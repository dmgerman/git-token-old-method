...
specifier|static
name|int
name|config_cover_letter
decl_stmt|;
end_decl_stmt
begin_decl_stmt
DECL|variable|config_output_directory
specifier|static
specifier|const
name|char
modifier|*
...
name|var
argument_list|,
literal|"format.outputdirectory"
argument_list|)
condition|)
return|return
name|git_config_string
...
end_function
begin_struct
DECL|struct|base_tree_info
struct|struct
name|base_tree_info
block|{
DECL|member|base_commit
name|struct
name|object_id
name|base_commit
decl_stmt|;
DECL|member|nr_patch_id
DECL|member|alloc_patch_id
name|int
name|nr_patch_id
decl_stmt|,
name|alloc_patch_id
...
end_struct
begin_function
DECL|function|get_base_commit
specifier|static
name|struct
name|commit
modifier|*
name|get_base_commit
parameter_list|(
specifier|const
name|char
modifier|*
...
argument_list|)
expr_stmt|;
comment|/* There should be one and only one merge base. */
if|if
condition|(
operator|!
name|base_list
...
name|_
argument_list|(
literal|"Could not find exact merge base."
argument_list|)
argument_list|)
expr_stmt|;
name|base
...
name|_
argument_list|(
literal|"Failed to get upstream, if you want to record base commit automatically,\n"
literal|"please use git branch --set-upstream-to to track a remote branch.\n"
literal|"Or you could specify base commit by --base=<base-commit-id> manually."
argument_list|)
argument_list|)
expr_stmt|;
block|}
...
name|total
expr_stmt|;
comment|/* 	 * Get merge base through pair-wise computations 	 * and store it in rev[0]. 	 */
while|while
condition|(
name|rev_nr
operator|>
...
name|_
argument_list|(
literal|"Failed to find exact merge base"
argument_list|)
argument_list|)
expr_stmt|;
name|rev
...
name|_
argument_list|(
literal|"base commit should be the ancestor of revision list"
argument_list|)
argument_list|)
expr_stmt|;
for|for
...
name|_
argument_list|(
literal|"base commit shouldn't be in revision list"
argument_list|)
argument_list|)
expr_stmt|;
block|}
...
argument_list|)
expr_stmt|;
comment|/* 	 * Traverse the commits list, get prerequisite patch ids 	 * and stuff them in bases structure. 	 */
while|while
condition|(
operator|(
name|commit
...
name|i
decl_stmt|;
comment|/* Only do this once, either for the cover or for the first one */
if|if
condition|(
name|is_null_oid
argument_list|(
...
argument_list|)
expr_stmt|;
comment|/* Show the prerequisite patches */
for|for
control|(
name|i
operator|=
...
name|printf
argument_list|(
literal|"prerequisite-patch-id: %s\n"
argument_list|,
name|oid_to_hex
argument_list|(
operator|&
...
name|N_
argument_list|(
literal|"mark the series as Nth re-roll"
argument_list|)
argument_list|)
block|,
block|{
...
name|N_
argument_list|(
literal|"set From address to<ident> (or committer ident if absent)"
argument_list|)
block|,
name|PARSE_OPT_OPTARG
block|,
...
name|N_
argument_list|(
literal|"add prerequisite tree info to the patch series"
argument_list|)
argument_list|)
block|,
name|OPT_FILENAME
...
name|N_
argument_list|(
literal|"add a signature from a file"
argument_list|)
argument_list|)
block|,
name|OPT__QUIET
...
block|{
empty_stmt|;
comment|/* --no-signature inhibits all signatures */
block|}
elseif|else
if|if
condition|(
...
block|{
empty_stmt|;
comment|/* non-default signature already set */
block|}
elseif|else
if|if
condition|(
...
name|_
argument_list|(
literal|"unable to read signature file '%s'"
argument_list|)
argument_list|,
name|signature_file
argument_list|)
