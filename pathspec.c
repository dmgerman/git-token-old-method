...
name|_
argument_list|(
literal|"global 'literal' pathspec setting is incompatible "
literal|"with all other global pathspec settings"
argument_list|)
argument_list|)
expr_stmt|;
if|if
...
name|_
argument_list|(
literal|"invalid parameter for pathspec magic 'prefix'"
argument_list|)
argument_list|)
expr_stmt|;
comment|/* "i" would be wrong, but it does not matter */
break|break;
block|}
block|}
if|if
...
name|die
argument_list|(
literal|"BUG: 'prefix' magic is supposed to be used at worktree's root"
argument_list|)
expr_stmt|;
if|if
condition|(
...
name|_
argument_list|(
literal|"%s: 'literal' and 'glob' are incompatible"
argument_list|)
argument_list|,
name|elt
argument_list|)
...
condition|)
block|{
comment|/* 		 * FIXME: should we enable ONESTAR in _GLOB for 		 * pattern "* * / * . c"? 		 */
block|}
else|else
block|{
if|if
