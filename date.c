...
name|die
argument_list|(
literal|"BUG: cannot create anonymous strftime date_mode struct"
argument_list|)
expr_stmt|;
name|mode
operator|.
...
name|timebuf
argument_list|,
literal|"%04d-%02d-%02dT%02d:%02d:%02d%c%02d:%02d"
argument_list|,
name|tm
operator|->
name|tm_year
...
argument_list|)
condition|)
comment|/* 		 * We take over "now" here, which usually translates 		 * to the current timestamp.  This is because the user 		 * really means to expire everything she has done in 		 * the past, and by definition reflogs are the record 		 * of the past, and there is nothing from the future 		 * to be kept. 		 */
operator|*
name|timestamp
operator|=
name|ULONG_MAX
...
name|die
argument_list|(
literal|"date format missing colon separator: %s"
argument_list|,
name|format
argument_list|)
expr_stmt|;
...
name|sys
decl_stmt|;
comment|/* If we overflowed our unsigned long, that's bad... */
if|if
condition|(
name|t
operator|==
...
literal|1
return|;
comment|/* 	 * ...but we also are going to feed the result to system 	 * functions that expect time_t, which is often "signed long". 	 * Make sure that we fit into time_t, as well. 	 */
name|sys
operator|=
name|t
expr_stmt|;
