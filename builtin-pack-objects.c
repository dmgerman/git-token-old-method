...
index|[]
init|=
literal|"git-pack-objects [-q] [--no-reuse-delta] [--non-empty] [--local] [--incremental] [--window=N] [--depth=N] {--stdout | base-name} [--revs [--unpacked | --all]*<ref-list |<object-list]"
decl_stmt|;
end_decl_stmt
begin_struct
...
end_function
begin_comment
comment|/*  * we are going to reuse the existing pack entry data.  make  * sure it is not corrupt.  */
end_comment
begin_function
DECL|function|revalidate_pack_entry
specifier|static
name|int
name|revalidate_pack_entry
parameter_list|(
name|struct
name|object_entry
modifier|*
...
literal|0
return|;
comment|/* the caller has already called use_packed_git() for us, 	 * so it is safe to access the pack data from mmapped location. 	 * make sure the entry inflates correctly. 	 */
name|used
operator|=
...
name|die
argument_list|(
literal|"corrupt delta in pack %s"
argument_list|,
name|sha1_to_hex
argument_list|(
name|entry
...
end_function
begin_comment
comment|/* copied from rev-list but needs to do things slightly differently */
end_comment
begin_function
...
argument_list|)
expr_stmt|;
comment|/* make sure we did not get pathspecs */
if|if
condition|(
name|revs
operator|.
...
argument_list|)
expr_stmt|;
comment|/* --unpacked and --all makes sense only with --revs */
if|if
condition|(
operator|!
name|use_internal_rev_list
