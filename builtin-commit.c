...
return|;
block|}
comment|/* update the user index file */
name|add_files_to_cache
argument_list|(
name|verbose
argument_list|,
...
name|die
argument_list|(
literal|"failed to read HEAD tree object"
argument_list|)
expr_stmt|;
block|}
comment|/* Use a lock file to garbage collect the temporary index file. */
name|next_index_lock
operator|=
name|xmalloc
argument_list|(
...
literal|"# (Comment lines starting with '#' will not be included)\n"
argument_list|)
expr_stmt|;
if|if
condition|(
...
argument_list|)
expr_stmt|;
comment|/* Start building up the commit header */
...
name|die
argument_list|(
literal|"failed to write new index"
argument_list|)
expr_stmt|;
name|rerere
argument_list|()
