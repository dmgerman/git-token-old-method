...
expr_stmt|;
block|}
comment|/* Clear out the Range: header after performing the request, so 	   other curl requests don't inherit inappropriate header data */
name|curl_result
operator|=
...
name|error
argument_list|(
literal|"unable to write index filename %s: %s"
argument_list|,
name|filename
argument_list|,
name|strerror
...
name|error
argument_list|(
literal|"unable to write pack filename %s: %s"
argument_list|,
name|filename
argument_list|,
name|strerror
...
argument_list|)
expr_stmt|;
comment|/* Note: if another instance starts now, it will turn our new 	   tmpfile into its prevfile. */
if|if
condition|(
name|local
operator|<
