...
end_decl_stmt
begin_comment
comment|/*  * update-cache --refresh may not touch anything at all, in which case  * writing 1.6MB of the same thing is a waste.  */
end_comment
begin_decl_stmt
DECL|variable|cache_changed
specifier|static
name|int
name|cache_changed
init|=
...
condition|)
block|{
comment|/* If we have not smudged the cache, be careful 		 * to keep it clean.  Find out if we have a matching 		 * cache entry that add_cache_entry would replace with, 		 * and if it matches then do not bother calling it. 		 */
name|int
name|pos
init|=
name|cache_name_pos
...
argument_list|)
expr_stmt|;
comment|/* magic to tell add_file_to_cache that 			 * we have not updated anything. 			 */
return|return
literal|999
return|;
block|}
